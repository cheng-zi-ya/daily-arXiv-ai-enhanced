...
Motivation: 慢性疾病如肥胖和糖尿病与饮食相关，需要准确监测食物摄入量。虽然AI驱动的饮食评估已有进展，但从单目图像恢复尺寸信息用于准确估计"吃了多少"仍是一个紧迫挑战。现有3D重建方法虽然几何重建效果不错，但无法恢复重建物体的真实世界尺度，限制了其在精准营养领域的应用。

Method: 提出一种方法，从单目图像恢复真实尺度的3D重建物体。利用在大规模数据集上训练的模型提取丰富的视觉特征来估计重建物体的尺度。这种学习到的尺度能够将单视图3D重建转换为真实、具有物理意义的模型。

Result: 在两个公开数据集上进行的大量实验和消融研究表明，该方法始终优于现有技术，实现了平均绝对体积估计误差减少近30%，展示了其在增强精准营养领域的潜力。

Conclusion: 该方法成功弥合了3D计算机视觉与数字健康之间的差距，通过从单目图像恢复真实尺度的3D重建，为精确的食物摄入量监测提供了有效解决方案，在精准营养领域具有重要应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型（如CLIP）的开放词汇语义分割方法存在两个关键限制：1）前景偏见，倾向于忽略背景区域；2）有限的空间定位能力，导致物体边界模糊。这些限制源于VLMs在图像-文本对预训练中对显著、物体中心区域的偏向。

Method: 提出DiSa框架，包含两个核心模块：1）显著性感知解耦模块（SDM），通过显式结合显著性线索，以分而治之的方式分别建模前景和背景集合特征；2）分层细化模块（HRM），利用像素级空间上下文，通过多级更新实现通道级特征细化。

Result: 在六个基准测试上的广泛实验表明，DiSa始终优于最先进的方法。

Conclusion: DiSa通过显著性感知的前景-背景解耦框架有效解决了开放词汇语义分割中VLMs的前景偏见和空间定位限制问题，在多个基准测试上取得了优越性能。
...
TL;DR: 提出Sparse CLIP方法，在CLIP训练中直接集成稀疏性，获得既具可解释性又保持高性能的表示，挑战了可解释性与性能相互冲突的传统观念。


<details>
  <summary>Details</summary>
Motivation: CLIP作为视觉-语言表示学习的基石，其密集不透明的潜在表示带来了显著的可解释性挑战。传统观点认为可解释性和性能存在矛盾：训练时强制稀疏性会降低准确性，而现有的后处理方法（如稀疏自编码器）又会导致下游性能下降和丧失CLIP固有的多模态能力。

Method: 提出Sparse CLIP方法，在CLIP训练过程中直接集成稀疏性，而不是采用后处理方式。这种方法产生稀疏表示，同时保持CLIP的多模态能力。

Result: 与稀疏自编码器相比，Sparse CLIP表示保持了强大的下游任务性能，实现了更优的可解释性，并保留了多模态能力。多模态稀疏特征能够实现直接的语义概念对齐，并揭示跨模态知识出现的训练动态。作为概念验证，在稀疏CLIP表示上训练了一个视觉-语言模型，实现了可解释的基于视觉的引导能力。

Conclusion: 研究结果挑战了可解释性需要牺牲准确性的传统观念，证明可解释性和性能可以共同优化，为未来模型设计提供了有前景的原则。
...
TL;DR: 该研究聚焦于H&E染色图像中的细胞核实例分割数据集标准化与评估，而非开发新算法。通过整合多个公开数据集，创建统一格式，并基于两种先进模型进行系统评估和排名，提出了用于公平跨数据集评估的统一测试集和提升性能的融合训练集。


<details>
  <summary>Details</summary>
Motivation: 当前细胞核实例分割研究主要关注算法开发，并在有限数量的任意选择的公共数据集上进行基准测试。本研究旨在解决数据集标准化和系统评估的不足，为H&E染色组织学图像的细胞核实例分割提供更可靠的训练、测试和评估基准。

Method: 1. 通过广泛的文献综述识别手动标注的公开H&E染色图像数据集；2. 将数据集标准化为统一的输入和标注格式；3. 使用两种最先进的分割模型（基于CNN和基于CNN与视觉Transformer混合架构）系统评估和排名这些数据集；4. 提出统一测试集（NucFuse-test）用于公平的跨数据集评估；5. 提出统一训练集（NucFuse-train）通过合并多个数据集的图像来提升分割性能。

Result: 1. 创建了标准化的数据集集合；2. 对数据集进行了系统评估和排名；3. 生成了融合数据集（NucFuse-test和NucFuse-train）；4. 进行了外部验证；5. 提供了公开可用的实现代码，为H&E染色组织学图像的细胞核实例分割模型训练、测试和评估建立了新基准。

Conclusion: 该研究通过数据集标准化、系统评估和融合数据集创建，为细胞核实例分割领域提供了更可靠和公平的评估框架。这项工作强调了数据集质量对算法性能的重要性，并为未来研究提供了实用的基准工具。
...
TL;DR: 提出SAP训练无关剪枝方法，通过识别中间层关键视觉补丁实现90%索引向量压缩，同时保持检索性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如ColPali）在细粒度视觉文档检索中索引向量过大，训练无关剪枝方法在高压缩场景（>80%）下性能不佳，甚至不如随机选择

Method: 提出结构锚点剪枝（SAP）方法，从中间层识别关键视觉补丁；引入Oracle分数保留（OSR）协议评估层间信息对压缩效率的影响

Result: 在ViDoRe基准测试中，SAP减少90%以上索引向量同时保持稳健检索保真度；OSR分析显示语义结构锚点补丁在中间层持续存在

Conclusion: SAP为视觉RAG提供高度可扩展解决方案，传统剪枝方法关注最终层（结构信号消散），而语义结构锚点在中间层持续存在
...
TL;DR: 针对扩散式大模型LLaDA-V的计算效率问题，提出结构化视觉token剪枝策略，在保持性能的同时显著降低计算成本


<details>
  <summary>Details</summary>
Motivation: 扩散式大模型如LLaDA-V存在显著计算开销问题，其双向注意力机制和迭代去噪范式导致视觉token在所有层和去噪步骤中被重复处理。研究发现LLaDA-V的跨模态信息聚合主要发生在中后层，存在语义对齐延迟

Method: 提出结构化token剪枝策略，受FastV启发但针对LLaDA-V特性改进：1）针对中后层进行剪枝以匹配延迟注意力聚合模式；2）在第一个去噪步骤进行剪枝，减少后续所有步骤的计算；3）选择性移除部分视觉token以减少FLOPs同时保留关键语义信息

Result: 在多个基准测试中，最佳配置将计算成本降低高达65%，同时保持平均95%的任务性能。这是首个研究扩散式大模型中结构化token剪枝的工作

Conclusion: 该框架为高效LLaDA-V推理提供了实证基础，突显了视觉感知剪枝在扩散式多模态模型中的潜力。通过针对性剪枝策略，在显著降低计算开销的同时保持了模型性能
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散变换器在内容保持风格迁移中的挑战，因为DiTs的内部表示中内容和风格特征存在固有纠缠，需要开发能够保持内容保真度同时实现风格泛化的模型

Method: 基于Qwen-Image-Edit构建，收集高质量特定风格数据集并合成多样化风格三元组，采用课程持续学习框架训练混合数据集，添加视频到视频风格化模块增强时间一致性

Result: 在三个核心评估指标上达到最先进性能：风格相似性、内容一致性和美学质量，能够泛化到未见过的风格而不损害内容保真度

Conclusion: TeleStyle是一个有效的轻量级图像和视频风格迁移解决方案，通过创新的数据集构建和训练策略解决了内容保持风格迁移的关键挑战
...
TL;DR: DenseGRPO：一种基于密集奖励的文本到图像生成对齐框架，通过评估每个去噪步骤的细粒度贡献来解决稀疏奖励问题，并采用奖励感知的探索空间校准方案。


<details>
  <summary>Details</summary>
Motivation: 现有的基于GRPO和流匹配模型的方法在文本到图像生成的人类偏好对齐方面取得了显著进展，但仍面临稀疏奖励问题：整个去噪轨迹的终端奖励被应用于所有中间步骤，导致全局反馈信号与中间去噪步骤的实际细粒度贡献不匹配。
...
Conclusion: DenseGRPO通过密集奖励评估和奖励感知探索空间校准，有效解决了现有GRPO方法中的稀疏奖励问题，显著提升了文本到图像生成的人类偏好对齐性能。
...
Conclusion: FPL是一种简单、高效且有效的方法，能够成功将CLIP预训练模型适应到下游任务，解决了现有方法的局限性
...
TL;DR: 提出可逆高效扩散模型（RED），通过显式监督训练框架解决扩散模型在图像融合中的细节损失问题，避免分布估计并保持计算效率


<details>
  <summary>Details</summary>
Motivation: 多模态图像融合需要将不同源图像的互补信息整合到统一表示中，但现有扩散模型在图像融合任务中常出现细节损失问题，这源于马尔可夫过程中的噪声误差累积，导致融合结果不一致和退化

Method: 提出可逆高效扩散模型（RED），这是一个显式监督训练框架，继承了扩散模型的强大生成能力，同时避免了分布估计，解决了计算效率问题

Result: 论文提出的方法能够有效解决扩散模型在图像融合中的细节损失问题，同时保持计算效率，避免了传统扩散模型训练中的分布估计挑战
...
TL;DR: 提出LVLMs-Saliency框架，通过融合注意力权重和输入梯度来量化视觉基础强度，发现幻觉常由前序token的低显著性导致，并提出了SGRS和LocoRE两种推理时机制来减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力动态的方法在可靠区分幻觉和事实性输出方面存在局限，因为它们仅依赖前向传播注意力模式，忽略了梯度信号。需要开发能更准确检测和缓解LVLMs中幻觉的方法。

Method: 提出LVLMs-Saliency框架，融合注意力权重和输入梯度来量化视觉基础强度。基于分析发现，提出两种推理时机制：1) SGRS：通过上下文自适应阈值动态过滤候选token；2) LocoRE：轻量级插件模块，增强当前token对最近前驱的注意力。

Result: 在多个LVLMs上的广泛实验表明，该方法显著降低了幻觉率，同时保持了流畅性和任务性能，为增强模型可靠性提供了鲁棒且可解释的解决方案。

Conclusion: LVLMs-Saliency框架通过梯度感知诊断和推理时干预，有效缓解了大型视觉语言模型中的幻觉问题，为模型可靠性提升提供了新途径。
...
TL;DR: 提出了一种新颖的无源域自适应方法，通过多视图增强和潜在空间一致性技术直接从目标域学习领域不变特征，无需源域数据或复杂的对抗训练。


<details>
  <summary>Details</summary>
Motivation: 传统域自适应方法通常需要访问源域数据、对抗训练或复杂的伪标签技术，这些方法计算成本高且实现复杂。为了解决这些挑战，本文旨在开发一种无需源域数据、更高效的自适应方法。

Method: 1. 使用多视图增强技术生成目标域数据的多个增强视图；2. 在潜在空间中强制这些增强视图的特征表示之间的一致性；3. 采用ConvNeXt-based编码器；4. 设计结合分类和一致性目标的损失函数；5. 直接从目标域学习可迁移表示，无需源-目标对齐或伪标签细化。
...
Conclusion: 该方法成功实现了无源域自适应，通过多视图增强和潜在空间一致性技术直接从目标域学习领域不变特征，在多个标准数据集上取得了优于现有方法的性能，为域自适应提供了一种更高效、更实用的解决方案。
...
Method: 1) 提出关注外观、运动和相机三个关键方面的评估协议，定义10种常见伪影类别；2) 构建包含8万个视频的GenVID数据集，由各种先进视频生成模型生成并标注；3) 开发DVAR密集视频伪影识别框架进行细粒度伪影检测与分类

Result: 实验表明该方法显著提高了伪影检测准确性，并能有效过滤低质量内容

Conclusion: 提出的评估协议、数据集和框架为视频生成质量评估提供了更精细的工具，有助于改进视频生成技术
...
<details>
  <summary>Details</summary>
Motivation: 现有方法中，SAM训练后剪枝会破坏鲁棒性，而剪枝后应用SAM又受限于早期剪枝模式的架构限制，需要在模型压缩过程中同时保持紧凑性和鲁棒性

Method: 提出C-SAM框架，将锐度感知学习从参数扰动转向掩码扰动，通过在训练中显式扰动剪枝掩码，促进对模型结构更平坦的损失景观

Result: 在CelebA-HQ、Flowers-102和CIFAR-10-C数据集上，使用ResNet-18、GoogLeNet和MobileNet-V2，C-SAM相比基线方法获得高达42%的认证鲁棒性提升，同时保持与未剪枝模型相当的任务准确率

Conclusion: C-SAM通过掩码扰动实现了在模型压缩过程中同时优化紧凑性和鲁棒性，解决了传统方法中剪枝与鲁棒性训练之间的冲突问题
...
TL;DR: 该研究提出了一种双域学习策略，通过结合有/无施源器的CT数据，仅需少量目标域数据即可显著提升膀胱分割性能，解决医学图像分割中的协变量偏移问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中协变量偏移导致的性能下降是一个主要挑战。在CT引导的妇科近距离放射治疗中，无施源器的CT扫描（NA）广泛可用，但带有施源器的扫描（WA）稀缺且存在显著的解剖变形和成像伪影，导致自动分割困难。研究旨在探索分布偏移的样本能否与有限的目标域数据有效结合来支持学习。

Method: 提出双域学习策略，整合NA和WA CT数据以提升鲁棒性和泛化能力。使用精心策划的混合数据集，通过系统实验在轴位、冠状位和矢状位平面上使用多种深度学习架构，研究不同比例WA数据对分割性能的影响。

Result: 仅使用NA数据无法捕捉WA图像的特征，但在NA训练集中引入少量WA数据（10-30%）即可显著提升性能。该方法达到Dice相似系数最高0.94和IoU最高0.92，性能与完全使用WA数据训练的模型相当。

Conclusion: 该研究证明了整合解剖相似但分布偏移的数据集的价值，能够克服数据稀缺问题，提升近距离放射治疗计划中基于深度学习的分割性能，实现有效的域适应和临床可靠性改进。
...
TL;DR: 提出一种基于物理结构的单图像质量估计框架，通过几何体积和材料密度推理来解决视觉质量估计的模糊性问题。
...
Method: 从单张RGB图像中，通过单目深度估计恢复物体中心三维几何来获取体积信息，使用视觉语言模型提取粗略材料语义来指导密度推理。通过实例自适应门控机制融合几何、语义和外观表示，并通过两个独立的回归头预测物理引导的潜在因子（体积相关和密度相关），仅使用质量监督进行训练。

Result: 在image2mass和ABO-500数据集上的实验表明，该方法在性能上持续优于现有最先进方法。

Conclusion: 提出的物理结构化框架通过将视觉线索与物理因素对齐，有效解决了单图像质量估计的模糊性问题，为视觉质量预测提供了物理上有意义的解决方案。
...
TL;DR: 提出SLDM模型，通过结构约束和语言引导的扩散模型，从低剂量对比剂CT生成正常剂量CT图像，减少对比剂用量同时保持诊断能力。
...
Conclusion: SLDM模型能够有效从低剂量对比剂CT生成高质量的正常剂量CT图像，在减少对比剂用量的同时保持诊断能力，解决了现有方法在不完全配对图像上增强不准确的问题。
...
TL;DR: OSDEnhancer：首个通过高效一步扩散过程实现真实世界时空视频超分辨率的框架，结合线性预插值、TR-SE MoE专家混合和双向变形VAE解码器，在保持时间一致性的同时提升空间细节。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在视频超分辨率方面表现出色，但在时空视频超分辨率（STVSR）方面探索不足，特别是需要同时提升分辨率和帧率。现有方法通常在简化的退化假设下工作，难以应对真实世界中复杂的未知退化，对重建保真度和时间一致性要求很高。

Method: 提出OSDEnhancer框架：1）通过线性预插值策略初始化基本时空结构；2）训练时间精化和空间增强混合专家（TR-SE MoE），让不同专家路径分别学习时间一致性和空间细节的专门表示；3）引入双向变形VAE解码器进行循环时空聚合和传播，增强跨帧重建保真度；4）采用高效的一步扩散过程。

Result: 实验表明该方法在真实世界场景中实现了最先进的性能，同时保持了优越的泛化能力。

Conclusion: OSDEnhancer是首个通过一步扩散过程实现真实世界时空视频超分辨率的方法，通过创新的架构设计有效解决了时间一致性和空间细节的平衡问题，在复杂退化场景中表现出色。
...
TL;DR: CPiRi是一个通道排列不变的多变量时间序列预测框架，通过解耦时空架构和排列不变正则化策略，解决了现有方法在通道依赖和通道独立之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有多变量时间序列预测方法存在局限性：通道依赖模型学习跨通道特征但容易过拟合通道顺序，当通道增加或重新排序时难以适应；通道独立模型虽然灵活但忽略了通道间依赖关系，限制了性能。需要一种既能学习跨通道结构又不依赖固定通道顺序的方法。

Method: CPiRi采用时空解耦架构和排列不变正则化训练策略：1）使用冻结的预训练时间编码器提取高质量时间特征；2）轻量级空间模块学习内容驱动的通道间关系；3）通道洗牌策略在训练中强制实现排列不变性。该方法在理论上分析了多变量时间序列预测中的排列等变性。

Result: 在多个基准测试中取得了最先进的结果。CPiRi在通道顺序被打乱时保持稳定，即使只在一半通道上训练，也能对未见过的通道表现出强大的归纳泛化能力，同时在大规模数据集上保持实际效率。

Conclusion: CPiRi通过通道排列不变框架解决了多变量时间序列预测中的结构漂移和分布漂移问题，无需重新训练即可适应通道结构变化，在保持性能的同时提供了更好的泛化能力和实际部署效率。
...
Method: 1. 高斯可见性感知的多视角几何一致性约束：通过聚合共享高斯基元在不同视角下的可见性，实现更准确稳定的几何监督。2. 渐进式四叉树校准的单目深度约束：从粗到细的空间尺度进行分块仿射校准，缓解深度先验的尺度模糊问题，同时保留细粒度表面细节。

Result: 在DTU和TNT数据集上的大量实验表明，该方法在几何精度上相比先前的高斯基方法和隐式表面重建方法都有持续改进。

Conclusion: 提出的高斯可见性感知多视角几何一致性约束和渐进式四叉树校准单目深度约束有效解决了现有方法的局限性，显著提高了3D高斯泼溅的表面重建精度。
...
Method: 提出TopoOT框架，集成多滤波持久性图与测试时适应，核心创新是最优传输链式对齐，跨阈值和滤波序列对齐持久性图，产生测地稳定性分数识别跨尺度一致保留的特征

Result: 在标准2D和3D异常检测基准测试中达到最先进性能，2D数据集上平均F1分数比最具竞争力方法高出+24.1%，3D异常分割基准上高出+10.2%

Conclusion: TopoOT通过拓扑感知的最优传输框架，实现了对分布偏移下异常分割的鲁棒适应，证明了拓扑特征在异常检测中的有效性
...
TL;DR: SpatialGenEval是一个新的基准测试，用于系统评估文本到图像模型的空间智能，包含1,230个信息密集的长提示，覆盖25个真实场景和10个空间子领域。研究发现高阶空间推理是主要瓶颈，并通过构建SpatialT2I数据集（15,400个文本-图像对）进行微调，使基础模型获得4.2%-5.7%的性能提升。
...
Method: 1. 提出SpatialGenEval基准测试：包含1,230个信息密集的长提示，覆盖25个真实世界场景，每个提示整合10个空间子领域和相应的10个多项选择题-答案对；2. 构建SpatialT2I数据集：包含15,400个文本-图像对，通过重写提示确保图像一致性同时保持信息密度；3. 对21个最先进模型进行广泛评估；4. 在基础模型（Stable Diffusion-XL、Uniworld-V1、OmniGen2）上进行微调。

Result: 评估发现高阶空间推理是文本到图像模型的主要瓶颈。通过SpatialT2I数据集微调后，基础模型在空间关系方面获得一致的性能提升（+4.2%、+5.7%、+4.4%）和更真实的效果。

Conclusion: SpatialGenEval基准测试系统评估了文本到图像模型的空间智能，揭示了高阶空间推理的瓶颈。通过数据中心的范式（构建信息密集的SpatialT2I数据集并进行微调），可以有效提升模型的空间智能，为文本到图像模型的空间关系处理提供了新的解决方案。
...
Method: 提出CURVE框架：1）集成变分不确定性建模与不确定性引导的结构正则化；2）应用原型条件去偏，从环境依赖变化中解耦不变交互动态；3）促进稀疏且领域稳定的拓扑结构。

Result: 在零样本迁移和低数据模拟到真实适应任务中评估CURVE，验证了其学习领域稳定稀疏拓扑的能力，并能提供可靠的不确定性估计以支持分布偏移下的风险预测。

Conclusion: CURVE通过抑制环境特定关系、解耦不变交互动态，有效提升了场景图在分布外场景下的泛化能力和可靠性。
...
TL;DR: RAW-Flow：一种基于流匹配的生成式RGB到RAW重建框架，通过潜在空间确定性向量场学习解决细节不一致和颜色偏差问题


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法将RGB到RAW重建视为直接回归任务，但由于逆ISP问题的病态性和量化RGB图像固有的信息损失，这些方法在处理细节一致性和颜色准确性方面存在困难

Method: 1. 将RGB到RAW重建重新定义为确定性潜在传输问题；2. 提出RAW-Flow框架，利用流匹配学习潜在空间中的确定性向量场；3. 引入跨尺度上下文引导模块，将分层RGB特征注入流估计过程；4. 设计具有特征对齐约束的双域潜在自编码器，支持潜在传输框架

Result: 大量实验表明，RAW-Flow在定量和视觉评估上都优于现有最先进方法

Conclusion: 通过生成式视角和流匹配技术，RAW-Flow能够有效解决RGB到RAW重建中的细节不一致和颜色偏差问题，实现高质量重建
...
TL;DR: HINT是首个用于多人运动生成的自动回归扩散框架，通过分层交互建模解决长序列、可变文本和参与者数量的问题


<details>
  <summary>Details</summary>
Motivation: 现有离线方法在处理长文本、可变文本和不同参与者数量时存在固有局限性，这促使需要自动回归方法来逐步预测未来运动

Method: 1) 在规范化潜在空间中使用解耦运动表示，分离局部运动语义和人际交互；2) 采用滑动窗口策略进行高效在线生成，聚合窗口内局部和跨窗口全局条件来捕捉历史、交互依赖和文本对齐

Result: 在公共基准测试中，HINT与强离线模型性能相当，并超越自动回归基线。在InterHuman数据集上，FID达到3.100，显著优于之前SOTA的5.154

Conclusion: HINT通过分层交互建模和自动回归扩散框架，有效解决了多人运动生成中的长序列、可变文本和参与者数量问题，实现了高质量的运动生成
...
Result: 在6个基准数据集上，BiFTA在ViT-based和ResNet-based CLIP模型上均取得优越的零样本性能
...
TL;DR: 提出Quartet of Diffusions框架，通过四个协调的扩散模型分别建模全局形状潜在表示、对称性、语义部件及其空间装配，实现结构感知的点云生成，保证对称性和部件一致性。
...
Method: 使用四个协调的扩散模型：1) 全局形状潜在表示模型，2) 对称性模型，3) 语义部件模型，4) 空间装配模型。通过解耦生成过程为可解释组件，使用中心全局潜在表示来增强结构一致性。

Result: 实验表明该方法达到了最先进的性能，能够保证对称性、实现连贯的部件放置，并生成多样化的高质量输出。这是首个在生成过程中完全集成并强制执行对称性和部件先验的3D点云生成框架。

Conclusion: Quartet of Diffusions框架通过显式建模部件组合和对称性，实现了结构感知的点云生成，支持对形状属性的细粒度控制，同时保持全局一致性，为3D形状生成提供了新的结构化方法。
...
TL;DR: Youtu-Parsing是一个高效的多功能文档解析模型，采用视觉Transformer和语言模型结合的架构，通过并行解码策略实现5-11倍加速，支持多种文档元素解析，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统文档解析模型在处理复杂文档结构时的效率瓶颈，特别是在需要同时处理文本、表格、公式、图表等多种元素的大规模文档智能应用中，需要开发一个既能保持高质量输出又能显著提升处理速度的解析系统。

Method: 采用解耦且特征可重用的架构：1）使用动态分辨率视觉编码器的原生Vision Transformer提取共享文档特征；2）结合提示引导的Youtu-LLM-2B语言模型进行布局分析和区域提示解码。核心创新是两种并行解码策略：令牌并行（每步生成64个候选令牌，通过验证机制筛选）和查询并行（同时预测多个边界框内容）。

Result: Youtu-Parsing在OmniDocBench和olmOCR-bench基准测试中达到最先进的性能。令牌并行策略相比传统自回归解码实现5-11倍加速，特别适合表格识别等高结构化场景；查询并行策略提供额外2倍加速且保持输出质量不变。模型能处理文本、公式、表格、图表、印章和层次结构等多种元素，对罕见字符、多语言文本和手写内容表现出强鲁棒性。

Conclusion: Youtu-Parsing通过创新的并行解码架构，在保持高质量文档解析的同时显著提升了处理效率，为大规模文档智能应用提供了具有重要实验价值和实用性的解决方案。
...
TL;DR: MARE：通过视觉语言模型进行可解释的深度伪造检测，采用多模态对齐和强化学习，结合人类反馈奖励函数和伪造解耦模块，提升检测准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型的快速发展，深度伪造检测面临新的挑战。现有方法主要将问题建模为分类或空间定位，需要更准确、可靠且可解释的检测方法。视觉语言模型在深度伪造检测和推理方面有潜力，但需要增强其准确性和可靠性。

Method: 提出MARE方法：1）设计综合奖励函数，结合人类反馈强化学习（RLHF），激励生成文本-空间对齐的推理内容；2）引入伪造解耦模块，从高级面部语义中捕获内在伪造痕迹，提升真实性检测能力。

Result: 对MARE生成的推理内容进行全面评估。定量和定性实验结果表明，MARE在准确性和可靠性方面达到了最先进的性能。

Conclusion: MARE通过多模态对齐和强化学习，结合人类反馈和伪造解耦技术，显著提升了视觉语言模型在深度伪造检测中的准确性和可靠性，为可解释的深度伪造检测提供了有效解决方案。
...
TL;DR: 提出Dummy Forcing方法，通过控制多头自注意力中历史帧的访问，减少上下文冗余并实现缓存压缩，在视频扩散模型中达到2.0倍加速且质量损失小于0.5%。


<details>
  <summary>Details</summary>
Motivation: 研究发现自回归视频扩散模型中的多头自注意力机制对历史帧的利用不足，约25%的注意力头几乎只关注当前帧，丢弃它们的KV缓存仅导致轻微性能下降。
...
Conclusion: 通过控制多头自注意力中历史帧的访问，有效减少上下文冗余并实现高效缓存压缩，显著提升视频扩散模型的推理速度。
...
<details>
  <summary>Details</summary>
Motivation: WMH和ISL是脑小血管病的影像特征，在FLAIR序列中视觉上容易混淆且常同时出现，开发准确分割模型面临挑战。由于完全标注数据稀缺，需要探索如何有效利用部分标注数据

Method: 研究了六种训练策略，结合私有和公开的部分标注数据集（共2052个MRI体积），其中1341个有WMH标注，1152个有ISL标注。重点比较了不同方法利用部分标注数据的效果

Result: 多种方法能有效利用部分标注数据提升模型性能，其中伪标签方法取得了最佳结果

Conclusion: 伪标签是处理部分标注数据训练WMH和ISL联合分割模型的有效策略，能显著提升模型性能
...
TL;DR: 本文提出了一种名为潜在时序差异（LTD）的运动先验方法，通过测量潜在空间中的帧间变化来指导损失权重分配，从而提升视频生成模型在动态场景中的表现。
...
Method: 引入潜在时序差异（LTD）作为运动先验来指导损失权重分配。LTD测量潜在空间中的帧间变化，为差异较大的区域分配更大的惩罚权重，同时对稳定区域保持常规优化。

Result: 在通用基准VBench和专注于运动的VMBench上进行广泛实验，该方法在VBench上比强基线提升3.31%，在VMBench上提升3.58%，在运动质量方面取得了显著改进。

Conclusion: 提出的LTD运动感知策略稳定了训练过程，使模型能够更好地重建高频动态，有效解决了视频生成模型在动态场景中的性能限制问题。
...
TL;DR: 本文提出了肖像收藏生成（PCG）新任务，通过自然语言指令编辑参考肖像图像来生成连贯的肖像收藏。针对多属性修改和高保真细节保留的挑战，作者构建了CHEESE大规模数据集并提出了SCheese框架。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体平台激增，用户需要直观的方式创建多样化、高质量的肖像收藏。现有方法面临两个独特挑战：复杂的多属性修改（如姿势、空间布局、相机视角）以及高保真细节保留（如身份、服装、配饰）。

Method: 作者构建了CHEESE数据集，包含24K肖像收藏和573K样本，通过基于大型视觉语言模型的流程构建并采用反转验证。提出了SCheese框架，结合文本引导生成与分层身份和细节保留，采用自适应特征融合机制保持身份一致性，并使用ConsistencyNet注入细粒度特征保持细节一致性。

Result: 综合实验验证了CHEESE在推进PCG任务中的有效性，SCheese框架实现了最先进的性能表现。

Conclusion: 本文提出了PCG新任务及其解决方案，通过构建大规模数据集和提出创新框架，成功解决了肖像收藏生成中的多属性修改和细节保留挑战。
...
TL;DR: 本文提出CoTA方法解决扩散式多模态大语言模型中的重复诅咒问题，通过分析信息流机制发现上下文令牌信息聚合与熵收敛规律，设计增强注意力与惩罚项来缓解重复生成。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的多模态大语言模型依赖缓存技术加速解码，但缓存机制常导致重复文本生成问题（重复诅咒）。作者旨在深入探究这一现象背后的信息流机制，并提出有效解决方案。

Method: 通过信息流分析揭示三个关键发现：1) 上下文令牌作为语义锚点聚合信息指导预测；2) 信息跨层传播时上下文令牌熵在深层收敛；3) 重复与上下文令牌信息流中断及熵不收敛相关。基于此提出CoTA方法：增强上下文令牌注意力以保持信息流模式，在解码时对置信度分数引入惩罚项以避免不确定上下文令牌驱动的输出。

Result: CoTA在缓解重复生成方面表现出显著效果，在通用任务上获得一致的性能提升。实验验证了该方法在保持模型原有能力的同时有效减少重复文本生成。

Conclusion: 通过信息流分析揭示了扩散式多模态大语言模型中重复诅咒的机制，提出的CoTA方法作为即插即用方案能有效缓解重复问题，同时提升模型在通用任务上的性能表现。
...
TL;DR: IOTA框架将预训练模型视为黑盒与白盒的结合，通过对比错误预测与正确认知生成纠正知识，将其转化为可解释提示来指导黑盒模块，实现更有效的下游任务适应。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个开源的世界模拟器，为社区提供实用的工具，应用于内容创作、游戏和机器人学习等领域，缩小开源与闭源技术之间的差距。

Method: 基于视频生成技术构建世界模拟器，通过优化算法实现高保真度、长时记忆和实时交互性，支持多种环境类型（写实、科学、卡通等）。

Result: LingBot-World实现了高保真度和鲁棒动态特性，支持分钟级时间跨度并保持上下文一致性，实时交互延迟低于1秒（16帧/秒），代码和模型已公开。

Conclusion: LingBot-World作为顶级世界模型，通过开源方式为社区提供强大的世界模拟能力，有望在多个应用领域产生实际影响。
...
TL;DR: DeepSeek-OCR 2提出了一种新颖的视觉编码器DeepEncoder V2，能够根据图像语义动态重新排序视觉标记，挑战传统视觉语言模型按固定光栅扫描顺序处理图像的方式。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型在处理图像时采用固定的光栅扫描顺序（从上到下、从左到右）和固定的位置编码，这与人类视觉感知方式相矛盾。人类视觉会根据图像的内在逻辑结构采用灵活但语义连贯的扫描模式，特别是对于复杂布局的图像，人类视觉表现出基于因果关系的顺序处理。

Method: 提出DeepEncoder V2编码器，赋予编码器因果推理能力，使其能够在基于LLM的内容解释之前智能地重新排序视觉标记。探索了一种新颖范式：是否可以通过两个级联的1D因果推理结构有效实现2D图像理解。

Result: 开发了DeepSeek-OCR 2系统，代码和模型权重已在GitHub上公开提供，为2D图像理解提供了一种新的架构方法。

Conclusion: 这项工作探索了通过级联的1D因果推理结构实现2D图像理解的新范式，为达到真正的2D推理提供了有潜力的新架构方法。
...
TL;DR: DiffVC-RT是首个实现实时扩散神经视频压缩的框架，通过高效架构设计、显隐式一致性建模和异步并行解码，在保持高质量的同时大幅降低计算复杂度，达到实时处理速度。
...
Method: 提出三方面创新：1）高效信息模型架构，通过模块替换和剪枝降低计算复杂度；2）显隐式一致性建模，引入零成本在线时间偏移模块和混合隐式约束；3）异步并行解码流水线，采用混合半精度和批量维度时间偏移设计。

Result: 在HEVC数据集上，相比VTM-17.0实现了80.1%的LPIPS比特率节省，在NVIDIA H800 GPU上对720p视频达到206/30 fps的实时编码和解码速度。
...
<details>
  <summary>Details</summary>
Motivation: 连续文本-视频检索面临灾难性遗忘的严重挑战，主要由于两种特征漂移：模态内特征漂移和跨模态非协作特征漂移导致的模态不对齐。
...
Conclusion: 通过同时解决跨模态非协作特征漂移和模态内特征漂移，StructAlign有效缓解了连续文本-视频检索中的灾难性遗忘问题。
...
TL;DR: 该论文评估了三种训练范式（监督学习、自监督学习、语言对齐模型）在行人重识别任务中的表现，特别关注跨域泛化能力，发现语言对齐模型在跨域场景中表现出意外鲁棒性。
...
Method: 比较三种训练范式：监督学习、自监督学习和语言对齐模型。通过分析11个模型在9个数据集上的表现，研究监督模型在跨域场景中的泛化能力，以及基础模型（如SigLIP2）在行人重识别任务中的表现。

Result: 监督模型在训练域内表现优异，但在跨域数据上表现崩溃；语言对齐模型尽管未针对行人重识别进行专门训练，却在跨域场景中展现出令人惊讶的鲁棒性。

Conclusion: 语言对齐模型在行人重识别的跨域泛化方面具有显著优势，为未来行人重识别研究提供了新的方向，即利用基础模型的丰富视觉表示来提升模型在未见域上的表现。
...
<details>
  <summary>Details</summary>
Motivation: 当前技能评估研究主要集中于体育领域，缺乏复杂程序性活动的大规模数据集，现有方法通常只涉及有限动作数量，且仅关注成对评估或二元标签，无法满足程序性视频技能评估的需求。

Method: 提出新颖且可扩展的标注协议：使用瑞士锦标赛方案进行高效的成对比较，然后通过ELO评分系统将这些成对评估聚合成一致、连续的全局技能分数，从而创建绝对技能评估排名。

Result: 创建了ProSkill基准数据集，并用该数据集对现有最先进的技能评估算法进行基准测试，包括基于排名和成对评估的范式，结果显示当前最先进方法的性能不理想。

Conclusion: ProSkill数据集为程序性视频技能评估提供了有价值的基准资源，现有方法的次优结果突显了该领域的挑战性，也证明了ProSkill在推动技能评估研究方面的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法在自然图像上表现良好，但在遥感图像上存在挑战：多标签场景、类内高变异性、空间分辨率多样，导致现有方法难以识别主导语义线索，在新类别上泛化能力差

Method: 使用冻结的图像描述模型（如BLIP-2）从遥感图像提取文本语义摘要，通过BERT分词器进行标记化，与CLIP编码器的高级视觉特征融合，轻量级交叉注意力模块基于融合的文本-视觉表示调节可学习查询提示

Result: 在四个遥感数据集上的三个域泛化任务中评估，BiMoRS表现出稳定的性能提升，平均比强基线高出2%

Conclusion: BiMoRS通过双模态提示学习有效解决了遥感图像的特殊挑战，在不改变CLIP主干的情况下实现了更好的跨域泛化性能
...
TL;DR: LEAF框架通过从MLLM教师模型蒸馏感知质量先验到轻量级学生回归器，显著减少图像质量评估所需的人工标注，同时保持与人类评价的高度相关性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在图像质量评估任务中表现出色，但适应这些大规模模型计算成本高，且仍需大量平均意见分数标注。作者认为MLLM-based IQA的核心瓶颈不在于MLLMs的质量感知能力，而在于MOS尺度校准。

Method: 提出LEAF框架，从MLLM教师模型蒸馏感知质量先验到轻量级学生回归器。教师通过点级判断和成对偏好进行密集监督，并估计决策可靠性。学生通过联合蒸馏学习教师的感知模式，并在少量MOS子集上进行校准以对齐人类标注。

Result: 在用户生成和AI生成的IQA基准测试中，该方法显著减少了对人类标注的需求，同时保持了与MOS的高度相关性，使轻量级IQA在有限标注预算下变得实用。

Conclusion: LEAF框架通过高效利用MLLM教师的感知能力，解决了MLLM-based IQA中的MOS尺度校准瓶颈，为有限标注预算下的轻量级图像质量评估提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉、音频和语言任务上取得了显著进展，但在长形式、知识密集且具有时间结构的教育内容上的表现尚未得到充分探索。为了填补这一空白，需要建立一个专门的评估基准来测试模型在复杂教育场景下的多模态理解能力。

Method: 研究者构建了LEMON基准，包含2,277个STEM讲座视频片段，涵盖5个学科和29门课程，平均时长196.1秒。基准包含4,181个高质量QA对（3,413个选择题和768个开放性问题），具有语义丰富性、学科密度高、视频-音频-文本模态紧密耦合、明确的时间教学结构以及上下文关联的多轮问答等特点。基准包含6个主要任务和12个子任务，覆盖从感知到推理再到生成的完整认知谱系。

Result: 综合实验揭示了各任务间存在显著的性能差距，即使是像GPT-4o这样的最先进MLLMs也在时间推理和教学预测方面表现不佳。基准展示了当前模型在处理长形式教学内容中的多模态理解挑战。

Conclusion: LEMON作为一个可扩展且具有挑战性的基准，有望推动多模态感知、推理和生成在长形式教学内容中的发展，帮助研究者更好地评估和改进模型在教育领域的应用能力。
...
TL;DR: 本文探讨了压缩与智能的关系，系统分析了视觉编码和视觉token技术两大技术体系，提出了统一优化框架，并展望了下一代视觉编解码和token技术的发展方向。


<details>
  <summary>Details</summary>
Motivation: 压缩效率与模型性能之间存在相关性，传统视觉编码技术已成熟，而新兴的视觉token技术与视觉编码有相似目标——在表示学习中最大化语义信息保真度同时最小化计算成本。需要统一理解这两种技术。

Method: 1. 全面概述视觉编码和视觉token技术两大技术家族；2. 从优化角度统一这两种技术，讨论压缩效率与模型性能权衡的本质；3. 基于提出的统一公式，综合双向见解并预测下一代技术；4. 通过实验展示任务导向token在MLLMs、AIGC和具身AI等实际任务中的潜力。

Result: 提出了连接视觉编码和视觉token技术的统一框架，展示了任务导向token在多种智能任务中的巨大潜力，并展望了未来可能像传统编解码器（如H.264/265）一样标准化通用token技术的前景。

Conclusion: 压缩与智能密切相关，视觉编码和视觉token技术本质上是相通的。通过统一优化框架可以更好地理解压缩效率与模型性能的权衡，未来有望发展出像传统编解码器一样高效、标准化的通用token技术，服务于广泛的智能任务。
...
TL;DR: FairT2V是一个无需训练的文本到视频生成去偏框架，通过锚点球面测地变换中和提示嵌入中的性别偏见，同时保持语义和时间连贯性。


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型存在未被充分探索的人口统计偏见，特别是性别偏见，这些偏见主要来自预训练文本编码器对中性提示的隐式性别关联编码。

Method: 1) 分析T2V模型的人口统计偏见，发现主要源于预训练文本编码器；2) 提出性别倾向评分量化偏见；3) 通过锚点球面测地变换中和提示嵌入；4) 使用动态去噪调度仅在早期身份形成步骤应用去偏以保持时间连贯性。

Result: 在Open-Sora模型上的实验表明，FairT2V显著减少了跨职业的人口统计偏见，同时对视频质量影响最小。

Conclusion: FairT2V提供了一个有效的训练免费去偏框架，能够显著减少文本到视频生成中的性别偏见，同时保持生成质量和时间连贯性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在生成3D人-场景交互时缺乏对物体功能和相应人-场景接触的显式推理，导致交互不真实或功能不正确。需要解决在体现AI、机器人和交互内容创建中生成功能正确3D人-场景交互的问题。

Method: FunHSI采用训练免费的功能驱动框架：1）通过功能感知接触推理识别功能场景元素、重建3D几何并通过接触图建模高级交互；2）利用视觉语言模型合成任务图像并估计3D身体和手部姿势；3）通过分阶段优化细化3D身体配置以确保物理合理性和功能正确性。

Result: FunHSI不仅能够合成更合理的通用3D交互（如"坐在沙发上"），还支持细粒度的功能人-场景交互（如"提高房间温度"）。在多样室内外场景中一致生成功能正确且物理合理的人-场景交互。

Conclusion: FunHSI通过功能驱动的接触推理和优化方法，成功解决了生成功能正确3D人-场景交互的挑战，在体现AI、机器人和交互内容创建中具有重要应用价值。
...
TL;DR: 提出了一种融合图像和惯性测量的多模态道路表面分类框架，通过双向交叉注意力模块和自适应门控层处理域偏移，并发布了包含真实多模态数据、视觉子集和合成数据的新数据集ROAD。
...
Method: 引入多模态框架，融合图像和惯性测量，使用轻量级双向交叉注意力模块，后接自适应门控层来调整域偏移下的模态贡献。同时创建了包含真实多模态记录、大规模视觉子集和合成数据的新数据集ROAD。

Result: 在PVS基准上比先前最优方法提升1.4个百分点，在多模态ROAD子集上提升11.6个百分点，在少数类别上获得更高的F1分数，在夜间、大雨和混合表面转换等挑战性视觉条件下表现稳定。
...
TL;DR: FreeFix：一种无需微调的2D-3D交替优化方法，利用预训练图像扩散模型提升神经辐射场和3D高斯泼溅的外推视图质量，通过像素级置信度掩码实现针对性改进。


<details>
  <summary>Details</summary>
Motivation: 神经辐射场和3D高斯泼溅虽然推进了新视图合成，但仍依赖密集输入且在外推视图上表现不佳。现有基于生成模型的方法面临泛化性与保真度的权衡：微调扩散模型提升保真度但可能过拟合，而不微调的方法保持泛化性但保真度较低。

Method: 提出FreeFix方法：1）采用交替的2D-3D优化策略，利用预训练图像扩散模型进行一致性优化，无需昂贵的视频扩散模型；2）引入像素级置信度掩码识别不确定区域进行针对性改进。

Result: 在多个数据集上的实验表明，FreeFix提升了多帧一致性，性能达到或超过基于微调的方法，同时保持了强大的泛化能力。

Conclusion: FreeFix在无需微调的情况下，通过创新的2D-3D交替优化和像素级置信度掩码，成功平衡了外推视图渲染的泛化性与保真度，为基于生成模型的新视图合成提供了有效解决方案。
