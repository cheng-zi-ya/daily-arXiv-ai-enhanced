...
TL;DR: 提出两步生成式数据增强框架，结合规则掩码变形与GAN图像转换，生成逼真口罩人脸样本，提升口罩人脸检测与识别性能


<details>
  <summary>Details</summary>
Motivation: 数据稀缺和分布偏移是口罩人脸检测与识别的主要挑战，需要超越纯合成变换的逼真数据增强方法

Method: 两步生成式数据增强框架：1) 规则掩码变形；2) 基于GAN的无配对图像到图像转换，引入非掩码保留损失和随机噪声注入以稳定训练并增强样本多样性

Result: 相比单纯规则变形，该方法在定性上获得一致改进，补充了现有GAN方法如IAMGAN，实验验证了所提组件的有效性

Conclusion: 该框架为面向人脸识别任务的数据中心化增强提供了有效方向，指出了未来改进的路径
...
TL;DR: JARVIS是一个JEPA启发的自监督视觉增强框架，用于提升多模态大语言模型在视觉推理任务中的表现，通过整合I-JEPA学习范式来学习图像的结构和语义规律。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在基础视觉推理任务上能力有限，主要因为：1）模型主要从文本描述学习视觉理解，这是主观且不完整的监督信号；2）多模态指令调优规模远小于纯文本预训练，导致模型过度依赖语言先验而忽略视觉细节。

Method: 提出JARVIS框架，将I-JEPA学习范式整合到MLLM的标准视觉-语言对齐流程中。使用冻结的视觉基础模型作为上下文和目标编码器，训练预测器（实现为LLM的早期层）从图像中学习结构和语义规律，而不完全依赖语言监督。

Result: 在标准MLLM基准测试上的广泛实验表明，JARVIS在不同LLM家族上都能持续提升视觉中心基准的性能，且不会降低多模态推理能力。

Conclusion: JARVIS框架通过自监督视觉增强有效解决了MLLMs在视觉推理中的局限性，为提升多模态模型的视觉理解能力提供了新思路。
...
TL;DR: 该论文提出了稀疏基础视觉导航任务和CityNav基准，用于评估多模态大语言模型在真实城市导航中的顺序决策能力，并提出了路径言语化方法来提升导航成功率。


<details>
  <summary>Details</summary>
Motivation: 当前评估基准主要关注语言中心或模拟环境，缺乏对实际复杂场景中知识密集型推理能力的测试，需要填补这一空白来评估MLLM在真实世界环境中的顺序决策能力。

Method: 提出了稀疏基础视觉导航任务，并构建了CityNav基准，涵盖四个全球城市，要求智能体仅依赖视觉输入和内部多模态推理进行导航。同时提出了路径言语化方法，通过从MLLM中提取显式认知地图来增强导航能力。

Result: 评估显示当前最先进的MLLM和标准推理技术在CityNav基准上表现显著不足，而提出的路径言语化方法能大幅提升导航成功率。

Conclusion: 稀疏基础视觉导航是一个具有挑战性的任务，现有MLLM在该领域表现有限，但通过显式认知地图的路径言语化方法可以有效改善智能体的导航性能。
...
TL;DR: R4是一个无需训练的检索增强推理框架，为视觉语言模型提供结构化、终身记忆的4D时空知识库，通过将对象级语义描述锚定在度量空间和时间中，支持在动态环境中的4D推理。


<details>
  <summary>Details</summary>
Motivation: 受人类在四维时空感知和推理能力的启发，希望为视觉语言模型构建类似人类的结构化内部表示，使其能够编码语义、空间布局和时间动态，从而支持回忆过去事件、推断未观察状态和上下文相关推理。

Method: R4框架持续构建4D知识数据库，将对象级语义描述锚定在度量空间和时间中，形成持久的世界模型。推理时将自然语言查询分解为语义、空间和时间键来检索相关观察，并将这些信息集成到视觉语言模型的推理过程中。

Result: 在具身问答和导航基准测试中，R4在时空信息检索和推理方面显著优于基线方法，展示了在动态环境中进行4D推理的新范式。

Conclusion: R4框架通过构建结构化、终身记忆的4D知识库，为视觉语言模型提供了无需训练的检索增强推理能力，推动了动态环境中具身4D推理的新范式发展。
...
TL;DR: 该论文提出了"感知观测台"框架，用于评估多模态大语言模型的感知能力，超越传统的端任务准确性评估，关注模型在扰动下的鲁棒性、归因保真度和推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然能力不断增强，但其感知能力缺乏系统评估。大多数模型家族在扩展语言组件时重用几乎相同的视觉编码器，这引发了一个关键问题：模型的进步是真正基于视觉基础，还是主要依赖互联网规模的文本世界知识。现有评估方法过于强调端任务准确性，忽略了鲁棒性、归因保真度和受控扰动下的推理能力。

Method: 提出了"感知观测台"框架，从多个垂直维度评估MLLMs：(1)简单视觉任务，如人脸匹配和视觉文本理解能力；(2)局部到全局理解，包括图像匹配、网格指向游戏和属性定位，测试一般视觉基础。每个垂直维度都使用真实数据集（人脸和文字），通过基于像素的增强和基于扩散的风格化幻觉进行系统性扰动。

Result: 该框架超越了排行榜准确性评估，能够深入分析MLLMs在扰动下如何保持感知基础和关系结构，为分析当前和未来模型的优势和弱点提供了原则性基础。

Conclusion: "感知观测台"框架为系统评估多模态大语言模型的感知能力提供了新方法，能够揭示模型在视觉基础方面的真实能力，而不仅仅是依赖文本知识，为模型开发和评估提供了更全面的视角。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单人在第一人称视角下的行为预测，但许多机器人应用需要从第三人称视角理解多人的行为，目前缺乏适合的数据集和方法来解决这一问题。
...
Conclusion: CAMP-VLM能够有效预测多人在场景中的行为，为机器人在人群环境中的操作提供了重要技术支持，特别是在缺乏真实数据集的情况下，合成数据训练显示出良好的效果。
...
TL;DR: 该论文探索了基于视觉语言模型（VLMs）的少样本多光谱目标检测方法，通过将Grounding DINO和YOLO-World适配到多光谱输入，在FLIR和M3FD基准测试中显著优于专门的多光谱模型。


<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测在自动驾驶和监控等安全关键应用中至关重要，但标注数据的稀缺限制了深度检测器的训练。文本类别信息可以作为有价值的语义监督源，而视觉语言模型在计算机视觉中的成功激发了将其应用于少样本多光谱目标检测的探索。

Method: 将两种代表性的VLM检测器（Grounding DINO和YOLO-World）适配到处理多光谱输入，并提出有效的机制来整合文本、视觉和热模态。通过文本模态提供语义监督，弥补多光谱标注数据的不足。

Result: 在FLIR和M3FD两个流行的多光谱图像基准测试中，VLM检测器在少样本场景下表现出色，显著优于使用可比数据训练的专门多光谱模型，在完全监督设置下也达到竞争性或更优的结果。

Conclusion: 大规模视觉语言模型学习的语义先验知识可以有效地迁移到未见的光谱模态，为数据高效的多光谱感知提供了强大的途径。这表明VLM在多光谱目标检测中具有重要潜力。
...
TL;DR: 本文评估了多种视觉语言模型在农业分类任务上的表现，发现零样本VLM显著落后于监督学习的YOLO11基线，当前现成的VLM尚不适合作为独立的农业诊断系统，但可作为辅助组件使用。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地被提议作为视觉识别任务的通用解决方案，但它们在农业决策支持中的可靠性仍不清楚。本文旨在评估各种开源和闭源VLM在农业分类任务上的表现。

Method: 在AgML集合的27个农业分类数据集上对多样化的开源和闭源VLM进行基准测试，涵盖162个类别，包括植物病害、害虫和损害、植物和杂草物种识别。使用零样本评估、多项选择提示和开放式提示等不同评估方法。

Result: 零样本VLM在所有任务中都显著落后于监督学习的YOLO11基线。最佳VLM（Gemini-3 Pro）在多项选择提示下达到约62%的平均准确率，而开放式提示的性能要低得多（通常低于25%）。使用基于LLM的语义判断提高了开放式准确率，并改变了模型排名。植物和杂草物种分类比害虫和损害识别更容易。

Conclusion: 当前现成的VLM还不适合作为独立的农业诊断系统，但可以作为辅助组件，配合约束界面、明确的标签本体和领域感知的评估策略使用。
...
TL;DR: 提出一种从初始图像和机器人关节状态生成遵循文本指令的视频-动作对的方法，通过自动提供动作标签解决视频扩散模型缺乏动作标注的问题，用于机器人策略学习。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：两阶段流水线限制了跨模态信息共享，或者依赖单模态扩散模型适应联合分布而无法充分利用预训练视频知识。视频数据缺乏动作标注也限制了其在机器人学习中的应用。
...
TL;DR: 提出3D多通道patch扩散模型，从单壳低角分辨率dMRI预测多壳高角分辨率FOD，通过FOD-patch适配器、体素级条件协调模块和SH注意力模块提升性能


<details>
  <summary>Details</summary>
Motivation: 单壳低角分辨率dMRI估计FOD精度有限，而多壳高角分辨率dMRI需要长扫描时间限制应用。扩散模型在从LAR-FOD估计HAR-FOD方面有潜力，但FOD中大量球谐系数使高效生成具有挑战

Method: 提出3D多通道patch扩散模型，设计FOD-patch适配器引入先验脑解剖知识实现更高效的基于patch的学习，引入体素级条件协调模块增强模型全局理解，设计SH注意力模块有效学习SH系数的复杂相关性

Result: 实验结果显示该方法在HAR-FOD预测中取得最佳性能，优于其他最先进方法

Conclusion: 提出的3D多通道patch扩散模型能够有效从LAR-FOD预测HAR-FOD，通过创新的架构设计解决了FOD中大量SH系数带来的挑战
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D目标检测方法虽然在训练期间能够检测未见过的类别，但仍需在训练和推理时依赖用户指定的类别。本文旨在研究真正的自动词汇3D目标检测，即无需任何用户输入即可自动生成检测物体的类别名称。

Method: 提出了一个新颖的AV3DOD框架，利用2D视觉语言模型通过图像描述生成丰富的语义候选，包括伪3D框生成和特征空间语义扩展。同时引入了语义评分（SS）来评估生成类别名称的质量。

Result: 在ScanNetV2和SUNRGB-D数据集上，AV3DOD在定位精度（mAP）和语义质量（SS）方面均达到最先进水平。在ScanNetV2上，相比当前最佳方法CoDA提升了3.48 mAP，并在SS上获得了24.5%的相对改进。

Conclusion: AV3DOD框架成功实现了真正的自动词汇3D目标检测，无需用户输入即可自动生成高质量的物体类别名称，在定位精度和语义质量方面均超越了现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机作为新型脑启发视觉传感器具有高动态范围和高时间分辨率等优势，但在长距离测量场景下的几何标定（包括内参和外参确定）仍然是一个重大挑战
...
Conclusion: 提出的基于准直仪闪烁星点图案的事件相机标定方法能够有效解决长距离高精度测量需求，为事件相机在相关应用场景中的实际部署提供了可靠的技术支持
...
TL;DR: TurboDiffusion是一个视频生成加速框架，通过多种技术手段将端到端扩散生成速度提升100-200倍，同时保持视频质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型计算量大、速度慢，需要高效的加速框架来提升实际应用中的生成效率。

Method: 采用三种主要技术：1) 注意力加速：使用低比特SageAttention和可训练的稀疏线性注意力；2) 步数蒸馏：采用rCM进行高效步数蒸馏；3) W8A8量化：将模型参数和激活量化为8位以加速线性层和压缩模型。此外还包括其他工程优化。

Result: 在多个视频生成模型上测试，TurboDiffusion在单张RTX 5090 GPU上实现了100-200倍的加速，同时保持了可比的视频质量。

Conclusion: TurboDiffusion是一个有效的视频生成加速框架，通过综合运用多种优化技术，显著提升了扩散模型的生成速度，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 相机标定是摄影测量和3D视觉应用中的关键步骤，需要更可靠、可控的标定环境。传统方法存在环境依赖性强、标定过程复杂等问题，需要更灵活快速的标定解决方案。

Method: 设计准直仪系统提供可靠可控的标定环境；利用准直仪系统的光学几何特性，引入角度不变性约束，证明标定目标与相机之间的相对运动符合球面运动模型；提出基于球面运动约束的多图像闭式线性求解器和两图像最小求解器；开发基于角度不变性约束的单准直仪图像标定算法。

Result: 在合成和真实世界实验中验证了准直仪系统标定的可行性，证明该方法优于现有基线方法。单图像标定算法消除了对相机运动的要求，提供了灵活快速的标定解决方案。

Conclusion: 基于准直仪系统的相机标定方法通过角度不变性约束简化了标定过程，实现了无需相机运动的单图像标定，为相机标定提供了更灵活、快速、可靠的解决方案。
...
TL;DR: C-DGPA提出了一种基于类别中心的双对齐生成提示适配方法，通过双分支架构同时优化边缘分布对齐和条件分布对齐，解决无监督域自适应中VLMs提示调优的领域差异问题。


<details>
  <summary>Details</summary>
Motivation: 在无监督域自适应任务中直接应用视觉语言模型进行提示调优面临显著挑战，现有方法主要关注边缘分布对齐而忽略了条件分布差异，导致类别原型错位和语义判别性下降等问题。

Method: 提出C-DGPA框架，包含两个分支：1）边缘分布对齐分支采用动态对抗训练框架弥合边缘分布差异；2）条件分布对齐分支引入类别映射机制，通过标准化语义提示理解和防止源域过度依赖来对齐条件分布差异。

Result: 在OfficeHome、Office31和VisDA-2017三个基准数据集上的广泛实验验证了C-DGPA的优越性，在所有基准测试中都取得了新的最先进结果。

Conclusion: C-DGPA通过双对齐策略将领域知识有效整合到提示学习中，确保了领域不变且语义可判别的表示，显著提升了无监督域自适应中视觉语言模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统相机在自动驾驶中存在领域差距问题，当训练数据条件与部署环境不匹配时性能大幅下降，特别是昼夜光照差异带来的挑战

Method: 提出使用事件相机作为传统相机的替代方案，研究其在昼夜光照差异领域差距下的性能表现

Result: 事件相机在不同光照条件下保持更一致的性能，其领域转移惩罚通常与灰度帧相当或更小，在跨领域场景中提供更优的基线性能

Conclusion: 事件相机是应对自动驾驶中昼夜光照领域差距的有效替代方案，能够在不需额外调整的情况下保持跨领域性能一致性
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注日常通用运动，灵活性有限，难以满足特定领域（如体育）对特定人类动作和运动模式的独特需求。需要一种能够生成可定制、可扩展且无需人工标注的合成数据集的方法。

Method: 开发了Avatar4D管道，能够生成高保真的4D（随时间变化的3D几何）人类运动序列，包含变化的玩家外观和多样化环境渲染。在此基础上创建了Syn2Sport大规模合成数据集，涵盖棒球和冰球等体育运动。

Result: 在Syn2Sport上评估了多种最先进的姿态估计模型，证明了其在监督学习、零样本迁移到真实数据以及跨体育泛化方面的有效性。同时评估了生成合成数据与真实数据集在特征空间的对齐程度。

Conclusion: Avatar4D展示了无需依赖特定领域真实数据，就能生成可扩展、可控且可迁移的人类数据集，为多样化特定领域任务提供了潜力。
...
TL;DR: VALOR提出了一种基于强化学习的医学视觉语言模型后对齐框架，通过GRPO方法改进放射学报告生成，提升临床准确性和视觉基础性


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告生成面临视觉基础性和临床准确性不足的挑战，现有方法依赖大量标注数据、昂贵的任务特定偏好数据或检索方法，未能有效缓解视觉与语言表示之间跨模态对齐不良导致的幻觉问题

Method: 提出VALOR方法，采用基于强化学习的后对齐框架，使用组相对近端优化(GRPO)。训练分两阶段：1)使用文本奖励改进Med-VLM以鼓励临床精确术语；2)将文本基础模型的视觉投影模块与疾病发现对齐，引导注意力到与诊断任务最相关的图像区域

Result: 在多个基准测试上的广泛实验表明，VALOR显著提高了事实准确性和视觉基础性，在报告生成方法上取得了显著的性能提升

Conclusion: VALOR通过强化学习后对齐框架有效解决了医学视觉语言模型在放射学报告生成中的视觉基础性和临床准确性问题，为自动化医疗工作流程提供了更可靠的解决方案
...
TL;DR: OAK模型通过引入可学习的上下文标记和结合CLIP的图像-文本对齐目标与GCD的视觉聚类目标，实现了开放ad-hoc分类的SOTA性能，在Stanford Mood数据集上达到87.4%的新类别准确率。


<details>
  <summary>Details</summary>
Motivation: AI智能体需要自适应地对视觉场景进行分类以应对变化的任务。与固定的常见类别（如植物或动物）不同，ad-hoc类别是动态创建的，用于服务特定目标。研究开放ad-hoc分类问题：给定少量标记样本和大量未标记数据，目标是发现底层上下文并通过语义扩展和视觉聚类来扩展ad-hoc类别。

Method: 基于ad-hoc和常见类别依赖相似感知机制的洞察，提出OAK模型：在冻结的CLIP模型输入处引入少量可学习的上下文标记，同时优化CLIP的图像-文本对齐目标和GCD的视觉聚类目标。

Result: 在Stanford和Clevr-4数据集上，OAK在多个分类任务中实现了最先进的准确率和概念发现性能，在Stanford Mood数据集上达到87.4%的新类别准确率，比CLIP和GCD高出50%以上。此外，OAK生成可解释的显著性图，针对动作关注手部、针对情绪关注面部、针对位置关注背景。

Conclusion: OAK模型通过结合语义扩展和视觉聚类，实现了自适应和可泛化的分类，同时提供可解释的显著性图，增强了透明度和可信度，为开放ad-hoc分类提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于信息几何的3D点云形状分析方法，通过将点云表示为统计流形上的高斯混合模型，并引入具有理论上界和下界的修正对称KL散度，解决了传统几何度量在点云比较中的局限性。
...
Method: 提出信息几何框架，将点云表示为统计流形上的高斯混合模型；证明GMM空间构成统计流形；提出修正对称KL散度，具有理论上界和下界保证，确保所有GMM比较的数值稳定性。

Result: 在人体姿态识别（MPI-FAUST数据集）和动物形状比较（G-PCD数据集）上的综合实验表明，MSKL提供稳定且单调变化的数值，直接反映几何变化，优于传统距离和现有KL近似方法。

Conclusion: 提出的信息几何框架和修正对称KL散度为3D点云形状分析提供了数值稳定、理论保证的方法，能够有效捕捉点云的全局统计结构，克服了传统几何度量的局限性。
...
TL;DR: SVD和低秩矩阵近似用于图像压缩，但性能不如JPEG等标准格式，在低误差下压缩文件甚至可能比原图更大
...
Result: 低秩近似通常能产生视觉上接近原图的图像，但在相同误差水平下，压缩效率始终比JPEG、JPEG2000和WEBP等标准格式差。在低容忍误差下，SVD压缩表示甚至可能超过原图大小。
...
TL;DR: ARMFlow是一个基于MeanFlow的自回归框架，用于3D人体反应生成，解决了高运动保真度、实时推理和在线场景自回归适应性三大挑战，在单步在线生成中性能超越现有方法40%以上。
...
Method: 提出ARMFlow框架，基于MeanFlow的自回归架构，包含因果上下文编码器和基于MLP的速度预测器。引入Bootstrap Contextual Encoding (BSCE)训练方法，编码生成的历史而非真实历史，减轻自回归生成中的误差累积。还提出离线变体ReMFlow。

Result: 单步在线生成在InterHuman和InterX数据集上超越现有在线方法40%以上（FID指标），同时匹配离线SOTA性能，尽管只使用部分序列条件。离线变体ReMFlow在离线方法中达到最快推理速度。
...
<details>
  <summary>Details</summary>
Motivation: 多视角人群计数可解决大范围场景中的严重遮挡问题，但多视角数据收集和标注困难，现有数据集视角帧和场景数量有限。需要解决数据有限的问题，避免依赖大量标注数据

Method: 提出两种半监督多视角计数框架：1) 基于预测排序的朴素模型：对输入不同数量相机视角的多视角融合模型预测结果进行排序，较少视角的预测不应多于较多视角的预测；2) 基于不确定性排序：对可变视角输入的多视角融合模型估计的不确定性进行排序，较多视角的模型不确定性不应大于较少视角的，并受多视角融合模型预测误差指导

Result: 实验证明所提出的多视角模型排序方法相比其他半监督计数方法具有优势

Conclusion: 通过引入基于预测和不确定性的排序约束，在有限标注数据下实现了有效的半监督多视角人群计数，为解决多视角数据稀缺问题提供了新思路
...
TL;DR: TextEditBench：首个专注于图像中文本区域编辑的评估基准，强调推理密集型编辑场景，提出语义期望新评估维度，揭示当前模型在上下文推理和物理一致性方面的不足


<details>
  <summary>Details</summary>
Motivation: 文本渲染已成为视觉生成中最具挑战性的前沿领域，但图像中的文本编辑仍未被充分探索，因为需要在生成清晰字符的同时保持语义、几何和上下文的一致性。现有研究缺乏专门针对文本中心区域编辑的评估基准。

Method: 提出了TextEditBench，一个专注于图像中文本中心区域的综合评估基准。除了基本的像素操作外，强调需要理解物理合理性、语言意义和跨模态依赖的推理密集型编辑场景。引入了新的评估维度——语义期望（SE），用于衡量模型在文本编辑过程中保持语义一致性、上下文连贯性和跨模态对齐的推理能力。

Result: 对最先进的编辑系统进行广泛实验发现，虽然当前模型能够遵循简单的文本指令，但在上下文依赖推理、物理一致性和布局感知集成方面仍然存在困难。TextEditBench为评估文本引导的图像编辑和推理能力提供了新的测试平台。

Conclusion: TextEditBench填补了文本编辑评估的空白，建立了一个新的测试平台，专注于这一长期被忽视但基本的能力，将推动文本引导图像编辑和多模态生成中推理能力的发展。
...
TL;DR: MACL提出多标签自适应对比学习框架，通过标签感知采样、频率敏感加权和动态温度缩放，解决遥感图像检索中语义重叠、标签分布不平衡和类别共现模式复杂的问题。


<details>
  <summary>Details</summary>
Motivation: 遥感图像多标签检索面临三大挑战：1）土地覆盖类别间的语义重叠；2）高度不平衡的标签分布；3）复杂的类别间共现模式。这些挑战使得传统对比学习方法难以获得平衡且可靠的表示学习。

Method: 提出多标签自适应对比学习（MACL）框架，包含三个关键技术：1）标签感知采样策略，考虑类别语义关系；2）频率敏感加权机制，平衡常见和稀有类别；3）动态温度缩放，适应不同类别的表示学习难度。

Result: 在DLRSD、ML-AID和WHDLD三个基准数据集上的实验表明，MACL持续优于基于对比损失的基线方法，有效缓解语义不平衡问题，在大规模遥感档案中提供更可靠的检索性能。

Conclusion: MACL通过自适应对比学习框架成功解决了遥感图像多标签检索中的语义不平衡问题，为大规模遥感档案的可靠检索提供了有效解决方案。
...
Method: 提出PixelArena基准，使用语义分割任务来客观检验多模态大语言模型的细粒度生成能力。通过零样本设置评估模型生成语义掩码的精度，并进行定性和定量比较分析。
...
TL;DR: QUIDS系统通过质量感知激励机制，在预算约束下优化非专用车载移动群智感知系统的信息质量，提升覆盖度和可靠性


<details>
  <summary>Details</summary>
Motivation: 解决非专用车载移动群智感知系统中传感覆盖度、传感可靠性和车辆动态参与度相互关联的挑战，在预算约束下实现最优信息质量

Method: 提出QUIDS系统，引入聚合传感质量指标整合覆盖度和可靠性，开发相互辅助信念感知车辆调度算法估计传感可靠性并在不确定性下分配激励

Result: 在真实世界大都市部署数据评估中，QUIDS比无调度场景提升ASQ 38%，比最先进方法提升10%，重建地图误差降低39-74%
...
<details>
  <summary>Details</summary>
Motivation: 传统部署中，边缘设备将视觉数据传输到服务器进行VLM推理，但将原始图像调整为视觉编码器输入分辨率时会丢失细粒度细节，导致精度下降。需要一种既能保持精度又能减少通信成本的方法。

Method: 设计了两阶段框架：第一阶段服务器在全局图像上进行推理，利用VLM内部注意力识别感兴趣区域，计算输出令牌的最小熵作为置信度度量；如果最小熵超过预设阈值，则请求边缘设备发送RoI的细节保留局部图像；第二阶段服务器联合利用全局和局部图像进行推理精炼。

Result: 在多种VLM架构上的实验表明，该框架显著降低了通信成本，同时保持了推理精度。

Conclusion: 提出的协作边缘到服务器推理框架通过选择性重传策略，在保持VLM推理精度的同时有效减少了通信开销，为边缘计算环境下的视觉语言模型部署提供了实用解决方案。
...
TL;DR: GMODiff是一个基于增益图的一步扩散框架，用于多曝光HDR重建，通过将HDR重建重新定义为条件引导的增益图估计任务，解决了传统LDM在HDR应用中的动态范围限制、推理成本高和内容幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 预训练的潜在扩散模型(LDMs)在低层视觉任务中显示出强大的感知先验，但直接应用于HDR重建面临三个主要挑战：8位潜在压缩导致的动态范围有限、多步去噪的高推理成本以及生成性质固有的内容幻觉问题。

Method: 提出GMODiff框架，将HDR重建重新定义为条件引导的增益图估计任务，增益图编码扩展的动态范围同时保持与LDR图像相同的位深度。从基于回归的信息估计而非纯噪声初始化去噪过程，实现单步生成高质量增益图。利用回归先验引导LDM的去噪过程和潜在解码，抑制幻觉同时保持结构准确性。

Result: 大量实验表明，GMODiff在性能上优于多个最先进方法，并且比之前的LDM-based方法快100倍。

Conclusion: GMODiff通过增益图驱动的一步扩散框架，有效解决了传统LDM在HDR重建中的动态范围限制、计算成本和内容幻觉问题，实现了高效且高质量的HDR重建。
...
<details>
  <summary>Details</summary>
Motivation: 单角色姿态驱动动画已取得显著进展，但扩展到多角色场景（特别是涉及位置交换时）面临挑战。核心问题在于确保参考帧和生成帧之间角色的正确身份对应关系。

Method: 提出身份匹配图将生成帧和参考帧中的角色建模为加权完全二分图的两个节点集，使用掩码查询注意力计算边权重。将身份对应正确性形式化为图结构度量并在训练中优化。还提出了身份嵌入引导、多尺度匹配策略和预分类采样等针对性策略。

Result: 构建了身份对应评估基准，专门用于多角色身份对应正确性评估。大量实验表明，EverybodyDance在身份对应和视觉保真度方面显著优于现有最先进基线方法。

Conclusion: EverybodyDance为多角色动画中的身份对应问题提供了系统性解决方案，通过图结构建模和针对性优化策略，有效解决了多角色位置交换时的身份一致性挑战。
...
TL;DR: FVG通过将文本到视频生成分解为推理、构图和时序合成三个阶段，解决了现有T2V模型在复杂场景构图和逻辑时序指令上的问题，显著提升了视频生成质量和效率。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的文本到视频扩散模型虽然能生成视觉上令人印象深刻的结果，但经常在构建复杂场景或遵循逻辑时序指令时失败。研究发现许多错误（包括明显的运动失败）源于模型无法构建语义正确或逻辑一致的初始帧。

Method: 提出因子化视频生成（FVG）管道，将文本到视频生成分解为三个专门阶段：1）推理阶段：使用大语言模型重写视频提示，仅描述初始场景，解决时间歧义；2）构图阶段：使用文本到图像模型从新提示合成高质量、构图正确的锚定帧；3）时序合成阶段：使用经过微调理解该锚定帧的视频模型，专注于场景动画和提示遵循。

Result: FVG在T2V CompBench基准测试中创造了新的最先进水平，在VBench2上显著提升了所有测试模型的性能。视觉锚定技术使得采样步骤减少70%而性能不损失，大幅提升了采样速度。

Conclusion: 因子化视频生成为更高效、鲁棒和可控的视频合成提供了一条简单而实用的路径，通过任务分解解决了现有T2V模型的核心限制。
...
TL;DR: 利用3D神经表示从少量未标定人脸图像构建统一解释，通过高斯泼溅和语义分割重建中性姿态，结合三角网格约束实现结构化重建，并将高斯泼溅转换为可视图相关神经纹理，实现标准图形管线兼容。


<details>
  <summary>Details</summary>
Motivation: 解决从少量未标定人脸图像中重建3D模型的问题，传统方法需要大量数据或视频序列，且难以与标准图形管线兼容。需要一种既能保持高视觉保真度，又能直接用于标准渲染流程的解决方案。

Method: 1. 使用高斯泼溅而非NeRF，因其更显式且易于约束；2. 利用语义分割标注对齐人脸区域，仅需11张图像即可重建中性姿态；3. 将高斯泼溅软约束到三角网格表面，实现结构化重建；4. 将高斯泼溅转换为视图相关的神经纹理，兼容标准图形管线；5. 使用可重光照高斯模型分离纹理和光照，获得高分辨率反照率纹理。

Result: 成功从仅11张未标定图像重建出高质量3D人脸模型，实现了结构化高斯泼溅重建，可将高斯泼溅转换为标准图形管线可用的神经纹理，获得可重光照的高分辨率反照率纹理，并展示了在文本驱动资产创建流程中的应用效果。

Conclusion: 该方法实现了从少量未标定图像到标准图形管线兼容3D资产的完整流程，结合了高斯泼溅的高视觉保真度和传统图形管线的实用性，为3D内容创建提供了灵活高效的解决方案。
...
Method: 采用两阶段训练：1)跨模态对齐预训练：通过自适应UV采样将Brep转换为图表示，设计分层BrepEncoder提取几何和拓扑特征，通过对比学习对齐全局token与CLIP文本嵌入；2)多阶段LLM微调：将预训练的BrepEncoder集成到LLM中，通过三阶段渐进训练策略对齐节点token序列。

Result: BrepLLM在3D物体分类和描述任务上取得了最先进的性能，并构建了包含269,444个Brep-文本问答对的数据集Brep2Text。

Conclusion: BrepLLM成功弥合了3D几何与自然语言之间的模态鸿沟，使LLM能够直接解析和推理原始Brep数据，为3D几何理解开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂场景中零样本物体计数的挑战，现有方法要么依赖开放词汇检测器产生多实例候选，要么通过随机补丁采样无法准确界定物体实例

Method: 提出CountZES框架，包含三个协同阶段：检测锚定示例（DAE）精炼开放词汇检测以隔离精确单实例示例；密度引导示例（DGE）通过密度驱动自监督范式识别统计一致且语义紧凑的示例；特征共识示例（FCE）通过特征空间聚类增强视觉一致性

Result: 在多样化数据集上的实验表明，CountZES在ZOC方法中表现出优越性能，并能有效泛化到自然、航拍和医学等多个领域

Conclusion: CountZES通过渐进式示例发现机制，产生多样化、互补的示例集，平衡了文本基础、计数一致性和特征代表性，为复杂场景中的零样本物体计数提供了有效解决方案
...
TL;DR: 提出一种无需训练的方法，通过几何视角精炼文本嵌入来抑制不需要的语义，解决多图像生成中的主题一致性和文本对齐问题。
...
Method: 提出一种简单有效的无需训练方法，从几何角度解决语义纠缠问题，通过精炼文本嵌入来抑制不需要的语义，避免跨帧嵌入纠缠导致的文本错位。
...
TL;DR: StageVAR：一种针对视觉自回归模型的分阶段感知加速框架，通过分析不同生成阶段的重要性差异，在保持早期关键步骤完整的同时，对后期细节优化步骤进行剪枝和近似，实现高效加速。
...
Method: 提出StageVAR框架：1）分析发现早期步骤对语义和结构一致性至关重要，应保持完整；2）后期步骤主要细化细节，可进行剪枝或近似；3）利用后期计算的语义无关性和低秩特性，提出即插即用的加速策略，无需额外训练。

Result: StageVAR实现了最高3.4倍的加速，在GenEval上仅下降0.01，在DPG上下降0.26，持续优于现有加速基线方法。
...
TL;DR: 该研究提出了一种让模型获得类人且自洽推理能力的盲图像质量评估方法，通过强化学习结合人类标注作为奖励信号，使模型能够从自我生成的描述中推断图像质量。


<details>
  <summary>Details</summary>
Motivation: 人类通过感知-推理级联来评估图像质量，结合感官线索与隐式推理形成自洽判断。研究旨在让模型获得类人且自洽的推理能力，用于盲图像质量评估。

Method: 1. 收集捕捉人类感知-推理过程多个方面的人类评估数据；2. 采用强化学习，使用人类标注作为奖励信号来引导模型实现类人感知和推理；3. 设计特殊奖励机制，使模型能够从自我生成的描述中推断图像质量，从而内化自洽推理能力。

Result: 1. 在通用指标（皮尔逊和斯皮尔曼相关系数）上，得分预测性能与最先进的BIQA系统相当；2. 在1000多个人类标注样本上，模型达到ROUGE-1分数0.512（基线为0.443），表明对人类解释的覆盖度显著提升，朝着类人可解释推理迈进一步。

Conclusion: 该方法成功让模型获得了类人且自洽的推理能力，不仅在图像质量评分上与现有方法相当，还能生成与人类感知-推理链相似的描述，为实现可解释的盲图像质量评估提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 当前情感识别（ER）领域过度依赖面部表情识别（FER），但面部表情常被用作社交工具而非真实内在情感的表现。为了理解和弥合FER与ER之间的差距，作者引入眼动行为作为重要的情感线索。

Method: 1. 构建EMER数据集：利用自发情感诱导范式收集真实情感数据，同时捕获眼动序列、眼动注视图等非侵入性眼动行为数据以及面部表情视频；2. 设计EMERT模型：采用模态对抗特征解耦和多任务Transformer，将眼动行为作为面部表情的强有力补充。

Result: 1. 建立了包含多模态数据和多视角情感标注的EMER数据集；2. 提出了七个多模态基准协议用于全面评估；3. EMERT模型在实验中大幅优于其他最先进的多模态方法，证明了眼动行为建模对鲁棒情感识别的重要性。

Conclusion: 眼动行为在情感识别中具有重要作用，能够有效弥补面部表情识别与真实情感识别之间的差距。EMER数据集和EMERT模型为研究更鲁棒的情感识别提供了重要资源，推动了解决FER与ER差距的研究进展。
...
TL;DR: 提出了VenusBench-GD，一个跨平台的双语GUI grounding基准，包含大规模数据集、高质量标注流程和层次化任务分类，评估发现通用多模态模型在基础任务上已匹敌专用GUI模型，但高级任务仍需要专用模型。


<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding基准存在数据量不足、领域覆盖窄、过度集中于单一平台或需要高度专业领域知识等局限性，需要构建更全面的评估框架。

Method: 1) 构建大规模跨平台基准，覆盖广泛应用、多样UI元素和丰富标注数据；2) 建立高质量数据构建流程，标注准确率高于现有基准；3) 提出层次化任务分类法，将grounding分为基础和高级两类，包含六个互补评估视角的子任务。

Result: 实验发现：通用多模态模型在基础grounding任务上已匹配甚至超越专用GUI模型；高级任务仍偏向GUI专用模型，但这些模型存在显著过拟合和鲁棒性差的问题。

Conclusion: 需要全面、多层次评估框架来准确评估GUI grounding能力，VenusBench-GD为此提供了有效工具，揭示了当前模型的优势和局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自监督预训练方法在骨架动作识别上取得了成功，但在时序动作定位任务中仍面临挑战。动作边界检测需要时间敏感的特征来捕捉相邻帧间的细微差异，而现有方法在这方面表现不足。

Method: 1. 提出片段区分预训练任务：将骨架序列密集投影到非重叠片段，通过对比学习区分不同视频中的片段；2. 构建U形模块：在骨架动作识别模型的基础上，融合中间特征以增强帧级定位的特征分辨率。

Result: 1. 在BABEL数据集的各种子集和评估协议上，该方法持续改进了现有的骨架对比学习方法；2. 在PKUMMD上实现了最先进的迁移学习性能，使用NTU RGB+D和BABEL进行预训练。

Conclusion: 该研究提出的自监督预训练框架有效提升了骨架时序动作定位的性能，通过片段区分任务和特征增强模块解决了动作边界检测中的时间敏感性需求。
...
TL;DR: N3D-VLM是一个统一框架，将原生3D物体感知与3D感知视觉推理结合，实现精确的3D定位和可解释的空间理解，显著超越现有方法。
...
Method: 提出N3D-VLM框架，赋予模型原生3D物体感知能力，使其能够基于文本描述直接在3D空间中定位物体。通过可扩展的数据构建流程，利用深度估计将大规模2D标注提升到3D空间，生成用于3D空间推理的链式思维问答数据集。

Result: 实验结果表明，该统一框架不仅在3D定位任务上达到最先进性能，而且在3D空间推理方面持续超越现有视觉语言模型方法。数据构建流程产生的3D物体定位数据比现有最大单图像3D检测数据集大六倍以上。

Conclusion: N3D-VLM成功地将原生3D物体感知与3D感知视觉推理相结合，实现了更精确、可解释的3D空间理解，为多模态模型在3D场景理解方面提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常只能重建当前可见的场景部分，无法处理动态场景中物体消失后的重建问题，缺乏对完整时空场景的持久感知能力

Method: 将场景分解为刚性3D基元，通过密集2D对应关系估计联合推断基元的刚体运动，引入运动外推机制处理不可见物体，使用运动分组技术保持连续性

Result: 在物体扫描和多物体数据集上，系统在定量和定性评估中显著优于现有方法，能够实现可重放的3D重建、多物体扫描和物体持久性

Conclusion: 该系统实现了4D时空感知，能够从单目视频中生成完整持久的动态场景重建，为动态场景理解和重建提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在微调视觉基础模型进行域泛化语义分割时，忽略了长期预训练模型存在的伪影问题。这些伪影与非因果因素相关，阻碍了有价值表征的利用，最终降低DGSS性能。

Method: 提出Causal-Tune方法：1）使用离散余弦变换提取每层特征的频谱；2）应用高斯带通滤波器分离因果与非因果成分；3）引入因果感知可学习token在频域精炼因果成分；4）通过逆DCT将精炼特征转换回空间域并传递到下一层。

Result: 在各种跨域任务上的实验证明Causal-Tune的有效性。特别是在恶劣天气条件下表现优异，在雪天条件下比基线方法提升了+4.8% mIoU。

Conclusion: 通过因果机制分析视觉基础模型中的特征因素，提出的Causal-Tune方法能够有效提取因果因素并抑制非因果因素，实现了更鲁棒的域泛化性能。
...
TL;DR: CRONOS是一个用于3D医学扫描时间演化的统一预测框架，支持从多个历史扫描进行多对一预测，同时处理离散和连续时间戳，首次实现3D医学数据的连续序列到图像预测。


<details>
  <summary>Details</summary>
Motivation: 现有模型要么依赖单个先验扫描，要么使用固定网格时间，要么针对全局标签，这限制了不规则采样下的体素级预测。需要一种能够处理多个历史扫描并支持连续时间预测的统一框架。
...
Conclusion: CRONOS是首个实现3D医学数据连续序列到图像预测的统一框架，能够处理多个历史扫描和连续时间预测，为疾病进展、治疗规划和发育评估提供了更好的工具。
...
TL;DR: SkiLa提出了一种新的多模态推理范式，通过让MLLM在推理过程中原生生成连续视觉嵌入（潜在草图标记）作为视觉思维，实现统一的视觉-文本推理。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM在需要视觉想象的场景中表现不足，而人类可以在大脑的统一空间中形成灵活的视觉-文本想象和交互。受此启发，考虑到当前MLLM已经在同一特征空间中编码视觉和文本信息，作者认为视觉标记可以无缝插入到文本标记携带的推理过程中。

Method: 提出Sketch-in-Latents (SkiLa)范式，扩展MLLM的自回归能力以原生生成连续视觉嵌入（潜在草图标记）。在多步推理中，模型动态交替使用文本思考模式生成文本思考标记和视觉草图模式生成潜在草图标记。提出潜在视觉语义重建机制确保这些潜在草图标记具有语义基础。

Result: 大量实验表明，SkiLa在视觉中心任务上实现了优越性能，同时在多样化的通用多模态基准测试中表现出强大的泛化能力。

Conclusion: SkiLa通过统一的多模态推理范式，使MLLM能够像人类一样在推理过程中自然地生成视觉思维，显著提升了视觉想象和推理能力。
...
Method: 提出Yuan-TecSwin模型：1) 用Swin-transformer块替换编码器和解码器中的CNN块；2) 通过精心选择的文本编码器、有效利用文本嵌入和仔细设计的文本条件融合来改进文本-图像对齐；3) 采用自适应时间步搜索策略在不同扩散阶段优化推理性能。

Result: 1) 在ImageNet生成基准上取得了1.37的SOTA FID分数；2) 通过自适应时间步搜索使推理性能提升10%；3) 在人类评估中，受访者难以区分模型生成的图像和人类绘制的图像。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在移动和嵌入式设备上实时运行的轻量级去雾系统，解决现有去雾方法计算复杂、难以在消费级硬件上部署的问题。

Method: 基于暗通道先验和大气散射模型，采用伽马自适应重建、快速传输近似（带数值稳定性的下界）、基于分数顶部像素平均的稳定大气光估计器，以及可选的颜色平衡阶段。

Result: 实验演示表明，在真实世界的图像和视频上，Hazedefy能够在不依赖GPU加速的情况下显著改善可见度和对比度，适合移动和嵌入式应用。

Conclusion: Hazedefy提供了一个计算简单、实际可部署的去雾解决方案，能够在消费级硬件上实现实时视频和直播摄像头增强，具有良好的实用价值。
...
TL;DR: LLSA提出了一种对数线性复杂度的稀疏注意力机制，通过分层结构解决DiTs在长序列上的二次计算成本问题，显著加速训练和推理同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）在视觉生成中达到最先进水平，但其二次自注意力成本从根本上限制了向长令牌序列的扩展。现有的Top-K稀疏注意力方法虽然通过压缩令牌和选择相关键块来减少计算，但仍存在两个问题：(1) 压缩令牌上的二次选择成本；(2) 随着序列增长，需要增加K值来维持模型质量。作者发现这种低效源于单层设计，因为单个粗略层级不足以表示全局结构。

Method: LLSA采用分层Top-K选择机制，利用分层结构将选择和注意力成本从二次降低到对数线性复杂度。具体包括：1）分层Top-K选择：在先前层级找到的索引基础上逐步采用稀疏Top-K选择；2）分层KV丰富机制：在注意力计算中使用不同粒度的更少令牌，同时保留全局上下文。此外，作者开发了高性能GPU实现，仅使用稀疏索引进行前向和反向传播，无需密集注意力掩码。

Result: 在256×256像素令牌序列上，LLSA将注意力推理加速28.27倍，DiT训练加速6.09倍，同时保持生成质量。该方法在不使用分块化和VAE编码的情况下，实现了高分辨率像素空间图像生成。

Conclusion: LLSA为高效训练长序列DiTs提供了一个有前景的方向，通过分层稀疏注意力机制显著降低了计算复杂度，同时保持了模型性能。该方法在长序列视觉生成任务中具有重要应用价值。
...
TL;DR: 本文提出一个基于电源插座的室内多媒体地理位置定位管道，通过检测、分类插座类型并映射到国家，为打击人口贩卖等犯罪提供数字取证工具。


<details>
  <summary>Details</summary>
Motivation: 室内多媒体地理位置定位在打击人口贩卖、儿童剥削等严重犯罪中具有重要价值，但面临室内布局相似、装修频繁、视觉模糊、光照变化、GPS信号不可靠以及敏感领域数据集有限等挑战。

Method: 提出三阶段深度学习管道：1) 使用YOLOv11检测电源插座(mAP@0.5=0.843)；2) 使用Xception将插座分类为12种类型(准确率0.912)；3) 将检测到的插座类型映射到国家(置信度>90%时准确率0.96)。为解决数据稀缺问题，创建了两个专用数据集。

Result: 在Hotels-50K数据集的TraffickCam子集上评估，该子集包含真实世界条件下的酒店图像（光照差、业余拍摄角度），比旅游网站的专业图像更具现实性。管道表现出良好的性能。

Conclusion: 该框架展示了向现实世界数字取证应用迈出的实际步骤，所有代码、训练模型和数据均已开源。
...
TL;DR: DeContext是一种保护输入图像免遭未经授权的上下文编辑的新方法，通过注入针对性扰动来削弱跨注意力路径，从而阻断输入与输出之间的关联。


<details>
  <summary>Details</summary>
Motivation: 上下文扩散模型虽然能轻松修改图像，但也带来严重的隐私问题：个人图像可能被恶意操纵用于身份冒充、虚假信息等目的，而现有针对个性化文本到图像生成的输入扰动方法对现代大规模DiT模型的鲁棒性尚未充分研究。

Method: DeContext通过注入小的针对性扰动来削弱多模态注意力层中的跨注意力路径，从而阻断源图像上下文信息向输出的传播。研究发现早期去噪步骤和特定transformer块主导上下文传播，因此可以将扰动集中在最关键的位置。

Result: 在Flux Kontext和Step1X-Edit上的实验表明，DeContext能持续阻止不需要的图像编辑，同时保持视觉质量，证明了基于注意力的扰动作为防御图像操纵的有效性。

Conclusion: 注意力机制中的跨注意力路径是上下文信息传播的关键通道，通过针对性扰动这些路径可以有效保护图像免遭未经授权的编辑，为防御图像操纵提供了一种高效且鲁棒的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有SAR深度学习面临数据稀缺和斑点噪声干扰两大挑战，限制了细粒度语义表示学习。需要开发能够处理SAR特有噪声并利用大规模数据的方法。
...
Conclusion: SARMAE通过构建大规模数据集、噪声感知学习和光学先验对齐，有效解决了SAR深度学习中的数据稀缺和噪声干扰问题，为SAR图像理解提供了强大的自监督表示学习方法。
...
TL;DR: REGLUE是一种新的潜在扩散模型框架，通过联合建模VAE潜在空间、局部VFM语义和全局CLS标记，并引入非线性语义压缩器和外部对齐损失，显著提升图像合成质量和训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有潜在扩散模型通过重建式去噪目标仅提供间接语义监督，高级语义出现缓慢，需要更长训练时间且限制样本质量。现有方法要么通过表示对齐外部注入语义，要么仅在扩散过程中联合建模VFM特征的狭窄切片，未能充分利用VFM丰富、非线性、多层空间语义。

Method: 提出REGLUE框架：1) 在单个SiT主干中联合建模VAE图像潜在、紧凑的局部（补丁级）VFM语义和全局（图像级）CLS标记；2) 使用轻量级卷积语义压缩器非线性聚合多层VFM特征为低维空间结构化表示；3) 在扩散过程中将语义表示与VAE潜在纠缠；4) 使用外部对齐损失进一步正则化内部表示朝向冻结的VFM目标。
...
Conclusion: REGLUE通过统一联合建模VAE潜在、局部VFM语义和全局CLS标记，并引入非线性语义压缩和外部对齐，有效利用了VFM的丰富空间语义，显著提升了潜在扩散模型的图像合成质量和训练效率。
...
TL;DR: FrameDiffuser是一个自回归神经渲染框架，通过结合G-buffer数据和自身先前输出来生成时间一致的光照真实帧，专门针对交互式应用优化
...
Result: FrameDiffuser能够在数百到数千帧中保持稳定的时间一致性生成，相比通用方法，环境专门化训练在光照真实度、阴影和反射方面获得更优质量

Conclusion: FrameDiffuser为交互式应用提供了一种高效的时间一致神经渲染解决方案，通过自回归框架和环境专门化训练平衡了生成质量与计算效率
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（NeRF、3DGS、RadiantFoam）在网格重建精度方面仍有不足，需要一种既能保持高效渲染又能提供精确几何重建的解决方案

Method: 联合学习显式Voronoi图和隐式有符号距离场（SDF），通过光线追踪优化场景，使用Eikonal正则化，使Voronoi单元面与零水平集对齐

Result: SDFoam在多种场景中显著改善了网格重建精度（Chamfer距离），同时保持了可比较的外观质量（PSNR、SSIM），且训练速度与RadiantFoam相当

Conclusion: 提出的混合隐式-显式方法SDFoam能够产生更清晰、视角一致的表面，减少漂浮物并改善拓扑结构，在保持渲染效率的同时大幅提升几何重建质量
...
<details>
  <summary>Details</summary>
Motivation: 目前胎儿生长评估依赖超声图像中手动识别解剖标志点来测量生物指标，这种方法耗时、操作者依赖性强，且在不同扫描设备和中心间存在变异性，限制了自动化方法的可重复性。需要多源标注数据集来开发AI辅助的胎儿生长评估方法。

Method: 创建了一个开放的多中心、多设备基准数据集，包含4,513张去标识化的超声图像，来自1,904名受试者，在三个临床中心使用七种不同的超声设备采集。提供了标准化的、受试者分离的训练/测试分割、评估代码和基线结果。使用自动生物测量模型量化域偏移，并比较单中心与多中心测试的性能差异。

Result: 数据集包含所有主要胎儿生物测量指标（头双顶径、枕额径、腹横径、腹前后径和股骨长度）的专家标注。通过自动生物测量模型发现，仅在单一中心训练和评估会显著高估性能，而多中心测试更能反映实际应用中的表现。这是首个公开覆盖所有主要胎儿生物测量指标的多中心、多设备、标志点标注数据集。

Conclusion: 该数据集为胎儿生物测量中的域适应和多中心泛化提供了稳健的基准，能够实现更可靠的跨中心AI辅助胎儿生长评估。所有数据、标注、训练代码和评估流程均已公开提供。
...
TL;DR: 该论文提出了OMG-Bench，首个大规模公开的骨架在线微手势识别基准，包含40个细粒度手势类别和13,948个实例，并提出了Hierarchical Memory-Augmented Transformer (HMATr)框架，统一了手势检测和分类任务。


<details>
  <summary>Details</summary>
Motivation: 在线微手势识别对VR/AR交互至关重要，但面临两大挑战：1）缺乏公开数据集，微手势的细微运动模式使得构建具有精确骨架和帧级标注的数据集困难；2）现有算法多为任务特定，缺乏统一框架。

Method: 1）开发多视图自监督管道自动生成骨架数据，结合启发式规则和专家精炼实现半自动标注，构建OMG-Bench基准；2）提出Hierarchical Memory-Augmented Transformer (HMATr)端到端框架，利用分层记忆库存储帧级细节和窗口级语义以保留历史上下文，并使用从记忆初始化的可学习位置感知查询隐式编码手势位置和语义。

Result: OMG-Bench包含40个细粒度手势类别、13,948个实例、1,272个序列，具有细微运动、快速动态和连续执行的特点。HMATr在检测率上比现有最优方法高出7.6%，为在线微手势识别建立了强基线。

Conclusion: 该研究通过构建首个大规模公开基准OMG-Bench和提出HMATr框架，有效解决了在线微手势识别中的数据稀缺和算法统一问题，为VR/AR交互提供了重要技术基础。
...
TL;DR: TODSynth框架：用于遥感语义分割的任务导向数据合成，包含多模态扩散变换器和任务反馈引导的采样策略，在少样本和复杂场景下显著提升合成数据质量。
...
Method: 提出TODSynth框架：1) 多模态扩散变换器(MM-DiT)，采用文本-图像-掩码联合注意力机制；2) 控制-校正流匹配(CRFM)方法，在采样早期阶段通过语义损失动态调整采样方向；3) 基于任务反馈的即插即用采样策略。

Result: 实验表明该方法在遥感语义分割数据合成中优于现有最先进的可控生成方法，特别是在少样本和复杂场景下，能生成更稳定、任务导向的合成数据。

Conclusion: TODSynth框架通过多模态联合注意力机制和任务反馈引导的采样策略，有效解决了遥感语义分割数据合成的控制复杂性和质量不确定性问题，为扩展标注数据集提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 降低计算复杂度是基于学习的图像压缩技术广泛采用的关键挑战，现有方法在计算效率方面仍有改进空间。
...
TL;DR: Make-It-Poseable是一个新颖的前馈框架，通过潜在空间变换实现3D角色摆姿，避免了传统方法中蒙皮权重预测不准确、拓扑缺陷等问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D角色摆姿方法（如自动绑定和姿态条件生成）存在蒙皮权重预测不准确、拓扑缺陷和姿态一致性差等问题，限制了其鲁棒性和泛化能力。

Method: 将角色摆姿重新定义为潜在空间变换问题，通过潜在姿态变换器基于骨骼运动操作形状标记，使用密集姿态表示进行精确控制，并引入潜在空间监督策略和自适应补全模块。

Result: 该方法在摆姿质量上表现出优越性能，并且自然地扩展到3D编辑应用，如部件替换和细化。

Conclusion: Make-It-Poseable通过潜在空间变换方法有效解决了传统3D角色摆姿方法的局限性，实现了高质量的摆姿效果和扩展的编辑能力。
...
TL;DR: Kling-Omni是一个端到端的多模态视频生成框架，能够从文本、图像、视频等多种输入直接合成高质量视频，支持生成、编辑和推理任务，旨在成为多模态世界模拟器。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成系统通常采用分离的流水线方法，不同任务（生成、编辑、推理）之间功能割裂。作者希望构建一个统一的端到端框架，将多种视频相关任务整合到单一系统中，实现更智能、更高质量的视频内容创作。

Method: 采用端到端架构，将文本指令、参考图像和视频上下文等多种输入处理为统一的多模态表示。构建了全面的数据系统作为多模态视频创作的基础，并采用高效的大规模预训练策略和推理基础设施优化。
...
<details>
  <summary>Details</summary>
Motivation: 现有真实数据集缺乏精确标注，而合成数据集虽然能低成本标注大量帧，但缺乏真实的车辆运动轨迹。需要一种既能提供精确标注又具有真实车辆运动的数据集。

Method: 创建合成3D环境，并从无人机拍摄的鸟瞰数据集SinD中提取真实世界轨迹，将这些真实轨迹整合到合成环境中。

Result: R3ST数据集填补了合成数据与真实轨迹之间的差距，提供了准确的多模态标注和真实的人类驾驶车辆轨迹。

Conclusion: 该数据集通过结合合成环境的标注优势和真实轨迹的运动真实性，推动了道路车辆轨迹预测研究的发展。
...
TL;DR: KineST：一种基于运动学引导的状态空间模型，用于从稀疏信号重建逼真多样的全身姿态，在轻量级框架下实现高精度和时间一致性。


<details>
  <summary>Details</summary>
Motivation: AR/VR应用中，基于头戴设备稀疏信号重建逼真多样的全身姿态具有挑战性。现有方法要么计算成本高，要么需要分别建模空间和时间依赖关系，难以平衡精度、时间一致性和效率。

Method: 提出KineST模型，核心创新包括：1）在状态空间对偶框架中引入运动学引导的双向扫描策略，嵌入运动学先验以捕捉复杂关节关系；2）采用混合时空表示学习方法紧密耦合空间和时间上下文；3）引入几何角速度损失函数施加物理约束以提升运动稳定性。

Result: 大量实验表明，KineST在轻量级框架下在精度和时间一致性方面均表现出优越性能。

Conclusion: KineST通过运动学引导的状态空间模型有效解决了AR/VR中基于稀疏信号重建全身姿态的挑战，在保持轻量级的同时实现了高精度和时间一致性。
