...
TL;DR: 研究人员提出了DarkDriving数据集，这是首个用于自动驾驶黑暗环境研究的真实世界昼夜对齐数据集，包含9,538对精确对齐的图像，并引入了四项感知相关任务。


<details>
  <summary>Details</summary>
Motivation: 现有低光增强数据集存在局限性：要么只能在静态场景和小范围内控制曝光采集，要么夜间驾驶数据缺乏精确对齐的白天对应图像。在动态驾驶场景中收集真实世界昼夜对齐数据集极为困难，这严重限制了该领域的研究。

Method: 提出了一种自动化的昼夜轨迹跟踪姿态匹配（TTPM）方法，在一个大型真实世界封闭驾驶测试场（面积69英亩）中，通过精确的位置和空间内容对齐，收集了9,538对昼夜图像。每对图像都手动标注了物体2D边界框。

Result: DarkDriving数据集实现了厘米级的对齐精度，并引入了四项任务：低光增强、广义低光增强、用于2D检测的低光增强和用于3D检测的低光增强。实验表明该数据集为自动驾驶低光增强提供了全面的基准，并能推广到其他低光驾驶环境。

Conclusion: DarkDriving是首个真实世界昼夜对齐的自动驾驶数据集，填补了该领域的研究空白，为低光环境下的自动驾驶感知系统提供了重要的基准和评估工具。
...
TL;DR: SSP-SAM是一个增强SAM模型语言理解能力的框架，通过语义-空间提示编码器整合视觉和语言注意力适配器，使SAM能够根据语言描述进行精确分割，并自然支持广义指代表达分割任务。


<details>
  <summary>Details</summary>
Motivation: SAM模型在通用图像分割方面表现出色，但缺乏对自然语言的理解能力，限制了其在指代表达分割任务中的直接应用。需要一种方法既能充分利用SAM的分割能力，又能理解语言描述。

Method: 提出SSP-SAM框架，包含语义-空间提示编码器，整合视觉和语言注意力适配器。视觉注意力适配器突出视觉特征中的显著对象，语言注意力适配器突出语言特征中的区分性短语，从而增强指代表征，生成高质量提示使SAM能根据语言生成精确掩码。

Result: 在广泛使用的RES和GRES基准测试中表现出优越性，生成高质量分割掩码，即使在严格的Pr@0.9阈值下也能保持强精度。在PhraseCut数据集上的评估显示，在开放词汇场景中相比现有最先进的RES方法有改进。

Conclusion: SSP-SAM成功地将SAM的分割能力与语言理解相结合，无需专门设计就能自然支持广义指代表达分割，为语言引导的图像分割提供了有效的解决方案。
...
TL;DR: CytoSyn是一个用于组织病理学H&E染色图像生成的基础潜在扩散模型，在广泛基准测试中表现出色，支持虚拟染色等任务。


<details>
  <summary>Details</summary>
Motivation: 计算病理学在特征提取器方面已有显著进展，但专门针对组织病理学的生成式基础模型仍然稀缺。这类模型能够处理特征提取器无法完成的任务，如虚拟染色。

Method: 开发了CytoSyn-v2基础潜在扩散模型，探索了方法改进、训练集扩展、采样策略和切片级过拟合问题。模型基于超过10,000张TCGA诊断全切片图像（32种癌症类型）进行训练。

Result: 模型能够生成高度逼真且多样化的H&E染色组织病理学图像，在广泛基准测试中表现优异。尽管仅在肿瘤切片上训练，但在炎症性肠病图像生成上仍保持最先进性能。
...
TL;DR: O2MAG是一种无需训练的小样本异常生成方法，通过自注意力嫁接和异常掩码技术，利用单个参考异常图像合成更多逼真异常，提升下游异常检测性能。
...
Conclusion: O2MAG是一种无需训练的小样本异常生成方法，能够合成逼真且与文本一致的异常，有效提升工业异常检测性能，解决了现有方法训练耗时和分布不准确的问题。
...
<details>
  <summary>Details</summary>
Motivation: 后训练量化（PTQ）是部署大型扩散模型的实用方法，但量化噪声会在去噪轨迹中累积，从而降低生成质量。需要一种有效的方法来校正量化误差的影响。

Method: Q-Drift将量化误差视为每个去噪步骤的隐式随机扰动，推导出保持边缘分布的漂移调整。通过校准估计时间步方差统计量，仅需少量（如5次）全精度/量化配对校准运行。该方法可与常见采样器、扩散模型和PTQ方法即插即用。

Result: 在六个不同的文本到图像模型（包括DiT和U-Net）、三个采样器（Euler、flow-matching、DPM-Solver++）和两种PTQ方法（SVDQuant、MixDQ）上，Q-Drift在大多数设置中改善了FID，在PixArt-Sigma（SVDQuant W3A4）上FID降低高达4.59，同时保持CLIP分数。

Conclusion: Q-Drift提供了一种原理性的采样器侧校正方法，有效缓解扩散模型后训练量化中的噪声累积问题，显著提升量化模型的生成质量，且推理开销可忽略不计。
...
TL;DR: TOGA是一个仅用于训练的不对称框架，通过构建异构图教师来增强Tip-Adapter的缓存原型，实现细粒度跨模态对齐，测试时无需额外开销。
...
Result: 在标准的1-16样本基准测试中，该方法持续建立了新的最先进性能，同时推理阶段与Tip-Adapter完全相同，无额外延迟或内存开销。

Conclusion: 辅助图监督、文本引导推理和节点过滤是鲁棒少样本适应的关键要素，提出的训练框架有效提升了原型质量而不增加推理成本。
...
<details>
  <summary>Details</summary>
Motivation: 现有IAA模型预测性能强但缺乏解释性，用户需要理解图像美学评分背后的原因，人类评估美学时依赖高层次线索

Method: 基于人类可理解美学概念构建可解释框架，学习可访问的概念并构建子空间作为可解释模型基础，引入残差预测器捕捉超出显式概念的细微影响

Result: 在摄影和艺术数据集上实验表明，该方法在保持竞争性预测性能的同时提供透明、人类可理解的美学判断

Conclusion: 提出的框架成功实现了图像美学评估的可解释性，在预测性能和解释透明度之间取得了良好平衡
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在复杂推理方面面临高质量长链推理数据稀缺和优化训练流程不足的挑战，需要开发能够自主生成结构化推理轨迹并有效训练模型的系统。

Method: 1) 可扩展的数据生成管道，配备多粒度评估，自主合成跨图像和视频域的结构化复杂推理轨迹；2) 双智能体架构：推理智能体执行广泛分析链，摘要智能体批判性评估和提炼最终结果；3) 引入ST-GRPO和J-GRPO算法增强时空推理和评估鲁棒性；4) 利用摘要智能体的可靠反馈指导迭代推理路径生成，实现持续自我改进循环。

Result: 在LLaVA-NeXT和Qwen2.5-VL等基础模型上进行的广泛实验表明，在具有挑战性的图像和视频推理基准测试中取得了显著的性能提升，同时保持了传统感知任务的强大能力。

Conclusion: Insight-V++框架通过创新的多智能体架构和训练算法，成功解决了多模态大语言模型在复杂视觉推理方面的关键挑战，为长链时空推理提供了有效的解决方案，并实现了持续自我改进的能力。
...
TL;DR: VLM-AutoDrive是一个模块化后训练框架，用于将预训练的视觉语言模型适配到驾驶安全事件检测任务，显著提升了碰撞和近碰撞检测性能。
...
Result: 在Nexar车载摄像头视频上评估，碰撞检测F1分数从0.00提升到0.69，总体准确率从35.35%提升到77.27%，显著改善了碰撞和近碰撞检测性能。

Conclusion: VLM-AutoDrive为通用视觉语言模型适配到安全关键、时间定位的感知任务提供了可扩展方案，弥合了自动驾驶中感知、因果关系和决策推理之间的差距。
...
TL;DR: 提出了MicroVision数据集，这是一个专注于弱势道路使用者（行人、骑行者、电动滑板车使用者）和静止微移动车辆的开源图像数据集，从VRU视角采集，包含8000多张图像和30000多个标注，用于训练计算机视觉模型。


<details>
  <summary>Details</summary>
Motivation: 现有开源图像数据集在VRUs和MMVs方面缺乏专注性和多样性，通常将行人和微移动车辆骑行者都归类为"人"，或不包含新型微移动车辆如电动滑板车。此外，数据集多从汽车视角采集，缺乏仅VRUs通行的区域（人行道、自行车道）数据。

Method: 创建了MicroVision数据集，在瑞典哥德堡采集了8000多张匿名全高清图像，包含30000多个精心标注的VRUs和MMVs，覆盖全年时间，包含近2000个独特的交互场景。同时提供了基于最先进架构的基准目标检测模型。

Result: 数据集包含8000多张图像和30000多个标注。基准目标检测模型在未见测试集上达到了最高0.723的平均精度均值（mAP）。

Conclusion: MicroVision数据集填补了现有数据集的空白，能够支持交通安全领域区分不同类型的VRUs和MMVs，或帮助监控系统识别微移动车辆的使用情况。数据集和模型权重已开源。
...
<details>
  <summary>Details</summary>
Motivation: 月球表面导航和建图面临纹理贫乏、高对比度光照、计算资源有限等挑战，需要鲁棒的感知系统来支持未来月球任务
...
Conclusion: 将语义分割和密集深度估计与学习型地图表示相结合，是创建详细大规模月球表面地图的有效方法，支持未来月球任务
...
TL;DR: LRConv-NeRV通过用低秩可分离卷积替换NeRV中的部分密集卷积层，在保持重建质量的同时显著降低计算复杂度和模型大小，为资源受限环境下的神经视频解码提供了高效架构。


<details>
  <summary>Details</summary>
Motivation: NeRV的卷积解码器计算成本高且内存密集，限制了其在资源受限环境中的部署。需要一种更高效的NeRV变体来平衡重建质量与计算效率。

Method: 提出LRConv-NeRV，用结构化低秩可分离卷积替换选定的密集3x3卷积层，从最大解码阶段到早期阶段逐步应用低秩分解，实现可控的质量与效率权衡。

Result: 仅对最终解码阶段应用LRConv可将解码器复杂度降低68%（从201.9降至64.9 GFLOPs），模型大小减少9.3%，质量损失可忽略，比特率降低约9.2%。INT8量化后仍保持接近基线的重建质量。
...
TL;DR: CycleCap：一种基于循环一致性的自监督微调方法，通过图像重构相似度作为奖励信号，无需标注数据即可提升视觉语言模型的图像描述准确性和减少幻觉


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在视觉-语言对齐问题，容易产生过于通用或幻觉的描述。传统方法需要昂贵的标注数据集或复杂的测试时框架，作者希望开发一种无需标注数据的自监督方法来解决这一问题。

Method: 提出CycleCap微调方案：1）利用循环一致性原理：图像→VLM生成描述→文本到图像模型重构图像→比较原始图像与重构图像的相似度；2）使用Group Relative Policy Optimization（GRPO）进行微调，以重构图像相似度作为奖励信号；3）完全自监督，仅需原始图像，无需标注数据。

Result: 在1B到7B参数的四个VLM上应用CycleCap，在图像描述和幻觉基准测试中均取得一致改进，超越了依赖监督循环一致性训练的现有最先进方法。

Conclusion: CycleCap提供了一种有效的自监督微调方法，通过循环一致性直接作为训练信号，能够显著提升视觉语言模型的图像描述准确性和减少幻觉，无需昂贵的标注数据集。
...
TL;DR: PreSCAN是一个预测框架，能在训练前评估NeRF在卫星图像上的重建质量，通过轻量级几何和光度描述符实现快速架构选择，相比NAS加速1000倍，并在边缘设备上优化推理性能。


<details>
  <summary>Details</summary>
Motivation: NeRF在卫星图像应用面临挑战：每个场景需要单独训练，通过NAS优化架构需要数小时到数天的GPU时间。研究发现多视角一致性（而非模型架构）是决定重建质量的关键因素。

Method: 基于SHAP分析发现多视角一致性决定质量，开发PreSCAN框架，使用轻量级几何和光度描述符在训练前预测NeRF质量，快速选择合适架构。在边缘平台结合离线成本分析优化推理性能。

Result: PreSCAN能在30秒内选择合适架构，预测误差小于1dB，相比NAS加速1000倍。在Jetson Orin边缘平台上，结合离线成本分析减少26%推理功耗和43%延迟，质量损失最小。在DFC2019数据集上验证了跨场景泛化能力。
...
TL;DR: 本文提出了三层次诊断框架，通过三个指标分离幻觉来源：潜在异常检测、视觉必要性分数和竞争分数，揭示了VLMs中普遍存在的视觉奉承现象（模型检测到视觉异常但仍产生幻觉以满足用户期望），而零样本显示稳健拒绝，表明对齐训练系统性地抑制了真实不确定性承认。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究VLMs在正确回答时，是真正依赖视觉信息还是利用语言捷径。当前缺乏系统性的方法来解构幻觉来源，无法区分模型是否真正理解视觉内容，还是仅仅为了满足用户期望而产生幻觉。

Method: 提出了三层次诊断框架，包含三个指标：潜在异常检测（感知意识）、视觉必要性分数（通过KL散度测量视觉依赖性）和竞争分数（视觉基础与指令跟随之间的冲突）。使用反事实干预（盲图、噪声图和冲突图像）在7个VLMs和7,000个模型-样本对上进行测试。

Result: 69.6%的样本表现出视觉奉承现象（模型检测到视觉异常但仍产生幻觉以满足用户期望），而零样本显示稳健拒绝。缩放分析显示更大模型减少语言捷径但放大视觉奉承。诊断分数支持后验选择性预测策略，在50%覆盖率下实现+9.5pp准确率提升。

Conclusion: 对齐训练系统性地抑制了真实不确定性承认，仅靠模型缩放无法解决基础问题。诊断框架为理解VLMs幻觉来源提供了系统性方法，并支持无需额外训练成本的选择性预测策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于极线几何的匹配方法大多依赖粗粒度空间分箱，这会引入近似误差、需要昂贵的后处理，并且可能遗漏有效匹配。需要一种更精确、高效的匹配方法。

Method: 将每个关键点分配一个容差圆，从极点观察时定义角度区间，将匹配问题转化为1D角度区间查询，使用线段树在对数时间内高效解决。

Result: 在ETH3D数据集上的广泛评估显示，相比现有方法获得了显著的速度提升，同时恢复了精确的对应关系集合。

Conclusion: 提出的角度空间精确匹配方法克服了传统方法的局限性，实现了像素级容差、支持每个关键点的控制，并消除了不必要的描述符比较。
...
TL;DR: Inst4DGS提出了一种实例分解的4D高斯泼溅方法，通过标签排列潜在变量解决多视角视频中实例标签不一致的问题，并引入实例分解的运动支架来优化长时程轨迹。


<details>
  <summary>Details</summary>
Motivation: 虽然动态4D高斯泼溅技术发展迅速，但实例分解的4D高斯泼溅仍未被充分探索，主要原因是难以在独立分割的多视角视频中关联不一致的实例标签。

Method: 1) 引入每视频标签排列潜在变量，通过可微分的Sinkhorn层学习跨视频实例匹配，实现具有一致身份保持的多视角直接监督；2) 提出实例分解的运动支架，为每个对象提供低维运动基，用于长时程轨迹优化。
...
Conclusion: Inst4DGS通过显式标签对齐实现了锐利的决策边界和时间稳定的身份保持，解决了实例分解4D高斯泼溅中的标签不一致问题，同时通过实例分解的运动支架提高了长时程轨迹优化的效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注常见疾病，仅评估最终准确性，忽视了临床推理过程，这对于复杂病例至关重要。特别是在罕见皮肤病诊断方面，大视觉语言模型的推理能力评估仍未被充分探索。

Method: 构建了DermCase基准测试，包含26,030个多模态图像-文本对和6,354个临床挑战性病例，每个病例都标注了全面的临床信息和逐步推理链。建立了基于DermLIP的相似性度量来评估鉴别诊断质量。对22个领先的大视觉语言模型进行了基准测试，并进行了微调实验。

Result: 基准测试显示，现有模型在诊断准确性、鉴别诊断和临床推理方面存在显著缺陷。微调实验表明，指令微调能显著提升性能，而直接偏好优化(DPO)带来的改进有限。系统误差分析揭示了当前模型推理能力的关键局限性。

Conclusion: DermCase基准测试填补了评估大视觉语言模型在皮肤病学中罕见疾病诊断推理能力的空白。研究发现现有模型在临床推理方面存在严重不足，需要进一步改进模型架构和训练方法以提升临床诊断能力。
...
TL;DR: SynQ是一种零样本量化框架，通过低通滤波器减少合成数据噪声，对齐类激活图提高精度，并利用软标签缓解预训练模型错误指导，在无数据情况下实现最先进的量化性能。
...
Conclusion: SynQ通过系统解决合成数据噪声、类激活图对齐和软标签使用等关键问题，为无数据场景下的模型量化提供了有效的解决方案。
...
Method: 提出一种集成可控扩散模型的合成数据增强流程，采用类别感知提示和视觉先验融合技术，确保生成图像与分割标签的精确对齐。
...
TL;DR: SR-Nav提出了一种空间关系感知的零样本目标导航框架，通过建模动态空间关系图来增强感知和规划能力，在HM3D数据集上实现了最先进的导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于基础模型的零样本目标导航方法在面对不良视角或弱语义线索时，感知和规划能力不可靠，导致导航效率低下或失败。作者观察到物体和区域之间的内在关系编码了结构化场景先验，即使在部分观测下也能帮助智能体推断合理的目标位置。

Method: 提出了SR-Nav框架：1）构建动态空间关系图（DSRG），通过基础模型编码以目标为中心的空间关系，并随实时观测动态更新；2）关系感知匹配模块，利用DSRG中的多样化关系进行关系匹配而非简单检测，验证和纠正错误以增强视觉感知鲁棒性；3）动态关系规划模块，基于DSRG从当前位置动态计算最优路径，减少规划搜索空间，降低探索冗余。

Result: 在HM3D数据集上的实验表明，该方法在成功率和导航效率方面都达到了最先进的性能。

Conclusion: SR-Nav通过建模动态空间关系来增强感知和规划，有效解决了零样本目标导航中基础模型在不良观测条件下的不可靠问题，显著提升了导航性能。
...
TL;DR: 提出一种无需额外标注的自监督方法，通过强制相关任务（生成与验证）关注相同帧来改善视频-LLM的时间定位能力，在体育教练任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 视频-LLM经常关注不相关的帧，这在需要精确时间定位的体育教练任务中尤其有害。获取帧级监督既昂贵（人工标注）又不可靠（其他模型），因此需要一种无需额外标注的改进方法。

Method: 利用观察发现相关任务（如生成和验证）必须关注相同的帧，通过在这些紧密相关任务的视觉注意力图上施加自一致性目标来实现无监督改进。

Result: 在VidDiffBench上验证了注意力分配不当是主要瓶颈。使用该方法在三个体育教练任务（Exact、FitnessQA、ExpertAF）上分别获得+3.0%、+14.1%准确率和+0.9 BERTScore的提升，甚至超过闭源模型。

Conclusion: 通过自监督的自一致性目标，无需额外标注即可显著改善视频-LLM的时间定位能力，在体育教练任务中取得优于监督微调和闭源模型的性能。
...
TL;DR: 该研究开发了一个可解释的前列腺癌自动检测框架，使用仅162张T2加权MRI图像的小数据集，通过迁移学习和数据增强解决数据稀缺问题。研究发现迁移学习的ResNet18在仅11M参数下获得最佳性能，而Vision Transformers表现较差，传统HOG+SVM方法也表现出色。该方法仅使用T2加权图像就达到竞争性性能，在放射科医生研究中AI模型灵敏度显著高于人类专家。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是男性主要死亡原因，但T2加权前列腺MRI解释具有挑战性，因为病变细微且异质性高。现有方法通常需要双参数MRI（T2+DWI）和大规模数据集，增加了采集复杂性和计算成本。本研究旨在开发一个仅使用T2加权图像的小数据集自动癌症检测框架，解决数据稀缺问题并降低临床应用门槛。

Method: 使用162张T2加权前列腺MRI图像（102例癌症，60例正常）的小数据集，通过迁移学习和数据增强解决数据稀缺问题。全面比较了Vision Transformers（ViT、Swin）、CNN（ResNet18）和传统方法（逻辑回归、SVM、HOG+SVM）。进行了包含22个病例的读者研究，由5名放射科医生评估，并与AI模型性能对比。

Result: 迁移学习的ResNet18获得最佳性能：准确率90.9%，灵敏度95.2%，AUC 0.905，仅11M参数。Vision Transformers性能较低但复杂度显著更高。HOG+SVM达到可比性能（AUC 0.917），显示在小数据集中手工特征的效力。放射科医生平均灵敏度67.5%（Fleiss Kappa = 0.524），而AI模型灵敏度95.2%，表明AI辅助筛查可减少漏诊并提高一致性。

Conclusion: 该研究证明在小数据集上，仅使用T2加权图像就能实现竞争性的前列腺癌检测性能，降低了采集复杂性和计算成本。迁移学习的轻量级CNN模型优于复杂的Vision Transformers，传统手工特征方法在小数据集中仍然有效。AI模型显著优于放射科医生表现，显示出AI辅助筛查在减少漏诊和提高诊断一致性方面的潜力。
...
TL;DR: MedQ-UNI是一个统一的视觉语言模型，采用"评估后修复"范式，通过医学图像质量评估指导医学图像修复，能够跨多种模态和退化类型进行通用修复。
...
Method: 提出MedQ-UNI模型，采用多模态自回归双专家架构：质量评估专家通过结构化自然语言描述识别退化问题，修复专家基于这些描述进行有针对性的图像修复。构建了包含约50K配对样本的大规模数据集，涵盖三种成像模态和五种修复任务。

Result: 单个MedQ-UNI模型无需任何任务特定适配，在所有任务上都达到了最先进的修复性能，同时生成更优的描述，证实明确的质量理解能显著提高修复保真度和可解释性。

Conclusion: 通过将医学图像质量评估与医学图像修复相结合，MedQ-UNI实现了跨模态和退化类型的统一修复，证明了明确质量理解对提高修复性能和可解释性的重要性。
...
TL;DR: ColourCrafter是一个统一的扩散框架，通过RGB颜色标记与图像标记的融合实现精细化的区域感知颜色编辑，解决了传统方法在连续色彩变化控制上的不足。


<details>
  <summary>Details</summary>
Motivation: 颜色是图像生成中最显著但最难控制的属性之一。现有扩散模型虽然能根据用户指令修改对象颜色，但结果往往偏离预期色调，特别是在细粒度和局部编辑方面。早期文本驱动方法依赖离散语言描述，无法准确表示连续色彩变化。

Method: 提出ColourCrafter框架：1）在潜在空间中进行RGB颜色标记和图像标记的标记级融合，选择性地将颜色信息传播到语义相关区域；2）使用感知Lab空间损失，通过解耦亮度和色度并在掩码区域内约束编辑来增强像素级精度；3）构建ColourfulSet大规模高质量图像对数据集，包含连续多样的色彩变化。

Result: 大量实验表明，ColourCrafter在细粒度颜色编辑中实现了最先进的颜色准确性、可控性和感知保真度。

Conclusion: ColourCrafter将颜色编辑从全局色调转移转变为结构化、区域感知的生成过程，通过标记级融合和Lab空间损失实现了精确可控的颜色编辑，为图像生成中的色彩控制提供了有效解决方案。
...
TL;DR: 研究评估视觉语言模型从游戏视频推断玩家参与度的能力，发现现有模型存在感知-理解差距，难以跨游戏稳健推断人类参与度
...
Method: 使用GameVibe Few-Shot数据集在9款第一人称射击游戏中评估3个视觉语言模型，采用6种提示策略：零样本预测、基于Flow/GameFlow/自我决定理论/MDA的理论引导提示、检索增强提示，同时考虑逐点参与度预测和连续窗口间参与度变化的成对预测

Result: 零样本VLM预测普遍较弱，常无法超越简单的每游戏多数类基线；检索增强提示在某些设置下改善逐点预测，而成对预测在所有策略中都持续困难；理论引导提示未能可靠帮助，反而可能强化表面级捷径

Conclusion: 当前视觉语言模型存在感知-理解差距：虽然能识别可见的游戏玩法线索，但仍难以跨游戏稳健推断人类参与度
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态OOD检测方法存在两个主要问题：1) 依赖固定融合规则，假设静态环境，无法应对时间漂移；2) 对协变量偏移输入缺乏鲁棒性
...
TL;DR: TexEditor：基于Qwen-Image-Edit-2509构建的文本引导纹理编辑模型，通过TexBlender数据集和StructureNFT训练方法解决现有模型在纹理编辑中结构保持不足的问题，并在TexBench基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的编辑模型在纹理编辑时经常难以保持结构一致性，尽管预期修改仅涉及外观变化。作者观察到这一现象，旨在从数据和训练两个角度联合增强结构保持能力。

Method: 1. 构建TexBlender：使用Blender生成的高质量SFT数据集，为冷启动提供强结构先验；2. 提出StructureNFT：基于强化学习的方法，集成结构保持损失，将SFT阶段学到的结构先验迁移到真实场景；3. 建立TexBench：通用真实世界纹理编辑基准测试。

Result: 在现有Blender纹理基准和TexBench上的广泛实验表明，TexEditor持续优于Nano Banana Pro等强基线。在通用基准ImgEdit上的评估验证了其泛化能力。
...
TL;DR: FILT3R提出了一种无需训练、可插拔的潜在状态过滤层，将循环状态更新建模为令牌空间中的随机状态估计，通过自适应平衡记忆保留与新观测来提高3D重建的长期稳定性。
...
Method: FILT3R采用卡尔曼滤波器风格的增益计算，为每个令牌维护方差，在线估计过程噪声（基于候选令牌的EMA归一化时间漂移），自适应平衡记忆保留与新观测。

Result: FILT3R在稳定区域随着证据积累不确定性收缩而减小增益，在真实场景变化增加过程不确定性时提高增益，相比现有方法在深度、姿态和3D重建的长期稳定性方面表现更好。

Conclusion: FILT3R提供了一个可解释、可插拔的更新规则，将常见的覆盖和门控策略作为特例，提高了流式3D重建的长期稳定性。
...
TL;DR: HOMEY是一个结合YOLO与领域特定掩码机制的新型房产风险检测框架，能够检测17种风险相关类别，包括结构损坏、维护疏忽和安全隐患，通过启发式对象掩码增强弱信号，实现更准确可靠的检测。


<details>
  <summary>Details</summary>
Motivation: 自动化房产风险检测在计算机视觉领域是一个高影响但未被充分探索的前沿方向，对房地产、承保和保险业务有直接影响。当前缺乏专门针对房产风险检测的有效解决方案。

Method: 提出HOMEY框架，结合YOLO检测器、领域特定掩码机制和自定义损失函数。引入启发式对象掩码来增强杂乱背景中的弱信号，以及风险感知损失校准来平衡类别偏斜和严重性权重。

Result: 在真实房产图像上的实验表明，HOMEY相比基线YOLO模型实现了更优越的检测准确性和可靠性，同时保持快速推理速度。

Conclusion: HOMEY不仅提供检测功能，还支持可解释和成本效益高的风险分析，为可扩展的AI驱动房产保险工作流程奠定了基础。
...
Result: AI已从结构分析工具转变为能够理解和重写生命动态语言的通用模拟器，但仍面临数据分布偏差、机制可解释性有限等瓶颈

Conclusion: 人工智能正在向物理一致的生成模型、多模态基础架构和实验闭环系统发展，标志着从静态结构分析到动态生命语言模拟的根本转变
...
TL;DR: 该手册系统性地提出了面向车险领域的垂直整合AI范式，统一感知、多模态推理和生产基础设施，构建用于车辆风险评估和理赔处理的智能栈。


<details>
  <summary>Details</summary>
Motivation: 针对车险行业需要处理大规模真实世界部署的挑战，建立将现代人工智能转化为高可靠性生产系统的原则性框架，解决车辆损伤分析、理赔评估和核保工作流程的端到端自动化需求。

Method: 开发领域适配的Transformer架构，包括结构化视觉理解、关系型车辆表示学习和多模态文档智能，将这些组件组合成可扩展的流水线，并在泰国全国车险系统的实际约束下运行。

Result: 建立了统一感知、多模态推理和生产基础设施的垂直整合AI范式，实现了车辆损伤分析、理赔评估和核保工作流程的端到端自动化，并在大规模真实世界部署中得到验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法多为离线或缺乏实例级理解，限制了在真实机器人任务中的应用。需要实时的开放词汇全景建图系统来支持具身应用的环境感知与交互。

Method: 采用滑动窗口的局部到全局范式：1) 构建局部一致性地图，利用几何和语义线索构建3D分割聚类图；2) 更新全局地图，通过显式空间属性网格和双向二分3D高斯实例匹配融合局部与全局；3) 利用VLM特征实现开放词汇场景理解。

Result: 在广泛使用的数据集上实验表明，该方法在在线方法中取得更好性能，同时保持实时效率。

Conclusion: OnlinePG是首个在线开放词汇全景建图系统，有效结合几何重建与开放词汇感知，为具身应用提供实时实例级场景理解能力。
...
TL;DR: LVLMs展现类似人类的计数行为：小数量精确，大数量估计。研究发现共享的"计数电路"，通过轻量级干预仅用计数微调即可提升视觉推理能力。
...
Method: 使用受控的合成和真实世界基准测试，结合机制分析。引入两种新颖的可解释性方法：Visual Activation Patching和HeadLens，用于揭示跨视觉推理任务共享的结构化"计数电路"。提出轻量级干预策略，利用简单易得的合成图像专门对任意预训练LVLM进行计数微调。

Result: LVLMs表现出类似人类的计数行为：小数量精确，大数量估计噪声较大。发现了跨多种视觉推理任务共享的"计数电路"。仅用计数微调的干预不仅提高了分布内合成数据的计数准确率，还在分布外计数基准上平均提升+8.36%，在Qwen2.5-VL的复杂通用视觉推理任务上平均增益+1.54%。

Conclusion: 计数在视觉推理中扮演核心且有影响力的角色，通过针对性增强计数机制可能是提升整体视觉推理能力的潜在途径。轻量级计数微调能带来超出计数任务本身的广泛性能提升。
...
TL;DR: 提出3DreamBooth和3Dapter框架，实现3D感知的视频定制，通过解耦空间几何与时间运动，避免传统2D方法在3D物体定制中的几何重建不足问题。
...
Method: 提出3DreamBooth框架，采用1帧优化范式解耦空间几何与时间运动，通过仅更新空间表示将3D先验嵌入模型；结合3Dapter视觉条件模块，通过非对称条件策略进行多视角联合优化，作为动态选择性路由器查询视角特定的几何提示。

Result: 该方法能够在无需大量视频训练的情况下，实现3D感知的视频定制，保持跨视角的几何一致性，避免时间过拟合，并加速收敛过程。

Conclusion: 提出的3DreamBooth和3Dapter框架解决了3D感知视频定制的核心挑战，通过解耦空间与时间表示，实现了对真实3D物体的高质量、视角一致的视频生成。
...
TL;DR: 本文提出一种受生物启发的中心-外围注意力精炼框架，用于解决跨域少样本目标检测中的目标域散光问题，通过重塑注意力模式提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 跨域少样本目标检测面临严重的域偏移和数据稀缺问题。研究发现模型在目标域表现出分散、不聚焦的注意力模式（称为目标域散光问题），导致定位不精确和冗余预测，需要解决这一被忽视的现象。

Method: 受人类中央凹视觉系统启发，提出中心-外围注意力精炼框架：1）正模式精炼模块使用类特定原型重塑对语义对象的注意力；2）负上下文调制模块通过建模背景上下文增强边界判别；3）文本语义对齐模块通过跨模态线索加强中心-外围区分。

Result: 在六个具有挑战性的CD-FSOD基准测试上，该方法持续提升了检测精度，并建立了新的最先进结果，成功将散光注意力转化为聚焦模式。

Conclusion: 提出的生物启发方法有效解决了目标域散光问题，通过中心-外围注意力精炼框架显著改善了模型对目标域的适应能力，为跨域少样本目标检测提供了新的解决方案。
...
TL;DR: MVLM在临床工作流中的可靠性不足，CoDA框架通过构建临床可信的管道偏移来评估模型鲁棒性，发现链式组合比单阶段更破坏性能，并提出轻量级对齐修复策略


<details>
  <summary>Details</summary>
Motivation: 医学视觉-语言模型在放射学管道中应用日益广泛，但其在真实临床工作流中的可靠性尚未充分探索。现有鲁棒性评估通常假设干净、经过整理的输入或研究孤立的损坏，忽略了常规采集、重建、显示和传输操作，这些操作在保持临床可读性的同时会改变图像统计特性。

Method: 提出CoDA（链式分布）框架，通过组合采集类阴影、重建和显示重映射、传输和导出退化来构建临床可信的管道偏移。在掩码结构相似性约束下，CoDA联合优化阶段组合和参数以诱导失败同时保持视觉可信度。还提出基于教师引导的令牌空间适应与补丁级对齐的后修复策略。

Result: 在脑MRI、胸部X光和腹部CT上，CoDA显著降低了CLIP风格MVLM的零样本性能，链式组合始终比任何单阶段更具破坏性。专有多模态模型在CoDA偏移样本上显示出审计可靠性下降和持续高置信度错误，而测试的医学专用MLLM在医学图像质量审计方面存在明显缺陷。后修复策略提高了对存档CoDA输出的准确性。

Conclusion: 研究结果描述了MVLM部署的临床基础威胁面，并表明轻量级对齐可以提高部署中的鲁棒性。CoDA框架为评估医学视觉模型的临床鲁棒性提供了系统方法，揭示了当前模型在实际工作流中的脆弱性。
...
Result: 在nuScenes等基准测试中，CausalVAD实现了最先进的规划精度和安全性，并且在针对因果混淆的数据偏差和噪声场景中表现出优越的鲁棒性

Conclusion: CausalVAD通过因果干预有效解决了自动驾驶规划模型中的因果混淆问题，提升了模型在复杂真实场景中的可靠性和安全性，为端到端驾驶系统提供了更稳健的解决方案
...
TL;DR: 提出跨层注意力传播方法HAViT，通过保存和融合历史注意力矩阵来增强Vision Transformers的层间信息流，在CIFAR-100和TinyImageNet上分别提升1.33%和1.25%准确率。
...
Method: 提出跨层注意力传播方法，保存历史注意力矩阵并在各编码器层进行融合，通过简单的注意力矩阵存储和混合操作实现，最小化架构改动。
...
Conclusion: 跨层注意力传播方法有效增强了Vision Transformers的特征学习和优化动态，通过历史注意力信息的保留和融合实现了性能提升，且方法简单易实现。
...
<details>
  <summary>Details</summary>
Motivation: 传统非局部方法直接从彩色图像的RGB通道提取图像块，由于相似性主要基于独立通道的灰度值，无法精细描述颜色信息。本文旨在通过考虑彩色图像在饱和度-亮度通道中的图像块相似性，提出新的非局部正则化方法。

Method: 首先建立基于饱和度-亮度相似度的非局部全变差，将彩色图像块的饱和度-亮度相似性融入非局部梯度中；然后基于此构建非局部变分模型；最后采用Bregmanized算子分裂方法设计有效算法求解优化问题，并研究算法收敛性。

Result: 数值实验表明，所提模型在视觉质量和定量指标（包括PSNR、SSIM、QSSIM和S-CIELAB颜色误差）方面均优于其他测试方法。

Conclusion: 提出的基于饱和度-亮度相似度的非局部变分方法能更有效地描述彩色图像的颜色信息，在彩色图像恢复任务中取得了更好的性能。
...
TL;DR: 提出了一种基于自然语言描述动作单元（AU）的面部行为合成方法，解决了传统线性AU组合在处理冲突AU时产生解剖学上不合理伪影的问题，并创建了首个大规模文本-图像配对数据集BP4D-AUText。


<details>
  <summary>Details</summary>
Motivation: 当前基于动作单元（AU）的面部行为合成方法通常将AU编码为独热向量，将复合表情建模为单个AU的简单线性组合。这种线性方法在处理冲突AU（激活相同面部肌肉但作用相反）时会产生解剖学上不合理的伪影和不自然的运动叠加，限制了面部行为合成的真实性和表现力。

Method: 提出了一种新颖的方法，通过AU的自然语言描述来表示面部行为。该方法包括：1）创建BP4D-AUText数据集，这是首个大规模文本-图像配对数据集，通过基于规则的动态AU文本处理器对BP4D和BP4D+数据集进行合成；2）提出VQ-AUFace生成模型，利用面部结构先验从文本合成真实多样的面部行为。

Result: 广泛的定量实验和用户研究表明，该方法在生成面部表情方面显著优于现有方法。特别是在涉及冲突AU的挑战性条件下，能够生成解剖学上合理、行为丰富且感知上令人信服的面部表情。

Conclusion: 通过将AU表示为自然语言描述，该方法既保留了AU框架的表现力，又能够显式建模复杂和冲突的AU，同时解锁了现代文本到图像模型进行高保真面部合成的潜力，为面部行为合成提供了更精确和自然的解决方案。
...
TL;DR: 对缅甸手写数字数据集myMNIST进行了首次系统基准测试，评估了11种架构，发现CNN表现最佳，PETNN(GELU)紧随其后，能量模型JEM表现良好，KAN变体提供有意义的替代基准。


<details>
  <summary>Details</summary>
Motivation: 缅甸手写数字数据集myMNIST对缅甸NLP/AI研究很重要，但缺乏系统基准测试。本研究旨在建立可复现的基准，评估不同建模范式在该数据集上的表现，促进缅甸数字识别研究。

Method: 评估了11种架构：经典深度学习模型（MLP、CNN、LSTM、GRU、Transformer）、近期替代模型（FastKAN、EfficientKAN）、能量模型（JEM）和物理启发的PETNN变体（Sigmoid、GELU、SiLU）。使用精确率、召回率、F1分数和准确率作为评估指标。

Result: CNN表现最佳（F1=0.9959，准确率=0.9970），PETNN(GELU)紧随其后（F1=0.9955，准确率=0.9966），优于LSTM、GRU、Transformer和KAN变体。JEM表现良好（F1=0.9944，准确率=0.9958）。KAN模型准确率约0.992，提供有意义的替代基准。

Conclusion: 建立了myMNIST数据集的可复现基准；PETNN相对于经典和Transformer模型表现出色；量化了物理启发PETNN与真实能量模型（JEM）之间的差距；发布基准以促进缅甸数字识别研究和区域文字评估。
...
TL;DR: 本文提出TGA-ZSR和Comp-TGA方法，通过文本引导注意力机制增强CLIP模型的零样本对抗鲁棒性，分别获得9.58%和11.95%的改进。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型（如CLIP）在零样本能力上表现出色，但被发现容易受到对抗样本攻击。研究发现对抗扰动会导致文本引导注意力偏移，因此需要增强CLIP的对抗鲁棒性同时保持其泛化能力。

Method: 提出TGA-ZSR框架，包含局部注意力精炼模块和全局注意力约束模块。进一步提出Comp-TGA方法，整合类别提示引导的前景注意力和非类别提示引导的反向注意力，形成互补的注意力机制。

Result: 实验验证TGA-ZSR和Comp-TGA在16个数据集上分别比当前最先进技术提高了9.58%和11.95%的零样本鲁棒准确率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Speculative Jacobi Decoding (SJD)方法在加速自回归文本到图像合成时，由于视觉生成的高熵特性，在复杂区域草稿token接受率低，形成了严重的吞吐量瓶颈。

Method: 提出SJD-PAC框架：1) 采用主动草稿策略提高高熵区域的局部接受率；2) 引入自适应延续机制，在初始拒绝后继续序列验证，避免完全重采样。

Result: 在标准文本到图像基准测试中，SJD-PAC实现了3.8倍的加速，同时保持无损的图像质量。

Conclusion: SJD-PAC通过主动草稿和自适应延续的协同优化，显著提高了每步的平均接受长度，在严格保持目标分布的同时大幅提升了推理速度。
...
TL;DR: 本文提出Cross-Modal Context Learning (CCL)方法，通过TARP、LCT、DCR和UCG等模块解决双流transformer在音视频生成中的局限性，实现更高质量的音视频内容生成。


<details>
  <summary>Details</summary>
Motivation: 当前基于双流transformer的音视频联合生成方法存在多个局限性：门控机制导致模型流形变化、跨模态注意力引入多模态背景区域偏差、多模态无分类器引导在训练和推理中的不一致性，以及多条件之间的冲突。

Method: 提出Cross-Modal Context Learning (CCL)框架，包含：1) TARP模块增强音视频潜在表示的时间对齐；2) CCA模块中的LCT和DCR提供稳定的无条件锚点并动态路由跨模态信息；3) UCG在推理时利用LCT的无条件支持促进不同形式的CFG。

Result: CCL在综合评估中相比近期学术方法取得了最先进的性能，同时所需资源显著减少。

Conclusion: CCL通过精心设计的模块有效解决了双流transformer在音视频生成中的多个关键问题，提高了模型收敛速度、生成质量和训练-推理一致性。
...
TL;DR: CNN模型SegResNet在腹部多器官分割任务中表现优于三种混合Transformer模型，在异构数据集上CNN仍具竞争力


<details>
  <summary>Details</summary>
Motivation: 准确的多器官分割对于计算机辅助诊断和治疗至关重要，需要系统比较CNN和Transformer架构在医学图像分割中的性能

Method: 在RATIC数据集（206个CT扫描，23个机构）上系统比较三种混合Transformer模型（UNETR、SwinUNETR、UNETR++）与CNN基线SegResNet，使用相同的预处理和训练条件，以Dice相似系数为主要评估指标

Result: CNN-based SegResNet在所有器官上表现最佳，优于所有混合Transformer模型；在Transformer模型中，UNETR++表现最接近CNN，UNETR收敛速度最快
...
TL;DR: OpenT2M是一个包含2800小时高质量人体运动数据的大规模开源数据集，通过物理可行性验证和多粒度过滤确保质量，并配有详细的时间标注。基于此数据集开发的MonoFrill模型使用新颖的2D-PRQ运动标记化器，在文本到运动生成任务上取得了优异性能。
...
Method: 1) 创建OpenT2M数据集：包含超过2800小时高质量人体运动数据，通过物理可行性验证和多粒度过滤进行质量控制，提供详细的秒级文本标注；2) 开发自动化流水线生成长时程序列；3) 基于OpenT2M构建MonoFrill模型，核心是2D-PRQ运动标记化器，通过将人体划分为生物部位来捕捉时空依赖关系。

Result: OpenT2M显著提升了现有T2M模型的泛化能力，2D-PRQ在重建任务上表现优异并具备强大的零样本性能。实验表明该数据集和模型能够解决长期存在的数据质量和基准测试问题。

Conclusion: OpenT2M大规模高质量数据集和MonoFrill模型通过创新的2D-PRQ标记化器，有效解决了文本到运动生成领域的数据质量和泛化问题，有望推动该领域的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估协议将AI生成视频检测视为二元分类问题，依赖粗粒度指标如整体准确率，无法深入了解LVLMs在哪些方面成功或失败，需要更细粒度的评估框架来诊断模型能力。

Method: 构建GenVideoLens基准测试，包含400个高度欺骗性的AI生成视频和100个真实视频，由专家在15个真实性维度上进行标注，涵盖感知、光学、物理和时序线索。评估了11个代表性LVLMs，并进行时序扰动实验。

Result: 研究发现明显的维度不平衡：LVLMs在感知线索上表现相对较好，但在光学一致性、物理交互和时序因果推理方面表现不佳。不同模型在维度间表现差异显著，较小的开源模型有时在特定真实性线索上优于更强的专有模型。时序扰动实验显示当前LVLMs对时序信息的利用有限。

Conclusion: GenVideoLens为LVLM行为提供了诊断性见解，揭示了关键能力差距，并为改进未来AI生成视频检测系统提供了指导。该基准测试有助于更全面地理解模型在细粒度维度上的表现，推动该领域的发展。
...
TL;DR: SVOO提出了一种无需训练的视频生成稀疏注意力框架，通过离线层间稀疏度分析和在线双向共聚类，解决了现有方法忽略层异质性和查询-键耦合的问题，实现了更好的质量-加速权衡。
...
Method: SVOO采用两阶段范式：1) 离线层间敏感度分析以获取每层固有的剪枝水平；2) 通过新颖的双向共聚类算法实现在线块级稀疏注意力。

Result: 在七个广泛使用的视频生成模型上的实验表明，SVOO在质量-加速权衡方面优于最先进方法，在Wan2.1上实现高达1.93倍的加速，同时保持高达29 dB的PSNR。

Conclusion: SVOO通过揭示注意力稀疏性是每层固有属性的关键洞察，提出了一种有效的无训练稀疏注意力框架，显著提升了视频生成的效率而不牺牲质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法在视觉保真度方面取得了进展，但确保物理一致的运动仍然是一个基本挑战。真实世界的物体运动发生在三维空间中，而视频观测仅提供这种动态的部分、视角相关的投影。

Method: 提出PhysVideo两阶段框架：第一阶段Phys4View使用物理感知注意力捕捉物理属性对运动动态的影响，通过几何增强的跨视图注意力和时间注意力增强时空一致性；第二阶段VideoSyn使用生成的前景视频作为指导，学习前景动态与背景上下文之间的交互，实现可控视频合成。

Result: 构建了包含40K场景、每个场景有四个正交视角的PhysMV数据集（总计160K视频序列）。实验表明PhysVideo在物理真实性和时空一致性方面显著优于现有视频生成方法。
...
TL;DR: MeInTime提出了一种基于扩散模型的跨年龄人脸修复方法，通过解耦身份和年龄建模，在仅有跨年龄参考图像的情况下实现身份保真和年龄一致的修复效果。
...
Method: 1. 解耦身份和年龄条件建模；2. 训练时通过新注意力机制有效注入身份特征；3. 引入门控残差融合模块整合退化特征和身份表示；4. 推理时提出无需训练的年龄感知梯度引导采样策略，将身份感知去噪潜在向量推向目标年龄语义流形。

Result: 大量实验表明，MeInTime在身份保持和年龄一致性方面优于现有的人脸修复方法。

Conclusion: MeInTime成功将基于参考的人脸修复从同年龄设置扩展到跨年龄设置，在仅有跨年龄参考图像的情况下实现了身份保真和年龄一致的修复效果。
...
<details>
  <summary>Details</summary>
Motivation: 直播电商已成为现代重要的传播形式，但主播在直播过程中需要高效处理产品推广和实时回应观众问题，这需要大量准备时间和专业能力。因此需要开发一个AI助手来简化产品推广流程，提高内容参与度，并实现与观众的即时互动。

Method: 系统包含两个核心模块：离线模块处理多模态产品信息（如图片、视频、文本），将其转化为结构化产品数据并生成合规的推广文案；在线模块在直播过程中，允许主播点击观众问题，利用离线模块生成的结构化产品信息和基于流式架构维护的事件级历史记忆，实时生成回应。

Result: 在收集的TikTok直播帧数据集上，该方法实现了0.913的问题识别准确率和0.876的回应质量评分。系统显著减少了推广准备时间，增强了内容参与度，并能及时响应观众询问，展示了实际应用的巨大潜力。

Conclusion: Click-to-Ask系统通过结合离线和在线AI组件，有效解决了直播电商中产品推广和实时互动的挑战，提高了直播电商的效率和效果，具有重要的实际应用价值。
...
TL;DR: 本文提出基于LLM作为评判的PDF表格提取评估框架，通过合成PDF和arXiv表格构建基准，相比传统规则指标显著提升与人类判断的相关性。


<details>
  <summary>Details</summary>
Motivation: PDF表格提取对于大规模科学数据挖掘和知识库构建至关重要，但现有评估方法依赖基于规则的指标，无法捕捉表格内容的语义等价性。

Method: 1) 使用arXiv表格创建合成PDF基准，确保真实复杂性和多样性；2) 采用LLM作为评判进行语义表格评估；3) 构建匹配管道以适应解析器输出的不一致性；4) 进行包含1500多个质量判断的人类验证研究。

Result: LLM评估与人类判断的相关性显著更高（Pearson r=0.93），优于树编辑距离相似性（TEDS，r=0.68）和网格表格相似性（GriTS，r=0.70）。评估21个当代PDF解析器在100个合成文档的451个表格上显示出显著性能差异。
...
Method: 提出了Switch框架，包含两个关键创新：1) 多尺度切换策略，采用分层补丁混合实现均匀空间覆盖；2) 频域切换策略，在傅里叶空间进行幅度切换并结合对比学习以获得鲁棒特征表示。这些组件集成在教师-学生架构中，有效利用标注和未标注数据。

Result: 在六个不同的超声数据集（淋巴结、乳腺病变、甲状腺结节和前列腺）上评估，Switch在5%标注比例下取得了显著改进：LN-INT数据集80.04% Dice，DDTI数据集85.52% Dice，前列腺数据集83.48% Dice。半监督方法甚至超越了全监督基线，同时保持了参数效率（180万参数）。

Conclusion: Switch框架通过创新的多尺度和频域切换策略，有效解决了医学超声图像分割中的数据稀缺和特征表示问题，在资源受限的医学成像应用中表现出优越性能。
...
TL;DR: 该综述系统回顾了多模态计算病理学的最新进展，重点关注全切片图像分析、自监督学习、多模态数据生成、参数高效适应和多智能体协同推理等方向，旨在解决计算病理学中的分辨率、标注、可解释性等挑战。


<details>
  <summary>Details</summary>
Motivation: 全切片成像技术虽然推动了数字病理学的发展，但面临四大挑战：超高分辨率带来的计算负担、专家标注数据有限、多模态信息整合困难、以及模型透明度不足影响临床可信度。需要系统总结现有方法并探索未来发展方向。

Method: 该综述采用系统性分析方法，从四个研究方向展开：1) 自监督表示学习和结构感知的token压缩技术；2) 多模态数据生成与增强；3) 参数高效适应和推理增强的少样本学习；4) 多智能体协同推理实现可信诊断。特别关注token压缩如何实现跨尺度建模，以及多智能体机制如何模拟病理学家的"思维链"过程。

Result: 综述系统梳理了多模态计算病理学的最新进展，展示了各种技术如何应对现有挑战。token压缩技术能够有效处理超高分辨率图像，多模态数据生成缓解了标注数据稀缺问题，参数高效适应提高了模型实用性，多智能体协同推理增强了诊断的可解释性和可信度。

Conclusion: 未来进展依赖于统一的多模态框架，需要整合高分辨率视觉数据与临床和生物医学知识，以支持可解释且安全的AI辅助诊断。多智能体协同推理机制通过模拟病理学家的思维过程，为实现不确定性感知的证据融合和透明化诊断提供了有前景的方向。
...
TL;DR: SCNP是一种高效的拓扑精度提升方法，通过惩罚logits中最差分类的邻居像素，强制模型在改进像素预测前先改善其邻居预测，从而提升分割结果的拓扑准确性。
...
Method: SCNP通过惩罚logits中最差分类的邻居像素来工作，强制模型在改进像素本身的预测之前，先改善其邻居的预测。这种方法可以集成到多种损失函数中，适用于不同的分割框架。

Result: 在13个不同结构形态和图像模态的数据集上验证了SCNP的有效性，成功集成到三个语义分割和实例分割框架中，并证明可以集成到多种损失函数中提升拓扑精度。

Conclusion: SCNP是一种高效、通用的拓扑精度提升方法，能够解决现有方法的局限性，易于集成到现有训练流程，适用于多种结构形态和分割任务。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成方面表现出色，但实际部署受到高内存使用和计算成本的严重限制。现有的静态位宽分配方法忽视了不同扩散时间步中激活值的量化难度，导致效率与质量之间的权衡不够优化。
...
Result: 大量实验表明，该方法实现了1.92倍的端到端加速和3.32倍的内存减少，为视频扩散变换器的高效推理设定了新的基准。

Conclusion: 提出的自适应精度策略和时间增量缓存机制能够在保持生成质量的同时实现显著的效率提升，为视频扩散变换器的实际部署提供了有效的解决方案。
...
TL;DR: WeNLEX：一种弱监督模型，用于生成多标签胸部X光分类的自然语言解释，通过特征空间图像匹配确保忠实性，使用少量临床标注数据保持合理性


<details>
  <summary>Details</summary>
Motivation: 现有方法通常使用标注解释数据进行显式监督，虽然生成的解释看似合理，但并不忠实于模型的实际推理过程。需要一种既能生成忠实解释又能保持合理性的方法。

Method: 提出WeNLEX弱监督模型：1）通过将自然语言解释生成的图像与原始图像在黑盒模型特征空间中进行匹配来确保忠实性；2）通过与少量临床标注解释数据库进行分布对齐来保持合理性；3）可在后处理和模型内两种设置下工作。

Result: 1）仅需每个诊断5个真实解释即可生成忠实且合理的解释；2）在模型内设置下，与独立分类器相比，分类AUC提升2.21%；3）通过更换数据库可适应不同目标受众（如非医学用户）。

Conclusion: WeNLEX能够生成既忠实于模型推理又合理的自然语言解释，且可提高下游任务性能，具有适应不同受众的灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 现有无源域适应方法过度依赖邻域预测相似性，这会加速源知识遗忘并增加对局部噪声过拟合的敏感性，需要一种能平衡知识保留和域适应的解决方案。

Method: 提出ProCal概率校准方法：1）通过双模型协作预测机制（源模型初始预测与当前模型在线输出结合）动态校准邻域概率；2）设计联合优化目标，结合软监督损失和多样性损失来指导目标模型。

Result: 理论分析表明ProCal收敛到源知识和目标信息有效融合的均衡状态，减少知识遗忘和过拟合。在4个公共数据集的31个跨域任务上进行广泛实验验证了方法的有效性。

Conclusion: ProCal通过概率校准机制成功解决了无源域适应中源知识遗忘和局部噪声过拟合的问题，实现了知识保留和域适应的平衡，为SFDA提供了一种有效的解决方案。
...
Method: 提出SEAR微调策略，在相对较小的RGB-T数据集上对预训练的几何Transformer进行适配，使其能够有效处理多模态输入

Result: 在所有指标上显著超越现有方法（如AUC@30提升超过29%），在低光照和浓烟等挑战性条件下仍能可靠工作，推理时间开销可忽略不计

Conclusion: SEAR成功将预训练几何模型适配到多模态场景，同时提出了新的RGB-T数据集为未来研究提供基准，代码和模型已开源
...
TL;DR: 本文对医学图像分割中的不确定性量化进行了全面实证研究，重点分析了数据不确定性（AU）和模型不确定性（EU）的交互作用和纠缠问题，评估了不同组合方法在下游任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 在医学图像分割等安全关键应用中，不确定性量化至关重要。虽然已有许多方法分别建模数据不确定性（如Probabilistic UNet、Diffusion）和模型不确定性（如集成学习、MC Dropout），但两者结合时的交互作用尚不清楚。此外，近期研究发现AU和EU之间存在显著纠缠，这削弱了不确定性分解的可解释性和实际应用价值。

Method: 1. 进行了全面的实证研究，涵盖了广泛的AU-EU模型组合；2. 提出了量化不确定性纠缠的指标；3. 在多个下游UQ任务中评估了不同组合方法的表现，包括分布外检测、模糊性建模和校准。

Result: 1. 在分布外检测任务中，集成学习方法表现出更低的纠缠和更优的性能；2. 在模糊性建模和校准任务中，最佳模型依赖于具体数据集，softmax/SSN方法表现良好，Probabilistic UNets纠缠程度较低；3. softmax集成在所有任务中都表现出色；4. 分析了不确定性纠缠的潜在来源。

Conclusion: 本文揭示了不确定性纠缠对医学图像分割中UQ方法可解释性和实用性的影响，为缓解这一效应提供了方向。集成学习方法在减少纠缠方面表现突出，而最佳模型选择需考虑具体应用场景和任务需求。
...
<details>
  <summary>Details</summary>
Motivation: 当前基础模型大多采用难以解释的架构，这在医疗影像等高风险领域是一个关键问题。需要开发既能保持高性能又具有内在可解释性的模型。

Method: 提出Dual-IFM模型，具有双重可解释性设计：1）通过类别证据图提供单个图像的局部可解释性；2）通过2D投影层提供整个数据集的全局可解释性。模型在超过80万张彩色眼底照片上进行自监督预训练。

Result: 模型性能达到与参数多16倍的SOTA基础模型相当的水平，同时能在分布外数据上提供可解释的预测。大规模自监督预训练与内在可解释性结合能产生稳健的视网膜成像表征。

Conclusion: 大规模自监督预训练与内在可解释性设计相结合，可以为视网膜成像等高风险医疗领域开发出既高性能又可信赖的基础模型。
...
TL;DR: HORNet是一个轻量级帧选择策略，使用GRPO训练，为冻结的VLM选择关键帧，减少99%输入帧和93%处理时间，同时提升VQA性能。
...
Result: 在6个基准测试的341,877个QA对和114.2小时视频上评估：减少99%输入帧和93%处理时间；MSVD-QA上F1提升1.7%；NExT-QA上比均匀采样高7.3分；策略可跨VLM迁移，无需重新训练。

Conclusion: 优化VLM看到的内容（输入选择）是优化其生成内容（输出）的实用补充方法，能同时提升效率和性能，且具有良好泛化能力和跨模型迁移性。
...
TL;DR: Motion-o：一种基于运动中心的视频理解扩展，通过显式轨迹推理增强视觉语言模型的时空推理能力


<details>
  <summary>Details</summary>
Motivation: 现有视频推理研究主要关注时空证据链，但缺乏对物体如何在观察间移动的显式推理。没有先前工作明确连接连续观察来阐述运动模式，导致轨迹理解隐式且难以验证。

Method: 1. 提出空间-时间-轨迹（STT）推理形式化框架；2. 引入轨迹基础数据集，通过增强稀疏关键帧监督获得更密集的边界框轨迹；3. 设计Motion Chain of Thought（MCoT）结构化推理路径，使用<motion/>标签总结每个物体的方向、速度和尺度变化；4. 设计奖励函数，使模型直接基于视觉证据推理，无需架构修改。

Result: Motion-o改进了时空基础和轨迹预测，同时保持与现有框架的完全兼容性，确立了运动推理作为基于证据的视频理解的关键扩展。

Conclusion: Motion-o通过显式轨迹推理增强了视频理解能力，使物体轨迹可验证，为证据驱动的视频推理提供了重要扩展。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉上下文学习（VICL）方法采用补丁级融合框架和模型无关的监督，限制了信息线索的利用和性能提升
...
<details>
  <summary>Details</summary>
Motivation: PET在神经退行性疾病诊断中很重要，但成本高且有辐射暴露；MRI无此限制但诊断敏感性较低。现有生成方法主要关注结构保持而忽视了病理感知的需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖类别特定模板或计算量大，且在3D中产生物理不一致的手-物体对齐。需要一种快速、类别无关的框架来重建动态手-物体交互，用于AR/VR、机器人和具身AI应用。

Method: 使用2D高斯泼溅表示手和物体为密集、视图一致的高斯圆盘。引入三个关键创新：1)几何先验检索和一致性损失补全被遮挡物体区域；2)抓取感知对齐细化手平移和物体尺度确保真实接触；3)手感知背景损失避免惩罚手遮挡的物体区域。

Result: 在ARCTIC、HO3D和野外数据集上进行了广泛实验，在3D重建和2D渲染质量方面达到了最先进的准确性。实现了完整、物理一致且可动画的重建，运行速度比现有类别无关方法快一个数量级。

Conclusion: GHOST为真实手-物体交互建模提供了一个高效、鲁棒的解决方案，能够从单目RGB视频快速重建动态交互，在准确性和效率方面均优于现有方法。
...
TL;DR: 本文提出了一种基于无监督对比学习的新型3D形状匹配方法，通过最大化正相似对一致性、最小化负相似对一致性来提升特征质量，并设计简化的功能图学习架构，无需昂贵的功能图解算器，在精度和效率上均达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有深度功能图方法主要关注优化点对点和功能图，而非直接提升嵌入空间的特征表示，导致特征质量不足和匹配性能欠佳。同时，这些方法严重依赖传统功能图技术，如耗时的功能图解算器，计算成本高昂。

Method: 1) 提出无监督对比学习框架，通过最大化正相似对一致性、最小化负相似对一致性来提升特征的一致性和可区分性；2) 设计简化的功能图学习架构，无需计算昂贵的功能图解算器和多个辅助功能图损失；3) 将这两个组件集成到统一的双分支流程中。

Result: 该方法在精度和效率上均达到最先进水平，在近等距、非等距和拓扑不一致等各种挑战性基准测试中表现优异，甚至超越了监督技术，同时计算效率显著提升。

Conclusion: 本文首次引入基于无监督对比学习的高效鲁棒3D形状匹配方法，通过改进特征学习和简化架构，在保持高精度的同时大幅提升计算效率，为3D形状匹配领域提供了新的解决方案。
...
TL;DR: VGGT-360是一个无需训练、零样本的几何一致全景深度估计框架，通过多视角3D重建和全景重投影实现，包含三个即插即用模块：不确定性引导自适应投影、结构显著性增强注意力、相关性加权3D模型校正。


<details>
  <summary>Details</summary>
Motivation: 现有无需训练的方法通常是视角独立的，缺乏全景一致性。本文旨在利用VGGT类基础模型的内在3D一致性，将碎片化的单视角推理统一为连贯的全景理解，实现几何一致的全景深度估计。

Method: 提出全景到3D再到深度的统一框架：1) 不确定性引导自适应投影：将全景切片为透视视图，通过梯度不确定性估计在几何贫乏区域分配更密集的视图；2) 结构显著性增强注意力：在VGGT注意力层中注入结构感知置信度，增强对几何可靠区域的关注；3) 相关性加权3D模型校正：使用注意力推断的相关性分数重新加权重叠点，优化重建的3D模型。

Result: 在多个分辨率和多样的室内外数据集上，VGGT-360超越了现有的训练方法和无需训练的最先进方法，表现出优越的性能。

Conclusion: VGGT-360通过将全景深度估计重新定义为基于多视角3D重建的全景重投影任务，实现了无需训练的几何一致性，其三个模块协同工作，显著提升了全景深度估计的准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型对齐方法存在两个主要问题：1）数据依赖性强（SFT需要高质量图像成本高，DPO方法需要大规模偏好数据集但质量不一致）；2）计算效率低下。需要一种既能减少训练数据需求又能保持计算效率的方法。

Method: 提出Composite Reward Assisted Fine-Tuning (CRAFT)框架，包含两个核心组件：1）Composite Reward Filtering (CRF)技术，用于构建高质量且一致的训练数据集；2）增强版SFT微调。该方法在理论上被证明优化了基于群体强化学习的下界。

Result: 仅使用100个样本的CRAFT就能轻松超越需要数千个偏好配对样本的最新SOTA偏好优化方法。在收敛速度方面，CRAFT比基线偏好优化方法快11-220倍，显示出极高的效率。

Conclusion: CRAFT提供了一种轻量级但强大的扩散模型微调范式，显著减少了训练数据需求并保持了计算效率，通过复合奖励过滤和增强SFT实现了高质量的对齐效果。
...
TL;DR: 本文提出了两种简约的指纹增强方法：上下文滤波方法和基于学习的方法，在保持简单性的同时超越了复杂的现有方法，在低质量指纹图像上表现优异。


<details>
  <summary>Details</summary>
Motivation: 指纹识别系统依赖指纹图像质量，但现有增强方法在处理低质量指纹时效果有限且计算复杂，需要更简单有效的方法。

Method: 提出了两种新颖方法：1）上下文滤波方法；2）基于学习的方法。这两种方法都采用简约设计理念，优先考虑简单性和有效性。

Result: 新方法在具有挑战性的潜指纹数据库上验证，一致优于复杂的现有方法，能生成更清晰、更准确、噪声更少的图像。
...
TL;DR: GenHOI框架通过整合层次语义知识和手部先验，结合多模态掩码建模，在严重遮挡条件下实现泛化的3D手-物体姿态估计


<details>
  <summary>Details</summary>
Motivation: 单张RGB图像中的3D手-物体姿态估计面临物体外观多样性和交互模式变化的挑战，特别是在严重遮挡条件下，现有方法泛化能力有限

Method: 1) 引入层次语义提示编码物体状态、手部配置和交互模式；2) 采用多模态掩码建模策略处理RGB图像、预测点云和文本描述；3) 利用手部先验作为空间参考提取隐式交互约束

Result: 在DexYCB和HO3Dv2基准测试中达到最先进性能，证明了在严重遮挡条件下对未见物体和新颖交互的泛化能力

Conclusion: GenHOI通过整合语义知识和多模态学习，有效解决了遮挡条件下的手-物体姿态估计问题，为未见物体和交互模式提供了鲁棒的泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的分割方法主要依赖专用掩码解码器或多重额外标记，本文旨在探索能否仅通过MLLM自身实现分割，消除对外部解码器的依赖。

Method: 1) 保留原始未压缩分辨率的图像特征，并用从MLLM处理的压缩特征中提取的残差特征进行补充；2) 分别对经过和未经过LLM处理的图像特征进行像素重排操作，增强特征细节；3) 设计具有图像-图像和图像-分割双感知路径的注意力掩码，促进像素与分割标记间的特征交互。

Result: 在多个分割任务上的综合实验验证了SELF1E能够达到与基于专用掩码解码器方法竞争的性能，证明了MLLM中无需解码器的分割可行性。

Conclusion: SELF1E成功展示了仅使用单个分割嵌入即可在MLLM中实现竞争性分割，为消除外部解码器依赖提供了有效解决方案，推动了MLLM在分割任务上的自包含发展。
...
TL;DR: TerraScope是一个统一的视觉语言模型，专门用于像素级地理空间推理，支持多模态（光学和SAR）输入和多时间序列分析，在像素级地理空间推理任务上显著优于现有VLM模型。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在地球观测任务中难以将复杂的空间推理与精确的像素级视觉表示相结合，需要一种能够进行像素级地理空间推理的模型。

Method: 提出TerraScope统一视觉语言模型，具有模态灵活推理（处理单模态输入并自适应融合多模态）和多时间推理能力；构建Terra-CoT数据集（100万样本，包含像素级掩码）；创建TerraScope-Bench基准测试（6个子任务，评估答案准确性和掩码质量）。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成模型能产生高保真视频，但常存在3D空间几何不一致问题。现有评估方法无法准确表征这些不一致：保真度导向的指标对几何失真不敏感，而一致性基准常惩罚有效的动态前景。
...
TL;DR: SwiftTailor是一个两阶段3D服装生成框架，通过几何图像表示统一缝纫图案推理和网格合成，显著提升推理速度同时保持高质量输出。
...
Method: SwiftTailor采用两阶段框架：1) PatternMaker - 轻量级视觉语言模型，从多种输入模态预测缝纫图案；2) GarmentSewer - 高效密集预测变换器，将图案转换为Garment Geometry Image（在统一UV空间中编码所有服装面板的3D表面）。最后通过逆映射过程重建3D网格，包含重新网格化和动态缝合算法。
...
<details>
  <summary>Details</summary>
Motivation: 当前的前瞻性视频大语言模型依赖逐帧触发决策，面临效率与准确性的权衡困境。需要一种能够在严格计算约束下实现高效准确前瞻性视频理解的解决方案。

Method: 提出Em-Garde框架，将语义理解与流式感知解耦：1）查询时，指令引导的提案解析器将用户查询转换为结构化、感知基础的视觉提案；2）流式处理时，轻量级提案匹配模块执行基于嵌入的高效匹配来触发响应。

Result: 在StreamingBench和OVO-Bench上的实验表明，相比先前模型，在前瞻性响应准确性和效率方面均取得一致改进，验证了在严格计算约束下实现前瞻性视频理解的有效性。

Conclusion: Em-Garde通过解耦语义理解与流式感知，解决了前瞻性视频理解中的效率-准确性权衡问题，为计算受限环境下的流式视频理解提供了有效解决方案。
...
TL;DR: SignAgent是一个基于大语言模型的智能框架，用于大规模、语言学基础的手语标注和数据集构建，解决了传统方法忽略语言学细节和人工标注效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 传统手语计算方法主要在gloss层面操作，忽略了重要的语言学细节，而人工语言学标注速度慢、成本高，成为构建大规模、音系学感知数据集的瓶颈。

Method: SignAgent包含两个核心组件：SignAgent Orchestrator（协调各种语言学工具的推理LLM）和SignGraph（提供词汇和语言学基础的基于知识的LLM）。框架通过多模态证据进行约束性标注，并结合视觉相似性和音系重叠进行推理。

Result: 该框架在两个下游标注任务中表现优异：1）伪gloss标注任务中，使用多模态证据提取和排序合适的gloss标签；2）ID glossing任务中，通过推理视觉相似性和音系重叠来检测和优化视觉聚类，正确识别和分组词汇手语变体。

Conclusion: SignAgent的智能方法在大规模、语言学感知的数据标注和构建方面取得了显著成效，为手语计算研究提供了有效的解决方案。
...
TL;DR: 提出一种鲁棒的实时RGB SLAM系统，通过可微分不确定性感知束调整处理动态环境，在杂乱动态场景中实现最先进的相机位姿和场景几何重建，运行速度约10FPS。
...
Result: 在杂乱动态场景中实现了最先进的相机位姿和场景几何重建，同时以约10FPS的速度实时运行。

Conclusion: 提出的系统通过可微分不确定性感知束调整和多视角特征不一致性估计，能够有效处理动态环境中的SLAM问题，在真实世界环境中实现鲁棒的跟踪和重建。
...
### [82] [SAVeS: Steering Safety Judgments in Vision-Language Models via Semantic Cues](https://arxiv.org/abs/2603.19092)
*Carlos Hinojosa,Clemens Grange,Bernard Ghanem*

Main category: cs.CV

TL;DR: 研究发现视觉语言模型的安全决策高度依赖语义线索而非真实的视觉理解，存在被简单语义干预操纵的漏洞


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地部署在现实世界和具身环境中，安全决策依赖于视觉上下文，但尚不清楚哪些视觉证据驱动这些判断。研究者希望探究多模态安全行为是否可以被简单的语义线索所引导。

Method: 提出了语义引导框架，在不改变底层场景内容的情况下应用受控的文本、视觉和认知干预。创建了SAVeS基准测试，用于评估语义线索下的情境安全性，并设计了评估协议来区分行为拒绝、基于安全推理和错误拒绝。

Result: 实验表明，多个视觉语言模型的安全决策对语义线索高度敏感，表明模型依赖学习的视觉-语言关联而非基于视觉的理解。进一步证明自动化引导管道可以利用这些机制，突显了多模态安全系统的潜在漏洞。

Conclusion: 视觉语言模型的安全决策容易被语义线索操纵，这揭示了当前多模态安全系统存在严重漏洞，需要开发更稳健的基于视觉理解的安全机制。
...
TL;DR: 本文提出了Roundabout-TAU数据集和TAU-R1框架，用于交通异常理解任务。数据集包含342个真实世界环岛视频片段和2000多个问答对，TAU-R1是一个双层视觉语言框架，通过两阶段训练策略在异常分类和推理任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 交通异常理解对智能交通系统的安全至关重要，但现有研究进展有限，主要原因是缺乏专门的基准数据集和任务特定的方法。为了解决这个问题，需要构建真实世界的交通异常数据集并开发相应的理解框架。

Method: 提出了TAU-R1双层视觉语言框架：第一层是轻量级异常分类器进行粗略异常分类，第二层是较大的异常推理器生成详细事件摘要。采用两阶段训练策略：分解问答增强的监督微调，然后是TAU-GRPO后训练方法（基于GRPO，使用TAU特定奖励函数）。

Result: 实验结果表明，TAU-R1在异常分类和推理任务上都取得了强大的性能，同时保持了部署效率。构建的Roundabout-TAU数据集包含342个视频片段和2000多个问答对，覆盖交通异常理解的多个方面。

Conclusion: 通过引入Roundabout-TAU数据集和TAU-R1框架，为交通异常理解任务提供了有效的基准和方法，推动了该领域的发展。数据集和代码已开源，便于进一步研究。
...
TL;DR: CustomTex是一个基于参考图像的3D室内场景纹理生成框架，通过双蒸馏方法实现实例级高保真纹理生成
...
Result: 实验表明CustomTex能实现与参考图像的精确实例级一致性，生成纹理具有更优的清晰度、更少的伪影和最小的烘焙阴影，优于现有方法
...
TL;DR: 该论文重新审视了基于视觉自回归模型的生成式分类器，通过利用任意顺序的自回归模型来估计顺序边缘化预测，显著提升了分类性能，在多个基准测试中超越了基于扩散模型的分类器，同时效率提高了25倍。


<details>
  <summary>Details</summary>
Motivation: 先前基于自回归模型的生成式分类器存在一个关键限制：它们依赖于固定的标记顺序，这为图像理解施加了限制性的归纳偏置。作者观察到单一顺序的预测更多地依赖于部分判别性线索，而平均多个标记顺序可以提供更全面的信号。

Method: 利用最近提出的任意顺序自回归模型来估计顺序边缘化预测，从而释放自回归模型的高分类潜力。这种方法通过考虑多个标记顺序来获得更全面的图像理解。

Result: 该方法在多样化的图像分类基准测试中始终优于基于扩散模型的分类器，同时效率提高了25倍。与最先进的自监督判别模型相比，该方法提供了具有竞争力的分类性能，这对于生成式分类器来说是一个显著的成就。

Conclusion: 通过解决固定标记顺序的限制并利用任意顺序自回归模型进行顺序边缘化预测，可以显著提升自回归生成式分类器的性能，使其在分类准确性和效率方面都超越了扩散模型方法。
...
TL;DR: ADAPT是一个无需训练的文生图框架，通过确定性规划和语义对齐的提示调度，解决了扩散模型在生成罕见组合概念时的挑战，显著提升了RareBench基准上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法如R2F使用LLM进行提示调度，但存在语言模型随机性导致的方差问题以及迭代文本嵌入切换的次优指导。扩散模型在生成训练数据中不常见的属性组合概念时仍然面临挑战。

Method: ADAPT是一个无需训练的框架，利用注意力分数和正交组件，通过确定性规划和语义对齐的提示调度，为罕见概念组合提供一致的指导。

Result: ADAPT在RareBench基准上显著提升了罕见组合概念的生成性能，准确反映了罕见属性的语义信息，提供了对罕见组合生成的确定性和精确控制，同时不损害视觉完整性。

Conclusion: ADAPT框架通过确定性规划和语义对齐的提示调度，有效解决了扩散模型生成罕见组合概念的挑战，无需额外训练或微调即可实现优异的性能。
...
TL;DR: AAPB框架通过自适应辅助提示混合，解决扩散模型在训练分布低密度区域生成语义错位或结构不一致的问题，提升罕见概念生成和图像编辑的稳定性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到图像生成方面取得显著进展，但当目标概念位于训练分布的低密度区域时，往往产生语义错位或结构不一致的结果。这是由于文本-图像数据集的长尾特性，罕见概念或编辑指令代表性不足。
...
Conclusion: AAPB提供了一个原则性的无训练框架，通过自适应提示混合稳定低密度区域的扩散过程，确保生成结果对目标提示的忠实性，解决了扩散模型在罕见概念生成和编辑中的局限性。
...
TL;DR: ARIADNE是一个两阶段框架，通过偏好对齐的感知和基于强化学习的诊断推理，实现拓扑连贯的冠状动脉狭窄检测，解决了传统像素级损失函数导致的血管树碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 传统像素级损失函数无法在冠状动脉血管分割中强制执行拓扑约束，导致尽管像素级准确率高，但血管树仍然碎片化。这影响了狭窄检测的临床实用性。

Method: 两阶段框架：1) 感知模块使用DPO（直接偏好优化）微调Sa2VA视觉语言基础模型，以Betti数约束作为偏好信号，使策略向几何完整的血管结构对齐；2) 推理模块将狭窄定位建模为马尔可夫决策过程，包含显式拒绝机制，自主推迟模糊的解剖候选（如分叉和血管交叉），从覆盖最大化转向可靠性优化。

Result: 在1,400张临床血管造影图像上，ARIADNE实现了最先进的中心线Dice分数0.838，与几何基线相比减少了41%的假阳性。在ARCADE和XCAD多中心基准上的外部验证证实了其在不同采集协议间的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有BEV感知框架多为端到端训练，将图像特征直接转换到BEV空间，缺乏显式3D几何理解和可解释性，导致性能次优。作者认为显式3D表示对准确BEV感知至关重要。

Method: 提出Splat2BEV框架：1）预训练高斯生成器从多视角输入显式重建3D场景，生成几何对齐的特征表示；2）将这些表示投影到BEV空间作为下游任务输入。

Result: 在nuScenes和argoverse数据集上的大量实验表明，Splat2BEV实现了最先进的性能，验证了将显式3D重建融入BEV感知的有效性。

Conclusion: 显式3D表示对准确BEV感知至关重要，Splat2BEV通过高斯泼溅辅助框架成功提升了BEV特征表示的几何精度和语义丰富性。
...
TL;DR: VLMs存在选择性视觉盲区，其视觉注意力受语言框架影响，导致相同视觉推理任务在不同框架下表现不一致，通过提示调优可改善视觉基础


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉语言模型存在选择性视觉盲区，其视觉注意力分配受语言框架影响，导致相同视觉推理任务在不同框架下表现不一致，这揭示了VLMs在视觉基础方面的系统性缺陷

Method: 使用视觉注意力作为探针，量化不同语言框架对注意力分配的影响；提出轻量级提示调优方法，使用可学习token来鼓励更稳健的视觉基础注意力模式

Result: 受限框架（如多项选择和是/否问题）相比开放式框架，导致对图像上下文的注意力显著降低，对任务相关区域的关注减少，注意力转向无信息token；这种注意力分配不当是准确性下降和跨框架不一致的主要原因

Conclusion: VLMs存在选择性视觉盲区，其视觉注意力受语言框架影响；通过提示调优方法可以改善视觉基础，提高跨框架性能一致性
...
<details>
  <summary>Details</summary>
Motivation: 现有基于预训练视觉表示模型的分词器方法存在两个主要问题：1）由于编码器被冻结，重建保真度有限，从而影响编辑质量；2）潜在空间维度过高，使扩散建模变得困难。需要一种既能保持语义结构又能提高重建质量的分词器。

Method: 提出表示枢轴自编码器（RPiAE），包含三个关键组件：1）表示枢轴正则化 - 允许表示初始化的编码器进行微调以改善重建，同时保持预训练表示空间的语义结构；2）变分桥接 - 将潜在空间压缩为紧凑形式以便更好的扩散建模；3）目标解耦的分阶段训练策略 - 依次优化生成可处理性和重建保真度目标。

Result: 实验表明，RPiAE在文本到图像生成和图像编辑任务上优于其他视觉分词器，同时在基于表示的分词器中提供最佳的重建保真度。

Conclusion: RPiAE通过创新的训练策略和架构设计，成功解决了现有表示基分词器的局限性，实现了在保持语义结构的同时提高重建质量，并降低了扩散建模的复杂度，为图像生成和编辑提供了更优的视觉分词器解决方案。
...
TL;DR: DreamPartGen是一个语义基础、部件感知的文本到3D生成框架，通过双部件潜变量和关系语义潜变量实现几何与语义一致的三维合成


<details>
  <summary>Details</summary>
Motivation: 大多数文本到3D方法忽略了部件的语义和功能结构，而现有的部件感知方法虽然引入了分解，但主要关注几何特征，缺乏语义基础，无法建模部件如何与文本描述对齐或部件间关系

Method: 提出DreamPartGen框架，引入双部件潜变量（DPLs）联合建模每个部件的几何和外观，以及关系语义潜变量（RSLs）捕捉从语言中推导出的部件间依赖关系。采用同步协同去噪过程强制执行几何和语义一致性
...
<details>
  <summary>Details</summary>
Motivation: 当前全模态大语言模型的评估主要关注10秒到5分钟的短视频片段，无法反映现实应用中通常长达数十分钟的视频理解需求，存在评估空白。

Method: 从开放平台收集高质量视频，通过严格的人工筛选和标注，构建包含275个10-90分钟视频和1014个问答对的LVOmniBench数据集，用于评估模型在长期记忆、时间定位、细粒度理解和多模态感知等方面的能力。

Result: 评估显示当前全模态大语言模型在处理长音频视频输入时面临显著挑战，开源模型准确率普遍低于35%，而Gemini 3 Pro最高达到约65%的准确率。

Conclusion: LVOmniBench填补了长音频视频理解评估的空白，揭示了现有模型的局限性，预计将推动更先进模型的发展，以解决长音频视频上下文中的复杂跨模态理解问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注架构调整或训练启发式方法，对连续流匹配目标与离散感知任务之间的内在不匹配理解有限。作者发现常用流匹配目标存在梯度消失和轨迹穿越两个关键限制，导致收敛缓慢和类别分离不佳。

Method: 提出基于向量场学习的扩散分割方法：1）向量场重塑策略，通过添加分离的距离感知校正项增强学习的速度场，引入吸引和排斥相互作用；2）受Kronecker序列启发的准随机类别编码方案，与端到端像素神经场框架无缝集成，实现像素级语义对齐。

Result: 大量实验一致表明，相比原始流匹配方法有显著改进，大幅缩小了生成式分割与强判别式专家方法之间的性能差距。

Conclusion: 通过向量场重塑和准随机类别编码，有效解决了扩散模型在生成式分割中的关键限制，为连续生成模型在离散感知任务中的应用提供了新思路。
...
TL;DR: DriveTok是一个用于自动驾驶场景的高效3D多视图tokenizer，通过3D可变形交叉注意力将多视图视觉特征转换为统一的场景tokens，支持多视图重建和3D语义理解。


<details>
  <summary>Details</summary>
Motivation: 现有tokenizer主要针对单目2D场景设计，在处理高分辨率多视图驾驶场景时存在效率低下和视图间不一致的问题，需要专门针对自动驾驶多视图场景的高效tokenizer。

Method: 首先从视觉基础模型获取语义丰富的视觉特征，然后通过3D可变形交叉注意力转换为场景tokens。解码时使用多视图transformer重建多视图特征，通过多头机制获得RGB、深度和语义重建。同时直接在场景tokens上添加3D头进行3D语义占用预测。

Result: 在nuScenes数据集上的大量实验表明，DriveTok的场景tokens在图像重建、语义分割、深度预测和3D占用预测任务上表现良好。
...
TL;DR: 本文提出了一种基于图像频谱特性的噪声调度设计方法，为像素扩散模型创建"紧凑"的每实例噪声调度，消除冗余步骤，在低步数情况下提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 去噪扩散模型在高质量图像和视频生成中广泛应用，但其性能依赖于噪声调度。传统噪声调度通常是手工设计的，需要针对不同分辨率进行手动调整，缺乏理论指导。

Method: 基于图像的频谱特性设计每实例噪声调度，推导最小和最大噪声水平的理论边界，创建"紧凑"的噪声调度以消除冗余步骤。在推理过程中，有条件地采样这些噪声调度。

Result: 实验表明，提出的噪声调度方法能够提升单阶段像素扩散模型的生成质量，特别是在低步数情况下效果显著。

Conclusion: 通过基于图像频谱特性设计每实例噪声调度，可以更有效地利用扩散步骤，提高生成效率和质量，特别是在计算资源受限的低步数场景中。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频修复和对象移除方法虽然能移除对象，但难以消除对象的视觉效应（变形、阴影、反射等）并合成连贯背景。此外，缺乏包含各种对象效应的综合数据集也阻碍了该领域的进展。

Method: 1. 构建VOR数据集：包含6万个高质量视频对，涵盖5种效应类型，来自真实拍摄和合成来源，覆盖广泛对象类别和复杂动态多对象场景。
2. 提出EffectErase方法：采用效应感知的视频对象移除框架，将视频对象插入作为逆向辅助任务，通过互惠学习方案实现。包含任务感知区域引导，专注于受影响区域的学习，并支持灵活任务切换。采用插入-移除一致性目标，鼓励互补行为和共享效应区域定位。

Result: 在VOR数据集上训练的EffectErase方法在广泛实验中表现出优越性能，能够在多样化场景中实现高质量的视频对象效应消除。

Conclusion: VOR数据集填补了视频对象移除领域缺乏综合数据集的空白，而EffectErase方法通过效应感知设计和互惠学习方案，显著提升了视频对象及其视觉效应的移除质量，为视频编辑任务提供了有效解决方案。
...
TL;DR: 本文提出了一种三阶段运动生成框架，结合了连续扩散模型和离散令牌生成器的优势，通过MoTok令牌化器实现语义抽象与细粒度重建的解耦，显著提升了运动生成的控制性和保真度。


<details>
  <summary>Details</summary>
Motivation: 现有运动生成方法存在两种主要范式：连续扩散模型擅长运动学控制，但语义条件能力有限；离散令牌生成器在语义条件方面有效，但运动学控制较弱。需要结合两者的优势，实现既具有良好语义理解又能精确控制运动细节的运动生成方法。

Method: 提出三阶段框架：1) 条件特征提取（感知）；2) 离散令牌生成（规划）；3) 扩散基运动合成（控制）。核心是MoTok扩散基离散运动令牌化器，将语义抽象与细粒度重建解耦，使用单层紧凑令牌保持运动保真度。运动学条件中，粗粒度约束指导令牌生成，细粒度约束通过扩散优化在控制阶段强制执行。

Result: 在HumanML3D数据集上，相比MaskControl方法，仅使用六分之一的令牌数量，轨迹误差从0.72cm降低到0.08cm，FID从0.083降低到0.029。在更强运动学约束下，其他方法性能下降，而本方法FID从0.033进一步降低到0.014，保真度反而提升。

Conclusion: 该方法成功结合了连续扩散模型和离散令牌生成器的优势，通过解耦语义规划与运动控制，实现了高保真、强可控的运动生成。MoTok令牌化器的设计使得在保持运动细节的同时使用更紧凑的表示，为运动生成领域提供了新的有效框架。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于指令的视频编辑模型难以同时平衡精确的语义修改和忠实的运动保持。现有方法依赖外部先验信息，这严重限制了模型的鲁棒性和泛化能力。

Method: 提出SAMA框架，将视频编辑分解为语义锚定和运动建模。语义锚定在稀疏锚定帧上联合预测语义标记和视频潜在表示；运动对齐通过视频修复预训练任务（立方体修复、速度扰动、管状打乱）学习时间动态。采用两阶段优化：分解预训练学习内在语义-运动表示，然后使用配对编辑数据进行监督微调。

Result: SAMA在开源模型中达到最先进性能，与领先的商业系统（如Kling-Omni）竞争力相当。仅分解预训练阶段就展现出强大的零样本视频编辑能力。

Conclusion: SAMA通过分解语义锚定和运动对齐的方法，成功解决了视频编辑中语义修改与运动保持的平衡问题，无需依赖外部先验信息，展现了优秀的鲁棒性和泛化能力。
...
TL;DR: MonoArt：通过渐进式结构推理从单图像重建关节3D物体的统一框架，无需多视图监督或外部模板
...
Method: MonoArt采用渐进式结构推理框架，不直接从图像特征预测关节，而是逐步将视觉观察转换为规范几何、结构化部件表示和运动感知嵌入，在一个单一架构内完成，无需外部运动模板或多阶段流程。

Result: 在PartNet-Mobility数据集上的大量实验表明，该方法在重建精度和推理速度上都达到了最先进的性能。框架还能推广到机器人操作和关节场景重建任务。

Conclusion: MonoArt通过渐进式结构推理实现了稳定且可解释的关节推断，无需外部运动模板或多阶段流程，在单图像关节3D重建任务中表现出色。
...
TL;DR: CubiD是首个针对高维表示（768-1024维）的离散生成模型，通过细粒度掩码和预测实现高质量视觉生成，同时保持原始表示能力。


<details>
  <summary>Details</summary>
Motivation: 当前离散生成方法仅限于低维潜在token（8-32维），牺牲了理解所需的语义丰富性。高维预训练表示（768-1024维）可以弥补这一差距，但其离散生成面临根本性挑战。

Method: 提出Cubic Discrete Diffusion (CubiD)，在高维离散表示中执行细粒度掩码——任何位置、任何维度都可以被掩码并从部分观察中预测。模型学习空间位置内部和之间的丰富相关性，生成步骤固定为T，远小于特征维度数。

Result: 在ImageNet-256上，CubiD实现了最先进的离散生成性能，参数规模从9亿到37亿表现出强大的扩展性。验证了离散化token保持原始表示能力，同一离散token可有效服务于理解和生成任务。

Conclusion: CubiD是首个高维表示离散生成模型，为统一多模态架构铺平道路，希望激发未来研究。
...
Method: 提出VEGA-3D框架，将预训练视频扩散模型重新用作潜在世界模拟器。通过提取中间噪声级别的时空特征，并通过token级自适应门控融合机制与语义表示集成，为MLLMs提供密集几何线索

Result: 在3D场景理解、空间推理和具身操作基准测试中，该方法优于现有最先进基线，验证了生成先验为物理世界理解提供了可扩展基础

Conclusion: 视频生成模型中隐含的时空先验可以作为增强多模态大语言模型几何推理能力的有效资源，无需显式3D监督，为物理世界理解提供了新范式
