...
Conclusion: 提出的视频轨迹编辑框架通过显式跨帧信息聚合和历史引导处理，解决了现有方法的局限性，实现了更好的相机控制和长期一致性
...
TL;DR: 不同视角的智能体通过预测学习获得的世界模型，其潜在空间存在近似线性等距关系，可实现表示对齐与知识迁移。


<details>
  <summary>Details</summary>
Motivation: 研究分散式视觉系统中，不同视角智能体通过预测学习获得的世界模型是否具有几何一致性，以及如何利用这种一致性实现表示对齐和知识迁移。

Method: 让独立智能体从同一环境的不同视角学习世界模型，不共享参数或协调训练。训练后分析其潜在空间的几何关系，发现存在近似线性等距映射，并利用这种对齐关系进行知识迁移。

Result: 两个智能体的潜在空间存在近似线性等距关系，即使视角差异大、原始像素重叠少，这种几何共识依然存在。利用这种对齐，可以在智能体间直接迁移分类器，无需额外训练；蒸馏式迁移能加速后续学习并显著减少计算量。

Conclusion: 预测学习目标对表示几何施加了强正则化，为分散式视觉系统的互操作性提供了轻量级路径。不同智能体学习的世界模型具有内在的几何一致性，可实现高效的知识迁移。
...
Method: 提出PDP框架：1) 双池提示解耦范式（共享池捕获任务通用知识，私有池学习任务特定特征）；2) 原型伪标签生成模块动态更新类原型空间并过滤有价值的伪标签

Result: 在MS-COCO上AP提升9.2%，在PASCAL VOC上AP提升3.3%，实现了最先进的性能，平衡了稳定性和可塑性

Conclusion: PDP框架通过解耦任务通用和特定提示，并保持监督信号一致性，有效缓解了提示退化和漂移问题，为增量目标检测提供了有前景的解决方案
...
TL;DR: AutoFFS：基于对抗攻击的数据驱动框架，通过变形生成反事实颅骨形态，为面部女性化手术提供定量术前规划指导。
...
Method: 提出AutoFFS框架，通过对预训练的二元性别分类器进行基于变形的针对性对抗攻击，生成反事实颅骨形态，将个体颅骨形状向目标性别转化。

Result: 通过分类器评估和人类感知研究验证，生成的形态确实表现出目标性别特征，为FFS术前规划提供了定量基础。

Conclusion: AutoFFS为面部女性化手术提供了数据驱动的定量规划框架，通过对抗攻击生成反事实颅骨形态，推动了这一被忽视患者群体的医疗技术进步。
...
TL;DR: HAMMER是一个利用多模态大语言模型进行交互意图驱动的3D可操作性定位框架，通过聚合图像中的交互意图到接触感知嵌入，引导模型推断文本可操作性标签，实现准确的3D可操作性定位。


<details>
  <summary>Details</summary>
Motivation: 受人类通过观察图像或视频中的交互来识别3D物体可操作性的启发，作者希望开发一个能够利用多模态大语言模型进行交互意图驱动的3D可操作性定位框架，以更好地挖掘物体语义和上下文线索。

Method: HAMMER框架：1）将图像中的交互意图聚合为接触感知嵌入，引导模型推断文本可操作性标签；2）设计分层跨模态整合机制，充分利用MLLM的互补信息进行3D表示细化；3）引入多粒度几何提升模块，将空间特征注入提取的意图嵌入中，促进准确的3D可操作性定位。

Result: 在公共数据集和新构建的损坏基准测试上的广泛实验表明，HAMMER相比现有方法具有优越性和鲁棒性。所有代码和权重都已公开可用。

Conclusion: HAMMER通过利用多模态大语言模型进行交互意图驱动的3D可操作性定位，提供了一种新颖且有效的框架，能够准确挖掘物体语义和上下文信息，实现精确的3D可操作性定位。
...
TL;DR: MERG3R是一个无需训练的分治框架，通过将无序图像分割成重叠子集进行独立重建，然后合并局部重建结果，使几何基础模型能够突破GPU内存限制，处理大规模无序图像集合。


<details>
  <summary>Details</summary>
Motivation: 现有的神经视觉几何模型（如VGGT和Pi3）虽然精度高，但由于使用全注意力机制，受GPU内存限制，无法扩展到大规模无序图像集合。

Method: MERG3R采用分治策略：1）对无序图像进行重新排序和分区，形成重叠且几何多样的子集；2）对各子集独立进行3D重建；3）通过高效的全局对齐和置信度加权的束调整程序合并局部重建结果。

Result: 在7-Scenes、NRGBD、Tanks & Temples和Cambridge Landmarks等大规模数据集上，MERG3R显著提高了重建精度、内存效率和可扩展性，能够在数据集超出内存容量限制时实现高质量重建。

Conclusion: MERG3R是一个模型无关的框架，能够使现有的神经几何模型突破内存限制，处理大规模无序图像集合，为3D重建任务提供了有效的可扩展解决方案。
...
TL;DR: 本文研究了视频时刻检索方法在训练时使用描述性查询但测试时使用搜索查询时的性能退化问题，提出了三个基准测试，识别了语言差距和多时刻差距两大挑战，并通过架构修改提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频时刻检索方法（特别是基于DETR架构的方法）在训练时使用描述性查询，但在实际应用中面对搜索查询时性能显著下降，需要研究这种泛化差距的原因并提出解决方案。

Method: 通过修改三个公开VMR数据集（HD-EPIC、YouCook2和ActivityNet-Captions）的文本查询创建三个基准测试，分析识别了语言差距和多时刻差距两大挑战，发现解码器查询崩溃是主要问题，并通过增加活跃解码器查询的架构修改来缓解该问题。

Result: 实验表明，提出的方法在搜索查询上的性能提升高达14.82% mAP_m，在多时刻搜索查询上提升高达21.83% mAP_m，显著改善了VMR方法在搜索查询场景下的泛化能力。

Conclusion: 视频时刻检索方法在描述性查询训练到搜索查询测试时存在显著的泛化差距，主要源于语言差距和多时刻差距，通过增加活跃解码器查询的架构修改可以有效缓解这一问题，提升模型在实际搜索场景中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统放射组学特征虽然提供透明度和可解释性，但性能通常不如端到端深度学习模型。现有的自适应放射组学方法使用深度学习预测特征权重并选择top-k特征，但这种方法可能过度选择冗余描述符并忽略特征间的互补交互作用。
...
Conclusion: 提出的两阶段检索方法能够近似穷举所有k特征选择，为每个患者生成紧凑、互补的特征集，在保持放射组学透明度和可解释性优势的同时，达到与端到端深度学习模型竞争的性能，使临床医生能够检查驱动预测的具体解剖结构和定量描述符。
...
TL;DR: 本文介绍了Cultural Counterfactuals数据集，用于测量大型视觉语言模型中的文化偏见，特别是宗教、国籍和社会经济地位方面的偏见，填补了现有研究主要关注视觉特征（如种族、性别）而忽视文化背景偏见的空白。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型偏见研究主要关注基于视觉特征的人口统计学偏见（如种族、性别），而文化差异相关的偏见（如宗教、社会经济地位）相对缺乏研究。主要挑战在于确定个体所属群体通常依赖于图像中的文化背景线索，而缺乏标注了文化背景线索的数据集。

Method: 作者创建了Cultural Counterfactuals数据集，包含近6万张合成反事实图像。使用图像编辑模型将不同人口统计学特征的人放置到真实的文化背景图像中，构建反事实图像集，使同一个人出现在多个不同的文化背景下，从而精确测量文化背景差异对模型输出的影响。

Result: 创建了一个高质量的数据集，可用于量化流行大型视觉语言模型中的文化偏见。该数据集能够精确测量文化背景差异对模型输出的影响，填补了文化偏见测量工具的空白。

Conclusion: Cultural Counterfactuals数据集为测量大型视觉语言模型中的文化偏见提供了有效工具，特别关注宗教、国籍和社会经济地位方面的偏见，有助于更全面地理解和减少模型中的偏见问题。
...
TL;DR: TorchGeo是一个基于PyTorch的地理空间机器学习库，提供专门处理地理空间数据的工具和教程，包括端到端的水体分割案例研究。


<details>
  <summary>Details</summary>
Motivation: 地理空间机器学习流程与标准计算机视觉工作流存在根本差异：图像通常是大尺寸地理参考场景，标签可能是栅格掩码或矢量几何体，且训练和评估需要空间感知的采样和分割策略。需要专门的工具来简化地理空间数据在机器学习中的应用。

Method: TorchGeo是一个基于PyTorch的领域库，提供数据集、采样器、变换和预训练模型。论文介绍了两个教程：1）通过代码示例展示TorchGeo核心抽象；2）使用Earth Surface Water数据集在Sentinel-2影像上进行多光谱水体分割的端到端案例研究。

Result: 教程展示了如何使用TorchGeo数据集训练语义分割模型，将模型应用于巴西里约热内卢的Sentinel-2场景，并将预测结果保存为GeoTIFF格式以供进一步地理空间分析。教程代码以两个Python笔记本形式分发。

Conclusion: TorchGeo通过提供专门的地理空间数据处理工具，简化了地理空间机器学习流程，使研究人员和从业者能够更轻松地将机器学习应用于地球观测数据。
...
TL;DR: OpenMarcie是最大的制造环境多模态人类动作监控数据集，包含37小时多视角数据，涵盖自行车和3D打印机装配任务，用于人类活动识别基准测试。


<details>
  <summary>Details</summary>
Motivation: 智能工厂需要准确量化工人绩效指标以提高效率和安全，但缺乏针对制造环境的大规模多模态数据集来支持人类活动监控研究。

Method: 创建包含36名参与者的多模态数据集，采用两种实验设置：自行车装配拆卸任务（12人，半真实条件）和3D打印机装配任务（25人，有指导说明），收集可穿戴设备和环境摄像头的八种数据类型。

Result: OpenMarcie包含超过37小时的自中心和他中心多模态数据，200多个独立信息通道，是目前最大的制造环境人类动作监控数据集，并在三个HAR任务上进行了基准测试。

Conclusion: OpenMarcie为制造环境中的人类活动识别研究提供了重要资源，支持活动分类、开放词汇描述和跨模态对齐等任务，有助于智能工厂的优化和安全提升。
...
TL;DR: QuADD是一个量化感知的数据集蒸馏框架，在固定比特预算下联合优化数据集紧凑性和精度，通过端到端协同优化合成样本和量化参数，在图像分类和3GPP波束管理任务上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前数据集蒸馏方法主要关注样本数量减少，对数据精度及其对效率的影响考虑有限。需要一种统一框架在固定比特预算下联合优化数据集紧凑性和精度。

Method: 提出量化感知数据集蒸馏(QuADD)框架，将可微分量化模块集成到蒸馏循环中，支持均匀和非均匀自适应量化，从速率-失真角度分析比特分配如何影响学习性能。

Result: 在图像分类和3GPP波束管理任务上的实验表明，QuADD在每比特精度上超越了现有的数据集蒸馏和后量化基线方法，为信息高效的数据集蒸馏建立了新标准。

Conclusion: QuADD通过联合优化数据集紧凑性和精度，在固定比特预算下实现了更高效的数据集蒸馏，为信息高效的数据集压缩提供了新的解决方案。
...
TL;DR: TruckDrive数据集填补了高速公路卡车自动驾驶的长距离感知空白，现有模型在150米以上范围性能大幅下降31%-99%


<details>
  <summary>Details</summary>
Motivation: 重型卡车高速公路自动驾驶面临挑战：由于制动距离长，需要数百米的场景理解进行预测性规划和安全制动。现有驾驶数据集主要覆盖城市场景，感知范围有限（仅100米内），无法满足高速公路长距离感知需求。

Method: 引入TruckDrive高速公路多模态驾驶数据集，使用专门为长距离感知设计的传感器套件：7个长距离FMCW激光雷达、3个高分辨率短距离激光雷达、11个8MP环绕摄像头（不同焦距）和10个4D FMCW雷达。数据集包含47.5万个样本，其中16.5万帧密集标注。

Result: 数据集支持2D检测（1000米）、3D检测（400米）、深度估计、跟踪、规划和端到端驾驶（20秒序列，高速公路速度）。研究发现最先进的自动驾驶模型无法泛化到150米以上范围，3D感知任务性能下降31%-99%，暴露了现有架构和训练信号无法解决的系统性长距离感知差距。

Conclusion: TruckDrive数据集填补了高速公路长距离感知的空白，揭示了当前自动驾驶模型在长距离感知方面的严重局限性，为未来研究和模型改进提供了重要基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理文档视觉问答时面临挑战，特别是在复杂推理和多步骤工作流方面。现有方法难以将复杂问题分解为可管理的子任务，并且无法针对不同文档元素采用专门的处理路径。

Method: ORCA采用多智能体框架：首先由推理智能体将查询分解为逻辑步骤，然后通过路由机制从专门的智能体库中激活任务特定的智能体。每个智能体专注于不同的模态，实现细粒度理解和跨文档组件的协作推理。为确保答案可靠性，采用辩论机制进行压力测试，必要时使用正反论证裁决过程，最后通过完整性检查器确保格式一致性。
...
TL;DR: 该研究提出了一个全面的肤色公平性框架，包括大规模开源数据集STW、经典CV与深度学习方法的基准测试，以及达到SOTA泛化性能的SkinToneNet模型，用于可靠的公平性审计。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型常从训练数据中继承偏见。虽然性别和种族的公平性研究较多，但由于缺乏细粒度标注数据集，肤色分析仍具挑战。现有方法依赖医学6色调Fitzpatrick量表（缺乏视觉代表性），或使用小型私有数据集（不可复现），或使用经典CV方法，忽视了训练-测试泄漏和数据集不平衡问题。

Method: 1. 引入STW数据集：包含42,313张图像、3,564个个体，使用10色调MST量表标注；2. 对经典CV方法（SkinToneCCV）和深度学习方法进行基准测试；3. 提出SkinToneNet：基于ViT微调的模型，在域外数据上实现最优泛化。

Result: 经典模型表现接近随机，而深度学习方法达到接近标注者准确率。SkinToneNet在域外数据上实现最先进的泛化性能，能够可靠地对CelebA和VGGFace2等公共数据集进行公平性审计。

Conclusion: 该工作提供了肤色分类和公平性评估的最先进结果，通过开源数据集和模型促进了肤色公平性研究的可复现性和进一步发展。
...
TL;DR: ModalPatch是一个即插即用的模块，能够在任意模态丢失场景下增强多模态3D目标检测的鲁棒性，无需架构更改或重新训练。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶中的多模态3D目标检测面临传感器数据瞬时中断或丢失的问题（如硬件故障、恶劣天气、遮挡），特别是在同时丢失多个模态时，车辆会暂时"失明"，这带来了关键安全风险。

Method: 1. 利用传感器数据的时间特性实现感知连续性，使用基于历史的模块预测和补偿瞬时不可用的特征；2. 引入不确定性引导的跨模态融合策略，动态估计补偿特征的可靠性，抑制有偏信号同时增强信息丰富的信号。

Result: 大量实验表明，ModalPatch在各种模态丢失条件下，能够持续增强最先进的3D目标检测器的鲁棒性和准确性。

Conclusion: ModalPatch是首个即插即用的模块，能够无缝集成到多种检测框架中，有效解决多模态3D检测中的瞬时模态丢失问题，提升自动驾驶系统的可靠性。
...
TL;DR: NeighborMAE通过联合重建相邻遥感图像来学习空间依赖性，利用动态调整掩码比例和像素级损失权重的启发式策略，在多个预训练数据集和下游任务中显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态和多时序的遥感数据在掩码图像建模中已被广泛探索，但相邻区域图像之间的空间依赖性在很大程度上被忽视了。由于地球表面是连续的，相邻图像高度相关，为自监督学习提供了丰富的上下文信息。
...
Conclusion: NeighborMAE通过利用相邻遥感图像的空间依赖性，在掩码图像建模中取得了显著改进，为遥感自监督学习提供了新的视角和方法。
...
TL;DR: EIMC提出了一种早期协作感知范式，通过轻量级协作体素和热图驱动的共识协议，在减少87.98%带宽的同时实现73.01% AP@0.5的性能


<details>
  <summary>Details</summary>
Motivation: 当前多模态协作感知采用"局部融合再通信"的顺序，需要在协作融合前传输大量特征数据，导致高带宽需求。需要一种更高效的协作范式来降低通信开销同时保持感知性能。

Method: 1) 早期协作范式：注入轻量级协作体素到局部模态融合步骤，生成紧凑的3D协作先验；2) 热图驱动共识协议：计算逐像素置信度热图识别需要协作的区域；3) 实例中心消息传递：仅查询Top-K低置信度区域的实例向量，通过交叉注意力融合；4) 精炼融合：收集各智能体Top-K最置信实例，通过自注意力增强特征。

Result: 在OPV2V和DAIR-V2X数据集上评估，EIMC达到73.01% AP@0.5，同时相比最佳已发布多模态协作检测器减少87.98%的字节带宽使用。

Conclusion: EIMC通过早期协作范式和实例中心消息传递，在显著降低通信开销的同时保持了高性能，为多模态协作感知提供了一种高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 森林环境中检测失踪人员具有挑战性，因为茂密的树冠遮挡使得无人机从顶部或倾斜角度拍摄的图像难以发现人员。现有无人机视角无法捕捉冠层下的关键视觉线索，需要专门针对冠层下视角的数据集来改进森林搜救中的人员检测能力。
...
Conclusion: ForestPersons填补了森林冠层下人员检测数据集的空白，为改进实际搜救场景中的人员检测能力提供了重要基准。数据集已公开可用，支持先进的人员检测算法开发。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）在开放世界动作理解方面表现出色，但在闭集设置中，将MLLMs作为生成式分类器存在效率低下和语义重叠导致模糊性的问题。判别式分类器虽然效率高、决策边界清晰，但性能仍有提升空间。

Method: 首先比较生成式和判别式分类器在闭集动作理解中的表现，然后设计策略提升生成式分类器的性能。最终提出生成辅助判别式分类器（GAD），该框架仅在微调阶段使用生成建模辅助判别式分类器，保持与MLLM预训练的完全兼容性。

Result: 在时序动作理解基准测试中，GAD在准确性和效率上都优于生成式方法，在5个数据集的4个任务上取得了最先进的结果。在最大的COIN基准上实现了平均2.5%的准确率提升和3倍的推理加速。

Conclusion: 生成建模可以有效地辅助判别式分类器，实现更好的性能同时保持效率。GAD框架为闭集动作理解提供了一种高效且高性能的解决方案。
...
TL;DR: SemGS是一个前馈框架，从稀疏图像输入重建可泛化的语义场，使用双分支架构提取颜色和语义特征，通过相机感知注意力机制建模几何关系，实现语义场景重建和语义感知的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有的语义场景重建和语义感知新视角合成方法通常依赖密集多视图输入并需要场景特定优化，限制了在实际应用中的实用性和可扩展性。需要一种能够从稀疏输入中有效重建语义场并具有良好泛化能力的方法。

Method: 提出SemGS框架：1) 使用双分支架构提取颜色和语义特征，共享浅层CNN层使语义推理能利用颜色外观中的纹理和结构线索；2) 在特征提取器中加入相机感知注意力机制，显式建模相机视角间的几何关系；3) 将提取的特征解码为共享几何一致性的双高斯表示；4) 引入区域平滑损失增强语义一致性。

Result: 在基准数据集上实现了最先进的性能，同时提供快速推理和强大的泛化能力，能够适应多样化的合成和真实世界场景。

Conclusion: SemGS通过创新的双分支架构和相机感知注意力机制，成功解决了从稀疏图像输入重建语义场的问题，在保持几何一致性的同时实现了高质量的语义理解和新视角合成，具有实际应用价值。
...
TL;DR: GKD是一个多阶段知识蒸馏框架，专门用于提升语义分割模型在分布偏移下的泛化能力，通过解耦表示学习和任务学习，并引入基于查询的软蒸馏机制。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法主要关注域内精度而忽视了域外泛化能力，这在分布偏移下尤为重要。随着视觉基础模型的出现，虽然它们对未见数据有很强鲁棒性，但用传统蒸馏方法会损害这种能力。

Method: GKD采用多阶段框架：第一阶段通过选择性特征蒸馏让学生学习域无关表示；第二阶段冻结这些表示进行任务适应，防止对可见域的过拟合。还引入了基于查询的软蒸馏机制，让学生特征作为查询从教师模型中检索可迁移的空间知识。

Result: 在五个域泛化基准测试中，GKD始终优于现有KD方法，在基础模型到基础模型蒸馏中平均提升1.9%，在基础模型到本地模型蒸馏中平均提升10.6%。

Conclusion: GKD通过解耦表示学习和任务学习，有效提升了知识蒸馏的泛化能力，特别是在视觉基础模型蒸馏中表现出色，为分布偏移下的模型压缩提供了有效解决方案。
...
TL;DR: VC-STaR提出了一种基于视觉对比的自学习推理框架，通过对比相似的视觉问答对来减少视觉幻觉，生成高质量推理路径用于视觉语言模型的微调。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的自学习技术能提升推理能力，但扩展到视觉语言模型时面临视觉幻觉问题——模型生成的推理路径中的视觉错误难以验证和纠正。

Method: 提出视觉对比自学习推理器(VC-STaR)，利用视觉对比原理：当呈现两个视觉相似图像和同义问题时，VLM能更精确识别相关视觉线索。通过收集多样化VQA数据集，根据多模态相似性筛选对比对，生成高质量推理路径。

Result: 创建了包含55K样本的视觉推理数据集VisCoR-55K，实验表明VC-STaR不仅优于现有自学习方法，也超越了在SOTA视觉推理数据集上微调的模型。

Conclusion: 视觉语言模型固有的对比能力可以引导自身的视觉推理提升，VC-STaR框架通过视觉对比有效缓解幻觉问题，显著增强了VLM的推理能力。
...
TL;DR: CAPT框架通过构建混淆库和多粒度挖掘模块，让视觉语言模型从自身分类错误中学习，显著减少相似类别间的系统误分类


<details>
  <summary>Details</summary>
Motivation: CLIP等视觉语言模型在跨模态表示学习方面取得了显著进展，但在视觉和语义相似的类别之间存在系统性误分类问题。研究发现这些混淆模式并非随机，而是在特定类别对之间持续发生，揭示了模型的内在偏见和细粒度判别能力有限的问题。

Method: 提出了CAPT（混淆感知提示调优）框架：1）构建混淆库来显式建模类别间的稳定混淆关系和误分类样本；2）引入语义混淆挖掘器（SEM）通过语义差异和共性提示捕捉全局类间混淆；3）设计样本混淆挖掘器（SAM）从库中检索代表性误分类实例，并通过Diff-Manner适配器整合全局和局部上下文；4）开发多粒度差异专家（MGDE）模块，联合利用语义级和样本级专家进行更鲁棒的混淆感知推理。
...
TL;DR: Track4World：基于前馈模型的单目视频全像素3D轨迹估计方法，通过3D相关方案同时估计任意帧间的2D/3D稠密流，实现高效的世界坐标系下的像素级3D跟踪


<details>
  <summary>Details</summary>
Motivation: 现有单目3D跟踪方法要么只能跟踪第一帧的稀疏点，要么采用缓慢的优化框架进行稠密跟踪，无法实现高效的全像素3D轨迹估计

Method: 基于VGGT风格ViT编码的全局3D场景表示，采用新颖的3D相关方案同时估计任意帧对之间的像素级2D和3D稠密流，结合重建的3D几何实现像素级3D跟踪

Result: 在多个基准测试中，该方法在2D/3D流估计和3D跟踪方面始终优于现有方法，展示了其鲁棒性和可扩展性

Conclusion: Track4World实现了高效的全像素3D跟踪，为现实世界4D重建任务提供了强大的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 当前数字孪生方法（如NeRF）主要关注视觉重建，缺乏底层材料属性（如介电常数、电导率），导致功能不完整。需要解决非接触、非侵入式传感获取场景材料属性的物理反演难题。

Method: NEMF框架采用系统解耦策略：1）利用图像获取高保真几何信息作为锚点；2）解析环境场；3）通过约束几何和环境场，将病态问题转化为良定的物理监督学习任务；4）使用解码器模块，在环境RF信号和可微分物理反射模型指导下，学习输出连续、空间变化的材料参数场。

Result: 在高保真合成数据集上的实验表明，NEMF能够高精度重建材料分布图，生成的功能数字孪生体支持高保真物理仿真，超越了传统被动视觉复制品。

Conclusion: NEMF框架通过非侵入式物理反演实现了功能完整的数字孪生构建，使数字孪生从被动视觉复制品转变为真正可仿真的物理世界模型，为计算机视觉领域的功能数字孪生创建提供了新途径。
...
TL;DR: 该研究评估了多种图像数据增强技术在孟加拉语手写字符识别中的应用，针对资源有限语言的数据稀缺问题，发现随机仿射变换与颜色抖动组合在轻量级模型EfficientViT上效果最佳。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在计算机视觉中表现出色，但严重依赖大数据集以避免过拟合。对于资源有限的语言如孟加拉语，大规模数据集往往不可得，因此需要研究数据增强技术来解决数据稀缺问题。
...
Conclusion: 该研究为资源稀缺语言（特别是孟加拉语手写字符识别）提供了全面的图像数据增强分析，证明适当的增强技术组合能显著提升轻量级模型的性能，为解决数据稀缺问题提供了有效方案。
...
TL;DR: VLMFusionOcc3D：一个用于自动驾驶中密集3D语义占据预测的鲁棒多模态框架，通过视觉语言模型和天气感知融合解决稀疏几何网格中的语义模糊和恶劣天气性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于体素的占据模型在稀疏几何网格中存在语义模糊问题，且在恶劣天气条件下性能下降。需要利用视觉语言模型的丰富语言先验来稳定语义概念，并动态适应不同天气条件下的传感器可靠性。

Method: 1. 双分支特征提取管道将多视角图像和LiDAR点云投影到统一的体素空间；2. 实例驱动的VLM注意力机制使用门控交叉注意力和LoRA适配的CLIP嵌入将高级语义和地理先验注入3D体素；3. 天气感知自适应融合利用车辆元数据和天气条件提示动态重新加权传感器贡献；4. 深度感知几何对齐损失确保相机几何与LiDAR的空间一致性。

Result: 在nuScenes和SemanticKITTI数据集上的广泛实验表明，该即插即用模块持续提升了最先进体素基线的性能，特别是在具有挑战性的天气场景中取得了显著改进。

Conclusion: 该框架为复杂城市导航提供了可扩展且鲁棒的解决方案，通过结合视觉语言模型的语义先验和天气感知的传感器融合，有效解决了自动驾驶中3D语义占据预测的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于多视图扩散模型的单视图3D人体重建方法在处理动态或挑战性姿态时，恢复的3D人体往往呈现不自然姿态。这主要是由于现有3D人体数据集中姿态多样性有限，而收集大规模多样化姿态的3D数据成本高昂。

Method: 提出DrPose算法，仅使用人体姿态与单视图图像对进行训练，通过直接奖励微调最大化PoseScore（提出的可微分奖励函数，量化生成的多视图潜在图像与真实人体姿态的一致性）。使用DrPose15K数据集进行优化，该数据集从现有人体运动数据集和姿态条件视频生成模型构建，具有更广泛的姿态分布。

Result: 在传统基准数据集、野外图像和新构建的基准上进行了验证，特别是在挑战性人体姿态评估方面，结果显示在所有基准上都取得了定性定量的持续改进。

Conclusion: DrPose通过直接奖励微调算法，仅使用姿态-图像对训练多视图扩散模型，有效解决了3D人体重建中的不自然姿态问题，无需昂贵3D资产，在多样化姿态上表现出色。
...
TL;DR: IB-IUMAD是一个新颖的去噪框架，用于增量统一多模态异常检测，通过Mamba解码器和信息瓶颈融合模块解决虚假和冗余特征对灾难性遗忘的负面影响。


<details>
  <summary>Details</summary>
Motivation: 增量统一多模态异常检测面临灾难性遗忘困境，现有方法忽略了虚假和冗余特征对此问题的影响，特别是在多模态框架中，通过简单聚合单模态架构更容易发生遗忘。

Method: 提出IB-IUMAD框架，包含两个核心组件：Mamba解码器用于解耦对象间特征耦合，防止虚假特征干扰；信息瓶颈融合模块用于从融合特征中过滤冗余特征，明确保留判别性信息。

Result: 在MVTec 3D-AD和Eyecandies数据集上进行的一系列理论分析和实验证明了IB-IUMAD的有效性和竞争性能。

Conclusion: IB-IUMAD通过解决虚假和冗余特征对灾难性遗忘的负面影响，为增量统一多模态异常检测提供了有效的解决方案，在多模态框架中表现出优越性能。
...
Method: 提出SEP-YOLO框架，集成双域协作机制：1）频域细节增强模块，通过可学习复数权重分离增强弱高频边界分量；2）多尺度空间细化流，包含内容感知对齐颈部和多尺度门控细化块，确保深度语义特征中的精确特征对齐和边界定位。

Result: 在Trans10K和GVD数据集上的大量实验表明，SEP-YOLO实现了最先进的性能。同时为Trans10K数据集提供了高质量的实例级标注，填补了透明物体实例分割的关键数据空白。

Conclusion: SEP-YOLO通过创新的双域协作机制有效解决了透明物体实例分割的挑战，为透明物体识别提供了新的解决方案，并通过提供高质量标注数据促进了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 时尚智能涵盖检索、推荐、识别和对话等多个任务，但受到碎片化监督和不完整时尚标注的限制，阻碍了视觉-语义结构的形成，使现有视觉语言模型难以成为统一的通用时尚大脑

Method: 构建FashionX百万级数据集，全面标注服装中的可见时尚单品，从全局到局部组织属性；提出OmniFashion统一视觉-语言框架，将不同时尚任务桥接到统一的时尚对话范式下

Result: 在多子任务和检索基准测试中，OmniFashion实现了强大的任务级准确性和跨任务泛化能力

Conclusion: 该研究为通用、对话导向的时尚智能提供了可扩展的路径
...
TL;DR: DREAM是一个统一的多模态学习框架，同时优化判别式和生成式目标，在视觉表示学习和文本到图像生成方面都表现出色。


<details>
  <summary>Details</summary>
Motivation: 将视觉表示学习和文本到图像生成统一在单一模型中是多模态学习的一个核心挑战，现有方法往往只能专注于其中一个任务。

Method: 采用两种关键技术：训练时的Masking Warmup（渐进式掩码调度），从最小掩码开始建立对比对齐，逐渐过渡到完全掩码；推理时的Semantically Aligned Decoding（语义对齐解码），将部分掩码的图像候选与目标文本对齐并选择最佳候选进行进一步解码。

Result: 仅在CC12M数据集上训练，DREAM在ImageNet线性探测准确率达到72.7%（比CLIP高1.1%），FID为4.25（比FLUID高6.2%），在少样本分类、语义分割和深度估计方面也有一致性提升，文本-图像保真度提高6.3%。

Conclusion: 判别式和生成式目标可以协同作用，允许统一的多模态模型在视觉理解和生成方面都表现出色。
...
TL;DR: VisionCreator是一个原生视觉生成智能体模型，通过UTPC（理解、思考、规划、创造）框架统一视觉创作能力，在多项评估中超越更大规模的闭源模型。


<details>
  <summary>Details</summary>
Motivation: 当前视觉内容创作任务需要理解设计惯例和创意工作流程，通用模型难以满足这些需求，而基于工作流的智能体又缺乏自主创意规划的专业知识。需要一种能够统一理解、思考、规划和创造能力的端到端可学习框架。

Method: 提出VisionCreator智能体模型，采用渐进专业化训练（PST）和虚拟强化学习（VRL）在高保真模拟环境中优化；构建VisGenData-4k数据集，使用基于元认知的VisionAgent生成具有明确UTPC结构的高质量创作轨迹；建立VisGenBench基准测试。

Result: VisionCreator-8B/32B模型在多个评估维度上表现出色，超越了更大规模的闭源模型；建立了包含1.2k测试样本的VisGenBench基准，为多步骤视觉创作能力提供标准化评估。

Conclusion: 该工作为视觉生成智能体系统的未来研究奠定了基础，通过统一的UTPC框架和专门的训练方法，实现了在复杂创作任务中的稳定高效能力获取。
...
<details>
  <summary>Details</summary>
Motivation: 现有的冷扩散和广义扩散模型在稀疏视图CT重建中表现出潜力，但现有采样策略通常依赖临时采样控制或固定调度，对误差累积和采样不稳定性敏感。
...
Conclusion: ReCo-Diff通过残差条件自引导采样，为稀疏视图CT重建提供了一种无需启发式干预的确定性采样框架，显著提升了重建性能。
...
TL;DR: FiDeSR是一个高保真度、保留细节的一步扩散超分辨率框架，通过细节感知加权策略和自适应增强器提升真实图像超分辨率质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的方法在真实图像超分辨率中难以同时保留精细细节和确保高保真重建，导致视觉质量不理想。

Method: 1. 训练时引入细节感知加权策略，自适应强调模型预测误差较高的区域；2. 推理时使用低频和高频自适应增强器细化重建，无需重新训练模型；3. 采用残差中的残差噪声细化，校正扩散噪声中的预测误差并增强细节恢复。

Result: FiDeSR在真实图像超分辨率方面优于现有基于扩散的方法，产生具有高感知质量和忠实内容恢复的输出。

Conclusion: FiDeSR框架通过创新的训练和推理策略，成功解决了现有扩散方法在细节保留和保真度平衡方面的挑战，实现了卓越的超分辨率性能。
...
TL;DR: 开发了名为GTDoctor的妊娠滋养细胞疾病病理诊断专家模型，能够进行像素级病变分割并输出诊断结论和个性化病理分析结果。基于该技术开发的GTDiagnosis软件系统在临床应用中显著提高了诊断准确性和效率。
...
TL;DR: 提出了一种基于双层级混合专家架构和预训练扩散模型的统一图像修复框架，通过粗粒度跨类别适应和细粒度类内调制有效处理多种退化类型。
...
Conclusion: 双层级MoE架构能够实现跨退化类别的粗粒度适应和特定类内变化的细粒度调制，确保在处理复杂真实世界退化时的高专业性。
...
TL;DR: CoR-Painter提出了一种"如何画-画什么"的新范式，通过约束推理指导自回归图像生成，解决现有方法只关注"画什么"而忽略"如何画"的结构问题，显著提升空间关系准确性。
...
Method: 提出CoR-Painter框架，采用"如何画-画什么"范式：首先从输入提示推导视觉约束集，明确控制空间关系、关键属性和构图规则；然后用这些约束指导生成详细描述"画什么"；同时引入双目标GRPO策略优化文本约束推理和视觉投影过程。

Result: 在T2I-CompBench、GenEval和WISE上的大量实验表明，该方法实现了最先进的性能，在空间指标上有显著提升（如在T2I-CompBench上提升+5.41%）。

Conclusion: CoR-Painter通过引入约束推理和"如何画-画什么"范式，有效解决了自回归图像生成中的结构推理问题，显著提升了生成图像的空间准确性和整体质量。
...
TL;DR: 本文提出了一种基于专家混合的Tensor分解结构搜索框架TenExp，能够动态选择和激活合适的Tensor分解方法，突破了现有方法局限于固定因子交互族和单一分解的限制。


<details>
  <summary>Details</summary>
Motivation: 当前Tensor分解结构搜索方法存在两个主要限制：1) 局限于固定的因子交互族（如张量收缩）；2) 无法实现分解方法的混合。如何选择能够精确捕捉数据背后低秩结构的合适Tensor分解仍然是一个具有挑战性且相对未被充分探索的问题。

Method: 设计了一个基于专家混合的Tensor分解结构搜索框架（TenExp），该框架能够以无监督的方式动态选择和激活合适的Tensor分解方法。该方法不仅能够提供超越固定因子交互族的合适单一分解，还能实现超越单一分解的分解方法混合。

Result: 在合成和真实数据集上的大量实验表明，TenExp相比最先进的基于Tensor分解的方法具有优越性。理论分析提供了TenExp的近似误差界，揭示了其近似能力。

Conclusion: TenExp框架成功解决了现有Tensor分解结构搜索方法的局限性，能够动态选择和混合合适的分解方法，为Tensor分解领域提供了一种灵活有效的结构搜索解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像分割需要既能保留精细解剖边界又能在临床部署中保持高效性的模型。传统Transformer虽然能捕捉长程依赖但存在二次注意力成本和大量数据需求的问题，而CNN计算友好但全局推理能力有限。线性注意力虽然具有O(N)复杂度，但存在训练不稳定和注意力稀释问题，导致分割图模糊。

Method: 提出PVT-GDLA解码器中心Transformer，核心是门控差分线性注意力(GDLA)：在互补的查询/键子空间上计算两个核化注意力路径，通过可学习的通道级尺度相减以消除共模噪声并放大相关上下文。轻量级的头特定门控注入非线性和输入自适应稀疏性，缓解注意力汇聚问题；并行局部令牌混合分支使用深度卷积增强相邻令牌交互，提高边界保真度，同时保持O(N)复杂度和低参数开销。结合预训练的金字塔视觉Transformer编码器。

Result: 在CT、MRI、超声和皮肤镜基准测试中，在相同训练预算下达到最先进准确率，与CNN、Transformer、混合模型和线性注意力基线相比，参数数量相当但FLOPs更低。
...
<details>
  <summary>Details</summary>
Motivation: 现有阴影生成方法主要关注单物体插入，但在实际应用中，现代合成流程和真实场景往往需要同时插入多个物体，需要阴影在几何、附着和位置方面具有联合一致性。多物体阴影生成问题尚未得到充分探索。

Method: 利用预训练文本到图像扩散模型的多模态能力，采用双路径设计：图像路径注入密集多尺度特征提供细粒度空间指导；文本路径将每个物体的阴影边界框编码为学习的位置标记，并通过交叉注意力融合。使用注意力对齐损失将这些标记与对应阴影区域进行关联。

Result: 实验结果表明，该方法在单物体和多物体阴影生成设置中都达到了最先进的性能。为支持该任务，作者还扩展了DESOBAv2数据集，构建了包含多个插入物体的合成场景。

Conclusion: 该方法能够为多个插入物体合成物理上合理的阴影，解决了多物体阴影生成的挑战，在实际图像合成应用中具有重要价值。
...
TL;DR: 提出RADAR方法，通过相对注意力驱动主动推理，减少遥感视觉问答中的幻觉问题
...
Method: 提出RADAR方法，利用MLLMs内在的注意力机制，在测试时引导渐进式定位和细粒度局部推理，无需额外训练
...
TL;DR: ITO框架通过多模态多重对齐和训练时融合解决图像-文本对比预训练中的模态间隙问题，提升表示质量同时保持推理效率
...
Method: 提出ITO框架，包含两个协同机制：1) 多模态多重对齐，通过挖掘多样化的图像-文本对应关系来丰富监督信号；2) 轻量级训练时多模态融合模块，强制结构化跨模态交互。推理时丢弃融合模块，保持标准双编码器架构的效率

Result: 在分类、检索和多模态基准测试中，ITO始终优于强基线方法。分析表明多重对齐驱动判别能力，而训练时融合作为关键的结构正则化器，消除模态间隙并稳定训练动态，防止激进对比学习中常见的早期饱和现象
...
TL;DR: HiLoRA：一个分层LoRA框架，通过根、簇、叶三级适配器分别捕获全局、子组和客户端特定知识，解决联邦学习中ViT模型个性化与泛化平衡问题。
...
Conclusion: HiLoRA通过分层适配器设计有效平衡了联邦学习中ViT模型的个性化和泛化需求，理论分析和实验验证了其有效性。
...
TL;DR: UNICORN是一个用于系统评估医学基础模型的公开基准测试，通过统一协议、标准化少样本适应和隔离测试集来支持跨模态、跨任务的泛化能力评估。


<details>
  <summary>Details</summary>
Motivation: 医学基础模型需要从大规模多样化数据中学习泛化特征，但目前缺乏公开、标准化、可复现的评估框架，现有基准测试往往局限于特定任务、器官或模态，限制了跨任务泛化能力的评估。

Method: 构建了一个两步骤框架：1) 将模型推理与基于标准化少样本适应的任务特定评估解耦，以隔离表示质量；2) 创建间接可访问的隔离测试集，源自临床相关队列；3) 开发标准化评估代码和提交接口；4) 引入UNICORN Score作为跨领域比较的统一指标。

Result: UNICORN测试数据集包含来自17个机构、8个国家的2,400多名患者数据，包括3,700多个视觉病例和2,400多份临床报告，涵盖8个解剖区域和4种成像模态。建立了任务特定和聚合排行榜。

Conclusion: UNICORN通过标准化多任务、多模态评估，为医学基础模型的可复现基准测试奠定了基础，支持跨领域、跨模态和跨任务类型的直接比较。
...
TL;DR: R3GW是一种可重光照的3D高斯溅射方法，能够从野外拍摄的无约束照片集中重建户外场景，支持任意光照条件下的物理重光照。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯溅射(3DGS)虽然重建静态场景效果优秀，但存在两个主要问题：1) 没有显式建模场景光照，无法进行重光照任务；2) 难以处理野外拍摄的无约束照片集（光照条件变化）。

Method: R3GW将场景分为可重光照的前景和非反射背景（天空），使用两组不同的高斯分布。前景通过将基于物理的渲染(PBR)与3DGS场景表示相结合，在变化光照设置下建模视图相关的光照效果。

Result: 在NeRF-OSR数据集上的定量和定性评估显示，该方法达到了最先进的性能，支持无约束场景的物理重光照，能够合成任意光照条件下的逼真新视图。天空表示还减轻了深度重建伪影，提高了天空-前景边界的渲染质量。

Conclusion: R3GW成功解决了3DGS在重光照和野外场景重建方面的局限性，实现了可重光照的3D高斯表示，为无约束户外场景的逼真渲染提供了有效解决方案。
...
TL;DR: NOVA是一个无需配对数据的视频编辑框架，通过稀疏控制（用户编辑关键帧）和密集合成（保持运动纹理一致性）实现高质量视频编辑。
...
Method: 提出NOVA框架：稀疏分支通过用户编辑的关键帧提供语义指导；密集分支持续整合原始视频的运动和纹理信息以保持高保真度和连贯性。采用退化模拟训练策略，通过在人工退化视频上训练学习运动重建和时间一致性，无需配对数据。

Result: 大量实验表明，NOVA在编辑保真度、运动保持和时间连贯性方面优于现有方法。

Conclusion: NOVA通过稀疏控制和密集合成的结合，以及无需配对数据的训练策略，成功解决了视频编辑中的配对数据瓶颈问题，实现了高质量的无配对视频编辑。
...
TL;DR: 本文研究了视觉语言模型在古希腊评注本结构感知文本识别中的应用，创建了大规模合成语料库和真实扫描基准，评估了三种先进模型，发现Qwen3VL-8B在真实扫描上达到1.0%字符错误率的最佳性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在端到端文档理解方面取得了进展，但在处理具有复杂布局语义的历史学术文本（特别是古希腊评注本）时能力有限。这些文本具有密集的引用层次结构和广泛的边缘注释，需要结构感知的识别方法。

Method: 1) 创建两个新资源：从TEI/XML源生成18.5万页图像的大规模合成语料库（控制排版和布局变化）；2) 构建跨越一个多世纪编辑和排版实践的真实扫描评注本基准数据集；3) 在零样本和微调两种设置下评估三种最先进的视觉语言模型。

Result: 实验显示当前VLM架构在处理高度结构化的历史文档时存在显著局限性。在零样本设置下，大多数模型表现显著低于成熟的现成软件。然而，Qwen3VL-8B模型实现了最先进的性能，在真实扫描上达到中位数字符错误率1.0%。

Conclusion: 研究结果既凸显了当前视觉语言模型在处理复杂学术文档结构感知识别方面的不足，也展示了其未来潜力。Qwen3VL-8B的优异表现表明，经过适当训练和优化，VLM能够有效处理具有复杂布局的历史学术文本。
...
TL;DR: ScribeTokens是一种将数字墨水分解为像素步长和笔状态标记的标记化方法，仅需10个基础标记就能表示任何数字墨水，在生成任务上显著优于向量表示，并通过自监督预训练在识别任务上达到最佳效果。
...
Method: 提出ScribeTokens标记化方法，将笔移动分解为单位像素步长，加上两个笔状态标记，形成固定的10标记基础词汇表，支持激进的BPE压缩；并引入下一墨水标记预测作为自监督预训练策略。

Result: 在手写文本生成任务上，ScribeTokens显著优于向量表示（17.33% vs 70.29% CER）；在识别任务上，ScribeTokens是唯一无需预训练就能超越向量表示的标记表示；通过预训练，在两个数据集上都达到了最佳识别结果（IAM上8.27% CER，DeepWriting上9.83% CER）。

Conclusion: ScribeTokens提供了一种高效的数字墨水表示方法，仅需极小词汇量就能表示任何数字墨水，在生成和识别任务上都表现出色，配合自监督预训练策略能进一步提升性能并加速收敛。
...
<details>
  <summary>Details</summary>
Motivation: 文本到视频(T2V)模型快速发展但商业潜力未充分开发，需要解决在自动生成的视频中嵌入广告品牌同时保持用户意图语义保真度的挑战。

Method: BrandFusion采用两阶段多智能体框架：离线阶段面向广告商构建品牌知识库，通过模型先验探测和轻量微调适应新品牌；在线阶段面向用户，五个智能体通过迭代优化利用共享知识库和实时上下文跟踪，确保品牌可见性和语义对齐。

Result: 在18个现有品牌和2个自定义品牌上，BrandFusion在多个最先进T2V模型中显著优于基线方法，在语义保持、品牌可识别性和融入自然性方面表现优异。人类评估进一步确认了更高的用户满意度。

Conclusion: BrandFusion为T2V可持续货币化建立了实用路径，成功解决了品牌植入中的语义保真度、品牌可识别性和上下文自然性三大核心挑战。
...
TL;DR: 提出了一种基于多模态先验引导重要性采样的分层3D高斯溅射方法，用于稀疏视角下的新视角合成，通过融合多种线索来指导高斯粒子的精细添加，实现更好的重建效果。


<details>
  <summary>Details</summary>
Motivation: 在稀疏视角下进行3D重建时，传统方法容易受到纹理诱导的过拟合和姿态/外观不一致带来的噪声影响。需要一种更稳健的方法来识别哪些区域可以可靠地恢复细节，而不是仅仅依赖渲染残差。

Method: 1. 多模态先验引导的重要性采样器：融合光度渲染残差、语义先验和几何先验，生成局部可恢复性估计，指导精细高斯的注入位置。
2. 粗到细的高斯表示：使用稳定的粗层编码全局形状，根据多模态指标在可恢复细节区域选择性添加精细基元。
3. 几何感知的采样和保留策略：将细化集中在几何关键和复杂区域，同时保护欠约束区域中新添加的基元免于过早剪枝。

Result: 在多个稀疏视角基准测试中实现了最先进的重建效果，在DTU数据集上PSNR提升了高达+0.3 dB。有效缓解了过拟合纹理诱导错误，抑制了姿态/外观不一致带来的噪声。

Conclusion: 通过融合多模态证据而非仅依赖原始残差，该方法能够更可靠地指导3D高斯溅射的层次化细化，在稀疏视角下实现更高质量的3D重建和新视角合成。
...
TL;DR: TaYS框架实现了视频流式推理，通过并行化思维链生成、流约束训练和流并行推理，显著提升大视觉语言模型在视频理解任务中的效率和响应速度。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型的思维链推理通常假设视频数据在推理前完全可用，采用批处理方式，这与现实世界中视频信息按顺序到达的流式特性不符。为了匹配视频数据的流式特性，需要研究更高效的流式推理范式。

Method: 提出了Think-as-You-See (TaYS)统一框架，包含：1) 并行化思维链生成；2) 流约束训练；3) 流并行推理。采用时间对齐的推理单元、流式注意力掩码和位置编码，以及解耦视觉编码与文本推理的双重KV缓存机制。

Result: 在Qwen2.5-VL系列模型上的实验表明，TaYS在事件动态分析、因果推理和主题理解等视频思维链任务中，始终优于批处理和交错式基线方法，显著提升了推理性能，同时大幅减少了首词生成时间和整体推理延迟。

Conclusion: TaYS框架通过数据对齐的流式推理，为大视觉语言模型实现了高效、响应迅速的视频理解能力，为处理真实世界流式视频数据提供了有效的解决方案。
...
TL;DR: SIGMark是一个用于视频扩散模型的可扩展、盲提取的生成中水印框架，解决了现有方法非盲提取、计算成本高和抗时间扰动能力弱的问题。


<details>
  <summary>Details</summary>
Motivation: AIGC视频生成快速发展，水印技术对保护AI生成视频和追踪有害内容至关重要。现有生成中水印方法存在非盲提取（需要存储大量消息-密钥对）、计算成本高，以及在因果3D VAE模型中抗时间扰动能力弱的问题。

Method: 提出SIGMark框架：1) 使用全局帧级伪随机编码密钥(GF-PRC)生成带水印的初始噪声，实现盲提取并保持噪声分布多样性；2) 针对因果3D VAE设计分段分组排序模块(SGO)，增强时间扰动下的水印反转鲁棒性。

Result: 在现代扩散模型上的综合实验表明，SIGMark在时间和空间扰动下都能实现很高的比特提取准确率，同时计算开销最小，证明了其可扩展性和鲁棒性。

Conclusion: SIGMark为视频扩散模型提供了一个可扩展、盲提取的生成中水印解决方案，有效解决了现有方法的局限性，在保护AI生成视频内容方面具有重要应用价值。
...
TL;DR: SemanticDialect是一种针对视频扩散变换器（VDiT）的高效量化方法，通过语义感知的方言分配、激活分解和格式书扩展，在保持视频质量的同时显著降低计算和内存成本。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiT）在视频生成方面表现出色，但其高内存和计算成本阻碍了边缘部署。现有量化方法在处理高激活变化和保持语义/时间一致性方面存在质量下降问题。
...
Result: 在视频DiT模型上的实验表明，SemanticDialect优于先前的VDiT量化方法和细粒度块级格式基线，在Open-Sora 2.0上接近FP16质量水平。

Conclusion: SemanticDialect通过创新的语义感知量化策略，在保持视频生成质量的同时显著降低了扩散变换器的计算和内存需求，为边缘部署提供了可行解决方案。
...
TL;DR: 提出StegaFFD框架，利用图像隐写术保护面部隐私，直接在隐写域进行人脸伪造检测，避免传统隐私保护方法引起的语义失真和攻击者警觉


<details>
  <summary>Details</summary>
Motivation: 现有面部伪造检测模型假设能访问原始面部图像，但在客户端-服务器框架下，隐私面部数据可能在传输中被截获或被不可信服务器泄露。传统隐私保护方法（如匿名化、加密或失真）虽然部分缓解泄漏问题，但会引入严重的语义失真，使图像看起来明显受到保护，从而引起攻击者警觉并激发更激进的攻击策略。此外，这些方法会严重操纵图像内容，引入可能混淆面部伪造检测模型的退化或伪影。

Method: 提出基于隐写术的面部伪造检测框架StegaFFD，将面部图像隐藏在自然封面图像中，直接在隐写域进行伪造检测。为了解决隐藏的伪造特定特征极其细微且受封面语义干扰的问题，提出了低频感知分解（LFAD）和空间频率差分注意力（SFDA）来抑制低频封面语义的干扰并增强隐藏面部特征的感知。此外，引入隐写域对齐（SDA）来对齐隐藏面部与其原始对应物的表示，增强模型在隐写域感知细微面部线索的能力。

Result: 在七个面部伪造检测数据集上进行的大量实验表明，StegaFFD实现了强大的不可感知性，避免了引起攻击者的怀疑，并且与现有的面部隐私保护方法相比，更好地保留了面部伪造检测的准确性。

Conclusion: StegaFFD框架通过将图像隐写术与面部伪造检测相结合，提供了一种有效的隐私保护解决方案，既保护了面部隐私又不引起攻击者警觉，同时保持了检测准确性。
...
Method: 采用模块化多智能体框架，包含四个阶段的专门智能体：查询解析与规划、地标推理、多模态检索和重排答案合成。关键组件包括地标知识智能体（将地标转化为视觉提示）、LLM辅助图像检索管道（自动检测地标并检索图像）和OCR细化模块（改进越南语文本识别）。

Result: 实验结果表明LLandMark实现了自适应、文化基础和可解释的检索性能，特别针对越南场景优化了CLIP语义匹配。

Conclusion: LLandMark框架通过多智能体协作和专门模块，有效解决了复杂多模态视频检索问题，特别是在处理文化地标和越南语内容方面表现出色。
...
TL;DR: MVD-HuGaS：通过多视角扩散模型从单张图像实现自由视角3D人体渲染，解决了现有方法中的人体结构扁平化和过度平滑问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的单图像3D人体重建方法存在明显缺陷：1）产生不理想的伪影（如扁平化的人体结构或多视角不一致导致的过度平滑结果）；2）在真实场景中泛化能力不足。需要一种能够生成高质量、一致多视角图像并准确估计相机姿态的方法。

Method: 1）使用增强的多视角扩散模型从单张参考图像生成多视角图像，该模型在高质量3D人体数据集上微调以融入3D几何和人体结构先验；2）引入对齐模块联合优化3D高斯和相机姿态，从稀疏生成的多视角图像中推断准确相机姿态；3）提出基于深度的面部失真缓解模块来细化生成的面部区域；4）利用细化的多视角图像和准确相机姿态优化目标人体的3D高斯以实现高保真自由视角渲染。

Result: 在Thuman2.0和2K2K数据集上的大量实验表明，MVD-HuGaS在单视角3D人体渲染任务上达到了最先进的性能。

Conclusion: MVD-HuGaS通过结合增强的多视角扩散模型、相机姿态联合优化和面部失真缓解技术，成功实现了从单张图像生成高质量自由视角3D人体渲染，解决了现有方法中的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D视觉定位任务仅处理句子级别的检测或分割，未能充分利用自然语言表达中丰富的组合上下文推理。为了解决这一挑战，需要开发能够支持更细粒度理解的3D视觉语言任务。

Method: 1) 提出3D-DRES新任务，实现短语到3D实例的映射；2) 创建DetailRefer数据集，包含54,432个描述和11,054个不同对象，采用创新的短语-实例标注范式；3) 设计DetailBase基线架构，支持句子和短语级别的双模式分割。

Result: 在DetailRefer数据集上训练的模型不仅在短语级别分割表现出色，而且在传统的3D-RES基准测试中也显示出令人惊讶的改进效果。

Conclusion: 3D-DRES任务和DetailRefer数据集为细粒度3D视觉语言理解提供了新的研究方向，通过短语级别的标注和分割能够显著提升模型性能，为更复杂的3D场景理解奠定了基础。
...
TL;DR: AiM框架从用户-物体交互视频和初始扫描中重建可交互3D数字副本，实现高质量分割、运动分析和渲染，无需部件数量先验知识


<details>
  <summary>Details</summary>
Motivation: 现有方法需要两个不同关节状态和部件数量先验知识，限制了应用范围和鲁棒性。当物体在两个状态中不能清晰可见时，性能会下降

Method: 提出AiM框架，使用双高斯场景表示从初始3DGS扫描和运动视频学习，利用运动线索分割部件并分配关节。采用顺序RANSAC进行部件运动分析，无需部件结构先验，自动确定部件数量

Result: 相比先前方法获得更高质量的部件分割，无需先验知识。在简单和复杂物体上的广泛实验验证了方法的有效性和强泛化能力

Conclusion: AiM框架能够从交互视频和初始扫描中重建可交互3D数字副本，实现高质量分割和运动分析，克服了现有方法的局限性
...
Conclusion: 该方法能够有效揭示视频扩散变换器中运动概念的时空定位机制，为理解模型如何将文本描述转换为视频提供了新的可解释性工具
...
<details>
  <summary>Details</summary>
Motivation: 现有开集目标检测方法严重依赖手动标注的细粒度训练数据集和资源密集的层间跨模态特征提取，需要更简洁高效的解决方案。

Method: 基于Transformer的DINO模型构建两阶段训练策略：第一阶段通过One-to-Many语义对齐机制将噪声样本作为额外正样本实例，实现视觉和文本模态对齐，并设计基于初始检测难度的难度加权分类损失挖掘困难样本；第二阶段使用轻量级特征融合模块增强对语言语义的敏感性。

Result: 在Swin Transformer-T设置下，HDINO-T在COCO数据集上达到49.2 mAP（使用220万训练图像），超越Grounding DINO-T和T-Rex2；微调后HDINO-T和HDINO-L分别达到56.4 mAP和59.2 mAP，展示了方法的有效性和可扩展性。

Conclusion: HDINO通过简洁高效的设计消除了对人工数据整理和资源密集型组件的依赖，在开集目标检测任务中取得了有竞争力的性能，为实际应用提供了实用解决方案。
...
TL;DR: GloPath是一个基于实体的基础模型，通过多尺度多视图自监督学习训练了超过100万个肾小球，用于肾小球病变评估和临床病理学发现，在52个任务中80.8%优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 肾小球病理学对肾脏疾病的诊断和预后至关重要，但肾小球形态的异质性和细粒度病变模式对当前AI方法仍具挑战性，需要开发更有效的评估和发现工具。

Method: 使用多尺度多视图自监督学习训练基于实体的基础模型，从14,049个肾活检标本中提取超过100万个肾小球，采用实体中心的方法处理肾小球病理学。

Result: 在三个独立队列的52个任务中，GloPath在42个任务（80.8%）上优于最先进方法；在大规模真实世界研究中，病变识别的ROC-AUC达到91.51%；在224个形态-临床变量对中揭示了统计学显著关联。
...
TL;DR: TRACE提出了一种统一生成式推理与判别式表示学习的多模态检索方法，通过生成结构化思维链来显式推理查询，然后将推理轨迹压缩为紧凑嵌入，在复杂查询时自动激活推理，简单查询时绕过推理，实现检索精度与推理吞吐量的最优平衡。
...
Result: 在M-BEIR基准测试中，TRACE达到新的最先进水平。关键的是，TRACE展示了学习的隐式路由行为：对复杂查询自动激活推理，对简单查询绕过推理，实现检索精度与推理吞吐量的最优平衡。同时展现出对未见领域和新约束的显著零样本迁移能力。

Conclusion: TRACE通过统一生成式推理与判别式表示学习，解决了多模态检索中复杂意图理解的问题，实现了自适应推理激活和优异的零样本迁移能力，为多模态检索提供了新的有效框架。
...
TL;DR: TC-Padé是一种基于Padé逼近的特征预测框架，通过有理函数建模特征演化，结合自适应系数调制和阶段感知策略，在低步数采样（20-30步）下实现稳定加速，相比现有特征缓存方法显著提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成质量优秀，但迭代采样过程计算负担重。现有特征缓存技术在较高步数（如50步）有效，但在实际应用的低步数（20-30步）存在严重限制：多项式外推器（如TaylorSeer）误差累积和轨迹漂移，传统缓存策略忽视不同去噪阶段的动态特性差异。

Method: 提出轨迹一致Padé逼近（TC-Padé）特征预测框架：1）使用Padé逼近（有理函数）建模特征演化，比泰勒方法更准确捕捉渐近和过渡行为；2）自适应系数调制，利用历史缓存残差检测细微轨迹转换；3）阶段感知预测策略，针对早期、中期和晚期采样阶段的不同动态特性进行定制。
...
Conclusion: TC-Padé通过Padé逼近和轨迹一致采样策略，有效解决了低步数采样下特征缓存方法的局限性，实现了高质量加速，为扩散模型的实际部署提供了有效解决方案。
...
TL;DR: 提出一种半监督方法，利用未标记数据传播文本信息伪标签，在极低样本医学图像分类中减少50%以上标注工作量


<details>
  <summary>Details</summary>
Motivation: 医学图像标注成本高昂，现有视觉语言模型在极低样本场景下受类别不平衡影响，少数类别性能下降明显

Method: 提出高效半监督求解器，在少样本适应过程中传播文本信息伪标签，利用未标记数据提升模型性能

Result: 方法在低样本场景下减少超过50%的标注工作量，实现更经济的标注流程

Conclusion: 通过利用未标记数据和文本信息伪标签传播，能够有效解决医学图像极低样本适应中的类别不平衡问题
...
TL;DR: 本文提出FMAS（基于基础模型的异常合成管道）和WDAM（小波域注意力模块）来解决工业异常检测中异常样本稀缺和真实异常复杂性的问题，无需微调或类别特定训练即可生成逼真异常样本。


<details>
  <summary>Details</summary>
Motivation: 工业异常检测面临两个主要挑战：1）异常样本稀缺，难以获得足够的训练数据；2）真实世界异常复杂性高，难以准确检测。作者观察到异常在频域具有独特的特征，这为改进异常检测提供了新思路。

Method: 方法包含两个核心组件：1）FMAS（基础模型异常合成管道）：利用基础模型生成高度逼真的异常样本，无需微调或类别特定训练；2）WDAM（小波域注意力模块）：利用自适应子带处理来增强异常特征提取，通过频域分析提升检测能力。

Result: 在MVTec AD和VisA数据集上的综合实验表明：1）WDAM作为即插即用模块，相比现有基线方法实现了显著的性能提升；2）FMAS和WDAM的组合显著提高了异常检测的灵敏度，同时保持了计算效率。

Conclusion: 本文提出的FMAS和WDAM方法有效解决了工业异常检测中的样本稀缺和检测精度问题。WDAM作为频域注意力模块能够有效增强异常特征提取，而FMAS则提供了高质量的异常样本生成能力，两者结合为工业异常检测提供了新的解决方案。
...
TL;DR: TagaVLM是一个用于视觉语言导航的端到端框架，通过将拓扑结构显式注入VLM主干，解决大视觉语言模型在导航任务中的架构不匹配问题，在R2R基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言导航对大型视觉语言模型提出了独特挑战，因为VLMs主要预训练于静态、非具身的视觉语言任务，这与导航的动态、具身和空间结构化特性存在根本冲突。现有方法通常将丰富的视觉和空间信息转换为文本，迫使模型隐式推断复杂的视觉-拓扑关系或限制其全局动作能力。

Method: 提出TagaVLM框架，显式地将拓扑结构注入VLM主干：1) 空间拓扑感知残差注意力(STAR-Att)直接将拓扑边信息集成到VLM的自注意力机制中；2) 交错导航提示增强拓扑节点信息的视觉-文本对齐；3) 通过嵌入的拓扑图实现全局动作推理，支持鲁棒的路径校正。

Result: 在R2R基准测试中，TagaVLM在未见环境中取得了51.09%的成功率和47.18的SPL，在大型模型方法中达到最先进性能，相比先前工作SR提高了3.39%，SPL提高了9.08。

Conclusion: 研究表明，对于具身空间推理，针对小型开源VLM进行有针对性的增强比暴力模型缩放更有效。该方法证明了将拓扑结构显式集成到VLM架构中的价值，为导航任务提供了更有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在真实手术条件下评估腹部软组织3D重建需要高质量的数据集，现有数据集缺乏配对的腹腔镜视频和精确几何数据，无法全面评估非刚性变形场景下的算法性能。

Method: 使用达芬奇Xi立体腹腔镜和Zivid结构光相机采集六次猪尸体实验数据，通过光学追踪和手动迭代配准方法进行数据对齐，包含三种序列类型：整体变形、增量变形和相机移动片段。

Result: 数据集包含超过300,000帧图像和369个点云，涵盖98个精心处理的记录，提供矫正立体图像、每帧器械掩码、立体深度、起始/结束结构光点云、精确相机位姿和相机内参。

Conclusion: D4D数据集为开发和评估非刚性SLAM、4D重建和深度估计方法提供了全面的基准测试资源，支持在可见和遮挡区域进行定量几何评估以及光度视图合成基线测试。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术在3D场景建模和渲染方面表现出色，但缺乏高效且逼真的场景外观编辑方法。现有方法难以保持视点相关效果（如镜面高光），且编辑过程不够快速便捷。

Method: 1. 提出新颖架构：将颜色分离为漫反射和视点相关分量；2. 多视图训练策略：整合来自多个视角的图像块；3. 快速编辑方案：仅需一张手动编辑图像，通过微调单个MLP权重和单次分割模块，在2秒内将颜色编辑传播到整个场景；4. 提供视点相关效果强度控制。

Result: 在多样化数据集上的验证表明，该方法在定量和定性评估上均显著优于基于神经辐射场表示的方法。能够准确重建场景，并作为重新着色任务的坚实基础，同时保持视点相关效果。

Conclusion: VIRGi为3D高斯泼溅场景提供了一种快速、逼真的颜色编辑解决方案，仅需一张手动编辑图像即可在2秒内完成整个场景的颜色传播，同时保持视点相关效果，实现了实时交互和编辑控制。
...
TL;DR: 提出URGT方法，通过多块Transformer联合处理高分辨率图像，实现深度和法线联合估计，在保持局部细节和全局一致性方面取得突破，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 高分辨率场景下的深度和法线联合估计面临局部细节保持与全局一致性之间的权衡难题，现有方法难以同时兼顾精细局部特征和全局几何一致性。

Method: 提出Ultra Resolution Geometry Transformer (URGT)，基于VGGT构建统一多块Transformer框架。将高分辨率图像分割成多个块，并加入预训练模型生成的粗略深度和法线先验，通过单次前向传播联合处理。采用跨块注意力机制确保全局一致性，引入GridMix块采样策略增强空间鲁棒性。

Result: 在UnrealStereo4K数据集上取得SOTA结果：AbsRel从0.0582降至0.0291，RMSE从2.17降至1.31，平均角度误差从23.36度降至18.51度。同时表现出强大的零样本和跨域泛化能力，能够有效扩展到极高分辨率。

Conclusion: URGT提供了一种高效可扩展的高质量几何细化解决方案，通过多块Transformer框架成功解决了高分辨率几何估计中局部细节与全局一致性的平衡问题，为整体3D场景理解提供了有效工具。
...
TL;DR: BRIGHT是首个专门针对乳腺病理设计的病理基础模型，在51,000张乳腺全切片图像上训练，在24项临床任务中表现出色，超越了通用病理基础模型。
...
Method: 提出BRIGHT模型，采用协作式通用-专家框架，在来自40,000多名患者的51,000张乳腺全切片图像上训练，包含约2.1亿个组织病理学图块。构建了最大的多机构验证队列，包含25,000多张WSI，涵盖24项临床任务。

Result: BRIGHT在24项内部验证任务中的21项达到SOTA性能，在10项外部验证任务中的5项达到SOTA，超越了三个领先的通用病理基础模型，并具有出色的热图可解释性。

Conclusion: BRIGHT不仅展示了在乳腺肿瘤学中的临床实用性，还验证了协作式通用-专家范式，为开发特定器官系统的病理基础模型提供了可扩展的模板。
...
TL;DR: EduAIGV-1k是首个专门评估教育导向AI生成视频质量的基准数据集，包含1130个数学教学视频，提供细粒度的感知质量和提示对齐标注，并提出了EduVQA评估框架和S2D-MoE模块来提升评估性能。


<details>
  <summary>Details</summary>
Motivation: 当前AI生成内容模型在生成逼真视频方面取得了显著成功，但其在教育领域支持视觉化、故事驱动学习的潜力尚未充分开发。特别是缺乏专门评估教育导向AI生成视频质量的基准数据集和评估框架。

Method: 1. 构建EduAIGV-1k数据集：包含1130个由10个先进文本到视频模型生成的短视频，基于113个教学导向提示；2. 提供细粒度标注：沿两个互补维度（感知质量：空间和时间保真度；提示对齐：词级和句级对齐）；3. 提出EduVQA评估框架：引入结构化2D专家混合模块，通过共享专家和动态2D门控矩阵增强整体质量与各子维度之间的依赖关系。

Result: EduVQA在广泛的实验中一致优于现有的视频质量评估基线方法。数据集和代码将公开可用。

Conclusion: 该研究填补了教育领域AI生成视频评估的空白，通过构建首个专门基准数据集和提出改进的评估框架，为教育导向AI生成视频的质量评估提供了重要工具和方法。
...
TL;DR: MoECLIP提出了一种基于混合专家（MoE）架构的零样本异常检测方法，通过动态路由图像块到专门的LoRA专家实现细粒度适应，解决了现有方法处理所有图像块时忽略其独特特征的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有零样本异常检测方法在处理CLIP模型时采用"块无关"设计，将所有图像块统一处理而忽略其独特特征，这限制了模型在异常检测任务中的专业化能力同时保持CLIP强大泛化能力的平衡。

Method: 提出MoECLIP架构：1）使用混合专家（MoE）系统，基于每个图像块的独特特征动态路由到专门的LoRA专家；2）引入冻结正交特征分离（FOFS）技术，正交分离输入特征空间以强制专家关注不同信息；3）采用单纯形等角紧框架（ETF）损失，调节专家输出形成最大等角表示。

Result: 在涵盖工业和医疗领域的14个基准数据集上的综合实验结果表明，MoECLIP超越了现有的最先进方法。

Conclusion: MoECLIP通过混合专家架构实现了细粒度的图像块级适应，有效解决了零样本异常检测中专业化与泛化能力的平衡问题，在多个领域数据集上表现出优越性能。
...
Method: 提出A trous Wavelet Diffusion (AWDiff)框架，集成a trous小波变换以保留精细结构，避免破坏性下采样；使用BioMedCLIP进行语义条件约束，确保与临床有意义的标签对齐

Result: 在LUS数据集上，AWDiff相比现有方法实现了更低的失真和更高的感知质量，展示了结构保真度和临床多样性

Conclusion: AWDiff框架能够有效生成高质量的肺部超声图像，保留关键诊断特征，为医学图像分析和疾病监测提供了有效的增强方法
...
<details>
  <summary>Details</summary>
Motivation: 现有方法利用2D扩散模型进行3D编辑时，难以保持多视角一致性，且缺乏3D一致的编辑配对数据，使得监督微调不可行。观察到生成多视角一致的3D内容困难，但验证3D一致性相对容易，这自然将强化学习定位为可行解决方案。

Method: 提出RL3DEdit框架：1) 利用强化学习优化，2) 从3D基础模型VGGT中提取新颖的奖励信号，3) 将编辑后的图像输入VGGT，利用其输出的置信度图和姿态估计误差作为奖励，4) 通过强化学习将2D编辑先验锚定到3D一致流形上。

Result: 大量实验表明，RL3DEdit实现了稳定的多视角一致性，在编辑质量上优于现有最先进方法，且具有高效率。该方法解决了3D编辑中的多视角一致性问题。

Conclusion: RL3DEdit通过强化学习框架有效解决了3D编辑中的多视角一致性问题，利用VGGT的3D先验作为奖励信号，实现了高质量、高效率的3D编辑。该方法为3D编辑领域提供了新的解决方案，并将开源代码和模型以促进该领域发展。
...
TL;DR: Kling-MotionControl是一个基于DiT的统一框架，专门用于鲁棒、精确和富有表现力的整体角色动画，通过异构运动表示和自适应身份无关学习实现高质量动画生成。


<details>
  <summary>Details</summary>
Motivation: 角色动画的目标是通过将驱动视频的运动动态转移到参考图像来生成逼真的视频。虽然生成模型取得了进展，但需要一种能够处理身体、面部和手部不同特性，同时保持结构稳定性和细节表现力的统一框架。

Method: 采用基于DiT的统一框架，使用分而治之策略协调异构运动表示；引入自适应身份无关学习实现跨身份泛化；通过精细的身份注入和融合设计保持外观忠实度；使用多阶段蒸馏加速框架提升推理速度10倍以上。
...
TL;DR: 本文提出CAT框架，通过几何条件机制和非线性传输映射，在保持图像质量的同时减少T2I模型生成不安全内容的问题。
...
Method: 首先构建包含2300个安全和不安全提示对的SafeSteerDataset，然后提出条件激活传输框架，采用几何条件机制和非线性传输映射，仅在激活区域不安全时进行干预。

Result: 在Z-Image和Infinity两种架构上验证，CAT能有效降低攻击成功率，同时保持与未引导生成相当的图像保真度。

Conclusion: CAT框架通过条件激活传输，在减少T2I模型生成不安全内容的同时，最小化对良性查询的干扰，实现了安全性和图像质量的平衡。
...
TL;DR: MoD-DPO是一种改进全模态大语言模型模态对齐的方法，通过解耦模态感知正则化和语言先验去偏惩罚，减少跨模态幻觉，提高感知准确性和抗幻觉能力。
...
Method: 提出Modality-Decoupled Direct Preference Optimization (MoD-DPO)框架，包含：1）模态感知正则化项，强制对不相关模态的扰动保持不变性，对相关模态的扰动保持敏感性；2）语言先验去偏惩罚，减少对文本先验的过度依赖。

Result: 在多个视听幻觉基准测试中，MoD-DPO在相似训练预算下优于之前的偏好优化基线方法，显著提高了感知准确性和抗幻觉能力。

Conclusion: MoD-DPO证明了模态忠实对齐的重要性，为构建更可靠、更具韧性的多模态基础模型提供了一条可扩展的路径。
...
TL;DR: 论文提出SpeciaRL框架，通过特异性感知的强化学习来微调推理大模型，解决开放世界细粒度图像分类中正确性与特异性难以平衡的问题。


<details>
  <summary>Details</summary>
Motivation: 在开放世界设置下进行细粒度视觉概念分类时，现有推理大模型虽然具备视觉理解能力，但倾向于产生过于通用的预测。模型本身具备细粒度领域知识，但如何在保持正确性的同时提升预测特异性是一个尚未充分研究的挑战。

Method: 提出SpeciaRL框架，这是一种特异性感知的强化学习方法，通过基于验证器的动态奖励信号来微调推理大模型。该奖励信号以在线rollout中的最佳预测为锚点，在提升特异性的同时尊重模型能力以防止错误预测。

Result: 在域外实验中，SpeciaRL在广泛的细粒度基准测试中实现了正确性与特异性的最佳平衡，超越了现有方法，推动了开放世界细粒度图像分类的发展。

Conclusion: SpeciaRL框架通过特异性感知的强化学习有效解决了细粒度图像分类中正确性与特异性的权衡问题，为开放世界细粒度视觉概念分类提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 地球观测应用需要整合多传感器数据，但不同模态间的关系是非单射的（相同条件信息对应多个可能观测结果）。确定性模型会坍缩到条件均值，无法表示数据完成和跨传感器翻译所需的不确定性和变异性。

Method: 提出COP-GEN多模态潜在扩散变换器，将跨模态映射参数化为条件分布，在原生空间分辨率下建模异构地球观测模态的联合分布，支持零样本模态翻译、光谱带填充和部分输入生成，无需任务特定重训练。

Result: 在大规模全球多模态数据集上的实验表明，COP-GEN能生成多样且物理一致的实现，同时在光学、雷达和高程模态上保持强峰值保真度。模型能捕捉有意义的跨模态结构，并随条件信息增加系统调整输出不确定性。

Conclusion: 研究强调了随机生成建模在地球观测中的实际重要性，并推动评估协议超越单一参考、逐点指标，为多模态地球观测数据集成提供了灵活的概率框架。
...
TL;DR: 该研究通过UniG2U-Bench基准测试发现，统一多模态模型的生成能力并不总能提升理解性能，生成-理解耦合在特定任务中才有效


<details>
  <summary>Details</summary>
Motivation: 当前统一多模态模型展现出强大生成能力，但生成是否以及何时能提升理解能力尚不明确，缺乏系统性的任务探索

Method: 引入UniG2U-Bench基准，将生成到理解评估分为7个机制和30个子任务，涵盖不同程度的隐式或显式视觉转换，评估超过30个模型

Result: 1) 统一模型通常表现不如基础视觉语言模型，生成后回答推理通常比直接推理性能更差；2) 在空间智能、视觉错觉或多轮推理任务中观察到一致改进；3) 相似推理结构的任务和共享架构的模型表现出相关行为

Conclusion: 生成-理解耦合在任务、预训练数据和模型架构上诱导类一致的归纳偏差，需要更多样化的训练数据和新的范式来充分释放统一多模态建模的潜力
...
TL;DR: DuoMo是一种从无约束视频中恢复世界坐标下人体运动的方法，通过两个扩散模型解决噪声或不完整观测下的运动重建问题。


<details>
  <summary>Details</summary>
Motivation: 从无约束视频中恢复世界坐标下的人体运动面临一个基本权衡：需要从多样且噪声的视频输入中泛化，同时保持全局运动一致性。现有方法难以同时处理噪声观测和保持全局一致性。

Method: 将运动学习分解为两个扩散模型：相机空间模型首先在相机坐标中估计运动，世界空间模型然后将这个初始估计提升到世界坐标并进行精炼以保持全局一致性。该方法直接生成网格顶点运动，绕过参数化模型。

Result: 在EMDB数据集上获得16%的世界空间重建误差减少，同时保持低脚部滑动；在RICH数据集上获得30%的世界空间误差减少，达到最先进性能。

Conclusion: DuoMo通过双扩散模型架构有效解决了从噪声或不完整视频观测中恢复世界坐标人体运动的挑战，在多个基准测试中表现出色，为无约束视频中的运动重建提供了通用解决方案。
...
TL;DR: LoGeR是一种新型长上下文几何重建架构，通过分块处理和混合记忆模块，将密集3D重建扩展到极长视频序列，无需后优化，在数千帧推理中保持全局一致性。


<details>
  <summary>Details</summary>
Motivation: 前馈几何基础模型在短窗口重建表现良好，但扩展到分钟级视频时受到二次注意力复杂度或循环设计有限有效内存的限制，需要解决长序列重建的瓶颈问题。

Method: LoGeR采用分块处理视频流，利用强双向先验进行高保真度块内推理。为解决跨块边界一致性关键挑战，提出基于学习的混合记忆模块：包含参数化测试时训练内存来锚定全局坐标系防止尺度漂移，以及非参数化滑动窗口注意力机制保留未压缩上下文以实现高精度相邻对齐。

Result: LoGeR在128帧序列上训练，推理时可泛化到数千帧。在标准基准和新重新利用的VBR数据集（高达19k帧序列）上评估，显著优于先前最先进的前馈方法——KITTI上ATE减少超过74%，在空前范围内实现鲁棒的全局一致重建。

Conclusion: LoGeR通过创新的混合记忆架构解决了长序列3D重建的关键挑战，实现了从训练到推理的有效泛化，在极长视频序列上取得了突破性的重建性能。
...
TL;DR: 该研究通过从头开始的多模态预训练实验，探索了超越语言的基础模型设计空间，发现视觉表示自动编码器(RAE)是最佳统一视觉表示，视觉和语言数据具有互补协同效应，统一多模态预训练自然导向世界建模，MoE架构能有效协调视觉和语言的不同扩展需求。
...
Method: 采用Transfusion框架，对语言使用下一个token预测，对视觉使用扩散模型，在多样化数据（文本、视频、图像-文本对、动作条件视频）上进行从头预训练。通过IsoFLOP分析计算两种模态的扩展规律，并使用MoE架构进行高效多模态扩展。

Result: 获得四个关键发现：1) RAE在视觉理解和生成方面表现最佳；2) 视觉和语言数据具有互补协同效应；3) 统一多模态预训练自然导向世界建模；4) MoE实现高效多模态扩展并自然诱导模态专业化。发现视觉比语言更数据饥渴，存在扩展不对称性。
...
TL;DR: Utonia是首个跨多个领域（遥感、室外LiDAR、室内RGB-D、CAD模型、RGB视频点云）训练的自监督点云Transformer编码器，能够学习一致的表示空间并提升跨域感知能力。


<details>
  <summary>Details</summary>
Motivation: 目标是实现一个能够统一处理来自不同领域点云的单一模型，让所有领域的点云数据能够共同塑造一个对所有领域都有益的基础模型。

Method: 提出Utonia方法，使用自监督学习训练一个点云Transformer编码器，跨越遥感、室外LiDAR、室内RGB-D序列、CAD模型和RGB视频提取的点云等多个不同领域。

Result: Utonia学习到了跨领域的一致表示空间，提升了感知能力，并展现出只有在多领域联合训练时才会出现的涌现行为。此外，Utonia表示还能提升机器人操作和空间推理任务。

Conclusion: Utonia是迈向稀疏3D数据基础模型的重要一步，有望支持AR/VR、机器人和自动驾驶等下游应用，展示了跨领域点云统一表示的巨大潜力。
