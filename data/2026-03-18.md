...
Motivation: NeRF在体积渲染时需要进行密集光线采样，导致计算效率低下。传统方法使用固定采样策略，无法根据场景特性自适应调整采样密度，存在计算资源浪费的问题。

Method: 提出SAC-NeRF框架，将采样问题建模为马尔可夫决策过程，使用Soft Actor-Critic强化学习算法学习自适应采样策略。包含三个关键技术：高斯混合分布颜色模型提供不确定性估计；多组件奖励函数平衡质量、效率和一致性；两阶段训练策略解决环境非平稳性问题。

Result: 在Synthetic-NeRF和LLFF数据集上的实验表明，SAC-NeRF减少了35-48%的采样点，同时渲染质量仅比密集采样基线低0.3-0.8 dB PSNR，在计算效率和渲染质量之间取得了良好平衡。

Conclusion: 虽然学习到的策略是场景特定的，且强化学习框架比简单启发式方法更复杂，但这项工作表明数据驱动的采样策略能够发现难以手动设计的有效模式，为NeRF的高效渲染提供了新思路。
...
TL;DR: 该研究评估了视觉语言模型在盲人和低视力人群导航任务中的表现，发现GPT-4o在空间推理和场景理解方面表现最佳，而开源模型在复杂环境中存在明显不足。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索视觉语言模型在辅助盲人和低视力人群导航方面的潜力，评估现有模型在关键视觉技能上的表现，为开发更有效的辅助技术提供指导。

Method: 方法包括评估闭源模型（GPT-4V、GPT-4o、Gemini-1.5-Pro、Claude-3.5-Sonnet）和开源模型（Llava-v1.6-mistral、Llava-onevision-qwen），测试它们在三个基础视觉技能上的表现：环境障碍物计数、相对空间推理、常识性寻路相关场景理解，并使用针对pBLV设计的提示词模拟真实世界辅助任务。

Result: 结果显示模型间存在显著性能差异：GPT-4o在所有任务中表现最佳，特别是在空间推理和场景理解方面；开源模型在复杂环境中存在推理和适应性困难；常见挑战包括在杂乱环境中准确计数困难、空间推理偏见、以及倾向于优先关注物体细节而非空间反馈。

Conclusion: 尽管存在局限性，视觉语言模型在寻路辅助方面显示出潜力，但需要更好地与人类反馈对齐并改进空间推理能力。该研究为开发者提供了将视觉语言模型有效集成到辅助技术中的实用见解，同时指出了需要解决的关键限制以增强可用性。
...
<details>
  <summary>Details</summary>
Motivation: WSI分析对肿瘤早期诊断至关重要，但超高分辨率使端到端训练不切实际。现有方法使用自然图像预训练的特征提取器无法捕捉领域特定特征，而领域特定预训练又计算昂贵且缺乏任务特异性。

Method: 提出eWSI方法，将参数高效微调(PEFT)与多实例学习(MIL)精心集成，实现WSI任务的端到端训练，既能利用预训练特征提取器，又能高效适应特定任务。

Result: 在Camelyon16、TCGA和BRACS数据集的七个WSI级任务上评估，eWSI使用ImageNet特征提取器即可达到或超越使用领域特定特征提取器的MIL方法；当与领域特定特征提取器结合时，性能进一步提升。

Conclusion: eWSI为WSI任务提供了任务导向、计算高效的解决方案，是计算病理学中任务特定学习的有前景方向，减少了昂贵的领域特定预训练需求。
...
TL;DR: 本文提出了MM-SafetyBench++基准用于评估多模态大语言模型的上下文安全性，并开发了EchoSafe训练框架来提升模型基于上下文理解的安全行为


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉推理任务上表现出色，但其安全漏洞仍然是一个紧迫问题。现有研究主要关注检测和拒绝显式不安全输入的越狱防御，往往忽略了上下文安全性——即模型需要区分看似相似但安全意图截然不同的场景

Method: 1) 构建MM-SafetyBench++基准：为每个不安全图像-文本对创建对应的安全版本，通过最小修改翻转用户意图但保留上下文含义；2) 提出EchoSafe框架：维护自反思记忆库来积累和检索先前交互的安全洞察，将相关历史经验整合到当前提示中，实现上下文感知推理和安全行为的持续演化

Result: 在多个多模态安全基准上的广泛实验表明，EchoSafe始终实现卓越性能，为推进MLLMs的上下文安全性建立了强大的基线

Conclusion: 本文提出的MM-SafetyBench++基准和EchoSafe框架有效解决了多模态大语言模型的上下文安全问题，通过上下文感知推理和持续学习机制提升了模型的安全行为适应能力
...
TL;DR: MATCH是一种多视角高斯配准方法，能够在0.5秒内从校准的多视角图像直接预测对应的高斯溅射纹理，无需数据预处理，比现有方法快10倍，支持个性化头像创建、表情迁移、语义编辑等应用。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的多视角头像方法需要耗时的头部跟踪和昂贵的头像优化，总创建时间通常超过一天。需要一种更快速、无需预处理的方法来创建高质量头像。

Method: 使用基于transformer的模型端到端建立对应关系，预测模板网格固定UV布局中的高斯溅射纹理。引入新颖的配准引导注意力块，每个UV贴图标记仅关注描绘其对应网格区域的图像标记，提高效率和性能。

Result: MATCH在新视角合成、几何配准和头像生成方面优于现有方法，使头像创建速度比最接近的竞争基线快10倍，每帧仅需0.5秒，无需数据预处理。

Conclusion: MATCH提供了一种快速高效的多视角头像创建和编辑方法，通过端到端的对应关系学习，支持多种应用如表情迁移、语义编辑和身份插值，显著提高了头像创建的效率和性能。
...
TL;DR: ModTrack是一个模块化多视角多目标跟踪系统，通过将学习限制在检测和特征提取阶段，使用闭式分析方法进行融合、关联和跟踪，实现了与端到端方法相当的性能，同时提供跨模态、传感器无关的泛化能力和可追踪的不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端多视角多目标跟踪方法缺乏原则性的不确定性处理，且与训练配置紧密耦合，限制了在不同传感器布局、模态或数据集上的泛化能力，需要重新训练才能适应新场景。
...
Conclusion: ModTrack提供了一种模块化、可泛化的多视角多目标跟踪解决方案，在保持与端到端方法相当性能的同时，实现了跨模态、传感器无关的部署灵活性，并提供了可追踪的不确定性处理。
...
TL;DR: ConflictAwareAH：一个多模态框架，通过检测视频、音频和文本表示之间的冲突特征来识别矛盾犹豫状态，在BAH数据集上达到0.694 Macro F1性能。


<details>
  <summary>Details</summary>
Motivation: 矛盾犹豫状态在临床环境中具有重要价值，但机器识别困难，因为关键证据存在于不同模态（语言、声音、面部表情）之间的不一致性中。

Method: 使用三个预训练编码器提取视频、音频和文本表示，通过成对模态嵌入的逐元素绝对差异作为冲突特征，采用文本引导的后期融合策略结合文本专用分类头和完整模型。

Result: 在BAH数据集上达到0.694 Macro F1（标注测试集）和0.715（私有排行榜），比纯文本方法提升F1-NoAH 4.6个百分点，缩小类别性能差距一半，比已发表多模态基线提升超过10个百分点。

Conclusion: ConflictAwareAH框架通过显式建模跨模态冲突特征，有效识别矛盾犹豫状态，解决了文本主导方法过度检测的问题，在单GPU上25分钟内完成训练，具有实用性和高效性。
...
TL;DR: 论文提出AdaRAG-CT框架，通过自适应检索增强解决3D CT报告生成中的表示瓶颈问题，显著提升临床效能


<details>
  <summary>Details</summary>
Motivation: 现有3D CT自动报告生成存在病理覆盖不全的问题，研究发现这是由于对比学习得到的3D CT嵌入存在严重的维度集中现象（512维中仅有2个有效维度），形成了视觉表示瓶颈，限制了生成和检索性能

Method: 提出AdaRAG-CT自适应增强框架：1）通过受控检索引入补充文本信息；2）在生成过程中选择性集成这些信息，补偿视觉表示瓶颈

Result: 在CT-RATE基准测试中达到最先进的临床效能，将Clinical F1从0.420（CT-Agent）提升至0.480（+6个点），消融研究证实检索和生成组件都对改进有贡献

Conclusion: 3D CT嵌入存在严重的表示瓶颈，通过自适应检索增强可以显著改善报告生成的临床效能，为解决医学图像分析中的表示瓶颈问题提供了有效方案
...
TL;DR: FEEL数据集首次将力测量数据与第一人称视角视频配对，包含约300万帧厨房环境中的自然操作数据，45%的帧涉及手-物体接触。该数据集用于接触理解和动作表征学习任务，在多个基准测试中取得先进性能。


<details>
  <summary>Details</summary>
Motivation: 力是驱动物理交互的根本原因，是物理动作理解的关键基础。目前缺乏大规模力测量与第一人称视频配对的数据集，限制了物理动作理解的研究发展。

Method: 开发定制压阻手套收集力测量数据，与第一人称视频同步，构建FEEL数据集。应用该数据集于两个任务：1）接触理解（时间接触分割和像素级接触物体分割）；2）动作表征学习（以力预测作为自监督预训练目标）。

Result: 在时间接触分割任务中取得最先进结果，在像素级分割任务中取得有竞争力的结果，且无需手动接触物体分割标注。动作表征学习在EPIC-Kitchens、SomethingSomething-V2、EgoExo4D和Meccano等数据集上提升了动作理解任务的迁移性能。

Conclusion: FEEL数据集证明了力测量数据对于物理动作理解的重要性，为接触理解和动作表征学习提供了有价值的资源，推动了物理交互理解领域的发展。
...
Method: 提出两阶段框架：第一阶段训练隐式神经模型学习稳定的形状嵌入，然后在形状潜在空间应用聚类获得伪疾病标签；第二阶段在紧凑变分空间中使用伪疾病标签和真实年龄标签解耦因子，通过结合协方差和监督对比损失的多目标解耦损失强制分离和可控性

Result: 在ADNI海马体和OAI远端股骨形状上，实现了接近监督方法的性能，在解耦和重建方面优于最先进的无监督基线，同时支持高保真重建、可控合成和基于因子的可解释性

Conclusion: 该框架能够在无需诊断标签的情况下有效分离3D医学形状中的病理变化与生理衰老，为开发可解释的生物标志物和患者分层提供了有力工具
...
Conclusion: 证明了在图像质量评估领域，不再需要为了达到最先进性能而牺牲可解释性。EvoIQA框架实现了高性能与完全可解释性的统一。
...
TL;DR: NAMD是一个多模态扩散框架，通过生成1年随访的肺结节CT图像来预测结节进展，结合基线扫描和电子健康记录，显著提升恶性预测性能。
...
Method: 提出NAMD框架，包含结节对齐的潜在空间（潜在距离对应结节属性变化）和基于LLM的控制机制，利用多模态扩散模型生成随访CT图像。
...
<details>
  <summary>Details</summary>
Motivation: 胸部CT扫描自动诊断面临两个临床部署中的持续挑战：采集站点间的分布偏移和人口亚组间的性能差异。研究旨在同时解决这两个问题，提升诊断模型的鲁棒性和公平性。

Method: 采用轻量级MobileViT-XXS切片编码器结合两层SliceTransformer聚合器进行体积推理，使用KL正则化的Group DRO优化目标，自适应地提升表现不佳的采集中心和人口亚组的权重，防止组权重崩溃。

Result: 在任务1（COVID-19二分类）中，最佳配置获得挑战F1分数0.835，比已发表的最佳挑战结果提升5.9分；在任务2（四类肺病理识别）中，Group DRO（α=0.5）获得平均每性别宏观F1分数0.815，比最佳挑战结果提升11.1个百分点，女性鳞状细胞癌F1比Focal Loss基线提升17.4分。

Conclusion: 提出的KL正则化Group DRO框架能有效同时处理分布偏移和公平性问题，在多个CT诊断任务中显著提升性能，特别是对代表性不足的人口亚组（如女性鳞状细胞癌患者）有显著改善。
...
TL;DR: 本文提出ATV-Pruning方法，通过非对称文本-视觉权重剪枝解决大型视觉语言模型剪枝中的模态差异问题，利用文本和视觉路径的信息性标记构建校准池，实现更准确的模型压缩。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型剪枝方法通常以统一方式处理不同模态的校准数据，忽视了模态特异性行为。这导致一个关键挑战：如何处理文本和视觉标记的差异行为以实现LVLMs的准确剪枝。

Method: 提出ATV-Pruning方法：1）通过解耦文本和视觉权重系统研究它们对剪枝操作的敏感性；2）自适应构建校准池，包含所有文本标记和部分视觉标记子集；3）设计层自适应选择策略来筛选重要视觉标记。

Result: 研究发现：1）文本路径应通过文本标记校准，因其比视觉路径更敏感；2）视觉路径冗余度高，可承受50%稀疏度。实验证明ATV-Pruning在标准多模态基准测试中优于现有方法。

Conclusion: ATV-Pruning通过考虑文本和视觉标记的差异敏感性，采用非对称剪枝策略，有效解决了LVLMs剪枝中的模态差异问题，实现了更准确的模型压缩。
...
Method: 创建了包含27万多个观测的FlatLands数据集，来自6个现有数据集的17,656个真实室内场景，提供对齐的观测、可见性、有效性和真实鸟瞰图地图，并建立了包含分布内和分布外评估协议的基准。

Result: 比较了无训练方法、确定性模型、集成方法和随机生成模型，并将任务实例化为端到端的单目RGB到地板地图的完整流程。
...
<details>
  <summary>Details</summary>
Motivation: 基于ViT的视觉基础模型在多种视觉任务上表现出色，但存在二次复杂度问题，限制了长序列的可扩展性。现有线性注意力方法需要从头训练消耗大量计算资源，而基于线性化的方法从大语言模型解码器迁移到ViT效果不佳。

Method: 提出ViT-AdaLA三阶段框架：1）注意力对齐阶段：在每个块中对齐普通线性注意力与原始softmax注意力，近似softmax注意力行为；2）特征对齐阶段：微调线性化ViT，使其最终层特征与冻结的softmax VFM教师模型对齐；3）监督微调阶段：将适应的先验知识迁移到下游任务。

Result: 在分类和分割任务上的大量实验表明，ViT-AdaLA在各种最先进的线性注意力对应方法中表现出有效性和通用性。

Conclusion: ViT-AdaLA成功解决了将视觉基础模型知识有效迁移到线性注意力ViT的挑战，通过三阶段对齐策略实现了计算效率与性能的平衡。
...
TL;DR: 该论文提出了USU方法，解决传统上采样技术在可解释AI中导致归因信号失真的问题，通过质量重分配算子保持归因质量和相对重要性排序。
...
Method: 提出通用语义感知上采样（USU），将上采样重新定义为质量重分配问题而非插值问题。使用比例形式的质量重分配算子，可证明地保持归因质量和相对重要性排序。基于特征归因的公理化传统，形式化了四个忠实上采样的期望条件。

Result: 在具有已知归因先验的模型上进行受控实验验证了USU的形式保证；在ImageNet、CIFAR-10和CUB-200上的评估确认了一致的忠实性改进和定性上更优越、语义更连贯的解释。

Conclusion: USU通过将上采样重新概念化为由模型推理驱动的质量重分配问题，而非孤立的插值问题，解决了传统方法的结构性缺陷，为可解释AI提供了更忠实、语义更连贯的归因可视化。
...
<details>
  <summary>Details</summary>
Motivation: 现有隐式配准方法主要依赖零水平集附近的监督，仅捕获表面对应关系，内部变形约束不足，无法满足解剖形状的组级分析需求

Method: 结合符号距离函数重建与神经微分同胚流，通过雅可比行列式和双调和惩罚等体积正则化抑制局部折叠，促进全局一致变形，构建共享规范模板

Result: 在胎盘MRI实验中，相比基于表面的隐式基线方法，本方法在几何保真度和体积对齐方面表现更优，产生解剖可解释且拓扑一致的展平结果

Conclusion: 该方法能够统一重建个体胎盘、对齐到群体隐式模板，并在规范空间中进行体素强度映射，适用于组级分析
...
TL;DR: 本文提出了一种新颖的无监督域适应框架，通过显式正则化语义特征结构来提升自动驾驶场景解析在真实场景中的性能，解决了合成数据到真实数据域适应中的语义结构建模不足问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶场景解析需要大量像素级标注，合成数据集成为替代方案但存在域适应问题。现有无监督域适应方法主要关注全局特征对齐，忽视了特征空间的语义结构，导致类别间语义关系建模不足，限制了模型的泛化能力。

Method: 提出了一种新颖的无监督域适应框架：1）利用类别特定原型强制类间分离和类内紧凑性，增强特征簇的判别性和结构一致性；2）基于熵的噪声过滤策略提高伪标签的可靠性；3）像素级注意力机制进一步细化特征对齐。

Result: 在代表性基准测试上的大量实验表明，该方法始终优于最近的最先进方法，验证了保持语义结构对于自动驾驶场景解析任务中合成到真实域适应的重要性。

Conclusion: 通过显式正则化语义特征结构，显著提升了自动驾驶场景解析在真实场景中的性能，强调了语义结构保持对于鲁棒的合成到真实域适应的关键作用。
...
TL;DR: Interact3D是一个从单图像生成3D组合物体的新框架，通过生成先验、几何对齐、SDF优化和VLM自主修正来解决遮挡下几何细节和空间关系保持的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单图像生成3D组合物体时，特别是在遮挡情况下，往往在隐藏区域几何细节退化，并且无法保持物体间的空间关系。需要一种能够生成物理合理交互3D组合物体的新方法。

Method: 1. 利用先进的生成先验创建高质量个体资产和统一的3D指导场景；2. 两阶段组合流程：基于指导场景通过全局到局部几何对齐固定主要物体，使用可微SDF优化集成后续几何体并惩罚几何交叉；3. 部署闭环自主修正策略：VLM分析多视角渲染，制定针对性修正提示，指导图像编辑模块迭代自修正。

Result: 大量实验表明，Interact3D成功生成了具有碰撞感知的组合，改善了几何保真度并保持了一致的空间关系。

Conclusion: Interact3D框架能够从单图像生成物理合理的交互3D组合物体，有效解决了遮挡下几何细节退化和空间关系保持的挑战。
...
TL;DR: Parallel-ICL：一种并行处理多模态上下文学习的推理算法，通过分块并行处理减少计算成本，同时保持与完整上下文相近的性能


<details>
  <summary>Details</summary>
Motivation: 多模态上下文学习中，增加演示示例数量能提升性能，但Transformer注意力机制的二次计算成本导致推理延迟显著增加，需要在准确性和效率之间进行权衡

Method: 提出Parallel-ICL算法：1) 将长演示上下文划分为多个短块；2) 并行处理这些块；3) 在logit层面使用加权Product-of-Experts集成方法融合预测结果；4) 基于集成学习理论设计聚类分块策略和相似性权重分配

Result: 在VQA、图像描述和分类基准测试中，Parallel-ICL实现了与完整上下文MM-ICL相当的性能，同时显著提高了推理速度

Conclusion: Parallel-ICL为多模态上下文学习中的准确性与效率权衡提供了有效解决方案，能够以显著减少的推理开销实现动态任务适应
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在图形设计领域缺乏结构化理解能力，需要大规模、层次化的设计数据集来支持结构化研究任务

Method: 构建包含155万多层设计组合的数据集，将每个设计表示为层次化组合的组件（文本、图像、矢量、组元素），包含丰富的元素级元数据，并引入图形设计视频作为新挑战

Result: 创建了覆盖20个设计类别、97万独特模板的大规模数据集，包含27,261个动画布局，建立了图形设计研究的新范式

Conclusion: LICA通过将设计表示为组合层和关系的系统，支持直接在设计结构而非像素级别操作的模型研究，为图形设计领域开辟了新的研究方向
...
TL;DR: OneWorld框架直接在3D表示空间中进行扩散，通过3D-URAE统一编码器、CVC一致性损失和MDF训练策略，解决了现有2D方法在跨视图一致性的挑战，生成了高质量且一致的3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的3D场景生成方法主要在2D图像/视频潜在空间操作，这使得保持跨视图的外观和几何一致性具有固有挑战性。为了弥补这一差距，需要直接在连贯的3D表示空间中进行扩散。

Method: 提出了OneWorld框架，核心是3D统一表示自动编码器（3D-URAE），利用预训练的3D基础模型，通过注入外观和提取语义来增强其几何中心特性。引入了token级别的跨视图对应（CVC）一致性损失来显式强制跨视图的结构对齐，并提出流形漂移强制（MDF）来缓解训练-推理暴露偏差，通过混合漂移和原始表示来塑造鲁棒的3D流形。

Result: 综合实验表明，OneWorld能够生成高质量的3D场景，与最先进的基于2D的方法相比，具有更优越的跨视图一致性。

Conclusion: OneWorld通过在3D表示空间直接进行扩散，有效解决了跨视图一致性问题，为3D场景生成提供了新的解决方案框架。
...
TL;DR: 该研究质疑CLIP类对比语言-图像训练模型在纯图像任务中表现不佳是由于模态内（图像-图像）对齐缺失的理论，通过理论分析和实验证明这种模态内错位假设不成立。
...
Method: 重新审视模态内错位假设的理论基础、支持该假设的经验指标以及受影响的性能指标。通过理论分析证明图像嵌入距离不存在所谓的自由度，并通过实验比较语言-图像训练模型（CLIP、SigLIP）和图像-图像训练模型（DINO、SigLIP2）的相似结果。

Result: 研究发现：1）理论上不存在图像嵌入距离的自由度；2）经验指标在语言-图像训练模型和图像-图像训练模型中表现相似；3）在检索和少样本分类等模态内任务中，解决任务模糊性而非假设的错位才是获得最佳结果的关键。

Conclusion: 模态内错位假设不成立，CLIP类模型在纯图像任务中的表现问题并非源于模态内对齐缺失，而是需要解决任务模糊性问题。语言-图像训练模型和纯图像训练模型在相关指标上表现相似。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）需要数百万个泼溅原语，导致存储和传输成本高。现有压缩方法依赖GPU密集型后训练优化和校准图像，限制了实际部署

Method: 将简化问题形式化为稀疏空间图上的局部成对合并。使用质量保持矩匹配近似一对高斯，通过原始混合与其近似之间的合并成本评估合并质量。限制合并候选到局部邻域并高效选择兼容对

Result: NanoGS显著减少原语数量同时保持高渲染保真度，可直接在现有高斯泼溅模型上运行，CPU高效，保持标准3DGS参数化

Conclusion: NanoGS为高斯泼溅简化提供了高效实用的解决方案，无需训练，轻量级，可无缝集成到现有渲染管线
...
<details>
  <summary>Details</summary>
Motivation: 当前病理学视觉语言模型缺乏可靠的自动评估指标，难以检测幻觉等细微错误，限制了其在临床中的广泛应用。

Method: 提出PathGLS框架，从三个维度评估：Grounding（细粒度视觉-文本对齐）、Logic（使用自然语言推理的蕴含图一致性）、Stability（对抗性视觉语义扰动下的输出方差）。支持patch级和WSI级分析。

Result: 在Quilt-1M数据集上，PathGLS对幻觉报告的敏感度下降40.2%，而BERTScore仅2.1%。与专家定义的临床错误层次结构验证显示，PathGLS的Spearman秩相关系数ρ=0.71，显著优于LLM方法（Gemini 3.0 Pro: ρ=0.39）。
...
TL;DR: SynOE-OD是一个基于合成离群点暴露的物体检测框架，利用生成模型和开放词汇检测器生成语义上有意义的离群数据，在训练中同时处理分布内和分布外物体检测。


<details>
  <summary>Details</summary>
Motivation: 当前物体检测器在处理分布外物体时存在严重问题：这些非典型物体经常被完全忽略并错误地归类为背景。现有的OOD检测方法通常依赖复杂架构或辅助分支，且缺乏统一处理ID和OOD的框架。

Method: 提出SynOE-OD框架，利用Stable Diffusion等强生成模型和开放词汇物体检测器生成语义上有意义的物体级数据作为训练中的离群点。通过迁移学习建立强大的ID任务性能，并为检测模型补充OOD物体检测的鲁棒性。

Result: 在已建立的OOD物体检测基准测试中达到了最先进的平均精度，而像GroundingDINO这样的开放词汇检测器在街景中检测OOD物体的零样本性能有限。

Conclusion: SynOE-OD框架成功解决了物体检测中OOD物体被忽视的问题，通过合成离群点暴露方法实现了对ID和OOD物体的统一检测，显著提升了OOD检测性能。
...
TL;DR: QICA框架通过数量感知和空间聚合解决零样本目标计数问题，在FSC-147数据集上表现优异，在CARPK和ShanghaiTech-A上验证了跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标计数方法存在三个主要问题：1）将计数视为粗粒度检索任务，缺乏细粒度数量感知；2）空间不敏感性；3）模型适应过程中的特征空间扭曲导致泛化能力下降。

Method: 提出QICA框架，包含三个核心组件：1）协同提示策略（SPS），通过数值条件提示适应视觉和语言编码器；2）成本聚合解码器（CAD），直接在视觉-文本相似度图上操作，通过空间聚合防止过拟合；3）多级数量对齐损失（L_MQA），在整个流程中强制执行数值一致性。

Result: 在FSC-147数据集上表现出竞争力，在CARPK和ShanghaiTech-A的零样本评估中验证了优秀的跨域泛化能力。

Conclusion: QICA框架通过整合数量感知和空间聚合，有效解决了零样本目标计数的关键挑战，实现了语义识别与数量推理的桥接，并保持了零样本可迁移性。
...
Result: 实验表明EPOFusion优于现有方法，能在过曝光区域保持红外线索，在非过曝光区域实现视觉保真融合，同时提升视觉保真度和下游任务性能。

Conclusion: EPOFusion通过曝光感知的融合方法有效解决了过曝光区域的融合问题，在保持红外信息的同时实现了高质量的视觉融合，为相关应用提供了有效解决方案。
...
TL;DR: DualPrim是一个紧凑的结构化3D重建框架，使用正负超二次曲面表示形状，通过可微布尔运算实现拓扑感知建模，相比传统方法能生成更紧凑、结构化且可解释的3D重建结果。
...
Method: DualPrim使用正负超二次曲面表示形状：正超二次曲面构建基础形状，负超二次曲面通过可微操作器雕刻局部体积，实现孔洞和凹面的拓扑感知建模。该框架嵌入体积可微渲染器中，支持从多视角图像进行端到端学习，并通过封闭形式布尔差实现无缝网格导出。

Result: DualPrim实现了最先进的精度，并产生紧凑、结构化且可解释的输出，比纯加法替代方案更好地满足下游需求。

Conclusion: DualPrim通过正负超二次曲面的加性-减性设计，在不牺牲紧凑性或可微性的情况下提高了表示能力，为3D重建提供了更结构化、可编辑的解决方案。
...
TL;DR: 本文通过对比三种数据增强方法在细粒度动物分类任务中的表现，发现FastGAN在极低数据量下不仅效果不佳，反而会加剧分类器偏见，而使用LoRA微调的Stable Diffusion表现最佳。


<details>
  <summary>Details</summary>
Motivation: 生成模型常用于解决AI训练中的类别不平衡问题，但它们在低数据条件下的失效模式尚未得到充分理解。本文旨在通过受控基准测试，比较不同数据增强策略在低数据条件下的表现。

Method: 使用Oxford-IIIT Pet数据集，人工制造8个代表性不足的品种，比较三种数据增强策略：传统变换、FastGAN和使用LoRA微调的Stable Diffusion 1.5。通过特征嵌入分析（t-SNE）和统计测试评估性能。

Result: FastGAN在极低训练集大小下不仅表现不佳，反而显著增加分类器偏见（偏见差距增加+20.7%，Cohen's d = +5.03，p = 0.013）。t-SNE分析显示FastGAN为严重少数类生成的图像形成紧密孤立簇，表明模式崩溃。Stable Diffusion with LoRA表现最佳，获得最高宏F1分数（0.9125±0.0047），并将偏见差距相对未增强基线减少13.1%。

Conclusion: 研究表明存在一个样本量边界（约20-50张图像/类），低于此边界时GAN增强在该设置中可能有害。Stable Diffusion with LoRA是低数据条件下最有效的增强方法，所有实验均在消费级GPU上完成，无需云计算资源。
...
TL;DR: IOMM提出了一种数据高效的两阶段训练框架，通过仅使用未标记图像数据进行视觉生成组件预训练，减少对稀缺文本-图像配对数据的依赖，显著提升训练效率并达到SOTA性能。
...
Method: 提出Image-Only Training for UMMs (IOMM)两阶段训练框架：第一阶段仅使用丰富的未标记图像数据预训练视觉生成组件，消除对配对数据的依赖；第二阶段使用未标记图像和少量精选文本-图像配对数据进行微调，提升指令对齐和生成质量。
...
Conclusion: IOMM不仅提高了训练效率，还实现了最先进的性能，为解决UMMs视觉生成组件预训练中的数据稀缺和效率问题提供了有效解决方案。
...
Method: 1) 将抓取合成重新表述为确定性常微分方程(ODE)过程；2) 引入免训练的物理感知能量引导策略，使用适应的显式物理能量函数定义目标分布；3) 通过局部蒙特卡洛近似在推理时估计引导项

Result: 在五个基准数据集上的广泛实验表明，EFF-Grasp在抓取质量和物理可行性方面表现优异，同时比基于扩散的基线方法需要更少的采样步骤

Conclusion: EFF-Grasp通过流匹配框架和物理感知能量引导，实现了高效、稳定且物理可行的灵巧抓取生成，显著优于现有的扩散模型方法
...
TL;DR: 该论文调查了主要学术期刊和出版商对AI生成图片的政策，分析了关键关注点，并提出了合规使用AI生成图片的最佳实践指南。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI能够制作出版质量的科学图片，学术出版商对此制定了不一致且模糊的政策，需要明确如何在保持学术诚信的前提下合规使用AI生成图片。

Method: 调查了Nature、Science、Cell Press、Elsevier、PLOS等主要期刊和出版商的AI生成图片政策，分析了他们关注的关键问题，并结合SciDraw等AI科学插图工具的实际案例，提出了最佳实践指南。

Result: 发现出版商主要关注可重复性、作者署名和视觉错误信息等问题，但通过适当的披露和质量控制，AI生成的图片可以在不损害学术诚信的前提下加速科学传播。

Conclusion: AI生成的图片可以成为科学传播的有价值工具，但需要透明披露和严格的质量控制，以确保符合学术出版标准。
...
TL;DR: 提出一种无监督、架构无关的条件化策略，通过预训练的细胞核分割基础模型生成连续细胞概率图作为显式输入先验，结合方差保持正则化，从普通免疫组化图像合成多重免疫荧光图像，显著提升细胞核计数保真度和感知质量。


<details>
  <summary>Details</summary>
Motivation: 多重免疫荧光（mIF）技术成本高、流程复杂且需要专用成像平台，限制了临床常规应用。现有虚拟染色方法主要优化像素级保真度，但未显式约束细胞核形态，这在病理学中具有重要临床意义，因为细胞核数量、形状或空间排列的细微失真会直接影响如Ki67增殖指数等量化终点，几个百分点的误差就可能导致治疗相关风险分类的变化。

Method: 引入无监督、架构无关的条件化策略：1）使用预训练的细胞核分割基础模型生成连续细胞概率图作为显式输入先验；2）添加方差保持正则化项，匹配局部强度统计以保持合成荧光通道中的细胞级异质性；3）软先验保留了二值化阈值处理中丢失的梯度级边界信息，提供更丰富的条件信号而无需任务特定调优。

Result: 在Pix2Pix（U-Net和ResNet生成器）、确定性回归U-Net和条件扩散模型上，使用两个独立数据集进行的控制实验表明，仅通过该方法修改就能在细胞核计数保真度和感知质量方面获得一致改进。

Conclusion: 该方法通过注入细胞核分割先验和方差保持正则化，有效提升了从普通免疫组化图像合成多重免疫荧光图像的质量，特别是在保持细胞核形态和计数准确性方面，为降低mIF技术临床应用的障碍提供了有前景的解决方案。
...
TL;DR: 该论文提出了360Bench基准测试来评估多模态大语言模型在360°图像感知方面的能力，并开发了Free360训练免费框架来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在传统图像理解方面表现出色，但在360°图像感知方面研究不足。360°图像能捕捉完整环境信息，但存在几何畸变和复杂空间关系等挑战，需要专门的评估和解决方案。

Method: 1. 构建360Bench基准测试，包含7K分辨率360°图像和7个代表性任务的VQA数据集；2. 提出Free360框架，采用训练免费的基于场景图的方法，将推理过程分解为模块化步骤，应用自适应球面图像变换，并将信息整合到统一的图表示中进行答案生成。

Result: 实验表明：1. 现有MLLM在360°图像感知方面存在明显不足；2. Free360框架能持续提升基础MLLM性能；3. Free360为360° VQA任务提供了强大的训练免费解决方案。

Conclusion: 该研究填补了MLLM在360°图像感知方面的评估空白，提出的360Bench基准和Free360框架为相关研究提供了重要工具和解决方案，源代码和数据集将在接受后公开。
...
TL;DR: KidsNanny是一个两阶段多模态内容审核架构，用于儿童安全保护。第一阶段使用视觉Transformer和物体检测器进行视觉筛查（11.7ms），第二阶段结合OCR和7B语言模型进行文本上下文推理（总延迟120ms）。在UnsafeBench数据集上表现优于现有基线模型。


<details>
  <summary>Details</summary>
Motivation: 开发高效的多模态内容审核系统，专门针对儿童安全保护，解决现有方法在处理文本嵌入威胁时的局限性，同时降低延迟提高效率。

Method: 采用两阶段架构：第一阶段使用视觉Transformer结合物体检测器进行快速视觉筛查；第二阶段将视觉输出转换为文本，结合OCR和7B语言模型进行上下文推理，避免传输原始像素数据。

Result: 第一阶段准确率80.27%，F1分数85.39%，延迟仅11.7ms；完整管道准确率81.40%，F1分数86.16%，延迟120ms。在文本依赖图像上表现优异，达到100%召回率，优于ShieldGemma-2和LlavaGuard等基线模型。

Conclusion: 基于OCR的专用推理架构在处理文本嵌入威胁方面具有召回率和精度的优势，同时保持较低延迟。该架构和方法为高效多模态儿童安全内容审核研究提供了有价值的贡献。
...
TL;DR: ECHO是一个边缘-云框架，通过语言指令控制人形机器人全身运动。云端扩散模型生成运动参考，边缘强化学习跟踪器在机器人上闭环执行，无需推理时重定向，支持零硬件微调。


<details>
  <summary>Details</summary>
Motivation: 实现自然语言到人形机器人全身运动的直接控制，消除传统方法中从人体模型到机器人重定向的复杂过程，提高运动生成效率和执行安全性。

Method: 采用边缘-云架构：云端使用基于扩散的文本到运动生成器（1D卷积UNet+CLIP文本特征），边缘部署强化学习跟踪器（教师-学生范式+证据适应模块）。使用紧凑的38维机器人原生运动表示，包含自动跌倒恢复机制。

Result: 在重定向的HumanML3D基准测试中表现优异（FID 0.029，R-Precision Top-1 0.686），在Unitree G1人形机器人上成功执行多样文本指令，保持高运动安全性和轨迹一致性，无需硬件微调。

Conclusion: ECHO框架实现了高效、安全的语言驱动人形机器人全身控制，通过边缘-云协同和机器人原生表示，在模拟和真实环境中均表现出色，为零硬件微调的机器人控制提供了可行方案。
...
TL;DR: CTRL-S是一个用于SVG生成的统一框架，通过思维链机制显式展示推理过程，使用多奖励优化提升生成质量
...
Method: 1. 构建SVG-Sophia高质量数据集（14.5万样本）；2. 引入思维链机制显式展示推理过程；3. 采用GRPO算法和多奖励优化框架（DINO、图像-文本相似度、格式、代码效率奖励）；4. 训练模型生成组级结构化SVG代码
...
<details>
  <summary>Details</summary>
Motivation: 当前视频动作模型要么依赖缓慢的多步视频生成，要么使用噪声的单步特征提取，无法同时实现实时推理和高保真预测，限制了其在机器人学习中的应用。
...
Conclusion: S-VAM通过创新的自蒸馏策略实现了实时推理和高保真预测的统一，为机器人学习中的视频动作模型提供了更实用的解决方案。
...
TL;DR: Leveling3D提出了一种结合前馈3D重建与几何一致生成的新方法，通过几何感知适配器对齐扩散模型与3D先验知识，改善外推视图中的缺失区域，提升3D重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在修复3D渲染结果时缺乏几何考虑，无法有效填充外推视图中的缺失区域，限制了3D重建的质量提升。

Method: 提出几何感知的leveling适配器，轻量级对齐扩散模型内部知识与前馈模型的几何先验；引入调色板过滤策略增强生成多样性，以及测试时掩码细化防止修复区域边界混乱。

Result: 在公开数据集上实现了SOTA性能，包括新视角合成和深度估计任务，并能通过增强的外推视图进一步提升3D高斯溅射的重建质量。

Conclusion: Leveling3D成功整合了前馈3D重建与几何一致生成，通过几何感知适配器实现了同时重建与生成，为3D视觉任务提供了更强大的基线。
...
TL;DR: PureCLIP-Depth：首个完全无需提示词和解码器的单目深度估计模型，直接在CLIP嵌入空间中学习从RGB到深度的映射


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计模型过度依赖几何特征，本文探索基于概念信息的新方法，直接在CLIP概念空间中进行深度估计

Method: 提出完全无提示词、无解码器的架构，在CLIP嵌入空间中学习从RGB域到深度域的直接映射，不依赖几何特征而利用概念信息

Result: 在室内外数据集上达到基于CLIP嵌入模型的最先进性能，代码已开源

Conclusion: PureCLIP-Depth展示了在CLIP概念空间中直接进行深度估计的可行性，为单目深度估计提供了新的概念驱动方法
...
<details>
  <summary>Details</summary>
Motivation: 半监督人群分析很重要，因为未标记数据丰富且便宜。传统基于点的标注限制了性能，因为个体区域本身具有模糊性，从稀疏标注中学习细粒度结构语义仍然是一个未解决的挑战。

Method: 首先提出基于最近邻排除圆约束的EDP-SAM来生成掩码监督；然后提出XMask通过判别性掩码目标强制空间分离；使用高斯平滑和可微分中心采样策略改善特征连续性和训练稳定性；基于XMask构建半监督人群计数框架，使用实例掩码先验作为伪标签。

Result: 在ShanghaiTech A、UCF-QNRF和JHU++数据集上（使用5%、10%和40%标记数据）的广泛实验验证，端到端模型实现了最先进的半监督分割和计数性能。

Conclusion: 该方法有效弥合了计数和实例分割之间的差距，在统一框架内实现了优越的半监督人群分析性能。
...
TL;DR: DiVA-Former是一个轻量级架构，通过视觉token作为动态查询来蒸馏长文本序列，有效整合视觉和文本信息，在13个表格基准测试中比纯文本基线提升23.9%。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个问题：1）LLMs将2D表格线性化为1D序列会削弱行列邻接和布局线索；2）纯视觉编码器能捕捉空间线索但难以保留精确单元格文本。分析发现这两种模态为LLMs提供高度不同的信息且具有强互补性，但直接拼接等融合方法收益有限且常引入跨模态干扰。

Method: 提出DiVA-Former轻量级架构，利用视觉token作为动态查询来蒸馏长文本序列为可消化向量，从而有效利用视觉-文本互补信息。

Result: 在13个表格基准测试中，DiVA-Former比纯文本基线提升23.9%，并在使用视觉输入、文本输入或两者组合的现有基线上实现一致增益。

Conclusion: DiVA-Former通过有效整合视觉和文本信息，解决了表格理解中模态融合的挑战，在多个基准测试中显著优于现有方法。
...
TL;DR: 提出混合框架解决白细胞分类中的类别不平衡问题，集成生成式修复、Transformer对比学习和生物启发式约束，在WBCBench 2026挑战中取得0.77139的Macro-F1分数


<details>
  <summary>Details</summary>
Motivation: 白细胞自动分类对白血病筛查至关重要，但面临极端类别不平衡、长尾分布和域偏移问题，导致深度学习模型过拟合主导类别，在罕见亚型上表现不佳
...
Conclusion: 该方法在严重类别不平衡条件下表现出色，证明了将生物先验知识融入深度学习对血液学图像分析的价值
...
Conclusion: SEVEX框架通过自动化、任务特定的视觉提示探索，为增强LVLM感知提供了新范式，能够发现传统方法难以获得的复杂视觉策略。
...
TL;DR: EVPV通过显式视觉前提验证来解耦感知不确定性和逻辑评估，提高视觉语言过程奖励模型的准确性


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言过程奖励模型(VL-PRMs)作为黑盒评判器存在感知与推理的纠缠问题，导致系统性误判（奖励幻觉视觉前提）和误罚（惩罚正确接地陈述），影响重排序和错误定位的准确性

Method: 提出显式视觉前提验证(EVPV)框架：1)策略生成步骤级视觉检查清单，明确所需视觉事实；2)约束提取器从输入图像独立推导结构化视觉约束；3)将检查清单声明与约束匹配计算视觉可靠性信号；4)通过可靠性门控校准PRM步骤奖励，视觉依赖步骤在可靠性低时奖励衰减，可靠性高时保留

Result: 在VisualProcessBench和六个多模态推理基准测试中，EVPV改进了步骤级验证，并持续提升了Best-of-N重排序准确性；通过向提取约束注入受控损坏产生单调性能下降，提供了因果证据表明增益源于约束保真度和显式前提验证而非偶然提示效应

Conclusion: EVPV通过显式视觉前提验证有效解耦了感知不确定性和逻辑评估，无需每步工具调用即可提高视觉语言过程奖励模型的验证准确性，为多模态推理系统提供了更可靠的评估框架
...
TL;DR: AW-MoE框架通过集成混合专家系统，结合图像引导的天气感知路由和统一双模态增强，显著提升了恶劣天气下的3D目标检测性能，相比现有方法性能提升约15%
...
Method: 提出AW-MoE框架：1）图像引导的天气感知路由（IWR），利用图像特征在不同天气条件下的判别能力和场景不变性进行精确天气分类；2）基于分类结果选择最相关的天气特定专家（WSE）处理数据差异；3）统一双模态增强（UDMA）实现LiDAR和4D雷达数据的同步增强同时保持场景真实性

Result: 在真实世界数据集上的实验表明，AW-MoE在恶劣天气下的性能比最先进方法提升约15%，推理开销可忽略不计；集成到现有基准检测器中也能获得超越当前最先进方法的性能提升

Conclusion: AW-MoE框架通过有效处理不同天气条件下的数据分布差异，显著提升了恶劣天气下的3D目标检测性能，具有良好的有效性和强扩展性
...
TL;DR: 提出FG-SGL框架，通过细粒度和类别级语义联合指导视觉-语言模型感知局部微手势运动，解决微手势识别中类间差异细微的挑战。
...
Method: 提出细粒度语义指导学习框架，包含FG-SA模块（细粒度语义指导局部运动特征学习）和CP-A模块（类别级语义增强特征可分性），构建细粒度文本数据集描述微手势动态过程，设计多级对比优化策略进行粗到细的联合优化。

Result: 实验表明FG-SGL取得了有竞争力的性能，验证了细粒度语义指导对微手势识别的有效性。

Conclusion: 细粒度语义指导能有效提升微手势识别性能，联合细粒度和类别级语义的框架能更好地捕捉局部运动差异。
...
Method: 1) 提出基于几何的奖励模型，利用预训练几何基础模型通过跨帧重投影误差评估多视角一致性；2) 采用点对点方式进行误差计算，获得更物理基础和鲁棒的误差度量；3) 引入几何感知采样策略，过滤低纹理和非语义区域，专注于具有可靠对应关系的几何有意义区域；4) 通过两种互补途径应用奖励模型：通过SFT或强化学习对双向模型进行后训练，以及通过测试时缩放对因果视频模型进行推理时优化。

Result: 实验结果表明，该方法设计有效，基于几何的奖励相比其他变体提供更优越的鲁棒性。通过实现高效的推理时缩放，该方法为增强开源视频模型提供了实用解决方案，无需大量计算资源进行重新训练。

Conclusion: 提出的几何奖励模型和几何感知采样策略能够有效解决视频扩散模型的几何不一致问题，通过点对点误差计算和推理时优化，为视频生成提供了更物理基础和鲁棒的几何监督方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有特征引导方法在所有层均匀应用，忽略了层间差异，可能干扰与幻觉无关的层，导致通用任务性能下降
...
Conclusion: 通过定位幻觉相关层并稀疏化特征引导，LTS-FS框架为减少LVLM幻觉提供了更精确有效的方法，平衡了幻觉缓解和通用性能保持
...
Method: 1. 提出"All-in-One-World Character Integrator"，在统一LoRA模块中实现连续角色适应，无需每个角色的优化模块；2. 通过MLLM-as-Judge建立"Character Quality Gate"，使用链式思维推理确保角色适应过程的保真度；3. 引入"Character-Aware Region-Focus Sampling"策略，解决多角色故事可视化中的身份退化和布局冲突问题。

Result: 实验结果表明，EverTale在单角色和多角色故事可视化任务上，相比更广泛的方法范围都取得了优越的性能表现。

Conclusion: EverTale通过创新的角色集成器、质量门控和采样策略，成功解决了故事可视化中连续角色定制的挑战，实现了准确的角色定制、语义对齐和新身份集成。
...
Method: 提出Micro-AU CLIP框架，将AU检测过程分解为局部语义独立性建模和全局语义依赖性建模。在LSI中设计Patch Token Attention，将AU区域内的多个局部特征映射到同一特征空间；在GSD中提出Global Dependency Attention和Global Dependency Loss来建模不同AU之间的全局依赖关系。此外，针对CLIP在微语义对齐上的局限性，设计了microAU对比损失来通过视觉和文本特征的细粒度对齐学习AU特征。

Result: 实验结果表明，Micro-AU CLIP能够充分学习细粒度的微表情动作单元特征，实现了最先进的性能，并成功应用于无情感标签的微表情识别。

Conclusion: 该研究探索了独立性到依赖性模式的有效性，提出的Micro-AU CLIP框架通过局部独立性建模和全局依赖性建模相结合的方式，有效解决了微表情动作单元检测中的特征学习问题，为细粒度的真实情感分析提供了客观线索。
...
TL;DR: DriveFix是一个用于自动驾驶场景的多视角修复框架，通过扩散变换器架构实现时空一致性，解决了现有4D场景重建方法中跨相机空间错位和时间漂移的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散先验的4D场景重建方法通常独立处理帧或逐视角处理，缺乏时空协同，导致跨相机空间错位和时间序列漂移问题，需要一种能确保驾驶场景时空一致性的方法。

Method: 提出DriveFix多视角修复框架，采用交错扩散变换器架构，包含专门模块显式建模时间依赖性和跨相机空间一致性。通过历史上下文条件生成和几何感知训练损失，确保修复视角符合统一的3D几何结构。

Result: 在Waymo、nuScenes和PandaSet数据集上的广泛评估表明，DriveFix在重建和新视角合成方面达到最先进性能，显著减少伪影，实现高保真纹理的一致传播。

Conclusion: DriveFix通过确保驾驶场景的时空一致性，在4D世界建模方面取得重要进展，为实现真实世界部署的鲁棒4D场景重建迈出重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 传统肺癌治疗方法（手术、化疗、放疗）因癌症异质性而效果有限，需要基于患者个体遗传信息的个性化治疗方案。深度学习可以分析大数据发现癌症早期迹象和肿瘤类型，但需要更精准的个性化预测模型。

Method: 使用癌症药物敏感性基因组学(GDSC)的多组学数据，构建XGBoost回归模型预测药物反应(LN-IC50)。通过交叉验证和随机搜索进行超参数调优，使用SHAP值解释特征重要性，并利用DeepSeek大语言模型验证特征的生物学有效性。

Result: 开发了一个能够预测肺癌药物敏感性的个性化治疗模型，通过SHAP分析识别了关键分子和细胞特征，DeepSeek验证了这些特征的生物学合理性，提供了基因和通路层面的解释。

Conclusion: 该研究成功整合了机器学习、可解释AI和大语言模型，为肺癌个性化治疗提供了有效工具，能够基于患者遗传特征预测药物反应，支持精准医疗决策。
...
TL;DR: Iris是一个确定性单目深度估计框架，通过将真实世界先验整合到扩散模型中，解决了传统前馈方法细节缺失和现有扩散方法合成到真实域迁移困难的问题。


<details>
  <summary>Details</summary>
Motivation: 传统前馈方法依赖大量训练数据但仍缺失细节，现有扩散方法虽利用生成先验但难以从合成域迁移到真实场景。需要一种既能保留细节、又能实现强泛化且数据高效的解决方案。

Method: 提出两阶段先验到几何确定性调度：先验阶段使用谱门控蒸馏转移低频真实先验，几何阶段使用谱门控一致性保证高频保真度并用合成真值精炼。两阶段共享权重，采用高到低时间步调度。

Result: 实验结果表明Iris在单目深度估计性能上取得显著提升，具有强大的野外场景泛化能力。

Conclusion: Iris框架通过整合真实世界先验到扩散模型中，实现了细节保留、强泛化和数据高效的单目深度估计，为解决合成到真实域迁移问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中基于LiDAR点云的人体姿态估计面临两大挑战：1) 人-物交互导致空间模糊性，交互区域容易产生错误的关键点预测；2) 交互频繁的身体部位（如手、脚）在LiDAR数据中观测稀疏，存在严重的类别不平衡问题

Method: 提出HOIL框架，包含三个核心组件：1) HOICL（人-物交互感知对比学习）增强人与物体点之间的特征区分度；2) CPPool（接触感知部分引导池化）自适应重新分配表示能力，压缩过表示点同时保留交互身体部位的信息点；3) 可选的基于接触的时序细化，利用时间上的接触线索细化每帧关键点估计

Result: HOIL框架有效利用人-物交互解决了交互区域的空间模糊性和类别不平衡问题，代码将开源

Conclusion: 通过人-物交互学习，能够构建更鲁棒的3D人体姿态估计框架，解决现有方法忽视人-物交互潜力的问题，为自动驾驶中的人体理解提供更可靠的解决方案
...
TL;DR: 本研究提出了一种基于深度学习的框架，使用YOLO架构结合高分辨率类激活映射（HiResCAM）来自动识别姬蜂总科寄生蜂，实现了超过96%的准确率，并通过可视化验证了模型关注分类相关的解剖学特征。
...
Method: 提出基于YOLO的深度学习框架，集成高分辨率类激活映射（HiResCAM）增强可解释性，使用包含3556张高分辨率膜翅目标本图像的数据集，主要包含姬蜂科、茧蜂科、蜜蜂科和胡蜂科等家族。

Result: 实验结果显示模型准确率超过96%，具有强大的泛化能力，HiResCAM可视化证实模型关注翅膀脉序、触角分节和腹部结构等分类相关解剖区域，验证了学习特征的生物学合理性。

Conclusion: 集成可解释AI技术提高了系统的透明度和可信度，使其适用于昆虫学研究，加速了这个描述不足的寄生蜂总科的生物多样性表征工作。
...
TL;DR: 提出一种实时水下图像增强框架，通过自适应加权通道补偿、多分支重参数化空洞卷积和统计全局颜色调整三个模块，在保持轻量化的同时实现高质量颜色恢复，仅需3,880个推理参数，达到409 FPS的推理速度。


<details>
  <summary>Details</summary>
Motivation: 水下图像增强对水下平台视觉信息获取至关重要，但现有高性能方法通常架构复杂难以部署，而轻量化方法在严重退化水下图像处理上效果不佳，需要在保持实时性的同时提升增强质量。

Method: 1. 自适应加权通道补偿模块：以绿色通道为参考锚点，动态恢复红色和蓝色通道颜色；2. 多分支重参数化空洞卷积：训练时采用多分支融合，推理时进行结构重参数化，实现大感受野表示且计算开销低；3. 统计全局颜色调整模块：基于统计先验优化整体颜色表现。

Result: 在八个数据集上的实验表明，该方法在七个评估指标上达到最先进性能，仅含3,880个推理参数，推理速度达409 FPS，UCIQE分数在不同环境条件下提升29.7%，在ROV平台部署和下游任务性能增益验证了其实时水下任务优越性。

Conclusion: 提出的实时水下图像增强框架在保持轻量化的同时实现了高质量颜色恢复，平衡了性能与效率，适用于实时水下任务部署。
...
TL;DR: InViC是一个轻量级插件框架，通过提取问题相关的视觉提示令牌来增强医学视觉问答中的图像证据利用，减少模型依赖语言先验的捷径回答。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在医学视觉问答中常表现出"捷径回答"问题，即过度依赖语言先验或数据集偏差，而忽视关键的视觉证据，这影响了临床可靠性，特别是在细微影像发现起决定性作用时。

Method: 提出Intent-aware Visual Cues (InViC)框架：1) Cue Tokens Extraction模块从密集视觉令牌中提取K个问题相关的紧凑提示令牌；2) 两阶段微调策略：第一阶段使用提示瓶颈注意力掩码阻断LLM直接访问原始视觉特征，强制所有视觉证据通过提示路径；第二阶段恢复标准因果注意力，训练LLM联合利用视觉和提示令牌。

Result: 在三个公开Med-VQA基准测试(VQA-RAD、SLAKE和ImageCLEF VQA-Med 2019)上评估，InViC在零样本推理和标准LoRA微调基础上均有持续改进，证明意图感知视觉提示与瓶颈训练是提高可信赖医学视觉问答的有效策略。

Conclusion: InViC通过引入问题相关的视觉提示令牌和瓶颈训练策略，有效增强了医学视觉问答中基于图像的答案生成，减少了模型对语言先验的依赖，提高了临床可靠性。
...
TL;DR: SemTok是一种语义一维标记化方法，将2D图像压缩为具有高级语义的1D离散标记，在图像重建和下游生成任务中取得优异效果。


<details>
  <summary>Details</summary>
Motivation: 现有视觉标记化方法主要将图像映射到固定2D空间网格，专注于像素级恢复，这阻碍了捕捉具有紧凑全局语义的表示。需要一种能够提取高级语义的标记化方法。

Method: 提出SemTok语义一维标记化器，包含三个关键创新：2D到1D标记化方案、语义对齐约束和两阶段生成训练策略。基于此构建了掩码自回归生成框架。

Result: SemTok在图像重建方面达到新的最先进水平，以极其紧凑的标记表示实现卓越保真度。在图像生成等下游任务中表现出显著改进。

Conclusion: SemTok通过语义一维标记化有效解决了现有方法的局限性，证明了语义1D标记化的有效性，为下游任务提供了更好的基础。
...
TL;DR: DermaFlux是一个基于rectified flow的文本到图像生成框架，专门用于合成临床基础的皮肤病变图像，通过自然语言描述生成多样化的皮肤病图像，显著提升皮肤病变分类性能。


<details>
  <summary>Details</summary>
Motivation: 皮肤病变分类系统受限于大型、多样化且标注良好的临床数据集的可用性不足，导致良性和恶性病变之间的类别不平衡，从而降低了模型的泛化性能。

Method: 基于Flux.1构建DermaFlux框架，使用参数高效的LoRA方法在大型公开临床图像数据集上进行微调。利用Llama 3.2生成包含皮肤病学标准（如不对称性、边界不规则性、颜色变化）的合成文本描述，构建图像-文本对。

Result: DermaFlux生成多样化且具有临床意义的皮肤病图像，在小规模真实数据集上增强时可将二元分类性能提升高达6%；当分类器在DermaFlux生成的合成图像而非基于扩散的合成图像上训练时，性能提升高达9%。仅使用2,500张真实图像和4,375个DermaFlux生成样本的ViT模型达到78.04%的二元分类准确率和0.859的AUC，超过次优皮肤病模型8%。

Conclusion: DermaFlux框架能够有效解决皮肤病变数据集稀缺和类别不平衡问题，通过生成临床基础的合成图像显著提升分类性能，为皮肤病诊断提供了有效的数据增强解决方案。
...
TL;DR: 提出了一种利用对称光源布置的近光光度立体线性求解方法，无需初始化即可获得表面法线和深度的闭式解


<details>
  <summary>Details</summary>
Motivation: 传统近光光度立体方法需要非凸优化和深度初始化，计算复杂且对光源校准要求高，需要更简单有效的解决方案

Method: 通过布置多组对称的近光源对，利用对称光源分布特性，推导出表面法线和深度的闭式解，无需初始化，对光源空间偏移的校准要求低

Result: 实验显示该方法在形状恢复精度上与最先进的校准近光光度立体方法相当，同时显著减少了对深度初始化和光源校准的要求

Conclusion: 提出的对称光源布置方法为近光光度立体提供了一种简单有效的线性解决方案，降低了实现门槛，提高了实用性
...
TL;DR: HGP-Mamba：一种基于Mamba的多模态框架，通过从全切片图像生成蛋白质特征来整合组织学和蛋白质信息，用于癌症生存风险预测，在四个公开数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 蛋白质标记物和组织病理学图像的联合预后潜力尚未得到充分探索，主要原因是蛋白质表达谱分析成本高且可用性有限。需要一种能够有效整合这两种模态信息的方法来改善癌症生存风险预测。
...
Conclusion: HGP-Mamba通过从组织学图像生成蛋白质特征，有效整合了多模态信息，为癌症生存风险预测提供了一种高效且性能优越的解决方案，克服了蛋白质表达谱分析的高成本和有限可用性问题。
...
TL;DR: SF-Mamba是一种新型视觉Mamba模型，通过辅助补丁交换和批量折叠技术，解决了传统Mamba在视觉任务中双向信息流不足和计算效率低的问题，在多个视觉任务上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前视觉Mamba模型存在两个主要问题：1）循环扫描机制限制了图像补丁之间的非因果交互，现有多扫描策略效率低下；2）在视觉任务常用的短令牌长度下，Mamba计算速度较慢。需要设计真正高效的视觉编码器。

Method: 提出SF-Mamba，包含两个关键技术：1）辅助补丁交换：在单向扫描下编码双向信息流；2）批量折叠与周期性状态重置：提高GPU并行性。这些设计优化了扫描操作和计算效率。

Result: 在图像分类、目标检测、实例分割和语义分割等任务上的广泛实验表明，SF-Mamba在不同模型规模下均显著优于现有基线方法，同时提高了吞吐量。

Conclusion: SF-Mamba通过重新思考视觉扫描操作和Mamba计算效率，提出了一种高效的视觉编码器，在保持计算效率的同时实现了更好的性能，为视觉Mamba模型的发展提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有高光谱图像分类方法存在两个主要问题：基于Transformer的方法由于自注意力机制二次复杂度导致可扩展性差，而基于傅里叶变换的方法通常依赖2D空间FFT，忽略了高光谱数据固有的关键波段间光谱依赖关系。

Method: 提出混合GFNet（HGFNet）架构，整合局部3D卷积特征提取和基于GFNet风格块的频域全局滤波。引入三种针对高光谱图像定制的频率变换：光谱傅里叶变换（沿光谱轴的1D FFT）、空间傅里叶变换（空间维度的2D FFT）和空间-空间傅里叶变换（光谱和空间维度的3D FFT）。此外还采用自适应焦点损失来缓解类别不平衡问题。

Result: 该方法能够实现全面和高维的频率建模，3D卷积层捕获细粒度局部空间-光谱结构，基于傅里叶的全局滤波模块高效建模长程依赖并抑制噪声，自适应焦点损失提高了对少数类别的识别能力。

Conclusion: HGFNet通过整合局部卷积特征提取和频域全局滤波，克服了现有方法在可扩展性和光谱依赖性建模方面的局限性，为高光谱图像分类提供了高效且鲁棒的空间-光谱表示学习解决方案。
...
TL;DR: 提出基于自监督跨模态协调的浮游生物识别方法，利用图像和光学测量数据，无需大量人工标注


<details>
  <summary>Details</summary>
Motivation: 现有浮游生物识别方法主要依赖监督学习，需要大量人工标注；同时现代成像仪器采集的光学测量数据（如散射和荧光剖面）未被充分利用。希望通过自监督方式利用多模态数据解决标注成本高的问题

Method: 受CLIP启发，训练图像和剖面数据的编码器，仅使用二元监督信息（判断图像和剖面是否来自同一颗粒）。识别时结合少量标注样本库和k-NN分类器，构建多模态识别模型

Result: 该方法在仅需少量标注图像的情况下实现高识别准确率，且优于仅使用图像的自监督基线方法

Conclusion: 自监督跨模态协调策略能有效利用多模态浮游生物数据，减少对人工标注的依赖，为大规模浮游生物识别提供可行方案
...
TL;DR: IRIS是一个用于无监督物理参数估计的高保真基准测试，包含220个4K分辨率、60fps的真实世界视频，涵盖单体和多体动力学系统，提供独立测量的真实参数和不确定性估计。
...
Method: 提出IRIS基准测试，包含220个在受控实验室条件下录制的真实世界视频，每个系统都配有控制方程。定义了标准化的评估协议，涵盖参数准确性、可识别性、外推性、鲁棒性和控制方程选择。

Result: 评估了多个基线方法，包括多步物理损失公式和四种互补的方程识别策略（VLM时间推理、描述-分类提示、CNN分类和路径标记），建立了所有IRIS场景的参考性能，并揭示了系统性的失败模式。

Conclusion: IRIS填补了无监督物理参数估计领域基准测试的空白，提供了高质量的真实世界数据集、标准化评估协议和基线实现，为未来研究提供了基础并揭示了需要改进的方向。
...
TL;DR: 本文提出了一种统一的雨滴和反射去除方法DiffUR³，并构建了RDRF真实数据集来解决雨天透过玻璃拍摄时同时出现的雨滴和反射问题。


<details>
  <summary>Details</summary>
Motivation: 雨天透过玻璃或挡风玻璃拍摄时，雨滴和反射经常同时出现，显著降低图像可见度。这是一个缺乏关注但急需解决的实际问题，现有的去雨滴、去反射或一体化模型都无法处理这种复合退化。

Method: 首先正式定义了统一的雨滴和反射去除任务，并构建了真实拍摄的RDRF数据集。然后提出了基于扩散模型的DiffUR³框架，利用强大的生成先验来同时去除两种退化。

Result: 在构建的基准数据集和具有挑战性的真实场景图像上，DiffUR³方法都实现了最先进的性能表现。

Conclusion: 本文首次定义了统一的雨滴和反射去除任务，提出了有效的扩散模型解决方案，并提供了高质量的数据集基准，为解决这一实际问题提供了新的工具和标准。
...
Method: 1) 多视角图像对齐：通过多尺度和多方向视角增强细粒度匹配；2) 双向对比学习：生成"困难查询"并建立视觉和文本消歧的互补学习路径以重新平衡监督；3) LLM元控制器：将模型状态摘要输入LLM，利用专家知识自适应调整训练课程以促进模型演化。
...
TL;DR: GAP-MLLM提出了一种几何对齐预训练范式，通过视觉提示联合任务和多级渐进融合模块，显式激活多模态大语言模型的几何感知能力，显著提升3D空间感知性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在纯RGB输入下3D空间感知能力不足，主要原因是文本主导的微调范式无法有效激活模型中的几何表示，现有方法采用简单的特征拼接且缺乏几何特定监督，导致结构利用不充分。

Method: 提出GAP-MLLM几何对齐预训练范式：1）引入视觉提示联合任务，强制模型同时预测稀疏点云图和语义标签，显式激活几何感知；2）设计多级渐进融合模块，采用令牌级门控机制，自适应整合几何先验而不抑制语义推理。

Result: 实验表明GAP-MLLM显著增强了几何特征融合能力，在3D视觉定位、3D密集描述和3D视频目标检测等任务上均取得一致性能提升。

Conclusion: 通过几何对齐预训练范式显式激活多模态大语言模型的几何感知能力，能够有效弥补基于图像的方法与使用显式3D数据方法之间的性能差距，为提升3D空间理解提供了新思路。
...
TL;DR: 提出UOT-Unlearn框架，用于一步生成模型的类别遗忘，通过非平衡最优传输实现目标类别抑制与生成质量保持的平衡


<details>
  <summary>Details</summary>
Motivation: 一步生成框架（如流映射模型）显著提高了图像生成效率，但这些强大生成器的机器遗忘问题尚未探索。现有的扩散遗忘方法依赖于多步迭代去噪过程，与一步模型不兼容

Method: 基于非平衡最优传输（UOT）的即插即用类别遗忘框架，将遗忘问题形式化为遗忘成本（抑制目标类别）与f-散度惩罚（通过松弛边际约束保持整体生成保真度）之间的权衡
...
TL;DR: VIEW2SPACE是一个用于稀疏多视角推理的多维基准测试，通过物理仿真构建高质量3D场景，评估发现现有模型在多视角推理上表现接近随机猜测，提出的Grounded Chain-of-Thought方法能显著提升性能并泛化到真实数据。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注单图像或密集视频场景，而真实世界需要从稀疏离散视角进行多视角推理，但缺乏大规模高质量标注数据。需要构建可扩展的数据集来评估和改进多视角推理能力。

Method: 利用物理仿真构建多样化高保真3D场景，创建VIEW2SPACE基准测试，包含数百万个有依据的问答对。提出Grounded Chain-of-Thought with Visual Evidence方法，并进行难度感知的扩展分析（模型大小、数据规模、推理深度、可见性约束）。

Result: 现有最先进的视觉语言和空间模型在多视角推理上表现接近随机猜测。提出的Grounded Chain-of-Thought方法在中等难度下显著提升性能，并能泛化到真实数据，在跨数据集评估中优于现有方法。几何感知在足够可见性下可通过扩展受益，但跨稀疏视图的深度组合推理仍是根本挑战。

Conclusion: 多视角推理仍是一个未解决的重要问题。通过物理仿真可构建可扩展的高质量数据集，提出的Grounded Chain-of-Thought方法能有效提升性能，但跨稀疏视图的深度组合推理需要进一步研究突破。
...
<details>
  <summary>Details</summary>
Motivation: 爆轰单元的分割和测量一直是爆轰研究中的长期挑战，现有方法主要依赖手动或原始的2D边缘检测，存在精度和效率问题，需要更精确的自动化解决方案。

Method: 开发了一种基于图论的无训练算法，使用分割模型从3D压力迹线中提取细胞模式，通过图论方法处理复杂的爆轰晶格结构。

Result: 在生成数据上分割效果良好，预测误差仅2%；在3D模拟数据中，统计和联合概率密度结果显示沿波传播轴排列的椭圆形细胞，偏差为17%；体积的较大离散反映了线性变异的三次放大。

Conclusion: 该图论框架对多种细胞几何结构具有良好泛化能力，是实用的爆轰分析工具，为未来三重点碰撞研究奠定了坚实基础，但高度复杂细胞模式的分割和量化仍具挑战性。
...
TL;DR: 提出了一种结合蒙特卡洛姿态采样和基于费舍尔信息的PnP优化的重定位框架，解决3D高斯泼溅在姿态优化中对初始姿态和重建几何敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)虽然能提供高质量的可微分渲染，但其姿态优化的鲁棒性对初始相机姿态和重建几何高度敏感。研究发现存在两个主要不确定性来源：(1)姿态先验不确定性，通常来自回归或检索模型输出的单一确定性估计；(2)几何不确定性，由3DGS重建中的缺陷引起，会传播到PnP求解器中。
...
Conclusion: 通过显式处理姿态和几何不确定性，提出的框架有效解决了3DGS姿态优化的敏感性问题，提高了重定位的鲁棒性和准确性。
...
TL;DR: 使用变分自编码器（VAE）和期望最大化（EM）方法解决扫描透射电子显微镜（STEM）的校准问题，通过模拟数据学习图像的低维表示，实现比现有方法更快、更一致的校准效果。


<details>
  <summary>Details</summary>
Motivation: 电子显微镜校准面临挑战：基于图像的参数调整需要克服光学像差，但诊断图像高维且噪声大，单张图像无法确定最优参数。现有方法通常只提取标量值，无法有效处理模拟数据与现实数据的差距。

Method: 1. 使用变分自编码器（VAE）在模拟数据上训练，学习STEM图像的低维表示；2. 采用期望最大化（EM）方法联合估计校准参数到编码表示的映射模型及最优校准参数；3. 利用光学系统的对称性确保联合估计问题的全局可识别性。

Result: 在真实STEM上验证，该方法比现有方法显著更快、更一致，估计误差减少2倍，且需要更少的观测数据。实现了STEM自动校准的显著进步。

Conclusion: VAE-EM框架不仅推进了STEM自动校准，展示了VAE在图像信息压缩中的潜力，还适用于存在模拟训练数据与现实差距、非单射映射会阻碍唯一解的其他逆问题。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现良好，但物体幻觉严重损害其可靠性。现有研究多关注文本模态，将幻觉归因于过强的语言先验和不足的视觉基础。作者发现视觉模态内部的异常注意力模式也会导致幻觉物体。

Method: 提出基于分割的注意力熵（SAE），利用语义分割在对象级语义空间中量化视觉注意力不确定性。基于SAE设计了幻觉检测的可靠性评分和SAE引导的注意力调整方法，在推理时修改视觉注意力以缓解幻觉。

Result: 在公共基准测试和四足机器人的真实具身多模态场景中评估，SAE显著减少了物体幻觉，无需额外训练成本，使LVLM驱动的感知和决策更可信。

Conclusion: SAE方法通过量化视觉注意力不确定性，有效检测和缓解大型视觉语言模型中的物体幻觉问题，提高了模型的可靠性，为可信的多模态感知和决策提供了新思路。
...
TL;DR: 基于Transformer的深度学习模型，直接从原始活细胞视频序列预测癌细胞在化疗下的命运，无需预定义特征，准确率达0.94，并提供了可解释性框架分析预测依据。


<details>
  <summary>Details</summary>
Motivation: 研究非遗传因素对细胞命运的影响对癌症治疗至关重要，因为基因相同的细胞在相同治疗条件下可能产生不同结果。需要直接从细胞行为观察中预测命运并理解其决定因素。

Method: 使用Transformer深度学习模型，直接从原始长期活细胞记录视频序列预测细胞命运，无需依赖预定义的形态或分子特征。引入全面的可解释性框架，包括注意力机制和掩码实验来分析预测依据。

Result: 模型在细胞命运预测上达到平衡准确率0.94和F1分数0.93。注意力实验表明预测信号不仅存在于细胞轨迹的最后帧，可靠预测可在事件发生前10小时完成。分析揭示了有丝分裂和凋亡序列中预测信息的独特时间分布，以及细胞形态和p53信号在决定细胞结果中的作用。

Conclusion: 基于注意力的时间模型能够准确预测细胞命运，同时提供对细胞决策中非遗传决定因素的生物学可解释性见解。该方法为理解细胞命运决定机制和治疗反应提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D内容创作中物理材质生成的挑战，传统方法需要专业知识和繁琐的手工调整，需要一种能够根据几何形状和文本描述自动生成高质量物理材质的方法。

Method: 采用视频扩散Transformer架构，输入几何形状和文本描述作为条件，联合建模多种材质属性（基础颜色、粗糙度、金属性、高度图）。引入定制变分自编码器，将多种材质模态编码到紧凑的潜在空间，实现多模态联合生成而不增加token数量。

Result: 能够根据文本提示为3D形状生成高质量的物理材质，生成结果与常见内容创作工具兼容，实现了多材质属性的联合生成。

Conclusion: 提出了一种基于扩散Transformer的3D材质生成框架，通过创新的多模态编码方法，实现了从文本到物理材质的有效生成，为3D内容创作提供了自动化工具。
...
TL;DR: Face2Scene：利用人脸作为感知引导，从人脸恢复中提取退化信息，指导整个场景（包括身体和背景）恢复的两阶段框架


<details>
  <summary>Details</summary>
Motivation: 现有基于参考的人脸恢复方法只关注面部区域，忽略了身体和背景的退化；而全场景恢复方法往往忽略退化线索，导致预测不确定和视觉伪影。需要一种既能利用人脸恢复优势，又能扩展到全场景的方法。

Method: 提出两阶段框架：1）使用Ref-FR模型从退化输入和身份参考中恢复高质量人脸；2）从恢复-退化人脸对中提取人脸衍生的退化编码，捕捉噪声、模糊、压缩等退化属性，转换为多尺度退化感知标记，用于条件化扩散模型，在单步中恢复整个场景。

Result: 大量实验证明该方法相比现有最先进方法具有显著优越性。

Conclusion: Face2Scene通过将人脸作为感知引导，能够有效估计退化信息并指导全场景恢复，解决了现有方法在面部恢复和全场景恢复之间的局限性。
...
TL;DR: REFORGE是一个黑盒红队框架，通过对抗性图像提示评估图像生成模型遗忘的鲁棒性，使用交叉注意力引导的掩码策略优化扰动，在保持视觉保真度的同时提高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 图像生成模型的进步带来了版权内容和冒犯性内容生成的风险，图像生成模型遗忘技术旨在消除有害概念而无需完全重新训练。然而，在黑盒设置下对抗性输入（特别是图像侧威胁）的鲁棒性研究不足。

Method: REFORGE框架初始化基于笔触的图像，使用交叉注意力引导的掩码策略优化扰动，该策略将噪声分配到概念相关区域，平衡攻击效果和视觉保真度。

Result: 在代表性遗忘任务和防御措施上的广泛实验表明，REFORGE显著提高了攻击成功率，同时实现了比基线更强的语义对齐和更高的效率。

Conclusion: 这些结果揭示了当前IGMU方法中持续存在的漏洞，并强调了针对多模态对抗攻击的鲁棒性感知遗忘的必要性。
...
TL;DR: 该研究将人脑的共线性视觉感知原理引入计算机视觉，探索其在工业检测等应用中的潜力，发现对晶圆、纳米材料等结构化图像有显著性能提升，但对ImageNet等自然图像效果有限。


<details>
  <summary>Details</summary>
Motivation: 人脑的共线性感知原理（对空间对齐的边缘有增强效应）在真实世界中的作用尚不明确，在计算机视觉和工程应用中的利用更是未充分探索。本研究旨在将这一原理引入计算机视觉，探索其在各种应用场景中的潜在价值。

Method: 开发了一个原型模型来体现共线性原理，通过四个用例进行系统测试和基准评估：1）结合深度学习进行晶圆缺陷检测；2）结合深度学习和显著性模型进行纳米材料缺陷识别；3）处理遮挡问题；4）在ImageNet数据集上测试。

Result: 1）晶圆缺陷检测：性能提升1.24倍（错误率从6.5%降至5.26%）；2）纳米材料缺陷识别：性能提升3.2倍（错误率从21.65%降至6.64%）；3）遮挡处理：共线性有帮助；4）ImageNet：效果不明显。总结出共线性对结构化、人造线条图像有益，对自然图像效果有限。

Conclusion: 共线性原理特别适用于工业应用，因为感兴趣的目标图像结构通常是人造的且包含线条。该研究为计算机视觉提供了新工具，旨在捕捉人类视觉处理的优势，为特定领域的视觉任务带来性能提升。
...
TL;DR: 该论文提出了ACPV-Net框架，用于从航拍图像一次性生成包含所有土地覆盖类别的完整矢量地图表示，解决了多类别多边形化中的拓扑一致性问题。
...
Method: 提出了ACPV-Net统一框架，引入语义监督条件机制（SSC），将语义感知与几何基元生成耦合，并通过拓扑重建设计强制共享边缘一致性。

Result: 在Deventer-512基准测试中，ACPV-Net在强制严格拓扑约束的同时，在所有类别的多边形质量上超越了所有类别特定基线。在单类别多边形化任务（WHU-Building）上也取得了最佳报告结果。

Conclusion: 该研究形式化了全类别多边形矢量化的新任务，提出了ACPV-Net框架，通过语义监督条件机制和拓扑重建设计，实现了从航拍图像一次性生成拓扑一致的多类别矢量地图表示。
...
TL;DR: TCATSeg是一种结合局部几何特征与全局语义上下文的三维牙科模型分割框架，通过稀疏但物理意义明确的超点捕获全局语义关系，在400个牙科模型数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 三维牙科模型的精确语义分割对正畸和种植牙等数字牙科应用至关重要。现有方法由于主要关注局部几何特征而忽视全局上下文信息，在复杂牙齿排列和相邻牙齿形状相似的情况下分割效果不佳。

Method: 提出TCATSeg框架，结合局部几何特征与全局语义上下文。引入一组稀疏但具有物理意义的超点来捕获全局语义关系，从而提高分割精度。还创建了包含400个牙科模型（包括正畸前样本）的新数据集来评估方法泛化能力。

Result: 大量实验表明，TCATSeg在牙科模型分割任务上优于现有最先进方法，证明了结合全局语义上下文对提升分割精度的重要性。

Conclusion: TCATSeg通过有效结合局部几何特征与全局语义上下文，显著提升了三维牙科模型的分割精度，为数字牙科应用提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型被提出用于为人脸识别决策生成自然语言解释，以增强人类可解释性，但其在无约束人脸图像上的可靠性尚未得到充分探索，特别是在极端姿态变化和监控图像场景下。

Method: 在具有挑战性的IJB-S数据集上系统分析MLLM生成的人脸验证解释，特别关注极端姿态变化和监控图像；研究结合传统人脸识别系统信息（分数和决策）的影响；引入基于似然比的框架来评估文本解释的证据强度。

Result: 即使MLLM产生正确的验证决策，其解释也经常依赖不可验证或幻觉的面部属性；结合传统人脸识别系统信息能改善分类验证性能，但并不能持续产生忠实的解释；基于似然比的评估框架揭示了当前MLLM在可解释人脸识别方面的局限性。

Conclusion: 当前多模态大语言模型在可解释人脸识别方面存在根本性局限，其生成的解释缺乏可靠性和可信度，需要在生物识别应用中建立原则性的可靠解释评估框架。
...
TL;DR: BUSSARD是一种基于标准化流的模型，用于检测场景图中的异常关系，通过语言模型嵌入对象和关系标记，利用标准化流学习从场景图三元组到高斯基分布的双射变换，在SARD数据集上比现有方法AUROC提升约10%，速度快5倍。


<details>
  <summary>Details</summary>
Motivation: 检测场景图中的异常关系对于理解图像语义和现实世界场景分析具有重要意义。现有方法在检测异常关系方面存在局限性，需要更有效的方法来利用语义知识并提高检测性能。

Method: 采用多模态方法，使用语言模型嵌入场景图中的对象和关系标记以利用现实世界语义知识。应用标准化流模型学习从对象-关系-对象三元组到简单基分布（通常是高斯分布）的双射变换，通过似然估计进行异常检测。

Result: 在包含办公室和餐厅场景的SARD数据集上评估，BUSSARD比当前最先进模型的AUROC结果提升约10%，同时速度快5倍。消融研究表明模型具有优越的鲁棒性和通用性，特别是在同义词使用方面，基线模型出现17.5%的性能偏差时，BUSSARD保持稳定性能。

Conclusion: 这项工作展示了基于学习的方法在场景图关系异常检测中的强大潜力。BUSSARD通过标准化流和语言模型嵌入的结合，实现了高性能、快速且鲁棒的异常关系检测。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的风格化方法主要局限于颜色驱动的变换，忽略了复杂的语义和材质细节。为了解决这一问题，作者提出了一个能够处理多样化语义层次风格的框架。

Method: 基于专家混合（MoE）架构，使用统一风格编码器将多样化风格嵌入到一致的潜在空间中，然后通过相似性感知的门控机制动态地将风格路由到MoE中的专门专家。
...
TL;DR: 提出基于深度学习的层状陷阱巢穴中蜂类育雏细胞检测与分类方法，引入约束假阳性损失策略减少标注工作量并缓解类别不平衡问题


<details>
  <summary>Details</summary>
Motivation: 监测洞穴筑巢的野生蜜蜂和黄蜂对生物多样性研究和保护至关重要，但手动评估层状陷阱巢穴（LTNs）检测和分类育雏细胞既费时又费力，需要自动化解决方案

Method: 提出深度学习方法来高效检测和分类LTNs中的育雏细胞，针对密集排列的育雏细胞和高标注成本问题，引入新颖的约束假阳性损失（CFPL）策略，动态屏蔽未标注数据的预测，防止它们干扰分类损失训练

Result: 在包含712张LTN图像、覆盖28个细粒度类别的数据集上评估，实验结果表明深度学习能有效检测LTNs中的育雏细胞，CFPL方法进一步提升了性能，在模型准确性和标注工作量之间取得平衡，同时缓解了类别不平衡问题

Conclusion: 深度学习可用于LTNs中育雏细胞的检测，CFPL策略能有效减少标注工作量并提高模型性能，为生物多样性监测提供了高效的技术支持
...
TL;DR: HeBA提出了一种异构瓶颈适配器，通过模态特定的结构归纳偏置来改进视觉语言模型的适配，使用2D卷积处理视觉token，线性投影处理文本token，并采用压缩瓶颈和主动梯度初始化策略。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉语言模型（如CLIP）在下游任务适配时采用"一刀切"的架构方法，视觉和文本token被统一的宽泛适配器处理。这种同质性忽略了模态的独特结构特性——图像的空间局部性与文本的语义密度。

Method: HeBA提出三个关键架构创新：1) 异构性：通过2D深度可分离卷积处理视觉token以保留空间相关性，通过密集线性投影处理文本token以捕捉语义关系；2) 瓶颈正则化：采用压缩瓶颈（D -> D/4）强制模型学习紧凑、鲁棒的特征；3) 主动梯度初始化：使用Kaiming初始化策略确保足够的初始梯度流以加速收敛。

Result: 在11个少样本基准测试中，HeBA的架构专业化设计实现了优越的稳定性和准确性，建立了新的最先进水平。

Conclusion: HeBA通过引入模态特定的结构归纳偏置，解决了视觉语言模型适配中的同质性问题，其异构设计、瓶颈正则化和主动梯度初始化策略共同提升了模型在下游任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 高光谱图像虽然具有丰富的光谱信息，但其高维度和传感器变异性使其对跨域分布差异更加敏感，影响分类性能。现有单源域泛化方法在数据增强中存在真实性与多样性的权衡问题：盲目增强可能产生不现实的样本，而过度强调真实性又会抑制多样性。

Method: 提出SPDDA方法：1）光谱多样性模块：沿光谱维度重采样生成不同光谱通道的样本，构建通道自适应光谱混合器避免固定增强模式；2）空间-光谱协同优化机制：联合优化空间保真约束和光谱连续性自约束，根据空间约束自适应调整光谱约束权重，防止光谱过平滑并保持空间结构。

Result: 在三个遥感基准数据集上的大量实验表明，SPDDA方法优于现有的最先进方法。

Conclusion: 通过显式考虑高光谱图像的内在特性（设备依赖的光谱通道数量和相邻通道混合），SPDDA方法有效解决了单源域泛化中真实性与多样性的权衡问题，提升了模型对目标域的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界动作模型（WAMs）采用"想象后执行"范式，在测试时需要迭代视频去噪，导致显著延迟。研究旨在探究WAMs在测试时是否需要显式未来想象，或者其优势主要来自训练时的视频建模。

Method: 提出Fast-WAM架构，在训练时保留视频协同训练，但在测试时跳过未来预测。实例化多个Fast-WAM变体以进行受控比较，分离训练时视频建模和推理时显式未来生成的影响。

Result: Fast-WAM在模拟基准（LIBERO和RoboTwin）和现实世界任务中与最先进方法保持竞争力，无需具身预训练。运行延迟仅190ms，比现有"想象后执行"WAMs快4倍以上。研究发现跳过视频协同训练会导致更大性能下降。

Conclusion: 视频预测在WAMs中的主要价值可能在于改善训练时的世界表示，而非在测试时生成未来观测。Fast-WAM展示了通过简化推理过程实现实时性能的可行性。
...
TL;DR: 提出x²-Fusion方法，通过事件相机提供的时空边缘信号构建统一的Event Edge Space，将图像和LiDAR特征对齐到这个共享表示中，实现多模态融合，并采用可靠性感知自适应融合和跨维度对比学习来提升2D光流和3D场景流的估计精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法将图像、LiDAR和事件数据结合预测2D和3D运动，但它们在分离的异构特征空间中操作，缺乏共享的潜在空间。这导致系统依赖多个模态特定模块，无法解决跨传感器不匹配问题，使融合变得不必要地复杂。

Method: 1. 利用事件相机提供的时空边缘信号作为内在边缘场，构建统一的Event Edge Space表示；2. 将图像和LiDAR特征显式对齐到这个共享的边中心同质空间中；3. 在共享空间内执行可靠性感知自适应融合，估计模态可靠性并强调退化条件下的稳定线索；4. 采用跨维度对比学习紧密耦合2D光流与3D场景流。

Result: 在合成和真实基准测试上的广泛实验表明，x²-Fusion在标准条件下达到了最先进的精度，并在具有挑战性的场景中提供了显著的改进。

Conclusion: 通过将多模态融合重新定义为表示统一，利用事件衍生的时空边缘定义边中心同质空间，x²-Fusion能够有效解决跨传感器不匹配问题，简化融合复杂性，并在各种条件下实现优越的2D光流和3D场景流估计性能。
...
Method: HMAR采用混合专家架构，包含双专家机制：Expert0提取全局特征进行整体相似性匹配，Expert1学习位置不变的局部表示用于精确病变区域检索。采用两阶段对比学习策略避免昂贵的边界框标注，使用滑动窗口匹配算法在推理时进行密集局部比较，并通过KAN层生成哈希码进行高效的汉明距离搜索。

Result: 在RadioImageNet-CT数据集（16种临床模式，29,903张图像）上，HMAR在64位和128位哈希码上分别实现了0.711和0.724的平均精度，相比最先进的ACIR方法分别提高了0.7%和1.1%。

Conclusion: HMAR通过自适应检索框架解决了医学图像检索中的关键限制，双专家机制和两阶段对比学习策略有效提升了检索性能，特别是在细粒度区域特定检索方面表现出色。
...
Method: 采用目标帧控制方法，利用首尾帧运动先验；通过语义引导的物体插入和鲁棒背景修复构建可靠目标帧；利用早期步自注意力图预测物体和相机动态；提出ACE-Seed轻量搜索策略提升运动保真度。

Result: 在FLF2V-obj和VBench基准测试中持续优于基线方法；提出了S2M-DAVIS和S2M-OMB两个稳定相机下的物体运动评估数据集。

Conclusion: Search2Motion提供了一个无需训练、无需额外输入的物体运动编辑框架，通过目标帧控制和早期注意力机制实现了高质量的物体重定位，同时保持了场景稳定性。
...
TL;DR: 本研究提出了一种基于物联网的高通量实时多智能体情感计算框架，用于通过情绪状态监测增强课堂学习效果，在三个教育机构测试中实现了88%的整体准确率。
...
Method: 开发了面向物联网设备的实时多智能体情感计算框架，解决了负载均衡和延迟挑战，使用包含1,500张标记图像和300个课堂检测视频的"课堂情绪数据集"进行评估。

Result: 系统在三个教育机构（小学A、初中B、高中C）测试中表现稳健，能够以25FPS检测最多50张人脸，课堂参与状态分类整体准确率达到88%，获得学生、教师和家长积极反馈。

Conclusion: 本研究建立了实用的物联网情感感知学习环境框架，并引入了"课堂情绪数据集"促进进一步验证和研究，为改善课堂互动和教学适应性提供了有效工具。
...
TL;DR: 该论文提出了一种从视频扩散模型生成不一致帧中重建高质量3D场景的方法，通过非刚性对齐帧到全局坐标系，并使用点云初始化和逆变形渲染损失来创建可探索的3D环境。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型能生成高质量多样的世界，但帧间缺乏3D一致性，导致难以重建3D世界。需要解决帧间不一致性问题以实现3D重建。

Method: 1. 使用几何基础模型将每帧提升为像素级3D点云；2. 提出定制化的非刚性迭代帧到模型ICP进行初始对齐；3. 全局优化进一步锐化点云；4. 利用点云作为3D重建初始化，提出新颖的逆变形渲染损失来创建高质量3D环境。

Result: 该方法能够从视频模型生成比基线更高质量的3D场景，有效将视频模型转化为3D一致的世界生成器。

Conclusion: 通过处理视频帧间的3D不一致性，提出的方法成功实现了从视频扩散模型到高质量、可探索3D环境的转换，为3D世界生成提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理细粒度视觉属性和因果推理方面存在困难，现有的基于相似性检索的方法倾向于选择相关但非因果的示例，这会放大虚假关联并限制模型鲁棒性

Method: 提出CIRCLES框架，通过属性引导的组合图像检索主动构建反事实风格的演示示例集，使模型能够隐式推理属性与结果之间的因果关系

Result: 在四个不同数据集上的实验表明，CIRCLES在多种架构上一致优于现有方法，特别是在小规模模型上，在信息稀缺情况下表现尤为突出，同时检索到更多样化和具有因果信息的示例
...
TL;DR: 提出基础设施教学的无标签3D感知范式，利用路边单元作为静态教师为自动驾驶车辆提供伪标签监督，无需人工标注即可训练车辆感知模型。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶在多样化城市部署时，大规模数据收集和人工标注成本高昂且不切实际。现代城市配备的路边单元为车辆感知训练提供了新的可能性。

Method: 提出三阶段无标签流程：1）路边单元从无标签数据学习本地3D检测器；2）向经过车辆广播预测结果；3）车辆聚合这些预测作为伪标签监督训练独立的感知模型。测试时无需基础设施支持。

Result: 在CARLA多智能体环境中，使用CenterPoint检测器达到82.3% AP（车辆检测），相比完全监督的上限94.4%。系统分析显示该方法具有可扩展性，并与现有以车辆为中心的无标签方法互补。

Conclusion: 城市基础设施本身可为自动驾驶提供可扩展的监督信号，基础设施教学学习成为减少3D感知标注成本的有前景的补充范式。
...
<details>
  <summary>Details</summary>
Motivation: EPI（回波平面成像）在fMRI中广泛应用，但存在磁化率引起的几何畸变问题。现有校正方法如FSL TOPUP需要手动参数调整，计算成本高。需要一种更高效、自动化的畸变校正方法。

Method: 使用最优传输理论，将反相位编码EPI图像对的畸变场建模为Wasserstein-2重心位移。在谱域使用弯曲能量惩罚进行正则化，通过Morozov差异原则自动选择正则化强度，无需手动调参。

Result: 在HCP数据集上，SuCor与T1结构图像的体积互信息达到0.341，优于FSL TOPUP的0.317。计算时间约12秒（单CPU核心），比现有方法更快。

Conclusion: SuCor提供了一种基于最优传输理论的EPI畸变校正方法，具有自动化、计算高效、精度高的特点，在fMRI预处理中具有实用价值。
...
TL;DR: 本文提出基于顶点帧的新范式，从稳定伪装状态的表情顶点帧识别真实情绪，并提出双流独立解耦框架来分离真实和伪装情绪特征。


<details>
  <summary>Details</summary>
Motivation: 现有方法从包含开始伪装帧的掩蔽表情片段识别真实情绪，但这可能无法反映实际伪装状态，因为开始帧泄露了真实情绪信息而未达到稳定伪装状态。

Method: 1) 引入基于顶点帧的新范式，从稳定伪装状态的顶点帧分类真实情绪；2) 提出双流独立解耦框架，解耦真实和伪装情绪特征；3) 设计解耦损失组，包括两个分类损失分别学习真实情绪和伪装表情特征，以及希尔伯特-施密特独立性损失增强两个特征的独立性。

Result: 实验证明基于顶点帧的范式具有挑战性，而提出的解耦框架提高了识别性能。

Conclusion: 通过顶点帧范式和双流独立解耦框架，能够更有效地从稳定伪装状态的表情中识别真实情绪，避免伪装情绪对真实情绪的干扰。
...
Method: 1. 引入噪声感知的一步扩散模型，生成多样化的超分辨率输出；2. 采用不等时间步策略，将噪声添加时间步与扩散时间步解耦；3. 提出GDPO策略，将GRPO原理整合到DPO中，计算在线生成样本的组相对优势；4. 设计属性感知奖励函数，基于平滑和纹理区域的统计信息动态评估每个样本的得分。

Result: 实验证明了GDPO在增强一步式生成图像超分辨率模型性能方面的有效性。

Conclusion: GDPO成功将强化学习整合到一步式生成图像超分辨率模型训练中，通过创新的噪声感知模型、不等时间步策略和属性感知奖励函数，有效提升了一步式生成方法的性能。
...
TL;DR: 本文系统研究了视觉协同去噪方法，提出了V-Co框架，通过四个关键要素显著提升了像素空间扩散模型的性能。


<details>
  <summary>Details</summary>
Motivation: 像素空间扩散模型虽然能生成高质量图像，但缺乏强语义监督和高层视觉结构捕捉能力。现有的视觉协同去噪方法设计选择混杂，不清楚哪些要素真正关键，需要系统研究。

Method: 提出V-Co框架，采用统一的JiT基础架构进行视觉协同去噪研究。关键设计包括：完全双流架构、结构化无条件预测、感知漂移混合损失、RMS特征重缩放校准。

Result: 在ImageNet-256上，V-Co在相同模型规模下超越了底层像素空间扩散基线和现有强像素扩散方法，同时使用更少的训练轮次。

Conclusion: 视觉协同去噪的四个关键要素为：完全双流架构、结构化无条件预测、感知漂移混合损失、RMS特征校准。这些发现为未来表示对齐生成模型提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有动物深度估计模型大多基于无度量尺度的数据集训练，这限制了图像模型的验证和可靠性。需要包含度量尺度的多模态数据集来推动动物感知系统的发展。

Method: 提出了WildDepth数据集，包含从家养到野生环境的各种动物类别，提供同步的RGB和LiDAR数据。建立了用于深度估计、行为检测和3D重建的基准套件。

Result: 实验结果显示，使用多模态数据可将深度可靠性提高最多10%的RMSE，而RGB-LiDAR融合可将3D重建保真度提高12%的Chamfer距离。

Conclusion: 通过发布WildDepth数据集和基准套件，旨在促进能够跨领域泛化的鲁棒多模态感知系统的发展。
...
TL;DR: 本文系统评估了三种数据中心方法在遥感数据中处理标签噪声的效果，分析了不同噪声类型和水平下的性能表现，为遥感领域标签噪声处理提供了方法选择指导。


<details>
  <summary>Details</summary>
Motivation: 遥感数据集中普遍存在标签噪声问题，这会严重影响深度学习模型的泛化能力，但该领域对标签噪声的自动化处理方法关注不足，缺乏对数据中心方法的系统性分析。

Method: 研究选择了三种数据中心方法，在两个基准数据集上注入不同噪声类型（10%-70%噪声水平），分析这些方法过滤标签噪声的效果及其对任务性能的影响。

Result: 分析证明数据中心方法在标签噪声识别和任务性能提升两方面都具有重要价值，研究提供了根据不同设置和目标选择最佳方法的见解，并指出了将数据中心方法应用于遥感数据时仍需研究的领域。

Conclusion: 该研究在连接数据中心标签噪声方法的方法论建立与遥感领域实际应用之间迈出了重要一步，为遥感数据处理标签噪声提供了实用指导。
...
<details>
  <summary>Details</summary>
Motivation: Vision transformers（特别是像DINOv2这样的特征基础模型）虽然能学习丰富的表示，但其架构选择（如位置编码）会导致模型表现出与语义内容无关的位置偏差和伪影。这在材料科学等领域造成问题，因为这些领域的图像通常是均匀微观结构的横截面（没有首选方向），使得零样本适应变得困难。

Method: 通过线性探测研究Vision transformers中的位置偏差，发现这种偏差存在于多种目标和位置编码中。然后通过微调模型使用ALiBi相对位置编码来减少位置偏差。

Result: 研究表明，经过ALiBi微调的模型能够保持理想的通用语义特征，其无偏差的特征可以成功用于复杂显微镜图像的可训练分割任务。

Conclusion: Vision transformers存在固有的位置偏差问题，但通过采用ALiBi相对位置编码进行微调，可以有效减少这种偏差，同时保持模型的语义表示能力，使其更适合材料科学等领域的应用。
...
TL;DR: M^3提出了一种结合多视角基础模型与单目高斯溅射SLAM的新方法，通过专用匹配头实现细粒度密集对应，提升未标定单目视频流式重建的精度和效率。


<details>
  <summary>Details</summary>
Motivation: 未标定单目视频的流式重建面临挑战，需要高精度姿态估计和动态环境中的高效在线优化。现有方法中，多视角基础模型通常以前馈方式估计姿态，产生的像素级对应关系缺乏几何优化所需的精度。

Method: M^3在多视角基础模型中增加专用匹配头以实现细粒度密集对应，并将其集成到鲁棒的单目高斯溅射SLAM框架中。还引入了动态区域抑制和跨推理内参对齐来增强跟踪稳定性。

Result: 在多样化的室内外基准测试中，M^3在姿态估计和场景重建方面均达到最先进精度。相比VGGT-SLAM 2.0，ATE RMSE降低了64.3%；在ScanNet++数据集上，PSNR比ARTDECO高出2.11 dB。

Conclusion: M^3通过增强多视角基础模型的匹配能力并集成到SLAM框架中，有效解决了未标定单目视频流式重建的精度瓶颈问题，在姿态估计和重建质量方面均取得显著提升。
...
TL;DR: SparkVSR是一个交互式视频超分辨率框架，允许用户通过稀疏关键帧控制超分辨率过程，支持灵活的关键帧选择和参考无关的引导机制。


<details>
  <summary>Details</summary>
Motivation: 现有VSR方法在推理时像黑盒一样工作，用户无法可靠地纠正意外伪影，只能接受模型产生的任何结果。需要一种允许用户控制超分辨率过程的交互式框架。

Method: 提出基于关键帧的交互式VSR框架SparkVSR，采用关键帧条件化的潜在-像素两阶段训练流程，融合低分辨率视频潜在特征与稀疏编码的高分辨率关键帧潜在特征，学习跨空间传播并细化感知细节。

Result: 在多个VSR基准测试中表现出改进的时间一致性和强大的恢复质量，在CLIP-IQA、DOVER和MUSIQ指标上分别超过基线24.6%、21.8%和5.6%。框架还可泛化应用于老电影修复和视频风格迁移等未见任务。

Conclusion: SparkVSR实现了可控的、关键帧驱动的视频超分辨率，是一个通用的交互式关键帧条件化视频处理框架，支持灵活的关键帧选择和参考无关的引导机制。
...
TL;DR: 该研究提出了MessyKitchens数据集和基于SAM 3D的多对象解码器(MOD)，用于改进杂乱场景中的物体级三维重建，重点关注物理合理性和物体间接触关系。


<details>
  <summary>Details</summary>
Motivation: 当前单目三维场景重建在深度估计方面取得进展，但将常见场景分解为独立三维物体仍面临挑战，包括物体多样性、频繁遮挡和复杂物体关系。机器人学和动画应用需要物理合理的场景重建，要求物体遵循非穿透和真实接触的物理原则。

Method: 1. 引入MessyKitchens数据集，提供杂乱真实场景的高保真物体级地面真值（三维物体形状、姿态和准确的物体接触关系）。2. 基于SAM 3D单物体重建方法，扩展了多对象解码器(MOD)进行联合物体级场景重建。

Result: MessyKitchens数据集在配准精度和物体间穿透方面显著优于先前数据集。MOD方法在三个数据集上的多物体重建任务中，相比现有技术取得一致且显著的改进。

Conclusion: 该研究通过新数据集和改进的多物体重建方法，推动了物体级场景重建的发展，特别是在物理合理性和物体间接触关系方面。所有资源将公开提供以促进进一步研究。
...
TL;DR: 研究发现视频生成模型中的推理机制并非沿视频帧序列展开，而是沿扩散去噪步骤进行，提出了"步骤链"机制，并揭示了多种涌现推理行为。


<details>
  <summary>Details</summary>
Motivation: 先前研究认为基于扩散的视频模型通过"帧链"机制进行推理，即推理沿视频帧序列展开。本研究挑战这一假设，旨在揭示视频模型中推理的真正机制。

Method: 通过定性分析和针对性探测实验，研究视频生成模型在扩散去噪过程中的推理行为。进一步提出了无需训练的简单策略作为概念验证，通过集成不同随机种子的相同模型的潜在轨迹来改进推理。

Result: 发现推理主要沿扩散去噪步骤展开，模型在早期去噪步骤中探索多个候选解，逐步收敛到最终答案，这一过程称为"步骤链"。还识别出三种关键推理行为：工作记忆、自我纠正与增强、感知先于行动。在扩散变换器中发现了功能专门化现象。

Conclusion: 视频生成模型的推理机制是沿扩散去噪步骤而非视频帧序列展开的"步骤链"机制。这一发现为理解视频模型的推理动态提供了系统框架，为未来研究如何更好地利用视频模型作为智能新基质奠定了基础。
...
TL;DR: 该论文提出了一种基于相机姿态的统一几何表示方法，用于改善视频扩散变换器在交互式游戏世界模型中的动作控制和长期3D一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散变换器方法在交互式游戏世界模型中存在两个主要问题：1）难以实现精确的动作控制，通常将用户动作视为抽象条件信号，忽略了动作与3D世界之间的几何耦合关系；2）缺乏长期3D一致性，动作引起的相对相机运动在长时间导航中难以保持空间一致性。

Method: 1）定义基于物理的连续动作空间，在Lie代数中表示用户输入以推导精确的6自由度相机姿态，通过相机嵌入器注入生成模型确保动作对齐；2）使用全局相机姿态作为空间索引检索相关历史观测，实现长时间导航中位置的几何一致重访；3）构建包含3000分钟真实人类游戏数据的大规模数据集，标注相机轨迹和文本描述。

Result: 实验表明，该方法在动作可控性、长期视觉质量和3D空间一致性方面显著优于现有的交互式游戏世界模型。

Conclusion: 相机姿态作为统一几何表示能够有效解决交互式游戏世界模型中的动作控制和长期3D一致性问题，为构建更真实、可控的生成式游戏环境提供了新思路。
