...
TL;DR: 提出SKETCHGAIT框架，使用无标签的草图模态进行步态识别，通过多模态分层解耦架构结合解析和草图两种互补的结构表示，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别研究主要基于轮廓和解析表示：轮廓稀疏且缺乏内部结构细节，解析依赖上游人体解析器导致性能不稳定。需要探索新的结构表示范式来提升识别性能。

Method: 提出SKETCH作为新的视觉模态，通过边缘检测器从RGB图像中提取高频结构线索（如肢体关节和自遮挡轮廓），无需语义标签。设计SKETCHGAIT框架，包含两个独立流进行模态特定学习，以及轻量级早期融合分支捕获结构互补性。

Result: 在SUSTech1K数据集上达到92.9%的Rank-1准确率，在CCPG数据集上达到93.1%的平均Rank-1准确率，验证了草图模态和框架的有效性。

Conclusion: 草图作为无标签的密集部分级结构表示，与标签引导的解析在语义上解耦且结构上互补，SKETCHGAIT框架通过多模态分层解耦有效结合两者，显著提升了步态识别性能。
...
<details>
  <summary>Details</summary>
Motivation: 关键点检测是许多视觉任务的基础，但现代神经模型对输入扰动仍然脆弱。由于高维输入和连续坐标输出，关键点检测器的形式化鲁棒性验证尚未充分探索。现有解耦方法独立验证每个关键点，忽略了关键点间的相互依赖关系，导致保守的保证。

Method: 提出耦合鲁棒性验证框架，将验证问题表述为使用混合整数线性规划（MILP）的证伪问题。该方法结合可达热图集合与编码联合偏差约束的多面体，通过不可行性证明鲁棒性，可行性提供反例。证明该方法具有可靠性：如果验证模型鲁棒，则关键点检测模型确实鲁棒。

Result: 实验表明，耦合方法实现了较高的验证率，在严格误差阈值下仍然有效，而解耦方法在此情况下失效。该方法能够捕捉关键点间的相互依赖关系，为下游任务提供更有意义的保证。

Conclusion: 该研究首次提出了针对基于热图的关键点检测器的耦合鲁棒性验证框架，通过验证集体行为而非独立关键点，提供了更严格和实用的鲁棒性保证，填补了该领域的形式化验证空白。
...
TL;DR: DreamCAD是一个多模态生成框架，能够直接从点级监督生成可编辑的BReps，无需CAD特定标注，并在ABC和Objaverse基准测试中取得最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有CAD生成方法受限于小规模标注数据集和BRep标签，而数百万未标注的3D网格数据未被充分利用，限制了可扩展CAD生成的发展。

Method: 将每个BRep表示为参数化曲面片集合，使用可微分细分方法生成网格，实现大规模3D数据集训练，同时重建连接且可编辑的表面。还引入了CADCap-1M数据集，包含100万+由GPT-5生成的描述。

Result: 在ABC和Objaverse基准测试中，DreamCAD在文本、图像和点模态上均取得最先进性能，提高了几何保真度，并超过75%的用户偏好。

Conclusion: DreamCAD通过点级监督直接生成可编辑BReps，无需CAD特定标注，解决了现有方法的局限性，并提供了大规模CAD标注数据集，推动了文本到CAD研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 高通量筛选产生的细胞成像数据存在技术变异导致的生物批次效应，这会引起协变量偏移并降低深度学习模型在未见数据上的泛化能力。现有批次校正方法通常需要额外先验知识或难以泛化到未见生物批次。

Method: 将生物批次缓解构建为领域泛化问题，提出对抗性批量表示增强（ABRA）。该方法通过参数化特征统计量作为结构化不确定性，在表示空间中主动合成最坏情况的生物批次扰动，采用严格的角几何边界保持细粒度类别可区分性，并引入协同分布对齐目标防止表示崩溃。

Result: 在RxRx1和RxRx1-WILDS基准测试上的广泛评估表明，ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性批量表示增强有效缓解了细胞成像中的生物批次效应，提高了深度学习模型在未见数据上的泛化能力，为高通量筛选数据分析提供了有效的解决方案。
...
Method: PFS在现有检测器的中间BEV表示上操作，生成精炼的特征图供原始检测头使用。设计包括：稳定域偏移下的特征统计、抑制传感器退化影响的空间区域、通过残差校正自适应恢复弱化线索，设计为近似恒等变换。

Result: 在nuScenes基准测试中，PFS在多种故障模式下达到最先进结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，同时保持轻量级（仅3.3M参数）。

Conclusion: PFS是一种有效的轻量级解决方案，能够提升现有BEV融合检测器在域偏移和传感器故障下的鲁棒性，无需架构修改或重新训练，适合实际部署。
...
TL;DR: CBM-Suite框架解决了概念瓶颈模型的四个核心问题：概念相关性预评估指标缺失、线性化问题导致概念瓶颈被绕过、与黑盒模型的准确率差距、以及缺乏对不同视觉骨干和视觉语言模型的系统研究。


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型虽然通过人类可理解的概念进行预测，但存在四个基本限制：1）缺乏预评估概念相关性的指标；2）"线性问题"导致模型绕过概念瓶颈；3）与不透明模型存在准确率差距；4）缺乏对不同视觉骨干和视觉语言模型的系统研究。

Method: CBM-Suite框架包含四个关键技术：1）基于熵的指标量化概念集的内在适用性；2）在概念激活和分类器之间插入非线性层解决线性问题；3）使用线性教师探针引导的蒸馏损失缩小准确率差距；4）系统分析不同视觉编码器、视觉语言模型和概念集对准确率和可解释性的影响。

Result: 广泛评估表明，CBM-Suite能够产生更准确的模型，并为改进基于概念的可解释性提供了深入见解。

Conclusion: CBM-Suite框架系统地解决了概念瓶颈模型的核心挑战，通过引入新的评估指标、解决线性问题、缩小准确率差距以及提供系统性分析，显著提升了概念瓶颈模型的实用性和有效性。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE重建FID（rFID）与潜在扩散模型生成FID（gFID）相关性很差，需要一种能预测扩散模型生成质量的指标

Method: 对数据集每个元素，在潜在空间中检索最近邻，插值潜在表示后解码，计算解码样本与原数据集的FID

Result: iFID与扩散gFID呈现强相关（Pearson和Spearman相关系数约0.85），是首个展示这种强相关的指标
...
TL;DR: 本文提出了一种名为隐式错误计数（IEC）的新方法，用于解决缺乏理想参考答案的强化学习任务。该方法通过枚举错误而非构建评分标准来生成奖励信号，在虚拟试衣等复杂任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法（如RLVR和RaR）依赖于理想参考答案来构建评分标准，但许多现实任务存在多个有效输出且缺乏单一理想答案。本文旨在填补这一空白，解决参考缺失场景下的后训练方法问题。

Method: 提出隐式错误计数（IEC）方法，通过枚举响应中的错误而非检查正确性来生成奖励。采用两个关键设计：隐式分数发射和组校准，将错误转换为校准后的每方面奖励。针对虚拟试衣任务，进一步提出级联错误计数（CEC）作为评估指标。

Result: 在Mismatch-DressCode基准测试中，IEC在所有指标上均优于RaR（CEC：5.31 vs 5.60）。在VITON-HD和DressCode数据集上，IEC在8个感知指标中的6个上匹配或超越了6个基线方法。CEC指标与人类偏好高度一致（60% top-1 vs 30%其他）。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号。IEC方法在缺乏单一理想答案的复杂任务中表现出色，为参考缺失场景下的强化学习提供了有效解决方案。
...
TL;DR: SemVID是一种无需训练的视频时间定位剪枝框架，通过保留关键证据和保持跨帧连接性，仅使用12.5%的视觉token就能达到95.4%的mIoU，实现5.8倍的预填充加速。


<details>
  <summary>Details</summary>
Motivation: 视频时间定位需要处理长视频，导致视频语言模型计算成本过高。现有的视觉token剪枝方法直接应用于VTG会导致性能显著下降，因为VTG依赖于边界敏感证据和跨帧推理链。

Method: 提出SemVID框架，基于两个VTG特定剪枝原则：证据保留（保留查询关键patch，特别是在事件边界周围）和连接强度（保持token级跨帧连接性）。首先通过平衡查询相关性和帧间变化分配每帧token预算，然后选择三种token：对象token（多样化查询关键证据）、运动token（捕获有意义的转换并作为跨帧中继）和少量上下文token（保持场景连续性）。

Result: 在VTG基准测试中，SemVID仅使用12.5%的视觉token就能保留高达95.4%的mIoU，提供高达5.8倍的预填充加速，在相同预算下持续优于先前方法。

Conclusion: SemVID通过识别VTG特定的剪枝原则并构建具有互补语义角色的紧凑token子集，实现了准确性和效率之间的强有力平衡，为高效视频时间定位提供了有效解决方案。
...
TL;DR: OWL是一种基于视觉运动线索的3D感知函数，直接从原始视觉运动图像序列中提取信息，无需环境先验知识，实现实时3D场景重建和相机航向估计。


<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，特别是在缺乏环境先验知识、移动物体或相机运动信息的情况下，实现实时、准确的3D感知。

Method: 基于两种基本视觉运动线索：1) 注视点附近点的感知局部视觉逼近；2) 刚性物体相对于注视点的感知旋转。通过像素级并行计算，直接从原始视觉运动图像序列中提取OWL值，无需测量或先验知识。

Result: 仿真实验表明，OWL能够实现3D物体的几何恒常性，仅凭视觉运动线索就能进行缩放3D场景重建。该方法提供了实时、统一的3D点相对运动表示。

Conclusion: OWL为机器感知提供了新视角，弥合了理论概念与机器人、自主导航实际应用之间的差距，可能成为下一代自主系统的基础构建模块，对自然感知和行为心理学也有启示意义。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准测试，包含超过46,000个多模态检索候选项和747个可验证问题，揭示当前MLLMs在异构多模态检索方面存在显著瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估文本、图像或视频的单独理解能力，但忽视了真实世界需要从大型异构多模态语料库中检索相关证据再进行推理的关键需求。大多数现有基准将检索限制在小规模、单模态候选项集中，过度简化了搜索空间并夸大了端到端可靠性。

Method: 提出MultiHaystack基准测试，包含超过46,000个跨文档、图像和视频的多模态检索候选项，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的验证证据项，要求跨模态的证据定位和细粒度推理。

Result: 研究发现：1）当提供相应证据时模型表现良好，但需要从完整语料库中检索时性能急剧下降；2）最强的检索器E5-V仅达到40.8%的Recall@1；3）最先进的MLLMs如GPT-5在提供相应证据时推理准确率为80.86%，但在top-5检索下降至51.4%。
...
TL;DR: 本文提出Audiovisual Geolocation框架，通过视听融合解决地理定位难题，在AVG基准上显著优于单模态基线


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在图像定位方面已有进步，但精确的全球地理定位仍面临挑战，主要因为视觉景观的固有模糊性和听觉线索的未充分利用

Method: 提出三阶段框架：1)感知阶段使用混合自回归稀疏自编码器将噪声音频分解为语义基础的"声学原子"；2)多模态推理阶段使用GRPO微调的MLLM融合声学原子与视觉特征；3)精确预测阶段在S^2流形上使用黎曼流匹配

Result: 实验表明该框架显著优于单模态基线，证明可解释的声音景观感知提供了关键的、正交的信号，与多模态推理结合可实现高精度全球定位

Conclusion: 通过可解释的感知和多模态推理，视听融合为全球地理定位提供了有效的解决方案，声音景观的感知提供了与视觉正交的关键信号
...
<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准强制单标签假设，但许多图像包含多个物体，导致标签噪声并限制学习信号的丰富性。多标签标注能更准确地反映现实世界视觉场景，使模型学习更丰富、更鲁棒的表征。

Method: 提出自动化流程：使用自监督视觉Transformer进行无监督物体发现，选择与原始标签对齐的区域训练轻量级分类器，然后应用于所有区域生成跨数据集的一致多标签标注。
...
Conclusion: 准确的多标签标注对于提升分类性能和表征学习至关重要。该方法为ImageNet训练集提供了可扩展、高质量的多标签标注，无需人工标注。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习天空模型在高分辨率下无法准确建模14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输偏差、阴影和色调失真。需要一种能够准确再现自然天空真实感和光照特性的模型。

Method: 提出Icarus模型，能够学习全动态范围物理捕捉户外图像的曝光范围，支持用户条件生成环境贴图，可直观定位太阳和云层位置，并扩展现有技术以实现用户控制的大气形成纹理。

Result: Icarus可与全动态范围物理捕捉户外图像或参数化天空模型互换，在基于图像的光照中提供前所未有的准确性、真实感、光照方向性（阴影）和色调。

Conclusion: Icarus是一个全天气天空模型，能够准确再现自然天空的光照特性，解决了现有深度学习天空模型在高动态范围太阳区域建模的局限性，为视觉艺术、虚拟现实和科学应用提供了高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有区域布局控制方法存在两个主要问题：基于训练的方法会继承数据偏差并降低图像质量；现有技术难以处理遮挡顺序，限制了实际应用。研究发现空间布局和遮挡在去噪早期阶段就已确定，因此重新排列早期潜在结构足以修改最终输出。

Method: 采用两阶段方案：1) 实例初始化阶段 - 利用多模态联合注意力中的上下文共享机制，创建每个实例的分支，在指定早期步骤中根据层顺序融合形成具有预设布局的统一潜在表示；2) 语义护理阶段 - 通过分层注意力增强强化区域细节并保持遮挡顺序，使用顺序分层注意力路径与标准全局路径并行，在层透明度调度器下进行更新合成。

Result: LayerBind在定性和定量结果上都表现出有效性，展示了在创意应用中的强大潜力。该方法无需训练、即插即用，可作为跨扩散变换器的区域和遮挡控制器，并原生支持可编辑工作流程。

Conclusion: LayerBind通过分层建模和绑定机制，解决了文本到图像生成中区域布局控制的现有局限性，实现了精确的区域控制和遮挡顺序管理，为创意应用提供了灵活实用的解决方案。
...
TL;DR: BM25-V：基于视觉词稀疏自编码的BM25检索方法，作为高效的第一阶段检索器，配合密集重排序实现高精度图像检索
...
Method: 使用稀疏自编码器（SAE）从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25评分。BM25的逆文档频率（IDF）权重能有效抑制常见低信息词，突出罕见判别性词。通过稀疏倒排索引操作实现高效检索。

Result: 在七个基准测试中，BM25-V的Recall@200达到≥0.993，仅需对每个查询重排序K=200个候选，就能恢复接近密集检索的准确率（平均差距0.2%）。SAE在ImageNet-1K上训练一次即可零样本迁移到七个细粒度基准，无需微调。

Conclusion: BM25-V作为高效的第一阶段检索器，配合密集重排序的两阶段管道，在保持高召回率的同时显著降低计算成本。检索决策可归因于特定视觉词及其IDF贡献，提高了可解释性。
...
Method: 提出LIPAR框架：1）检测并跳过重复的潜在补丁计算；2）引入注意力恢复机制，近似被修剪token的注意力值，避免视觉伪影。该方法无需额外训练即可与模型集成。

Result: 在NVIDIA A6000上，视频编辑吞吐量平均提升1.45倍，达到12.2 FPS（基线为8.4 FPS）。生成质量不受影响，有效弥合传统压缩算法与现代生成管道之间的差距。

Conclusion: LIPAR框架通过利用视频潜在补丁的时间冗余性，显著提高了视频生成的计算效率，实现了高质量实时视频编辑，为生成模型的实时应用提供了有效解决方案。
...
TL;DR: Cog2Gen3D：一个通过3D认知图指导的扩散框架，用于解决3D生成中语义保真度和几何合理性的问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法通常使用几何特征作为条件，但只能建模相对关系，容易产生绝对几何尺度不一致的问题。作者认为语义信息和绝对几何信息共同赋能3D认知，从而实现对物理世界的可控3D生成。

Method: 提出Cog2Gen3D框架，包含三个关键设计：1）认知特征嵌入：将不同模态编码为语义和几何表示，并提取逻辑表示；2）3D潜在认知图：构建双流语义-几何图，通过基于公共的交叉注意力融合获得3D认知图；3）认知指导的潜在扩散：使用融合的3D认知图作为条件指导潜在扩散过程生成3D高斯。

Result: 在基于Marble World Labs构建的验证子集上进行广泛实验，证明Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。

Conclusion: Cog2Gen3D通过统一的3D认知图框架确保了3D生成的物理合理性和结构合理性，为解决3D生成中的语义-几何一致性挑战提供了有效方案。
...
TL;DR: VS3R是一个视频稳定框架，通过结合3D重建与生成视频扩散技术，解决了传统方法在几何鲁棒性和全帧一致性之间的权衡问题，实现了高保真、全帧稳定的效果。


<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性与全帧一致性的根本权衡：2D方法因裁剪过度而受限，3D技术则因优化流程脆弱而在极端运动下失效。需要一种能在所有场景下可靠工作的解决方案。

Method: VS3R框架结合前馈式3D重建与生成视频扩散技术：1）联合估计相机参数、深度和掩码以确保全场景可靠性；2）引入混合稳定渲染模块融合语义和几何线索实现动态一致性；3）采用双流视频扩散模型修复遮挡区域并校正伪影，结合结构引导与语义锚点。
...
TL;DR: 该研究提出了TumorChain框架，通过多模态链式推理方法改进临床肿瘤分析，包括构建TumorCoT数据集（150万条带CoT标注的VQA指令）和开发结合3D CT扫描与临床文本理解的推理系统。


<details>
  <summary>Details</summary>
Motivation: 准确的肿瘤分析对临床放射学和精准肿瘤学至关重要，需要从影像发现到临床印象再到病理结论的逐步解释。链式思维推理能提高可追溯性并减少诊断错误，但目前缺乏专门针对临床肿瘤分析的大规模基准和系统框架。

Method: 1）构建TumorCoT数据集：包含150万条带CoT标注的视觉问答指令，与3D CT扫描配对，具有步骤对齐的推理过程和跨模态对齐；2）提出TumorChain框架：结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代因果推理进行证据定位、结论聚合和病理预测。

Result: 实验显示在病灶检测、印象生成和病理分类方面均优于强基线方法，在DeepTumorVQA基准测试中表现出强大的泛化能力。该框架提高了可追溯性并减少了幻觉风险。

Conclusion: 该研究证明了多模态推理在临床实践中实现可靠且可解释的肿瘤分析的潜力，为临床诊断和治疗规划提供了新的技术支持。
...
TL;DR: PatchCue是一种基于图像块的视觉提示范式，通过将图像分割成块并在块级别表示视觉提示，显著增强视觉语言模型的视觉推理能力，优于像素级边界框和点级提示。
...
Method: 提出PatchCue：基于图像块的视觉提示范式，将图像分割成块并在块级别表示视觉线索，更符合人类感知习惯。采用两阶段训练方法：1）冷启动监督微调以输出块级提示；2）使用过程监督提示奖励的强化学习来引导中间视觉推理步骤。

Result: 在多个视觉语言模型和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue能持续提升整体模型性能。块级提示在性能上优于像素级边界框和点级提示。

Conclusion: PatchCue提供了一种更有效且认知对齐的视觉推理范式，通过块级视觉提示显著增强了视觉语言模型的视觉推理能力，为多模态理解任务提供了更好的解决方案。
...
TL;DR: 本文系统评估了合成新视角在视频地点识别(VPR)中的应用，发现小规模添加新视角能改善识别统计，而大规模添加时，视角变化幅度不如添加数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新视角生成在机器人导航中有多种潜在应用价值：在基于图像的导航中，地面机器人拍摄的场景可生成新的俯视视角来引导空中机器人；在视频地点识别中，空中视角的地面新视图可帮助无人机识别地面机器人见过的地点，反之亦然。

Method: 使用五个公开的VPR图像数据库和七种典型的图像相似性方法，对合成新视角在VPR中的应用进行系统评估。

Result: 对于小规模合成添加，新视角能改善VPR识别统计；对于大规模添加，视角变化幅度的重要性不如添加的视图数量和数据集中的图像类型重要。

Conclusion: 合成新视角在VPR中具有实际应用价值，特别是在小规模添加时能有效改善识别性能，为跨视角机器人导航提供了技术支持。
...
TL;DR: CylinderSplat：一种用于全景3D高斯泼溅的前馈框架，通过圆柱形Triplane表示和双分支架构，在稀疏视图场景中实现高质量的全景新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯泼溅方法在全景图像应用中面临挑战：多视图成本体积在稀疏视图场景中难以解决遮挡问题，标准笛卡尔Triplane表示无法有效捕捉360°场景的固有几何结构，导致失真和混叠。

Method: 提出CylinderSplat框架，核心是新的圆柱形Triplane表示，与全景数据和曼哈顿世界假设更匹配。采用双分支架构：基于像素的分支重建观测良好的区域，基于体积的分支利用圆柱形Triplane完成遮挡或稀疏观测区域。框架灵活支持从单视图到多视图的输入。

Result: 大量实验表明，CylinderSplat在单视图和多视图全景新视角合成中均达到最先进水平，在重建质量和几何精度方面均优于先前方法。

Conclusion: CylinderSplat通过创新的圆柱形Triplane表示和双分支架构，有效解决了全景3D高斯泼溅中的几何表示和遮挡处理问题，为实时全景新视角合成提供了高质量解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖隐式有符号距离场和事后布局优化，缺乏统一的场景重建框架。需要一种能够直接从单张图像生成完整、连贯、可直接用于下游应用的3D场景网格的方法。
...
Conclusion: PixARMesh提供了一种有效的单图像3D室内场景重建方法，通过统一的自回归框架联合优化布局和几何，生成可直接使用的艺术家级网格，在重建质量和实用性方面均表现出色。
...
TL;DR: InnoAds-Composer：单阶段电商产品海报生成框架，通过重要性分析和条件路由实现主题、文字和风格的三重控制，解决现有多阶段方法存在的主题保真度差、文字不准确和风格不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的电商产品海报生成方法通常采用多阶段流程，难以同时控制主题、文字和风格，且存在主题保真度差、文字不准确和风格不一致三大问题。

Method: 提出单阶段框架InnoAds-Composer，通过重要性分析确定各条件对层和时间步的响应位置，仅将每个条件路由到最敏感位置以缩短活动token序列；设计文本特征增强模块（TFEM）整合字形图像和裁剪特征以提升中文文字渲染准确性；构建首个同时包含主题、文字和风格条件的高质量电商产品海报数据集。

Result: 实验表明InnoAds-Composer在显著优于现有产品海报生成方法的同时，没有明显增加推理延迟。

Conclusion: InnoAds-Composer通过高效的三重条件控制和文本特征增强，解决了电商产品海报生成中的关键问题，为单阶段多条件图像合成提供了有效解决方案。
...
TL;DR: 论文提出三种方法来减少概念瓶颈模型中的偏见，包括使用top-k概念过滤器减少信息泄露、移除偏见概念和对抗性去偏见，在公平性与性能的权衡上优于先前工作。


<details>
  <summary>Details</summary>
Motivation: 概念瓶颈模型虽然理论上通过将图像映射到人类可解释的概念来增强可解释性和公平性，但实际中概念会泄露与概念语义无关的信息，且在性别偏见等敏感属性上的减少效果有限。

Method: 提出三种偏见缓解技术：1. 使用top-k概念过滤器减少信息泄露；2. 移除偏见概念；3. 应用对抗性去偏见方法。

Result: 提出的去偏见CBM在公平性与性能的权衡上优于先前工作，为公平且可解释的图像分类提供了重要进展。

Conclusion: 通过结合三种偏见缓解技术，改进的概念瓶颈模型在保持可解释性的同时显著提高了公平性，是迈向公平且可解释图像分类的重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 无人机图像中的小目标检测面临尺度变化、结构细节退化以及计算资源有限的挑战。在高空场景中，细粒度特征在分层下采样和跨尺度融合过程中进一步减弱，导致定位不稳定和鲁棒性降低。
...
Conclusion: CollabOD通过协同设计策略有效解决了无人机图像中小目标检测的关键问题，在保持轻量化的同时提升了检测性能。
...
TL;DR: 论文提出Art3D框架，将2D转3D从几何精度转向艺术性合成，通过双路径架构分离全局深度参数和局部艺术效果，学习专业3D电影数据


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽然几何准确但缺乏艺术性，无法复制专业3D电影的沉浸感和情感共鸣，因为它们将艺术意图误认为数据噪声或歧义

Method: 提出Art3D框架，采用双路径架构解耦全局深度参数（宏观意图）和局部艺术效果（视觉笔触），通过间接监督从专业3D电影数据中学习

Result: 实验表明该方法在复制关键局部出屏效果和与电影3D内容的全局深度风格对齐方面显示出潜力

Conclusion: 为艺术驱动的转换工具新类别奠定了基础，将2D转3D从物理精确的视差估计转向艺术连贯的视差合成
...
TL;DR: Pano3DComposer是一个高效的前馈框架，用于从全景图像生成3D场景，通过解耦对象生成和布局估计，使用Object-World Transformation Predictor模块将现成的图像到3D模型生成的对象从局部坐标转换到世界坐标，并在约20秒内生成高保真3D场景。
...
Method: 设计了Pano3DComposer框架，包含两个核心技术：1) Object-World Transformation Predictor模块，将现成图像到3D模型生成的对象从局部坐标转换到世界坐标，使用Alignment-VGGT架构，结合目标对象裁剪、多视角对象渲染和相机参数来预测变换；2) 针对未见域输入图像的Coarse-to-Fine对齐机制，通过场景渲染反馈迭代优化几何一致性。使用伪几何监督训练以解决生成对象与真实对象之间的形状差异。

Result: 在合成和真实世界数据集上的图像/文本到3D任务中取得了优越的几何精度，能够在RTX 4090 GPU上约20秒内生成高保真3D场景，显著提升了生成效率和场景完整性。

Conclusion: Pano3DComposer通过解耦对象生成和布局估计，结合Object-World Transformation Predictor和Coarse-to-Fine对齐机制，实现了高效、高质量的全景图像到3D场景生成，解决了现有方法的效率低下和视场限制问题。
...
TL;DR: 该论文提出了CORE-Seg框架，通过语义引导提示适配器将推理与分割相结合，在ComLesion-14K基准上实现了最先进的复杂病灶分割性能。
...
Method: 提出了CORE-Seg端到端框架，通过语义引导提示适配器集成推理与分割。设计了从SFT到GRPO的渐进训练策略，配备自适应双粒度奖励机制以缓解奖励稀疏性问题。

Result: 在ComLesion-14K基准上实现了37.06%的平均Dice分数（比第二名基线高14.89%），同时将失败率降低到18.42%。

Conclusion: 该研究通过推理驱动的复杂病灶分割方法，成功填补了通用MLLMs和传统分割模型之间的能力差距，为医学图像分析提供了新的认知推理范式。
...
TL;DR: BlackMirror是一个用于黑盒设置下检测文本到图像模型中后门的框架，通过分析语义偏差的稳定性来检测视觉多样化的后门攻击。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖图像级相似性分析，假设后门触发的生成具有强一致性，但难以应对新兴的视觉多样化后门攻击。

Method: BlackMirror包含两个组件：MirrorMatch（对齐视觉模式与指令以检测语义偏差）和MirrorVerify（评估这些偏差在不同提示下的稳定性以区分真正后门行为）。

Result: 综合实验表明BlackMirror在多种攻击类型下都能实现准确检测，是一个通用的、无需训练的即插即用框架。

Conclusion: BlackMirror通过分析语义偏差的稳定性，有效解决了视觉多样化后门攻击的检测问题，适用于模型即服务应用。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建-生成差距问题，解码路径不够灵活，且需要独立的编码器和解码器，参数效率不高。RAC旨在通过Rectified Flow的直线可校正路径和双向推理能力来解决这些问题。

Method: RAC采用Rectified Flow框架：1）在流时间步上应用解码器实现多步解码，路径直线且可校正；2）通过时间反转实现双向推理，解码器同时作为编码器；3）生成式解码方法允许沿路径校正潜在变量。

Result: 实验表明RAC在重建和生成质量上均超越SOTA VAE，同时计算成本降低约70%，参数数量减少近41%。

Conclusion: RAC通过Rectified Flow框架成功替代传统VAE，实现了更高效、更高质量的重建和生成，部分解决了重建-生成差距问题，为生成模型提供了新的架构思路。
...
TL;DR: RAID数据集：用于驾驶员风险感知研究的大规模视频数据集，包含4,691个标注视频片段，涵盖多样化交通场景，支持风险对象识别和行人注意力分析。


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞智能车辆系统需要理解驾驶员的风险感知过程，这是一个复杂的认知过程，涉及驾驶员对外部刺激的自愿反应以及周围道路使用者对自车的关注度。目前缺乏专门用于驾驶员风险感知研究的大规模数据集。

Method: 1. 构建RAID数据集：包含4,691个标注视频片段，涵盖多样化交通场景，标注内容包括驾驶员意图操作、道路拓扑、风险情况、驾驶员反应和行人注意力。2. 提出弱监督风险对象识别框架：通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。3. 分析行人注意力在风险评估中的作用。

Result: 实验评估表明，该方法在RAID和HDDS数据集上分别比现有最先进方法性能提升20.6%和23.1%。

Conclusion: RAID数据集为驾驶员风险感知和上下文风险评估研究提供了有价值的资源，提出的弱监督风险对象识别框架在性能上显著优于现有方法，行人注意力分析进一步增强了风险评估能力。
...
Conclusion: TAR-ViTPose通过有效的时序聚合和恢复机制，显著提升了视频中2D人体姿态估计的性能，证明了利用时序信息对提升姿态估计鲁棒性的重要性
...
TL;DR: 提出了一种前馈三角形基元生成框架，能够从多视角图像直接预测连续三角形表面，实现单次前向传播即可生成仿真就绪的3D模型，无需逐场景优化或后处理。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS方法虽然渲染质量高，但依赖耗时的逐场景优化，限制了实时部署；而新兴的前馈高斯泼溅方法效率提升但缺乏显式流形几何结构，无法直接用于仿真。

Method: 提出了一个前馈框架，包含像素对齐的三角形生成模块，并引入相对3D点云监督来增强几何学习的稳定性和一致性，直接从标定多视角图像预测连续三角形表面。

Result: 实验表明该方法能够实现高效重建，同时保持与标准图形和机器人仿真器的无缝兼容性，生成仿真就绪的模型。

Conclusion: 该方法通过前馈三角形基元生成，克服了现有方法的局限性，实现了单次前向传播即可生成仿真就绪的3D模型，为机器人和仿真应用提供了高效解决方案。
...
TL;DR: 提出OD-RASE框架，通过检测导致交通事故的道路结构并连接基础设施改进，提升自动驾驶系统安全性。使用本体论和视觉语言模型自动生成改进方案，构建新数据集。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统在处理罕见情况或复杂道路结构时仍有局限。现有道路基础设施为人类驾驶员设计，安全改进通常在事故发生后才进行，这种被动方法对需要主动风险缓解的自动驾驶系统构成挑战。

Method: 1. 基于道路交通系统专业知识形式化本体论；2. 使用大规模视觉语言模型生成基础设施改进方案；3. 通过本体驱动数据过滤提高可靠性；4. 在事故前道路图像上自动标注改进方案构建新数据集；5. 提出OD-RASE基线方法，结合视觉语言模型和扩散模型生成改进方案和改进后道路环境图像。

Result: 实验表明，本体驱动数据过滤能够高精度预测导致事故的道路结构和相应改进计划。该方法有助于提升整体交通安全环境。

Conclusion: 这项工作有助于提升交通环境整体安全性，是推动自动驾驶系统更广泛应用的重要一步。通过主动识别和解决道路基础设施问题，为自动驾驶系统提供更安全的环境。
...
Method: 在2D离散傅里叶幅度谱上应用自适应径向投影来提取文档图像的主导倾斜角度，同时创建了DISE-2021数据集用于评估不同估计器的性能。

Result: 提出的方法表现出鲁棒性和可靠性，在所有对比方法中表现最优，源代码已在GitHub上公开。

Conclusion: 该方法为文档倾斜估计提供了有效的解决方案，并通过高质量数据集和综合分析推动了傅里叶基方法的发展。
...
TL;DR: LucidNFT：基于多奖励强化学习的流匹配真实图像超分辨率框架，通过Lucidity一致性评估器、解耦优势归一化和大规模真实退化数据集，解决真实图像超分辨率中的语义幻觉和优化瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 真实图像超分辨率（Real-ISR）存在语义和结构幻觉问题，即生成结果虽然视觉上清晰但可能不忠实于低分辨率输入证据。现有偏好强化学习方法面临三个主要挑战：缺乏退化鲁棒的低分辨率参考忠实度信号、rollout组优化瓶颈导致优势崩溃、以及真实退化数据覆盖有限。

Method: 提出LucidNFT框架：1）LucidConsistency - 退化鲁棒的语义评估器，使低分辨率锚定的忠实度可测量和优化；2）解耦优势归一化策略 - 在融合前保留每个低分辨率条件下的rollout组内目标间对比，防止优势崩溃；3）LucidLR - 大规模真实世界退化图像集合，支持鲁棒的强化学习微调。

Result: 实验表明，LucidNFT能够持续改进基于流的真实图像超分辨率基线方法，在多样化的真实世界场景中实现更好的感知-忠实度权衡，并保持稳定的优化动态。

Conclusion: LucidNFT通过解决真实图像超分辨率中的语义幻觉和优化瓶颈问题，为基于流匹配的真实图像超分辨率提供了有效的多奖励强化学习框架，实现了更好的感知质量和输入忠实度平衡。
...
<details>
  <summary>Details</summary>
Motivation: 少样本分割在处理大外观或视角变化时面临结构错位和跨视角不一致的挑战，需要同时解决结构一致性和前景模糊性问题。

Method: 1) 在骨干特征上构建空间-视角图：空间图捕捉局部几何拓扑，视角图连接不同视角特征以传播视角不变的结构语义；2) 从支持-查询特征差异中提取判别性先验，通过重加权SAM特征强调显著区域并重新校准骨干激活；3) 通过掩码交叉注意力逐步集成前景增强的SAM特征和结构丰富的ResNet特征，生成类别一致原型作为SAM解码器的自适应提示。

Result: 在多个少样本分割基准测试上的广泛实验验证了VINE的有效性和鲁棒性，特别是在具有视角偏移和复杂结构的挑战性场景中表现优异。

Conclusion: VINE通过联合建模结构一致性和前景判别性，成功解决了少样本分割中的结构错位和跨视角不一致问题，为处理视角变化和复杂结构场景提供了有效的统一框架。
...
TL;DR: OVGGT是一个训练无关的框架，通过自选择缓存和动态锚点保护技术，在固定内存和计算预算下处理任意长度的视频流，实现3D几何重建。


<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全注意力机制导致二次计算成本，只能处理短序列；因果注意力变体如StreamVGGT虽然支持单次流式处理，但KV缓存会无限增长，GPU内存很快耗尽，无法实现真正意义上的长序列流式推理。

Method: 1. 自选择缓存：利用FFN残差幅度来压缩KV缓存，同时保持与FlashAttention完全兼容；2. 动态锚点保护：保护坐标关键令牌不被驱逐，以抑制在扩展轨迹上的几何漂移。

Result: 在室内、室外和超长序列基准测试中，OVGGT能够在固定的VRAM限制内处理任意长度的视频，同时实现最先进的3D几何精度。

Conclusion: OVGGT框架通过结合自选择缓存和动态锚点保护，成功解决了流式视频3D重建中的内存和计算瓶颈，实现了真正意义上的长序列流式推理。
...
Method: 采用两阶段策略：先进行目标分割，然后识别。裁剪目标区域后，使用CLIP生成图像和文本嵌入，或引入CNN/MLP方法提取特征并与文本嵌入对齐。通过SVD构建共享表示空间，最后通过嵌入相似度匹配进行识别。

Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无需训练、基于CLIP的编码（无SVD）获得了最高的平均AP，超越了当前最先进方法。同时验证了CNN/MLP图像编码在开放词汇目标识别中的潜力。

Conclusion: 提出的两阶段开放词汇目标识别框架有效解决了现有方法的局限性，无需复杂重训练和标注，在多个数据集上表现出色，为开放词汇识别提供了简洁高效的解决方案。
...
TL;DR: S2I方法将3D人体骨架序列转换为图像格式，使预训练视觉模型能用于自监督骨架表示学习，解决了骨架数据格式差异和大规模数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉模型难以直接应用于3D骨架数据，因为数据格式存在根本差异。同时，大规模骨架数据集稀缺，且需要在不增加额外模型分支的情况下将骨架数据融入多模态动作识别。

Method: 提出Skeleton-to-Image Encoding (S2I)表示方法，通过基于身体部位语义对关节进行分区排列，并将骨架序列转换为标准图像尺寸的图像格式数据。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上的实验表明，该方法在自监督骨架表示学习中表现出有效性和泛化能力，包括在跨格式评估的挑战性设置下。

Conclusion: S2I表示首次实现了使用预训练视觉模型进行自监督骨架表示学习，将丰富的视觉域知识转移到骨架分析中，并为异构骨架数据提供了统一的图像格式。
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测模型虽然能够通过视觉-语言对齐检测新类别，但模型规模庞大难以部署在资源受限设备上。量化虽然提供压缩方案，但极端低比特量化会严重破坏细粒度视觉-语言对齐并扭曲区域间关系结构。

Method: 提出CR-QAT框架，包含两个核心组件：1）课程量化感知训练（CQAT），通过分阶段模型分区进行渐进量化，实现误差隔离和稳定优化；2）文本中心关系知识蒸馏（TRKD），针对任务相关模块，通过构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在极端低比特设置下持续优于现有QAT基线方法，分别实现了相对AP提升高达38.9%和40.9%。

Conclusion: CR-QAT框架有效解决了开放词汇目标检测模型在极端量化下的性能退化问题，通过课程式训练和关系知识蒸馏的结合，在保持模型压缩的同时显著提升了检测性能。
...
Conclusion: 通过将变化建模从静态图像比较重构为动态过程建模，ProCap能够更好地理解和描述变化过程，为变化描述任务提供了新的视角和方法。
...
TL;DR: DynUAV是一个新的无人机视角多目标跟踪基准，专门针对无人机快速运动和敏捷机动带来的挑战，包含42个视频序列和170万个边界框标注，涵盖车辆、行人和工业设备等类别。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角多目标跟踪基准缺乏真实世界中的复杂性，主要包含可预测的相机动态和线性运动模式，无法反映无人机快速运动和敏捷机动带来的显著观测挑战。

Method: 创建了DynUAV基准，包含42个视频序列，超过170万个边界框标注，涵盖车辆、行人和工业设备（挖掘机、推土机、起重机）等类别。该基准以强烈的自我运动和由此产生的复杂表观轨迹为特征。

Result: 与现有基准相比，DynUAV引入了由自我运动带来的重大挑战，包括剧烈的尺度变化、视角变化和运动模糊。对最先进跟踪器的综合评估揭示了它们在处理这种动态条件下的检测和关联交织挑战时的局限性。

Conclusion: DynUAV建立了一个严格的基准，预计将作为苛刻的测试平台，推动真实世界无人机视角多目标跟踪的进展。所有资源将在指定链接上提供。
...
TL;DR: 该报告探讨了巴基斯坦制造的外科器械及其制造缺陷，通过深度学习架构（YOLOv8、ResNet-152、EfficientNet-b4）开发自动缺陷检测方法，使用4,414张高分辨率图像数据集进行质量保证分析。


<details>
  <summary>Details</summary>
Motivation: 现代医疗中外科器械的质量标准对临床成功至关重要，即使是微小缺陷也可能导致致命后果。巴基斯坦作为外科器械制造中心，需要提升制造精度和质量控制，以防止经济损失并保障患者安全。

Method: 与Sialkot外科器械集群的行业领导者合作，收集4,414张高分辨率图像数据集，采用深度学习架构（YOLOv8、ResNet-152、EfficientNet-b4）进行自动缺陷检测，通过自动光学检测（AOI）工具分析裂纹、锈蚀和结构异常等关键缺陷。

Result: 开发了基于深度学习的自动缺陷检测方法，能够有效识别外科器械的各种制造缺陷。该方法为制造商、医疗专业人员和监管机构提供了质量保证工具，有助于提升巴基斯坦制造外科器械的质量标准。

Conclusion: 通过整合行业专业知识和前沿技术，特别是深度学习架构，能够显著提升外科器械制造的质量标准。自动缺陷检测方法有助于创建更可靠的医疗环境，保障患者安全并减少制造商的经济损失。
...
<details>
  <summary>Details</summary>
Motivation: 现有不规则采样时间序列预测方法仅使用历史观测数据，无法学习上下文语义和细粒度时间模式，需要更全面的多模态方法来提升预测性能。

Method: 提出两阶段编码机制：1)跨模态视觉-文本编码模块自动生成信息丰富的视觉图像和文本数据；2)ISTS编码提取历史观测的补充时间特征。还包括自适应查询特征提取器和多模态对齐模块。

Result: 在真实数据上的广泛实验验证了所提解决方案的有效性，表明MM-ISTS能够显著提升不规则采样时间序列的预测性能。

Conclusion: MM-ISTS通过整合时间、视觉和文本模态，利用多模态大语言模型的能力，成功解决了不规则采样时间序列预测中上下文语义和细粒度时间模式学习不足的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度基础模型在透视图像上训练表现良好，但在360°图像上泛化能力差，因为透视域和全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据，这在实际应用中成本高昂。
...
Result: RePer-360仅使用1%的训练数据就超越了标准微调方法。在相同域内训练设置下，RMSE指标提升了约20%。该方法在保留预训练透视先验的同时，有效适应了全景域。
...
<details>
  <summary>Details</summary>
Motivation: 传统CNN和Transformer在遥感图像分类中通常作为不可解释的黑盒模型，缺乏对分类决策的物理可解释性，这限制了其在需要可解释性的地球观测应用中的实用性。
...
TL;DR: EffectMaker是一个统一的推理-生成框架，通过多模态大语言模型和扩散变换器实现基于参考视频的视觉特效定制，无需针对每种特效进行微调。


<details>
  <summary>Details</summary>
Motivation: 当前视觉特效制作需要专业知识和高成本生产流程，现有AIGC系统面临特效特定数据稀缺、超自然或风格化效果建模困难的问题，且通常需要针对每种特效进行微调，限制了系统的可扩展性和泛化能力。

Method: 采用语义-视觉双路径引导机制：1）多模态大语言模型解释高级特效语义并推理如何适应目标主体；2）扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索。同时构建了包含130k视频、3k特效类别的EffectData数据集。

Result: EffectMaker在视觉质量和特效一致性方面优于现有最先进基线方法，提供了可扩展且灵活的定制视觉特效生成范式。

Conclusion: EffectMaker通过统一的推理-生成框架解决了视觉特效定制中的关键挑战，无需针对每种特效进行微调，实现了准确、可控且特效一致的合成，为定制化视觉特效生成提供了可扩展的解决方案。
...
TL;DR: MOSIV：首个针对视频中多物体系统识别的框架，通过可微分模拟器直接优化连续物体材料参数，超越现有单物体方法


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注单物体场景或基于固定材料原型的离散分类，无法处理视频中多物体交互的系统识别问题

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续物体材料参数，通过视频几何目标进行指导

Result: 在新合成基准测试中，MOSIV显著提升接地精度和长时程模拟保真度，成为该新任务的强基线

Conclusion: 物体级细粒度监督和几何对齐目标对于复杂多物体场景的稳定优化至关重要，MOSIV为多物体系统识别提供了有效解决方案
...
TL;DR: StruVis是一个新颖的文本到图像生成框架，通过结构化视觉表示增强推理能力，避免了传统方法的计算成本高和视觉限制问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成推理框架存在两个主要问题：纯文本推理缺乏视觉上下文导致关键空间和视觉元素缺失；文本-图像交错推理虽然增强视觉基础但计算成本高且受生成器表示能力限制。
...
Conclusion: StruVis作为一个生成器无关的推理框架，能够无缝集成到不同的文本到图像生成器中，高效提升其在推理型文本到图像生成中的性能。
...
Method: 提出OA-SORT框架，包含三个核心组件：遮挡感知模块（OAM）使用高斯地图减少背景影响分析遮挡状态；遮挡感知偏移（OAO）利用OAM描述的遮挡状态缓解成本混淆；偏差感知动量（BAM）抑制估计不稳定性。

Result: 在DanceTrack测试集上，OA-SORT达到63.1% HOTA和64.2% IDF1。将遮挡感知框架集成到其他四个跟踪器中，平均提升HOTA 2.08%和IDF1 3.05%，证明了框架的可重用性。

Conclusion: 遮挡处理对多目标跟踪至关重要，OA-SORT框架通过创新的遮挡感知机制有效解决了2D MOT中的遮挡问题，且具有良好的可重用性和即插即用特性。
...
TL;DR: 该论文提出使用分层子采样和集成学习来处理VGG16超列特征，以降低计算复杂度并提高脑肿瘤分割性能，特别是在小样本情况下。


<details>
  <summary>Details</summary>
Motivation: 受生物视觉启发的高维超列特征虽然可用于图像分割，但由于计算复杂度随训练集大小线性增长，实际应用较少。本文旨在解决这一计算挑战。

Method: 对VGG16生成的超列特征应用分层子采样，并研究在稀疏超列上的集成学习方法（堆叠和投票集成）。

Result: 在脑肿瘤数据集上，当N≤20的极低样本情况下，简单的逻辑回归分类器最有效。在10%分层子采样率下，N=20时的最佳平均Dice得分为0.66，相比标准多尺度UNet基线有24.53%的显著改进。

Conclusion: 分层子采样和集成学习能有效处理超列特征的计算复杂度问题，在小样本脑肿瘤分割任务中表现出色，特别是在极低样本情况下，简单分类器优于复杂模型。
...
TL;DR: 提出FontUse数据集和标注流程，通过数据驱动方法改进文本到图像模型中的排版控制，使模型能更准确地根据用户友好的提示生成符合要求的字体样式和应用场景


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在排版控制方面存在不足，用户请求的排版外观经常被忽略或仅被弱遵循，需要改进模型对字体样式和应用场景的响应能力

Method: 采用数据驱动方法，构建专门针对排版的大规模数据集FontUse（约7万张图像），使用分割模型和多模态大语言模型自动标注用户友好提示、文本区域位置和OCR识别字符串，通过微调现有生成器来改进排版控制

Result: 实验表明，使用该流程训练的模型在多样化的提示和布局下，生成的文本渲染比竞争基线更符合提示要求，并引入了基于Long-CLIP的评估指标来衡量排版与请求属性的一致性

Conclusion: 提出的数据驱动方法和FontUse数据集有效提升了文本到图像模型对排版控制的准确性，使模型能够一致地解释样式和使用场景条件，无需架构修改即可实现更好的排版生成
...
TL;DR: 提出GvU方法，通过自监督强化学习框架，利用统一多模态模型的理解能力来提升其生成质量，缩小理解与生成之间的能力差距。
...
Method: 提出GvU方法：1) 基于token级别的内在文本-图像对齐奖励机制，让模型同时作为教师和学生；2) 设计自监督强化学习框架，通过理解分支评估自身输出并指导生成；3) 无需外部监督，利用内在奖励信号迭代提升生成质量。

Result: 实验结果表明，该方法显著提升了统一多模态模型的生成能力，同时增强了其细粒度视觉理解能力，缩小了视觉理解与生成之间的能力差距。

Conclusion: 通过利用统一多模态模型内部的理解能力来指导生成过程，可以有效提升生成质量并缩小理解与生成之间的能力差距，实现理解与生成的协同增强。
...
<details>
  <summary>Details</summary>
Motivation: 当前手-物交互视频合成面临两大挑战：现有HOI重演方法局限于域内训练，难以泛化到复杂野外场景；而通用视频编辑模型虽然鲁棒性更强，但在HOI特定问题上（如对象外观不一致）表现不佳。

Method: 提出GenHOI框架，包含两个核心技术：1) Head-Sliding RoPE：通过为参考令牌分配头部特定的时间偏移，均匀分布其跨帧影响，缓解3D RoPE的时间衰减问题；2) 两级空间注意力门：将对象条件注意力集中在HOI区域，并自适应调整强度，保持背景真实性的同时增强交互保真度。

Result: 在未见过的野外场景上进行广泛定性和定量评估，GenHOI在物体一致性、交互保真度和背景保持方面显著优于当前最先进的HOI重演方法和通用视频编辑方法。

Conclusion: GenHOI通过轻量级增强预训练视频生成模型，有效解决了复杂野外场景中手-物交互视频合成的关键挑战，在保持背景真实性的同时显著提升了交互保真度和对象一致性。
...
TL;DR: Curious-VLA框架通过两阶段设计解决VLA模型的探索限制问题：IL阶段使用可行轨迹扩展策略生成多样化数据，RL阶段采用自适应多样性感知采样和跨度驾驶奖励来提升探索潜力，在Navsim基准上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 本文发现自主VLA模型存在一个根本性的窄策略限制：驾驶模仿学习倾向于使探索崩溃，限制了后续强化学习阶段的潜力，导致RL阶段因反馈多样性不足而过早饱和。

Method: 提出Curious-VLA两阶段框架：1) IL阶段：引入可行轨迹扩展策略生成多个物理有效轨迹，使用逐步归一化轨迹表示来适应多样化数据；2) RL阶段：提出自适应多样性感知采样优先选择高多样性样本，引入跨度驾驶奖励和焦点风格加权来扩大奖励值跨度，提高对驾驶质量的敏感性。

Result: 在Navsim基准测试中，Curious-VLA实现了SOTA结果（PDMS 90.3，EPDMS 85.4），Best-of-N PDMS达到94.8，证明了其在释放VLA模型探索潜力方面的有效性。

Conclusion: Curious-VLA通过解决VLA模型中的探索-利用困境，有效提升了自主驾驶模型的性能，为视觉语言动作模型的探索潜力开发提供了有效框架。
...
TL;DR: 研究通过分析视觉语言模型在自动驾驶场景中的中间激活，发现模型在简单视觉问题上存在两种失败模式：感知失败（视觉信息未线性编码）和认知失败（信息存在但语义对齐失败），且物体距离增加会降低视觉概念的线性可分性。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶应用中越来越普遍，旨在利用其推理和泛化能力处理长尾场景。然而，这些模型经常在自动驾驶高度相关的简单视觉问题上失败，且失败原因尚不清楚。

Method: 创建仅目标视觉概念不同的反事实图像集，使用四种SOTA视觉语言模型的激活训练线性探针，评估特定视觉概念是否被线性编码，并分析视觉信息流动瓶颈。

Result: 物体存在等概念被显式线性编码，而物体方向等空间概念仅通过视觉编码器保留的空间结构隐式编码。即使概念被线性编码，模型仍可能回答错误，识别出感知失败和认知失败两种失败模式。物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 研究提高了对视觉语言模型在自动驾驶相关简单视觉任务中失败案例的理解，揭示了视觉信息编码的局限性，为改进模型在自动驾驶应用中的性能提供了见解。
...
Method: 采用参考条件的潜在扩散框架，通过教师-学生蒸馏方法：使用标准噪声预测目标训练的扩散教师指导轻量级学生去噪器，实现更少推理步骤；结合身份锚定和时间正则化减少身份漂移和闪烁；使用视素音频条件提供粗略唇部运动控制。

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强大教师的大部分重建行为，同时实现显著降低的延迟推理；包括CPU和边缘计算的延迟特性测量表明边缘部署的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法虽然能生成自然的面部动作，但通常只能表达固定目标情感，缺乏人类在传达信息时那种连续变化的自然表情能力

Method: 提出TIE-TFG模型，采用时序密集情感波动建模技术，根据输入文本生成对应的情感变化序列，驱动合成视频中连续的面部表情变化

Result: 广泛评估表明该方法能够产生平滑的情感过渡，在不同情感状态下保持高质量的视觉效果和运动真实性

Conclusion: EC-TFG任务和TIE-TFG模型成功解决了说话人脸生成中情感连续变化的问题，实现了更自然、更富表现力的数字人脸合成
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在图像描述和视觉问答等任务上表现出色，但往往难以准确理解视觉数据中的深度信息，这限制了它们在三维场景理解方面的能力。

Method: 1. 构建深度图像-文本对数据集和深度指令数据集：使用GLPN模型从视觉图像生成深度图，利用GPT-4生成对应的深度指令，并通过LLaVA验证；2. 改进CLIP中的ViT编码器，融入局部物体信息以更好地捕捉深度的细微连续变化；3. 开发基于现有深度图像数据集的深度问答基准测试。

Result: 实验结果表明，DeepSight显著增强了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专门用于深度感知的MLLM，通过创新的数据集构建和模型改进，有效解决了现有模型在深度信息理解方面的不足，为三维场景理解提供了新的解决方案。
...
TL;DR: FedARKS框架通过鲁棒知识提取和知识选择机制，解决联邦域泛化行人重识别中全局特征不足和均匀聚合忽略客户端差异的问题


<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征和简单平均聚合，存在两个局限：1）仅用全局特征难以捕捉细粒度域不变局部细节；2）均匀参数平均忽略客户端在鲁棒特征提取能力上的差异，稀释高质量客户端的贡献

Method: 提出FedARKS框架，包含两个机制：RK（鲁棒知识）机制提取细粒度域不变特征；KS（知识选择）机制根据客户端质量差异进行选择性知识集成

Result: 未在摘要中明确说明具体实验结果，但暗示该方法能提升模型在未见域上的泛化能力

Conclusion: FedARKS框架通过鲁棒知识提取和选择性知识集成，有效解决了联邦域泛化行人重识别中的特征表示和聚合问题
...
TL;DR: RMD是一种跨分辨率分布匹配蒸馏框架，通过logSNR曲线划分时间步区间、跨分辨率分布匹配和噪声重注入机制，实现高保真、少步数的多分辨率级联推理加速。
...
Conclusion: RMD通过跨分辨率分布匹配有效解决了现有扩散蒸馏方法的局限性，实现了高质量、高效率的多分辨率级联推理，为加速图像和视频生成提供了新思路。
...
TL;DR: Place-it-R1是一个端到端的视频物体插入框架，利用多模态大语言模型的思维链推理能力，通过"思考-放置"范式实现物理一致的视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑技术主要关注视觉保真度而忽略了物理因果关系，导致插入的物体与周围环境物理不一致。需要开发能够理解物理场景并进行环境感知推理的系统。

Method: 采用MLLM的思维链推理来协调视频扩散模型：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌并推断有效插入区域；2) 引入MLLM引导的空间直接偏好优化，通过反馈评分机制确保视觉自然性；3) 推理时MLLM迭代触发精炼循环，形成闭环优化；4) 提供两种用户可选模式：增强物理合理性的灵活模式和保持场景完整性的标准模式。

Result: 大量实验表明，Place-it-R1在物理一致性方面优于现有的最先进解决方案和商业模型，实现了物理连贯的视频物体插入。

Conclusion: 该框架通过结合MLLM的认知推理能力和扩散模型的生成能力，成功解决了视频物体插入中的物理一致性问题，为用户提供了在合理性和保真度之间的明确控制。
...
TL;DR: 论文研究了视觉语言模型在结构化色彩扭曲下的感知弱点，发现即使底层场景对人类仍可识别，模型预测也会大幅退化，而人类表现远优于模型，简单预处理可部分恢复性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在基准测试中表现优异，但存在系统性感知弱点：即使底层场景对人类仍可识别，像素值的结构化大幅变化也会导致模型产生自信但无意义的预测。研究者希望量化这种人类与模型之间的感知差距。

Method: 使用空间色彩混合方法，创建程序化色彩扭曲家族，在RGB和Ostwald色彩系统中叠加结构化图案到自然图像上。设计了8种空间色彩混合变体，评估了9个来自3个模型家族的视觉语言模型在4个数据集上的表现。还进行了61人参与的人类研究，比较人类与模型在相同扭曲下的表现。

Result: 随着扭曲程度增加，所有模型和数据集上的准确率急剧下降；扩大语言模型规模并不能可靠地缓解这种失败；在动物识别数据集上，人类在相同扭曲下的表现显著优于视觉语言模型；简单的人类启发式预处理步骤能恢复多种扭曲类型下的部分性能。

Conclusion: 视觉语言模型存在严重的感知弱点，对结构化色彩扭曲高度敏感。人类感知鲁棒性远优于当前模型。感知感知的预处理和工具使用是提高视觉语言模型鲁棒性的实用策略。
...
TL;DR: 提出虚拟治疗框架，将非小细胞肺癌放疗期间肿瘤演变建模为剂量感知的多模态条件图像翻译问题，用于合成放疗诱导的解剖变化随访CT图像。


<details>
  <summary>Details</summary>
Motivation: 预测放疗期间肿瘤演变是临床关键挑战，特别是当纵向变化同时受解剖结构和治疗影响时。现有方法难以准确模拟放疗诱导的解剖变化。

Method: 提出虚拟治疗框架，将NSCLC进展建模为剂量感知的多模态条件图像翻译问题。基于CT扫描、基线临床变量和特定辐射剂量增量，合成反映治疗诱导解剖变化的随访CT图像。使用222名III期NSCLC患者的895个CT扫描数据集，在2D和2.5D配置下对GAN和扩散模型进行基准测试。

Result: 定量和定性结果表明，基于扩散的模型能更一致地从多模态、剂量感知的条件中受益，产生比GAN基线更稳定、解剖学上更合理的肿瘤演变轨迹。

Conclusion: 虚拟治疗框架作为NSCLC治疗监测和自适应放疗研究的硅内工具具有潜力，扩散模型在模拟放疗诱导解剖变化方面表现优于GAN模型。
...
TL;DR: VLM-RobustBench基准测试显示，当前视觉语言模型对空间扰动（特别是重采样和几何变形）非常脆弱，而视觉严重程度并不能准确预测性能下降程度。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在标准高质量数据集上表现良好，但它们在真实世界图像失真下的性能尚未被充分理解，需要系统评估模型对各种图像扰动的鲁棒性。

Method: 构建VLM-RobustBench基准，包含49种增强类型（噪声、模糊、天气、数字、几何扰动），在分级严重程度（低/中/高）和二元变换下评估，共133种损坏设置。评估四个模型家族（Qwen、InternVL、Molmo、Gemma）在两个基准上：MMBench（视觉基础）和MMMU-Pro（推理导向）。

Result: 视觉严重程度是性能下降的弱预测指标：低严重度的空间扰动通常比视觉上严重的光度损坏更降低性能。低严重度的glass_blur使MMBench准确率平均下降约8个百分点，而最大下降来自重采样和几何变形（如upsample、elastic_transform），可达34个百分点。
...
TL;DR: 本文提出RF-Sampling，一种专门为流匹配模型设计的无需训练推理增强框架，通过理论推导证明其隐式执行文本-图像对齐分数的梯度上升，显著提升生成质量和提示对齐效果。
...
Method: 提出Reflective Flow Sampling (RF-Sampling)：基于理论推导的无需训练推理增强框架，利用文本表示的线性组合并结合流反演，使模型能够探索与输入提示更一致的噪声空间。

Result: 在多个基准测试中，RF-Sampling一致提升了生成质量和提示对齐效果，是首个在FLUX上展示一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling为流模型提供了有效的推理增强解决方案，通过理论推导和实验验证证明了其在提升生成质量和文本对齐方面的有效性，填补了流模型推理增强方法的空白。
...
Method: 使用预训练基础模型：1) 可提示基础分割模型提取每视角全景先验；2) 重建基础模型恢复度量三维点；3) 深度和置信度感知过滤将可靠标签提升到三维；4) 确定性细化堆栈进行时空融合和体素化；5) 通过拟合和合并鲁棒的三维框候选恢复实例。

Result: 在Occ3D-nuScenes上，无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当。作为伪标签生成管道训练下游模型时，达到21.1 RayIoU，超越先前弱监督基线。在全景占据预测上，无需训练和弱监督分别达到3.1和3.9 RayPQ。

Conclusion: FreeOcc展示了基础模型驱动的感知是实现无需训练三维场景理解的实用途径，为语义和全景占据预测建立了新的基准。
...
TL;DR: 提出了一种用于乳腺超声图像分割的半监督框架，通过免训练的伪标签生成和标签细化，在仅有2.5%标注数据下达到全监督模型性能


<details>
  <summary>Details</summary>
Motivation: 传统半监督学习在标注极度有限时存在伪标签不稳定问题，导致监督不准确和性能下降。现有视觉语言模型在医学图像上效果有限，因为领域特定提示难以迁移。

Method: 1) 利用简单外观描述实现跨域结构迁移，让视觉语言模型生成结构一致的伪标签；2) 用伪标签预热静态教师模型以捕获病灶全局结构先验；3) 结合指数移动平均教师，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习来改善边界判别能力。

Result: 在四个乳腺超声数据集上的实验表明，该方法在仅使用2.5%标注数据时就能达到与全监督模型相当的性能，显著优于现有半监督方法。
...
Method: 将RGB-D全景图像转换为切向透视图像和3D点云，利用这些模态提取和对齐基础视觉语言特征，通过自然语言查询生成两种输入模态的语义掩码。

Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D能够产生跨全景和3D域的一致且有语义意义的分割结果，相比现有技术有显著提升。

Conclusion: 提出的方法在开放和封闭词汇的2D和3D语义分割方面相比最先进技术取得了显著改进，实现了语言驱动的跨模态场景理解。
...
TL;DR: 本文提出了一种使用强化学习优化3D医学图像扩散模型的方法，通过多尺度反馈机制提升生成图像质量
...
Method: 先预训练3D扩散模型建立生成先验，然后使用PPO算法进行微调，结合2D切片评估和3D体积分析的多尺度奖励系统

Result: 在BraTS 2019和OASIS-1数据集上验证，FID显著改善，合成数据在下游肿瘤和疾病分类任务中表现出更好的实用性

Conclusion: 强化学习反馈能有效引导生成过程向更高质量分布发展，提升3D医学图像扩散模型的临床相关性
...
TL;DR: 本文提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器在语义分割任务中的性能瓶颈问题，使生成能力更好地转化为分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有的训练免费扩散分割方法通常依赖交叉注意力图来捕捉图像像素和文本标记之间的语义关系，理论上更强的生成模型应该带来更好的分割性能。但实际观察发现现有方法未能有效随模型能力提升而扩展，主要存在两个问题：1）跨多个头和层的交叉注意力图与统一全局表示之间存在差异；2）即使有全局图，由于不同文本标记的得分不平衡，也不能直接转化为准确的分割语义相关性。

Method: 提出两种核心技术：1）自动聚合（auto aggregation）：解决多个注意力头和层之间的表示差异问题；2）逐像素重缩放（per-pixel rescaling）：解决不同文本标记得分不平衡问题，使生成能力更好地转化为分割性能。

Result: 在标准语义分割基准上评估了该方法，并进一步将其集成到生成技术中，展示了改进的性能和广泛的适用性。

Conclusion: 通过自动聚合和逐像素重缩放两种技术，成功解决了训练免费扩散分割器在利用生成模型能力时的瓶颈问题，使分割性能能够更好地随生成模型能力的提升而扩展。
...
TL;DR: 提出两阶段框架解决字形相似度度量问题：第一阶段在有标签人造字母表上训练对比学习编码器，第二阶段通过师生蒸馏扩展到历史文字系统，实现无监督表征学习。


<details>
  <summary>Details</summary>
Motivation: 字形和文字系统的相似度度量面临根本挑战：人造字母表中的单个字素可以可靠标记，但不同文字系统之间的历史关系不确定且存在争议。

Method: 两阶段框架：1) 在有标签的人造字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；2) 通过师生蒸馏扩展到历史文字系统，学生在教师知识指导下学习无监督表征，但可以自由发现潜在的跨文字相似性。

Result: 在多样化的文字系统上实验表明，该方法实现了有效的少样本字形识别和有意义的文字聚类，无需真实进化关系作为监督信号。
...
<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的逐帧标注且对模糊动作边界敏感，而点监督方法可以显著减少标注成本，同时保持较好的分割性能。

Method: 使用多模态骨架数据（关节、骨骼、运动信息），通过预训练统一模型提取特征；提出原型相似性方法，并与能量函数和约束K-Medoids聚类集成生成伪标签；采用多模态伪标签集成指导模型训练。
...
TL;DR: VG3S是一个利用视觉基础模型的几何先验来增强3D高斯溅射占用预测的新框架，在nuScenes基准上显著提升了性能。
...
Method: 提出VG3S框架，通过即插即用的分层几何特征适配器，将冻结的视觉基础模型的丰富3D几何先验转化为适合占用预测的特征表示。适配器通过特征聚合、任务特定对齐和多尺度重组来有效转换通用VFM标记。

Result: 在nuScenes占用基准上的实验表明，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。该框架能够无缝泛化到不同的视觉基础模型，持续提升占用预测精度。

Conclusion: VG3S证明了将预训练的几何基础视觉基础模型的先验知识整合到高斯溅射占用预测中的巨大价值，为自动驾驶场景理解提供了更强大的解决方案。
...
TL;DR: CoE是一个无需训练的多模态摘要框架，通过事件链和层次事件图进行结构化推理，在8个数据集上显著超越现有视频CoT基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：依赖领域特定监督、隐式融合导致跨模态关联弱、平面时间建模缺乏事件转换。需要解决这些限制以提升摘要质量。

Method: 提出CoE框架，基于层次事件图（HEG）进行结构化推理，通过事件链指导跨模态定位、建模事件演化和因果转换，并使用轻量级风格适应进行领域对齐。

Result: 在8个多样化数据集上的实验表明，CoE一致优于最先进的视频CoT基线方法，平均提升+3.04 ROUGE、+9.51 CIDEr和+1.88 BERTScore。

Conclusion: CoE框架通过显式的事件层次结构和结构化推理，解决了多模态摘要中的关键挑战，展现了强大的鲁棒性、可解释性和跨领域泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 标准3DGS产生的高斯中心与底层物体几何对齐不佳，而专注于表面的重建方法常牺牲光度精度。需要一种既能保持几何精度又能保证渲染质量的方法。

Method: 提出基于特征值熵的几何感知策略，从每个高斯中心k近邻的协方差矩阵特征值计算3D形状特征Eigenentropy，量化局部结构有序度。在交替优化框架中结合标准梯度致密化和特征值熵感知致密化，优先在低熵（有序、平坦）区域致密化以捕捉表面细节，在高熵（无序、球形）区域修剪。

Result: 在DTU和TUM2TWIN数据集上的实验表明，该方法几何精度提升达33%，渲染质量提升达7%，高斯数量减少达50%，训练时间减少达23%。

Conclusion: EntON通过避免不必要的场景扩展，在几何精度、渲染质量和效率之间实现了良好的平衡。
...
TL;DR: WAFL将时间伪造定位从回归和连续检测转变为离散词级二分类，通过特征对齐、预训练模型适配和不对称损失函数，显著提升定位性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有时间伪造定位方法存在特征粒度不对齐和计算成本高的问题，需要一种更高效、更准确的定位范式。

Method: 提出词锚定时间伪造定位(WAFL)：1) 将伪造定位转为离散词级二分类；2) 引入法证特征重对齐模块适配预训练模型；3) 设计伪影中心不对称损失处理类别不平衡。

Result: WAFL在数据集内和跨数据集设置下均显著优于现有方法，同时需要更少的可学习参数并保持高计算效率。

Conclusion: WAFL通过范式转变、特征对齐和不对称损失设计，为时间伪造定位提供了高效准确的解决方案，在性能和效率上均取得突破。
...
TL;DR: TaPD是一个用于自动驾驶轨迹预测的观测自适应框架，通过渐进知识蒸馏和时序回填处理可变历史长度观测，显著提升短观测条件下的预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在实际场景中（如遮挡或感知范围限制）观测长度可变或极短，导致性能显著下降。需要一种能够自适应不同观测长度的统一框架。

Method: 提出TaPD框架，包含两个协同模块：观测自适应预测器（OAF）和时序回填模块（TBM）。OAF基于渐进知识蒸馏（PKD），通过层次特征回归将长时观测"教师"的运动模式知识迁移到短时观测"学生"，并引入余弦退火蒸馏权重平衡预测监督和特征对齐。TBM在极短历史条件下，基于场景演化回填缺失的历史片段，生成上下文丰富的轨迹以增强PKD。

Result: 在Argoverse 1和Argoverse 2数据集上的广泛实验表明，TaPD在所有观测长度下均优于强基线方法，在极短输入条件下提升尤其显著，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD提供了一个统一的即插即用框架，通过渐进知识蒸馏和时序回填有效处理可变历史长度的轨迹预测问题，显著提升了短观测条件下的预测性能，在实际自动驾驶场景中具有重要应用价值。
...
TL;DR: HCF-RES提出多模态框架解决3D-GRES问题，通过分层视觉语义分解和渐进多级融合，在稀疏点云基础上引入2D视觉语义信息，提升细粒度描述下的目标定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D-GRES方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理匹配多个目标或零目标的细粒度自然语言描述。

Method: 提出HCF-RES多模态框架：1) 分层视觉语义分解，利用SAM实例掩码指导CLIP编码，获取像素级和实例级特征；2) 渐进多级融合，通过模态内协作、跨模态自适应加权和语言引导细化整合2D语义与3D几何特征。

Result: 在ScanRefer和Multi3DRefer数据集上取得最先进的结果。

Conclusion: HCF-RES通过有效整合2D视觉语义和3D几何信息，显著提升了3D-GRES的性能，特别是在处理复杂自然语言描述时表现优异。
...
TL;DR: NOVA提出了一种新的开放词汇3D多目标跟踪范式，通过将轨迹建模为结构化时空语义序列，利用大语言模型的自回归能力进行下一步序列补全，显著提升了未知目标的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D多目标跟踪方法受限于闭集假设和"语义盲"启发式方法，无法有效处理开放世界中的未知目标。需要一种能够泛化到未知目标的新范式。

Method: 将3D轨迹重新定义为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为基于原则的下一步序列补全过程。通过语言空间的分层结构解决细粒度语义歧义，利用高层常识推理保持复杂长序列中的身份一致性。
...
Conclusion: NOVA通过将3D跟踪从传统的基于距离的碎片化匹配转向生成式时空语义建模，为开放世界感知提供了创新解决方案，显著提升了未知目标的泛化能力。
...
TL;DR: GazeMoE：基于混合专家框架的端到端视线目标估计方法，通过选择性利用冻结基础模型中的多模态线索，结合类别平衡损失和数据增强，在基准数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 从可见图像估计人类视线目标对机器人理解人类注意力至关重要，但现有方法在泛化性神经架构和训练范式方面仍面临挑战。虽然预训练视觉基础模型为定位视线目标提供了有前景的途径，但整合眼睛、头部姿态、手势和上下文特征等多模态线索需要自适应且高效的解码机制。

Method: 提出GazeMoE框架，受大型视觉语言模型中混合专家（MoE）自适应领域专业知识的启发，通过MoE模块选择性利用冻结基础模型中的视线目标相关线索。为处理视线目标分类（框内vs框外）的类别不平衡并增强鲁棒性，结合了类别平衡辅助损失以及区域特定裁剪和光度变换等策略性数据增强。

Result: 在基准数据集上的广泛实验表明，GazeMoE实现了最先进的性能，在具有挑战性的视线估计任务上超越了现有方法。

Conclusion: GazeMoE是一个有效的端到端框架，通过自适应利用多模态线索和解决类别不平衡问题，显著提升了视线目标估计的性能，为机器人理解人类注意力提供了有力工具。
...
TL;DR: HiPP-Prune：一种分层偏好条件结构化剪枝框架，通过单一策略调用输出全局剪枝蓝图，实现视觉语言模型在任务效用、幻觉鲁棒性和压缩之间的可控权衡。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型剪枝面临挑战，因为压缩不仅影响任务效用，还会影响视觉基础能力，即使在相同稀疏度下也会加剧物体幻觉问题。

Method: 采用分层偏好条件结构化剪枝框架，将剪枝视为多目标下的条件资源分配。通过单一策略调用输出全局剪枝蓝图，决策分解为总体稀疏度预算和分层分配。策略状态整合视觉敏感性信号，防止过度剪枝视觉关键层。使用计划级组相对策略优化在多目标回报下优化剪枝计划。

Result: 在LLaVA模型上的POPE和ScienceQA实验表明，HiPP-Prune能够发现多样化的非支配剪枝计划，并在匹配的稀疏度预算下提供可控的鲁棒性-效用权衡。

Conclusion: HiPP-Prune框架通过分层偏好条件剪枝，有效解决了视觉语言模型剪枝中的多目标权衡问题，实现了任务效用、幻觉鲁棒性和压缩之间的可控平衡。
...
TL;DR: 提出StrSR框架，通过对抗蒸馏解决扩散变换器在真实图像超分辨率中的一步蒸馏难题，包含轨迹正则化和频谱正则化来消除伪影


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在真实图像超分辨率中表现出潜力，但其迭代采样计算成本高，需要一步蒸馏。现有的一步蒸馏方法在真实图像超分辨率上存在轨迹不匹配和网格状周期性伪影问题

Method: 提出StrSR框架，采用非对称判别蒸馏架构来弥合轨迹差距，并设计频率分布匹配策略来抑制由高频频谱泄漏引起的DiT特定周期性伪影

Result: 大量实验表明，StrSR在真实图像超分辨率任务中，在定量指标和视觉感知方面都达到了最先进的性能

Conclusion: StrSR通过频谱和轨迹正则化的对抗蒸馏框架，有效解决了扩散变换器在真实图像超分辨率中的一步蒸馏问题，显著提升了性能并消除了伪影
...
<details>
  <summary>Details</summary>
Motivation: 3D语义占据预测是机器人感知的核心，但真实世界的体素标注存在结构伪影和动态拖尾效应等固有噪声。这引发了一个关键但未被充分探索的问题：自主系统能否安全依赖这种不可靠的占据监督？
...
<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习面临两个内在挑战：1) 类级属性无法捕捉实例特定的视觉外观，导致类-实例差距；2) 语义与视觉特征分布不匹配，产生语义-视觉域差距。
...
Conclusion: ADiVA通过联合建模属性分布和执行显式语义-视觉对齐，有效解决了生成式零样本学习中的关键挑战，为未见类生成更准确的特征表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有无需训练的方法依赖预训练T2V模型的中间输出构建运动指导，导致计算开销大且灵活性有限。需要一种更高效灵活的运动迁移方法。
...
Conclusion: FlowMotion是一种无需训练的高效灵活视频运动迁移框架，通过直接利用光流模型预测和流指导机制，在保持性能的同时显著提升了效率。
...
<details>
  <summary>Details</summary>
Motivation: 锥束CT在牙科广泛应用，但高密度物体（如牙科植入物）会产生伪影，影响图像质量和诊断准确性。现有基于扩散模型的植入物修复方法仅处理独立的2D投影，忽略了投影间的相关性，导致重建图像不一致。
...
Conclusion: 提出的3D牙科植入物修复方法通过建模投影序列的3D分布，解决了现有2D方法忽略投影相关性的问题，为改善临床CBCT成像质量提供了有前景的解决方案。
...
TL;DR: DEX-AR是一种为自回归视觉语言模型设计的动态可解释性方法，通过生成逐标记和序列级热图来突出显示影响模型文本响应的关键图像区域。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法针对分类任务设计，难以处理现代自回归VLMs复杂的逐标记生成过程以及视觉和文本模态之间的复杂交互。

Method: DEX-AR通过计算逐标记生成过程中注意力图的层间梯度来解读自回归VLMs，包括层和生成标记的不同重要性。该方法引入了两个关键创新：1)动态头过滤机制，识别专注于视觉信息的注意力头；2)序列级过滤方法，聚合逐标记解释同时区分视觉基础和纯语言标记。

Result: 在ImageNet、VQAv2和PascalVOC上的评估显示，使用新的归一化困惑度度量的基于扰动的指标以及基于分割的指标都有一致的改进。

Conclusion: DEX-AR为自回归视觉语言模型提供了一种有效的可解释性方法，能够更好地理解这些复杂模型的决策过程，特别是在处理视觉-文本交互时的动态特性。
...
TL;DR: 提出了一种基于原型库的无监督点云语义分割方法P-SLCR，通过结构学习和一致性推理实现无标注点云场景的语义分割
...
Method: 提出P-SLCR方法：1）一致性结构学习，通过选择高质量特征在一致性点和一致性原型库之间建立结构特征学习；2）语义关系一致性推理，分别构建一致性和模糊性原型库之间的原型互关系矩阵，通过该矩阵对两种原型库施加约束以保持语义一致性

Result: 在S3DIS、SemanticKITTI和Scannet数据集上进行了广泛评估，相比无监督方法取得了最佳性能。在S3DIS数据集Area-5上实现了47.1%的mIoU，比经典全监督方法PointNet高出2.5%

Conclusion: 提出的P-SLCR方法通过原型库驱动的结构学习和一致性推理，有效解决了无监督点云语义分割的挑战，在多个数据集上超越了现有无监督方法，甚至在某些情况下超过了全监督方法
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散的世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程对交互使用和长时程推演来说成本过高。虽然特征缓存可以在不训练的情况下加速推理，但针对单模态扩散设计的策略在迁移到世界模型时效果不佳，主要面临两个特定障碍：多模态耦合和空间变化导致的令牌异构性，以及非均匀时间动态性（少量困难令牌驱动误差增长）
...
Conclusion: WorldCache是针对扩散世界模型量身定制的缓存框架，有效解决了令牌异构性和非均匀时间动态性这两个关键挑战，显著提升了推理效率而不牺牲推演质量，为资源受限场景下的世界模型应用提供了实用解决方案
...
TL;DR: K-MaT是一个知识锚定的流形传输框架，通过提示学习和最优传输实现医学视觉语言模型从高端到低端成像模态的零样本跨模态迁移，无需低端训练图像。


<details>
  <summary>Details</summary>
Motivation: 现有大规模生物医学视觉语言模型在高端成像模态（如CT）上表现良好，但迁移到前线低端模态（如X光）时容易陷入模态特定的捷径学习，导致性能崩溃。

Method: 提出K-MaT框架：1）分解提示并锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形对齐到视觉基础的高端空间；3）无需低端训练图像即可实现跨模态迁移。

Result: 在四个跨模态基准测试（皮肤镜、乳腺X光到超声、CT到胸部X光）中取得最先进结果：平均准确率调和均值提升至44.1%（原42.0%），宏F1提升至36.2%。在乳腺成像任务中有效缓解了灾难性遗忘问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D网格生成方法（如基于等值面的扩散模型和自回归生成）在生成复杂几何形状、保持良好拓扑结构以及推理效率方面存在局限，需要一种更高效、拓扑感知的网格生成方法。

Method: 1. 使用顶点位移场（VDF）表示网格，锚定在表面上；2. 采用稀疏体素变分自编码器（VAE）将显式信号压缩为结构化、拓扑感知的体素潜在表示；3. 解码器通过逐步细分和剪裁体素来实例化精确顶点位置；4. 专用连接头查询体素潜在表示以直接预测顶点对之间的边连接性；5. 采用两阶段流匹配过程：首先生成结构体素，然后细化体素级拓扑特征。

Result: 与现有的等值面/三角形扩散模型和自回归生成方法相比，LATO能够生成具有复杂几何形状、良好拓扑结构的网格，同时在推理时具有高效率。

Conclusion: LATO提供了一种新颖的拓扑保持潜在表示方法，实现了可扩展的、基于流匹配的显式3D网格合成，在保持网格拓扑结构的同时提高了生成效率。
...
Result: OralGPT-Plus在MMOral-X基准测试和现有全景基准测试中均优于强基线模型，表明交互式和对称感知推理的有效性
...
<details>
  <summary>Details</summary>
Motivation: 获取密集像素级标注成本高昂，稀疏标注作为弱监督替代方案仍存在性能差距，需要利用3D场景重建作为辅助监督信号来提升分割质量

Method: 采用双师生架构，利用前馈式3D重建从2D视频恢复3D几何结构，强制2D图像与重建3D点云之间的语义一致性，通过3D几何监督传播稀疏标注

Result: Rewis3d在稀疏监督下达到最先进性能，比现有方法提升2-7%，无需额外标注或推理开销

Conclusion: 3D几何结构为2D图像弱监督语义分割提供了有效的辅助监督信号，通过结合3D重建技术可以显著提升稀疏标注下的分割性能
...
TL;DR: CHMv2是一个全球米级分辨率冠层高度图，利用基于DINOv3的深度估计模型从高分辨率光学卫星图像生成，相比现有产品在精度、偏差和细节保留方面有显著提升。


<details>
  <summary>Details</summary>
Motivation: 机载激光扫描（ALS）提供的高精度冠层高度数据在全球范围内分布不均，限制了森林碳量化、恢复监测和栖息地结构评估等应用。

Method: 使用基于DINOv3的深度估计模型，从高分辨率光学卫星图像生成冠层高度；通过大规模地理多样化训练数据扩展、自动化数据整理和配准，以及针对冠层高度分布定制的损失函数和数据采样策略。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，更好地保留了冠层边缘和空隙等精细结构；在主要森林生物群落中表现出稳定性能。

Conclusion: CHMv2通过先进模型和优化策略，提供了全球范围内高精度的冠层高度数据，解决了ALS数据分布不均的问题，为森林监测和相关应用提供了可靠工具。
...
Method: 将提示敏感性重新定义为组一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，提出提示组感知训练框架，包括质量引导的组正则化和带停止梯度策略的logit级一致性约束

Result: 在多数据集细胞核基准测试中，在文本提示下获得一致提升，显著降低了不同提示质量水平下的性能方差；在六个零样本跨数据集任务中，Dice系数平均提升2.16分

Conclusion: 该方法无需修改架构且推理过程不变，显著提升了计算病理学中视觉语言分割的鲁棒性和泛化能力
...
TL;DR: REACT++是基于REACT架构改进的实时场景图生成模型，通过高效特征提取和原型空间中的主体到客体交叉注意力机制，在保持物体检测性能的同时提升了关系预测准确率和推理速度。


<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法往往只专注于关系预测准确性、物体检测准确性或降低延迟中的单一目标，缺乏同时平衡这三个目标的解决方案。为了满足实时应用需求，需要开发一个能同时兼顾性能与推理速度的模型。

Method: 基于REACT架构，采用高效特征提取和原型空间中的主体到客体交叉注意力机制，平衡延迟和表征能力。

Result: REACT++在现有SGG模型中达到最高推理速度，在保持物体检测性能的同时提升了关系预测准确性。相比前代REACT版本，速度提升20%，关系预测准确率平均提升10%。

Conclusion: REACT++通过创新的架构设计，成功平衡了场景图生成中的三个关键目标：关系预测准确性、物体检测准确性和推理速度，为实时应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 真实世界考古文物碎片重组面临侵蚀、缺失区域、不规则形状和大规模模糊性等重大挑战。传统的拼图求解器通常为干净的合成场景设计，在这些条件下表现不佳，特别是在碎片数量达到数千个时（如RePAIR基准测试）。

Method: 提出了一种人机协同拼图求解框架，将自动松弛标记求解器与交互式人工指导相结合。用户可以通过迭代锁定已验证的放置位置、纠正错误，并引导系统实现语义和几何上一致的组装。引入了两种互补的交互策略：迭代锚定和连续交互细化。

Result: 在多个RePAIR组上的实验表明，这种混合方法在准确性和效率上显著优于完全自动化和手动基线方法。

Conclusion: 该框架为大规模专家参与式文物重组提供了一个实用的解决方案，能够有效处理真实世界文化遗产重建的复杂性和规模问题。
...
TL;DR: DiffInf：基于自影响引导扩散的框架，用于缓解面部属性学习中的标注不一致问题，通过生成式校正而非丢弃样本来提升分类性能
...
Method: 1. 训练基线分类器并计算样本级自影响分数（使用一阶近似）来识别对优化过程影响过大的训练实例；2. 应用基于潜在扩散自编码器的针对性生成校正，更好地对齐视觉内容与分配标签，同时保持身份和真实感；3. 训练轻量级高影响成员预测器作为替代影响正则器，实现可微分引导；4. 用编辑后的样本替换原始样本，生成大小不变的影响精炼数据集。

Result: 在多类面部属性分类任务中，DiffInf相比标准噪声标签训练、鲁棒优化基线和基于影响的过滤方法，持续改善了泛化性能。结果表明，在图像层面修复有影响的标注不一致能增强下游面部属性分类，同时不牺牲分布覆盖范围。

Conclusion: 通过生成式校正而非丢弃样本来处理标注不一致问题，DiffInf框架有效提升了面部属性分类的性能，证明了在图像层面修复有影响的标注不一致是一种有前景的方法。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在定位BEiT模型中实现格式塔凸性先验的内部组件，探究视觉Transformer在局部几何证据与全局组织先验冲突时的感知决策机制。

Method: 使用基于飞镖形状合成刺激的受控感知冲突实验，系统掩蔽同时允许凹性和凸性完成的区域；通过logit归因将内部激活投影到离散视觉码本空间；分解注意力头的直接影响。

Result: BEiT可靠地偏好凸性完成；图形-背景组织在早期和中间层保持模糊，在深层突然解决；识别出头L0H9作为早期种子引入弱凸性偏置；下调该单个注意力头可改变感知冲突的分布质量，允许凹性证据引导完成。

Conclusion: BEiT的凸性偏好由可识别的功能单元控制，特定注意力头在感知决策中起关键作用，为理解Transformer内部感知组织机制提供了新见解。
...
TL;DR: PSIVG框架通过物理模拟器增强视频生成，解决现有扩散模型违反物理定律的问题，生成更符合真实物理规律的视频


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成技术虽然视觉质量高，但经常违反重力、惯性和碰撞等基本物理定律，导致物体运动不一致、动力学不合理，限制了AI生成视频的真实性和可靠性

Method: 提出PSIVG框架：1）从预训练扩散模型生成的模板视频中重建4D场景和前景物体网格；2）在物理模拟器中初始化并生成物理一致的轨迹；3）用模拟轨迹指导视频生成器实现时空物理一致性运动；4）提出TTCO技术，基于模拟器的像素对应关系调整文本和特征嵌入以改善纹理一致性

Result: 综合实验表明，PSIVG生成的视频能更好地遵循真实世界物理规律，同时保持视觉质量和多样性

Conclusion: PSIVG通过将物理模拟器集成到视频扩散过程中，显著提高了生成视频的物理一致性，为更真实可靠的AI视频生成提供了有效解决方案
...
TL;DR: CLoPA是一种持续适应策略，通过在标注缓存上微调SAM模型的小部分参数，快速将医学图像分割性能提升到专家水平，无需改变推理流程或增加新参数。


<details>
  <summary>Details</summary>
Motivation: 现有零样本模型（如SAM）在多样化医学影像任务中无法稳定达到专家级性能，而标注活动产生的任务特定标注数据流为在线模型适应提供了自然补充。

Method: 提出CLoPA持续适应策略：在标注缓存上微调SAM模型的小部分参数，通过轻量级训练周期调度触发，无需新参数或改变推理流程，完全在现有标注工作流中运行。

Result: 在8个医学分割十项全能任务中，CLoPA快速将性能提升到专家水平，即使SAM先前失败的任务也能成功，多数增益在单个训练周期后实现；不同参数组调优效果取决于任务特性和数据量。

Conclusion: CLoPA通过持续适应策略有效提升医学图像分割性能，但对于复杂几何目标（如肝血管），实例归一化和低层特征调优会饱和，表明在最具挑战性场景中需要更深层特征表示对齐。
...
TL;DR: WanderDream是首个用于心理探索模拟的大规模数据集，包含15.8K全景视频和158K问答对，使模型能够在无需实际探索的情况下进行空间推理。
...
TL;DR: CaTok提出了一种1D因果图像分词器，通过MeanFlow解码器学习因果1D表示，支持快速一步生成和高保真多步采样，在ImageNet重建上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉分词器将2D块展平为非因果序列或使用启发式排序，与"下一个token预测"模式不对齐；扩散自编码器要么缺乏因果性，要么引入不平衡。需要解决视觉领域的因果分词问题。
...
Conclusion: CaTok成功解决了视觉领域的因果分词挑战，学习到的因果1D表示支持高效生成和高质量采样，同时自然捕捉跨token间隔的多样化视觉概念。
...
TL;DR: Pinterest Canvas是一个大规模图像生成系统，专门针对图像编辑和增强用例，通过基础扩散模型和任务特定微调来满足严格的产品需求。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活但难以通过提示或简单推理适应来控制，不适合有严格产品要求的用例，因此需要构建专门针对图像编辑和增强任务的系统。

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对特定下游任务在任务特定数据集上快速微调变体，生成专门化模型。系统包括数据集管理、训练和推理的最佳实践。

Result: 在线A/B实验显示增强图像分别获得18.0%和12.5%的参与度提升，人工评估表明模型在背景增强和宽高比外绘任务上优于第三方模型。系统还能推广到多图像场景合成和图像到视频生成等任务。

Conclusion: Pinterest Canvas通过基础模型加任务特定微调的方法，成功构建了满足严格产品要求的大规模图像生成系统，在多个图像编辑任务上表现出色，并能推广到各种下游应用。
...
TL;DR: 本文系统研究去噪生成模型的训练目标，重点分析损失权重和输出参数化（噪声、干净图像、速度）如何与数据流形内在维度、模型架构和数据集大小相互作用，通过实验提供实用设计建议。


<details>
  <summary>Details</summary>
Motivation: 当前去噪生成模型训练中存在多种设计选择（损失权重、输出参数化等），但这些选择如何与数据特性、模型架构等因素相互作用尚不清楚，需要系统分析以提供实用指导。

Method: 通过系统数值研究，在合成数据集（可控几何）和图像数据上比较不同训练目标，使用PSNR（去噪精度）和FID（生成质量）等定量指标评估，分析训练选择与数据内在维度、模型架构、数据集大小的相互作用。

Result: 实验揭示了损失权重、输出参数化与数据流形内在维度、模型架构、数据集大小之间的复杂相互作用，为流匹配模型训练提供了实用设计见解，而非提出新方法。

Conclusion: 去噪生成模型的训练目标设计需要综合考虑数据特性、模型架构和数据集规模等因素，本文通过系统分析为流匹配模型的训练提供了实用的设计指导原则。
...
TL;DR: GreenRFM是一个资源高效的放射学基础模型预训练框架，通过MUST监督设计实现SOTA性能，大幅降低计算需求，挑战"规模至上"的教条。


<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型依赖暴力扩展，直接移植自然图像处理方法，导致模型脆弱且昂贵，临床实践中难以应用。需要资源高效的方法来开发鲁棒的放射学基础模型。

Method: 提出GreenRFM框架，采用MUST监督设计原则：更多蒸馏、普遍性、语义强化和任务对齐的监督，最大化利用监督信号而非简单堆叠数据量。提供两种配置：高性能模型（24GB GPU 24小时）和轻量模型（6GB VRAM 4小时）。

Result: 在超过20万张图像（来自4个机构、2种模态）的实验中，GreenRFM在胸部和腹部CT数据集上表现优异，超越多种基线模型。在内部肌肉骨骼MRI图像上的结果也表明监督原则在不同模态间可迁移。

Conclusion: GreenRFM的性能和效率挑战了"规模至上"的教条，使临床医生即使在笔记本电脑上也能公平地开发SOTA放射学基础模型，为资源高效的RFM发展提供了新范式。
...
TL;DR: Match4Annotate：一个轻量级框架，用于视频内和视频间的点和掩码标注传播，通过SIREN隐式神经表示和变形场学习实现跨视频标注传播。


<details>
  <summary>Details</summary>
Motivation: 在医学影像等专业领域，逐帧视频标注成本高昂且耗时。现有标签传播方法存在局限性：视频跟踪器和分割模型需要逐视频初始化且无法跨视频泛化；传统对应关系方法在低纹理场景中表现不佳；密集特征匹配和一次性分割方法缺乏时空平滑性且不支持点和掩码标注的统一处理。

Method: 提出Match4Annotate框架：1）在测试时使用SIREN隐式神经表示拟合DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应关系匹配；3）支持点和掩码标注的跨视频传播。

Result: 在三个具有挑战性的临床超声数据集上评估，Match4Annotate在跨视频传播方面达到最先进水平，优于特征匹配和一次性分割基线方法，同时在视频内传播方面与专用跟踪器保持竞争力。

Conclusion: 轻量级、测试时优化的特征匹配流程有潜力为可扩展的标注工作流提供高效且易用的解决方案，特别是在专家标注成本高昂的专业领域。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖外部模型存在三个问题：需要单独训练、目标不一致、表现出意外的缩放行为。这种依赖源于模型训练目标（去噪任务）缺乏学习语义表征的激励
...
TL;DR: 基于45,139张超声图像训练的人工智能系统，能够以超过93%的敏感性和95%的特异性诊断胎儿口面裂，性能与资深放射科医生相当，并显著优于初级医生。
...
Method: 开发了一个人工智能系统，使用来自22家医院9,215名胎儿的45,139张超声图像进行训练。该系统可作为医疗副驾驶使用，并在涉及24名放射科医生和实习生的试点研究中评估其对专业知识发展的影响。
...
TL;DR: 提出一种针对漫反射直接飞行时间LiDAR的空间校准方法，通过扫描反光片并背景减除，恢复每个像素在RGB图像平面中的有效区域和相对空间灵敏度。
...
Method: 使用扫描的反光片配合背景减除技术，恢复每个LiDAR像素在共置RGB图像平面中的足迹（有效支持区域）和相对空间灵敏度，建立显式的LiDAR到RGB对应关系。

Result: 在ams OSRAM TMF8828设备上演示了该方法，能够实现跨模态对齐和融合所需的LiDAR-RGB对应关系。
...
<details>
  <summary>Details</summary>
Motivation: 否定是基本的语言运算符，但在基于扩散的生成系统中建模不足。现有方法缺乏对语言否定的形式化处理，无法有效处理对象缺失、分级否定语义、多重否定组合和范围敏感消歧等复杂否定现象。

Method: 将语言否定重新解释为扩散动态中语义引导的结构化可行性约束。将无分类器引导定义为语义更新方向，通过将更新投影到从语言结构导出的凸约束集上来强制执行否定。该方法无需训练，与预训练扩散主干兼容，可从图像生成扩展到时间演化的视频轨迹。

Result: 实验表明该方法实现了稳健的否定合规性，同时保持了视觉保真度和结构连贯性。建立了扩散生成模型中语言否定的第一个统一公式，超越了表示级评估。还引入了结构化否定中心基准套件，用于隔离生成系统中的不同语言故障模式。

Conclusion: 该工作为扩散生成模型中的语言否定提供了第一个统一的形式化处理框架，通过结构化约束投影方法实现了对复杂否定现象的有效建模，为生成系统语言能力的研究开辟了新方向。
...
TL;DR: 研究自动驾驶数据集中的冗余问题，通过分析多源摄像头和多模态图像-LiDAR数据，发现选择性移除冗余标签能提升目标检测性能


<details>
  <summary>Details</summary>
Motivation: 自动驾驶研究过于关注算法设计而忽视数据质量分析，特别是冗余这一基本但未被充分探索的数据质量问题。自动驾驶系统依赖多源多模态数据，但数据质量因环境和传感器限制而异，冗余问题直接影响决策性能

Method: 使用nuScenes和Argoverse 2数据集，建模和测量多源摄像头数据和多模态图像-LiDAR数据的冗余，通过选择性移除冗余标签评估对YOLOv8目标检测任务的影响

Result: 选择性移除共享视野摄像头中的冗余多源图像目标标签能提升检测性能：在nuScenes中，三个代表性重叠区域的mAP50从0.66提升到0.70、0.64到0.67、0.53到0.55；在AV2中移除4.1-8.6%标签后mAP50保持在0.64基线附近。多模态分析显示图像和LiDAR数据间存在显著冗余
...
TL;DR: Penguin-VL提出了一种新的视觉语言模型架构，使用文本LLM初始化视觉编码器，替代传统的对比预训练视觉编码器，在保持轻量化的同时提升细粒度视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前VLM发展过度依赖模型规模扩展，难以部署在计算资源受限的移动和边缘设备上。同时，主流VLM依赖对比预训练（如CLIP/SigLIP）初始化的视觉编码器存在目标不匹配问题：对比学习优化的判别性任务会抑制细粒度视觉线索，而这些线索对于密集描述和复杂推理至关重要。

Method: 提出Penguin-VL，其视觉编码器从纯文本LLM初始化，而不是传统的对比预训练视觉编码器。这种方法解决了目标不匹配问题，能够更好地保留细粒度的空间和时间线索，为多模态理解提供更高的视觉保真度和数据效率。

Result: 在各种图像和视频基准测试中，Penguin-VL在数学推理上与领先的VLM（如Qwen3-VL）性能相当，在文档理解、视觉知识和多视角视频理解等任务中表现更优。这些优势是通过轻量级架构实现的，表明改进视觉表示而非模型缩放是性能提升的主要驱动力。

Conclusion: Penguin-Encoder作为传统对比预训练编码器的优越替代方案，能够保留对密集感知和复杂推理至关重要的细粒度空间和时间线索，使其成为计算高效VLM的强大即插即用替代方案，在资源受限环境中实现高性能。
...
TL;DR: SUREON是一个大规模手术视频问答数据集，从手术教学视频中提取专家讲解的意图、原理和预测等推理信息，用于训练具备手术推理能力的人工智能模型。


<details>
  <summary>Details</summary>
Motivation: 当前手术AI缺乏理解手术推理的能力，如理解手术器械选择的原因、风险评估和下一步操作等。虽然专家教学视频中包含了这些推理信息，但大规模标注这些数据非常困难。

Method: 通过多智能体管道从手术学术视频中提取和结构化监督信息，构建包含12个问题类别（安全评估、决策原理、预测等）的大规模视频问答数据集SUREON。开发了两个模型：SureonVLM（通过监督微调）和SureonVLM-R1（使用Group Relative Policy Optimization训练）。

Result: SUREON数据集包含134.7K个视频片段、170种手术类型和206.8k个问答对，以及354个专家验证的基准示例。两个模型在SUREON基准测试中准确率超过84%，在标准手术感知任务中优于更大的通用领域模型。

Conclusion: 从手术教学视频中提取的监督信号能够有效训练具备手术推理能力的AI模型，这些模型能够回答复杂的手术问题，并表现出明确的推理行为，如从视觉上下文中推断手术意图。
...
TL;DR: BEVLM框架将空间一致的BEV表示与LLM连接，提升自动驾驶中的3D空间推理和语义理解能力
...
Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLM连接，使用BEV特征作为统一输入，同时将LLM的语义知识蒸馏到BEV表示中

Result: BEVLM使LLM在跨视角驾驶场景中的推理准确率提升46%，通过语义蒸馏到BEV表示，在安全关键场景中端到端驾驶性能提升29%

Conclusion: BEVLM成功结合了BEV的空间一致性和LLM的语义推理能力，显著提升了自动驾驶系统的3D空间推理和决策性能
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要采用传统的自回归架构，在架构设计上存在探索有效且高效替代方案的空间。同时，离散扩散模型在视觉理解和图像生成等领域展现出作为多模态系统骨干的巨大潜力。

Method: 采用统一的基于掩码的离散扩散模型直接捕捉离散多模态标记的联合分布，支持双模态任务以及涉及多个模态的更复杂场景。

Result: 在多样化基准测试中，该方法优于或与现有的处理两个或更多模态的多模态系统性能相当。

Conclusion: 扩散模型在驱动下一代多模态基础模型方面展现出显著潜力，Omni-Diffusion证明了基于扩散模型的架构在多模态任务中的有效性。
...
TL;DR: 该论文发现多模态大语言模型（MLLM）分类性能评估存在严重偏差，主要源于评估协议问题和标注质量不足。通过修正这些问题，MLLM性能显著提升，与监督模型的差距大幅缩小。


<details>
  <summary>Details</summary>
Motivation: 当前关于MLLM分类性能的研究存在矛盾结论，作者认为这些矛盾源于评估协议的问题（如模型输出被不当丢弃、多选题干扰项过弱、输出映射不佳）以及标注质量不足，导致MLLM性能被低估。

Method: 1) 识别并修正常见评估协议的关键问题；2) 量化常被忽视的设计选择（批量大小、图像顺序、文本编码器选择）的影响；3) 使用ReGT（ImageNet-1k的625类多标签重新标注）进行评估；4) 进行控制案例研究，评估MLLM辅助人类标注的效果。

Result: 1) MLLM从修正标注中获益最大（提升达+10.8%）；2) MLLM与监督模型的感知差距大幅缩小；3) 模型性能对标注质量敏感，特别是那些较少依赖监督训练信号的模型；4) 在困难案例中，标注者确认或整合MLLM预测的比例约为50%。

Conclusion: MLLM在分类任务中的表现不佳主要是由嘈杂的标注和有缺陷的评估协议造成的，而非模型本身的缺陷。MLLM对标注质量敏感，且能有效辅助大规模数据集标注工作。
