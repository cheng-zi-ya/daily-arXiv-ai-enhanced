...
Conclusion: 基于最优传输的分布对齐为视觉令牌剪枝提供了稳定且语义保真的理论基础，显著提升推理效率
...
TL;DR: 提出一个结合图表去渲染、自动分析和迭代改进的框架，为可视化设计提供可操作、可解释的反馈，通过LLM驱动的推荐系统提升可视化质量和用户素养。


<details>
  <summary>Details</summary>
Motivation: 数据可视化在科学传播、新闻和日常决策中至关重要，但常存在错误。现有基于规则的检查器缺乏上下文且无法提供有意义的改进建议，而通用LLM在可视化质量评估上不可靠，缺乏遵循设计原则的训练。

Method: 开发了一个结合图表去渲染、自动分析和迭代改进的框架。系统从图像重建图表结构，使用视觉语言推理识别设计缺陷，基于可视化研究中的既定原则提出具体修改建议，用户可选择性地应用改进并重新渲染更新图表。

Result: 在Chart2Code基准测试的1000个图表上，系统生成了10,452个设计建议，聚类为10个连贯类别（如轴格式化、颜色可访问性、图例一致性）。

Conclusion: 展示了LLM驱动的推荐系统在提供结构化、基于原则的可视化设计反馈方面的潜力，为更智能、更易用的创作工具开辟了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经网格压缩方法独立处理每一帧网格，未能充分利用时间冗余。受2D视频编解码器帧间压缩启发，需要开发能够利用网格序列时间相关性的压缩方法。

Method: 1) 将连续的不规则网格帧转换为规则的4D张量；2) 使用自动解码器压缩张量，捕捉时空相关性；3) 引入基于Transformer的插值模型，根据跟踪体积中心的潜在嵌入预测中间网格帧，消除运动模糊。

Result: N4MC在率失真性能上优于现有最先进方法，同时能够实时解码4D网格序列。代码已开源。

Conclusion: N4MC成功将2D视频压缩中的帧间压缩思想扩展到4D网格序列，通过利用时间冗余实现了高效的压缩性能，为时变网格压缩提供了新思路。
...
TL;DR: 该论文提出了一种名为GSNR（图平滑零空间表示）的新方法，通过在重建框架中引入有意义的零空间信息来解决成像逆问题中的病态性。该方法利用图拉普拉斯算子构建零限制拉普拉斯，从最平滑的谱图模式中设计低维投影矩阵，从而改善重建质量。


<details>
  <summary>Details</summary>
Motivation: 成像逆问题是病态的，由于传感矩阵的非平凡零空间，存在无限多个与测量一致的解。传统的图像先验（如稀疏性、平滑性或评分函数）虽然能促进一般图像流形上的解，但无法约束零空间分量，可能导致重建偏差。因此需要将有意义的零空间信息纳入重建框架。

Method: 提出Graph-Smooth Null-Space Representation (GSNR)方法：基于图上的平滑图像表示，构建零限制拉普拉斯算子来编码零空间信号中相邻像素之间的相似性，并从最平滑的谱图模式（最低图频率）中设计低维投影矩阵。该方法可以集成到PnP、DIP和扩散求解器等现有逆问题求解器中。

Result: 在图像去模糊、压缩感知、去马赛克和图像超分辨率四种场景中，GSNR方法相比基线公式在PSNR上实现了高达4.3 dB的改进，相比端到端学习模型也有高达1 dB的提升。该方法具有三个重要优势：通过零空间图正则化改善收敛性、更好的覆盖范围（p个模式捕获的零空间方差）、以及高可预测性（这些模式从测量中推断的能力）。

Conclusion: GSNR方法通过将有意义的零空间信息纳入重建框架，有效解决了成像逆问题中的病态性。该方法具有坚实的理论基础和实际应用价值，能够显著提升各种逆问题求解器的性能，为图像重建提供了新的有效途径。
...
TL;DR: 提出首个视觉语言模型透明电路追踪框架，揭示多模态推理机制
...
Method: 使用转码器、归因图和基于注意力的方法，通过特征引导和电路修补验证

Result: 发现VLMs分层整合视觉和语义概念，特定视觉特征电路处理数学推理并支持跨模态关联

Conclusion: 这些电路具有因果性和可控性，为构建更可解释和可靠的视觉语言模型奠定基础
...
TL;DR: 该研究提出了一种端到端无人机视频流实时3D重建系统，结合3D高斯泼溅技术，实现低延迟的AR/VR可视化应用。


<details>
  <summary>Details</summary>
Motivation: 无人机在实时空中感知应用中广泛使用，3D高斯泼溅技术已展示实时神经渲染潜力，但将其集成到端到端无人机重建系统中尚未充分探索。

Method: 提出高效架构，整合RTMP直播视频采集、同步传感器融合、相机位姿估计和3DGS优化，实现连续模型更新和低延迟部署到交互式可视化环境。

Result: 相比NeRF方法，该方法在保持视觉保真度的同时，显著提高渲染性能并大幅降低端到端延迟，重建质量达到离线高保真参考的93-96%。

Conclusion: 该系统适用于实时、可扩展的空中平台增强感知应用，为AR/VR沉浸式体验提供有效解决方案。
...
Conclusion: BiRQA成功解决了当前FR IQA模型在速度、鲁棒性和准确性方面的平衡问题，为图像压缩、恢复和生成建模提供了高效可靠的评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成技术快速发展，但评估生成视频的真实性仍然主要依赖人工标注或有限范围的评估数据集，缺乏自动化的评估方法。需要一种能够捕捉语义和连贯3D结构、无需参考视频的自动化评估框架。

Method: 提出3DSPA（3D时空点自编码器），将3D点轨迹、深度线索和DINO语义特征整合为统一的视频表示。该方法建模物体运动方式和场景内容，能够评估视频的真实性、时间一致性和物理合理性。

Result: 实验表明，3DSPA能够可靠识别违反物理规律的视频，对运动伪影更敏感，在多个数据集上与人类对视频质量和真实性的判断更一致。该方法为生成视频模型提供了更强的基准测试基础。

Conclusion: 将3D语义信息融入基于轨迹的表示中，为生成视频模型的基准测试提供了更强的基础，并能够隐式捕捉物理规则违反。该方法代码和预训练权重将公开。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）在跨模态推理方面表现出色，但在域适应方面脆弱，特别是从合成到真实点云的适应。传统3D域适应方法依赖重型可训练编码器，虽然准确但效率低下。需要开发高效且鲁棒的3D域适应方法。

Method: 1. 将3D样本投影到多个深度图，利用冻结的CLIP主干；2. 知识驱动提示调优方案，集成高级语言先验和轻量3D编码器的几何线索；3. 参数高效微调CLIP编码器；4. 熵引导视图采样策略选择置信投影；5. 最优传输对齐损失和不确定性感知原型对齐损失协同工作，弥合源-目标分布差距并保持类别可分性

Result: 在PointDA-10和GraspNetPC-10基准测试中，CLIPoint3D相比基于CLIP和传统编码器的基线方法，实现了3-16%的准确率提升。代码已开源。

Conclusion: CLIPoint3D成功构建了首个基于CLIP的少样本无监督3D点云域适应框架，通过结合语言先验和几何线索，在保持效率的同时显著提升了域适应性能，为3D视觉任务提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的伪造图像检测方法容易过拟合训练数据，在分布偏移的困难测试集上表现灾难性下降。需要更原则性的方法来提高检测器的泛化能力和鲁棒性。

Method: 提出SimLBR框架，采用潜在混合正则化（LBR）学习紧致的真实图像决策边界，将伪造图像类别视为汇类。该方法训练效率高，比现有方法快几个数量级。

Result: 在Chameleon基准测试中，SimLBR显著提升跨生成器泛化能力，准确率提升达+24.85%，召回率提升达+69.62%。同时引入面向可靠性的评估指标和风险调整指标来更好评估模型鲁棒性。

Conclusion: 学习紧致的真实图像决策边界并将伪造类别视为汇类是更原则性的伪造图像检测方法。SimLBR框架在跨生成器泛化、训练效率和鲁棒性评估方面都表现出色，为伪造图像检测提供了新思路。
...
TL;DR: 该论文提出了一种将大型文本到图像潜在扩散模型适应于量子爆发成像光子受限领域的方法，通过整合潜在空间恢复和爆发级时空推理，在极端光子限制条件下实现高质量图像重建。


<details>
  <summary>Details</summary>
Motivation: 单光子雪崩二极管（SPAD）传感器在传统相机失效的极端条件下具有成像潜力，但原始量子帧包含稀疏、噪声、二进制的光子检测。从这样的帧序列中恢复连贯图像需要在远超标准恢复流程或现代生成模型假设的噪声统计下处理对齐、去噪和去马赛克（彩色）问题。
...
Conclusion: 该方法展示了将大型生成先验适应于极端光子受限传感的潜力，能够在高速运动等挑战性条件下产生既保持光度保真度又具有良好感知质量的图像重建。
...
Method: 提出MedCLIPSeg框架，通过概率跨模态注意力机制利用patch级CLIP嵌入，实现图像和文本token的双向交互，并显式建模预测不确定性；同时采用软patch级对比损失，促进跨不同文本提示的细粒度语义学习

Result: 在涵盖5种成像模态、6个器官的16个数据集上的广泛实验表明，MedCLIPSeg在准确性、效率和鲁棒性方面优于现有方法，并能提供可解释的不确定性图，突出分割结果的局部可靠性
...
TL;DR: SceMoS：使用2D场景表示而非3D数据进行场景感知的3D人体运动合成，通过分离全局规划和局部执行，在效率和保真度之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要计算昂贵的3D场景数据（如点云或体素占用网格），同时学习高级规划和低级接触推理。本文旨在证明结构化2D场景表示可以作为3D监督的替代方案，用于物理基础的运动合成。

Method: 1）基于文本条件的自回归全局运动规划器，使用鸟瞰图（BEV）作为场景表示，编码DINOv2特征；2）基于几何基础的运动分词器，通过条件VQ-VAE训练，使用2D局部场景高度图，将表面物理嵌入离散词汇表。

Result: 在TRUMANS基准测试中达到最先进的运动真实感和接触精度，场景编码的可训练参数减少超过50%，证明2D场景线索可以有效支撑3D人-场景交互。

Conclusion: 结构化2D场景表示可以作为完整3D监督的强大替代方案，通过分离全局规划和局部执行，在效率和保真度之间取得良好平衡，同时保持物理可行性。
...
TL;DR: 本文提出了一种基于双曲几何的路径解耦流匹配方法（HFM），用于解决跨模态少样本适应中欧几里得流匹配存在的路径纠缠问题，在11个基准测试中取得了新的最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有的跨模态少样本适应方法将视觉-语义对齐视为连续特征传输问题，采用欧几里得流匹配。然而，欧几里得几何的平坦特性导致多项式体积增长无法适应多样化的特征分布，造成严重的路径纠缠问题。

Method: 提出路径解耦的双曲流匹配（HFM），利用洛伦兹流形的指数扩展特性实现轨迹解耦。主要包含两个关键设计：1）向心双曲对齐：通过锚定文本根节点构建向心层次结构，将视觉叶节点推向边界以初始化有序流；2）路径解耦目标：作为"语义护栏"，通过逐步监督将轨迹严格限制在孤立的类特定测地线走廊内。此外，还设计了基于自适应直径的停止机制，防止基于内在语义尺度的过度传输到拥挤的源点。

Result: 在11个基准测试上的广泛消融实验表明，HFM建立了新的最优性能，始终优于其欧几里得对应方法。

Conclusion: 通过引入双曲几何和路径解耦机制，HFM有效解决了跨模态少样本适应中的路径纠缠问题，为视觉-语义对齐提供了更有效的特征传输框架。
...
TL;DR: 本文提出PipStereo，一种面向边缘设备的实时高精度立体匹配方法，通过渐进迭代剪枝、协作单目先验迁移和硬件感知的FlashGRU算子，在保持大模型精度的同时实现边缘实时推理。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像和视频生成方面取得了显著成功，但扩散变换器的高计算需求限制了实际部署。现有的特征缓存方法无法适应扩散过程的复杂阶段依赖动态，导致质量下降和与标准去噪过程不一致。

Method: 提出基于两阶段训练的LEarnable Stage-Aware (LESA)预测器框架，利用Kolmogorov-Arnold Network (KAN)从数据中准确学习时间特征映射，并引入多阶段多专家架构，为不同噪声级别阶段分配专门的预测器。

Result: 在FLUX.1-dev上实现5.00倍加速且质量仅下降1.0%；在Qwen-Image上实现6.25倍加速且质量提升20.2%；在HunyuanVideo上实现5.00倍加速且PSNR提升24.7%。在文本到图像和文本到视频合成任务上均达到最先进性能。

Conclusion: LESA框架通过可学习的阶段感知特征预测，在保持高保真生成的同时显著加速扩散模型，验证了基于训练的方法在不同模型上的有效性和泛化能力。
...
TL;DR: 通过融合DINO的几何原型和Flux的交互注意力图，在零样本训练下实现与弱监督方法相当的affordance估计，证实几何感知和交互感知是视觉基础模型中affordance理解的基本构建块。


<details>
  <summary>Details</summary>
Motivation: 探究视觉系统真正理解affordance（可供性）的含义，认为这依赖于两个互补能力：几何感知（识别支持交互的对象结构部分）和交互感知（建模智能体动作如何与这些部分互动）。

Method: 系统性地探测视觉基础模型（VFMs），发现DINO编码了部分级几何结构，而Flux等生成模型包含丰富的动词条件空间注意力图作为隐式交互先验。通过训练免费、零样本的方式融合DINO的几何原型和Flux的交互图进行affordance估计。

Result: 融合DINO几何原型和Flux交互图的方法在affordance估计上达到了与弱监督方法竞争的性能，证实几何感知和交互感知不仅是相关的，而且是affordance的可组合元素。

Conclusion: 几何感知和交互感知是视觉基础模型中affordance理解的基本构建块，为感知如何支撑动作提供了机制性解释。
...
<details>
  <summary>Details</summary>
Motivation: 研究扩散修复模型引入的视觉伪影如何影响视觉语言模型的语言生成质量，理解视觉重建质量对多模态系统语言生成的影响机制

Method: 采用两阶段诊断框架：首先对掩蔽图像区域进行重建，然后将重建图像提供给字幕生成模型，通过对比原始图像和重建图像生成的字幕进行控制比较

Result: 发现像素级和感知重建指标与词汇和语义字幕性能之间存在一致关联；中间视觉表示和注意力模式分析显示修复伪影导致模型行为的系统性、层级依赖性变化

Conclusion: 研究结果为检查视觉重建质量如何影响多模态系统中语言生成提供了实用的诊断框架
...
Method: 使用MobileNetV3提取UI布局视觉特征，DistilBERT提取文本特征，通过带Swish激活的门控融合模块融合多模态特征，最后使用MLP回归头进行预测
...
TL;DR: PFGNet是一个完全卷积的时空预测学习框架，通过像素级频率引导的门控机制动态调制感受野，实现了高效且自适应的运动模式捕捉。


<details>
  <summary>Details</summary>
Motivation: 纯卷积模型在时空预测学习中具有效率和并行性优势，但其固定感受野限制了自适应捕捉空间变化运动模式的能力。需要一种既能保持卷积效率又能自适应调整感受野的方法。
...
Conclusion: PFGNet通过频率引导的门控机制实现了无需循环或注意力机制的结构感知时空建模，在保持高效性的同时提升了自适应运动捕捉能力。
...
TL;DR: 该研究开发了一个结合深度学习和视觉语言模型的多智能体框架，用于自动检测菌落形成单位，在制药制造中实现了85%的自动化验证率。


<details>
  <summary>Details</summary>
Motivation: 制药制造中的菌落形成单位检测是环境监测和质量控制的关键环节。传统人工计数劳动密集且易出错，而现有深度学习方法虽然准确，但对样本质量变化和伪影敏感，无法满足制药级要求（需要接近100%的准确率）。

Method: 1. 评估了YOLOv5、YOLOv7和YOLOv8等目标检测模型，但准确率仅97.08%，不达标
2. 使用GSK超过50,000张培养皿图像训练自定义Detectron2模型，达到99%检测率
3. 针对异常情况（污染板、塑料伪影、光学清晰度差）开发多智能体框架：VLM智能体首先分类板为有效/无效；对于有效样本，DL和VLM智能体独立估计菌落数；预测差异在5%内时自动记录，否则送专家审查
4. 专家反馈支持持续再训练和自我改进

Result: 1. 初始DL自动化将疫苗制造站点的人工验证减少了50%
2. VLM集成后，人工验证进一步减少到85%
3. 实现了显著的运营节省
4. 自定义Detectron2模型达到99%检测率，2%假阳性，0.6%假阴性

Conclusion: 提出的多智能体系统为微生物质量控制提供了一个可扩展、可审计且符合法规要求的解决方案，推进了生物制药生产中的自动化进程。该系统通过结合DL和VLM的优势，并利用专家反馈进行持续改进，有效解决了传统方法的局限性。
...
TL;DR: 论文证明了所有成像前向模型都可以用11个基本原语构建的有向无环图近似表示，并提出了构造算法和最小性证明，为物理世界模型框架奠定数学基础。


<details>
  <summary>Details</summary>
Motivation: 传统计算成像前向模型通常以特定模态的单一代码实现，缺乏统一的数学框架。本文旨在证明所有成像模态（包括线性和非线性）都可以用一组有限的基本操作原语来近似表示，从而为成像系统提供统一的数学基础。

Method: 提出了有限原语基定理，定义了包含临床、科学和工业成像模态的算子类Cimg。证明任何在该类中的前向模型都可以用11个规范原语构建的类型化有向无环图进行ε近似表示。提供了构造算法，并证明该原语库是最小的（移除任一原语都会导致某些模态无法近似表示）。

Result: 在31个线性模态上验证了εimg低于0.01，最多使用5个节点和深度5。为9个非线性模态提供了构造性的DAG分解。证明了原语库的最小性，并建立了物理世界模型（PWM）框架的数学基础。

Conclusion: 所有成像前向模型都可以用11个基本原语构建的有向无环图进行有效近似，这为成像系统提供了统一的数学框架，支持跨模态的系统设计和分析，奠定了物理世界模型的理论基础。
...
TL;DR: 提出基于CAD模型提示的分割框架，使用CAD模型的多视角渲染作为提示输入，解决工业场景中语言描述困难或外观变化大的物体分割问题。


<details>
  <summary>Details</summary>
Motivation: 在制造和3D打印环境中，语言提示分割受限于自然语言的表达能力，难以描述不常见、特定实例或难以描述的物体。基于图像示例的方法主要编码颜色和纹理等外观线索，但在工业场景中，同一组件可能使用不同材料、表面处理或颜色制造，使得基于外观的提示不可靠。

Method: 基于SAM3构建CAD提示分割框架，使用CAD模型的标准多视角渲染作为提示输入。渲染视图提供独立于表面外观的几何条件。模型使用从网格渲染生成的合成数据进行训练，涵盖多样化的视角和场景上下文。

Result: 该方法实现了单阶段、CAD提示的掩码预测，将可提示分割扩展到无法仅通过语言或外观可靠描述的物体。

Conclusion: CAD提示分割框架通过几何条件解决了工业场景中物体分割的挑战，为无法用语言或外观可靠描述的物体提供了有效的分割解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D手部重建方法主要依赖受控环境数据，在真实世界场景中（如手-物交互、极端姿态、光照变化、运动模糊等扰动）性能会显著下降。

Method: 1. 动态扰动解耦模块：将扰动表示为3D高斯属性上的时变偏差；2. 扰动感知优化策略：生成每帧各向异性加权掩码指导优化；两者结合可在时空维度识别和抑制扰动。
...
<details>
  <summary>Details</summary>
Motivation: 现有文档伪造数据集依赖传统数字编辑工具，而最先进的检测器对日益增长的AI伪造文档欺诈威胁存在严重盲区，需要专门针对AI修复伪造的基准测试。

Method: 使用两种AI修复API（Gemini 2.5 Flash Image和Ideogram v2 Edit）系统性地伪造真实收据和表单图像中的数字字段，从四个公共文档数据集生成4,061张伪造图像，提供像素级精确的篡改区域掩码。

Result: 三种代表性检测器（TruFor、DocTamper和零样本GPT-4o）性能大幅下降：TruFor AUC=0.751 vs NIST16的0.96；DocTamper AUC=0.563 vs 原分布0.98；GPT-4o仅0.509，接近随机猜测，表明AI伪造值对自动检测器和VLMs难以区分。

Conclusion: AIForge-Doc代表了文档取证领域一个全新的、尚未解决的挑战，现有检测方法对AI修复伪造的文档存在严重漏洞，需要新的检测技术来应对这一威胁。
...
TL;DR: 论文提出了交互增强驾驶数据集(IEDD)，通过挖掘自然驾驶数据中的交互片段，并生成严格对齐的BEV视频和结构化语言，以解决自动驾驶VLA模型发展中交互场景稀疏和多模态对齐不足的问题。
...
Method: 开发可扩展的流水线从自然驾驶数据中挖掘百万级交互片段，基于交互轨迹设计量化指标；构建IEDD-VQA数据集，生成语义动作与结构化语言严格对齐的合成鸟瞰图(BEV)视频。
...
TL;DR: MVLAD-AD是一个用于自动驾驶的新型框架，通过掩码视觉-语言-动作扩散模型，在高效规划和语义可解释性之间架起桥梁，解决了现有LLM/VLM在推理延迟、动作精度和可解释性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有LLM和VLM在自动驾驶应用中面临推理延迟、动作精度和可解释性三大挑战。自回归方法存在逐token生成的缓慢问题，而基于扩散的方法通常使用冗长的通用语言token，缺乏明确的几何结构。

Method: 提出MVLAD-AD框架：1) 引入离散动作标记化策略，从真实驾驶分布构建运动学可行的路径点紧凑码本；2) 提出几何感知嵌入学习，确保潜在空间嵌入近似物理几何度量；3) 引入动作优先解码策略，优先生成轨迹。

Result: 在nuScenes及其衍生基准上的大量实验表明，MVLAD-AD实现了卓越的效率，在规划精度上优于最先进的自回归和扩散基线，同时提供高保真和可解释的推理。

Conclusion: MVLAD-AD成功解决了自动驾驶中LLM/VLM的效率和可解释性问题，通过创新的动作标记化、几何感知嵌入和优先解码策略，为端到端自动驾驶提供了更高效、精确且可解释的解决方案。
...
TL;DR: PropFly：无需配对视频数据集，利用预训练视频扩散模型动态生成监督信号，通过引导调制流匹配损失训练传播式视频编辑模型


<details>
  <summary>Details</summary>
Motivation: 传播式视频编辑需要大规模配对视频数据集进行训练，但这类数据获取成本高且复杂。现有方法依赖现成的或预计算的配对数据集，限制了模型的训练效率和泛化能力。

Method: 提出PropFly训练管道，利用预训练视频扩散模型动态生成监督信号。通过不同CFG尺度的一步干净潜在估计，在线合成"源"（低CFG）和"编辑"（高CFG）潜在对。源潜在提供视频结构信息，编辑潜在提供目标变换。使用引导调制流匹配损失训练附加适配器，学习传播编辑操作。

Result: PropFly在多种视频编辑任务上显著优于现有最先进方法，能够生成高质量的编辑结果。模型能够学习时间一致且动态的变换，无需依赖离线配对数据集。

Conclusion: PropFly通过在线监督机制解决了传播式视频编辑中配对数据集获取困难的问题，利用预训练视频扩散模型动态生成训练对，实现了高效且高质量的传播式视频编辑。
...
TL;DR: 提出一种针对长期环境监测的联合SfM重建方法，通过跨时段特征匹配解决传统方法在时间跨度大、外观变化显著场景下的失效问题。


<details>
  <summary>Details</summary>
Motivation: 传统SfM方法假设图像采集时间相近且外观变化有限，无法处理长期环境监测（如珊瑚礁调查）中因时间跨度大、视觉和结构变化显著而导致的失效问题。

Method: 1. 在联合SfM重建中直接强制执行跨时段对应关系；2. 结合手工特征和深度学习特征建立跨大时间间隔的鲁棒对应；3. 通过视觉位置识别筛选可能匹配的图像对，限制昂贵的深度学习特征匹配计算量。

Result: 在长期珊瑚礁数据集上评估，该方法能够在现有方法失效的情况下实现一致的联合重建，显著提升跨时段3D模型对齐的鲁棒性和准确性。

Conclusion: 通过跨时段特征匹配的联合SfM重建方法有效解决了长期环境监测中的3D模型对齐问题，在计算效率和重建质量之间取得了良好平衡。
...
TL;DR: 本文提出了一种端到端的交互感知Transformer（InterFormer），通过动态查询生成器、双上下文特征选择器和条件共现损失三个核心组件，解决了手-物交互解析中的查询初始化、特征选择和物理一致性等关键问题，在EgoHOS和mini-HOI4D数据集上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的手-物交互解析方法存在三个主要问题：1）查询初始化机制主要依赖语义线索或可学习参数，对变化场景中活动物体的适应性有限；2）使用像素级语义特征迭代优化查询，可能引入与交互无关的内容；3）容易产生"交互幻觉"，即物理上不一致的预测。这些问题限制了模型对精细粒度自我中心人-环境交互的理解能力。

Method: 提出InterFormer框架，包含三个核心组件：1）动态查询生成器（DQG）：基于手-物接触的空间动态显式地初始化查询，针对性地生成交互感知的手和活动物体查询；2）双上下文特征选择器（DFS）：融合粗粒度交互线索与语义特征，抑制交互无关噪声，强调交互关系学习；3）条件共现损失（CoCo）：引入手-物关系约束，增强预测的物理一致性。

Result: 在EgoHOS数据集和具有挑战性的分布外mini-HOI4D数据集上均取得了最先进的性能，证明了模型的有效性和强大的泛化能力。代码和模型已在GitHub上公开。
...
TL;DR: 该论文提出视频引导的3D物体可供性定位方法，通过动态交互序列解决静态视觉/文本方法无法准确识别真实接触区域的问题。
...
Conclusion: 通过视频引导的3D可供性定位方法，利用动态交互序列提供功能监督，能够更准确地识别物体上支持人-物交互的区域，为具身视觉推理提供了重要能力。
...
TL;DR: 本文提出了IPOW框架，通过概念分解模型使开放世界目标检测具有可解释性，解决已知-未知混淆问题，提高未知目标召回率。


<details>
  <summary>Details</summary>
Motivation: 现有开放世界目标检测方法主要关注提高未知目标召回率，但忽视了可解释性，导致已知-未知混淆和预测可靠性降低。本文旨在使整个OWOD框架具有可解释性，让检测器真正"知道未知"。

Method: 提出概念驱动的可解释OWOD框架(IPOW)，引入概念分解模型(CDM)，将Faster R-CNN中的耦合RoI特征显式分解为判别性概念、共享概念和背景概念。判别性概念用于区分已知类别，共享和背景概念由于其强泛化能力可迁移用于检测未知类别。还提出概念引导修正(CGR)来解决已知-未知混淆问题。

Result: 大量实验表明，IPOW显著提高了未知目标召回率，同时缓解了混淆问题，并为已知和未知预测提供了概念级别的可解释性。

Conclusion: IPOW框架通过概念分解实现了开放世界目标检测的可解释性，有效解决了已知-未知混淆问题，提高了检测性能，为OWOD提供了新的可解释性视角。
...
<details>
  <summary>Details</summary>
Motivation: AI生成内容的泛滥导致人脸操纵技术日益复杂，严重破坏视觉完整性并威胁知识产权。现有主动防御方法依赖脆弱水印，但假设攻击者不知道水印存在，忽视了水印移除攻击的固有脆弱性。双重水印策略中鲁棒水印和脆弱水印相互干扰且嵌入容量有限，进一步削弱了脆弱水印的有效性。

Method: RecoverMark采用两个关键洞察：1) 利用攻击者必须保持背景语义一致性以避免视觉检测的现实约束；2) 使用图像自身内容（人脸）作为水印增强提取鲁棒性。框架将受保护的人脸内容作为水印嵌入周围背景，设计鲁棒的两阶段训练范式，包含模拟全面潜在攻击的失真层和渐进训练策略，实现非脆弱方式的水印嵌入。

Result: 大量实验证明RecoverMark对已知和未知攻击具有鲁棒性，在分布内和分布外数据上具有良好泛化能力，能够同时实现鲁棒的篡改定位、内容恢复和图像IP保护。

Conclusion: RecoverMark通过将人脸内容作为水印嵌入背景，并采用鲁棒的两阶段训练策略，成功解决了传统脆弱水印易受移除攻击的问题，实现了同时支持篡改定位、内容恢复和所有权验证的综合性水印框架。
...
<details>
  <summary>Details</summary>
Motivation: 单目3D目标检测本质上是病态问题，需要大量标注数据。但由于人类偏见，训练数据中的物体、场景和相机姿态总是紧密耦合，特定物体总是在特定场景中以固定相机姿态出现，导致数据多样性不足，引发训练数据利用不足和过拟合问题。

Method: 提出在线物体-场景-相机分解与重组方案：1）将训练图像高效分解为带纹理的3D物体点云模型和背景场景；2）在每个训练周期中，通过将3D物体插入背景场景的自由空间，并使用扰动后的相机姿态从纹理3D点表示进行渲染，不断重组新的训练图像。

Result: 该方法作为即插即用组件，可应用于五种代表性M3OD模型，在KITTI和更复杂的Waymo数据集上均能提升性能。支持全监督和稀疏监督设置，在稀疏监督设置中可灵活控制标注成本。

Conclusion: 通过解耦物体、场景和相机姿态的紧密耦合关系，提出的数据增强方案能更高效地利用训练数据，生成覆盖完整组合谱系的新训练样本，有效缓解数据多样性不足问题，提升单目3D目标检测模型的性能。
...
TL;DR: TraqPoint：一种基于强化学习的端到端关键点检测框架，通过序列决策优化关键点的跨视图跟踪质量，在相对位姿估计和3D重建任务中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的关键点匹配方法通常在图像对上训练，未能显式优化关键点在挑战性视角和光照变化下的长期可跟踪性。需要一种能直接在图像序列上优化关键点跟踪质量的方法。

Method: 将关键点检测重构为序列决策问题，提出TraqPoint框架：1) 使用强化学习优化关键点的跟踪质量；2) 核心创新是跟踪感知的奖励机制，联合鼓励关键点在多视图中的一致性和独特性；3) 采用策略梯度方法进行训练。

Result: 在稀疏匹配基准测试（包括相对位姿估计和3D重建）上的广泛评估表明，TraqPoint显著优于一些最先进的关键点检测和描述方法。

Conclusion: 通过将关键点检测重构为序列决策问题，TraqPoint能够直接优化关键点的长期跟踪质量，在3D视觉任务中表现出优越性能，为关键点匹配提供了新的强化学习框架。
...
TL;DR: SIFormer是一个用于4D毫米波雷达和相机融合的3D目标检测方法，通过场景-实例感知的transformer架构，结合BEV级和视角级融合的优势，解决雷达稀疏几何线索带来的实例激活困难问题。


<details>
  <summary>Details</summary>
Motivation: 4D毫米波雷达在自动驾驶中具有鲁棒性和成本优势，但其稀疏和弱的几何线索使得可靠的实例激活变得困难。现有的雷达-相机融合范式存在局限性：BEV级融合提供全局场景理解但缺乏实例关注，视角级融合捕获实例细节但缺乏整体上下文。
...
Conclusion: SIFormer通过桥接BEV级和视角级融合范式，结合两者的互补优势，有效解决了雷达稀疏性的固有挑战，提高了检测精度，增强了实例感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法常将视觉注意力估计与下游相机控制混为一谈，或依赖直接的对象中心假设，缺乏对手术注意力跟踪的系统性建模

Method: 将手术注意力跟踪建模为时空学习问题，提出SurgAtt-Tracker框架，通过提案级重排序和运动感知细化利用时间一致性，而非直接回归

Result: 在多个手术数据集上实验表明，SurgAtt-Tracker在遮挡、多器械干扰和跨域设置下均达到最先进性能，表现出强鲁棒性

Conclusion: 该方法不仅实现手术注意力跟踪，还提供帧级视野引导信号，可直接支持下游机器人视野规划和自动相机控制
...
<details>
  <summary>Details</summary>
Motivation: 大规模图像数据集存储需求高，现有方法通过丢弃样本来减少数据集大小，但忽略了每个图像内部的颜色空间冗余问题

Method: 提出数据集颜色量化(DCQ)框架，通过跨相似图像强制一致调色板表示、基于模型感知选择性保留语义重要颜色、保持有效特征学习所需结构细节来压缩数据集
...
TL;DR: SD4R是一个将稀疏4D雷达点云转换为密集表示的框架，通过前景点生成器和logit查询编码器实现降噪和前景点稠密化，在View-of-Delft数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 4D雷达测量提供经济且天气鲁棒的3D感知方案，但其点云固有的稀疏性和噪声给准确3D目标检测带来挑战，现有稠密化方法难以处理极端稀疏场景且鲁棒性有限。
...
Conclusion: SD4R通过创新的FPG和LQE组件，有效解决了4D雷达点云极端稀疏和噪声问题，为雷达点云稠密化提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 手动提升任务是工作相关肌肉骨骼疾病的主要原因，需要有效的工效学风险评估。修订版NIOSH提升方程（RNLE）是广泛使用的评估工具，但其水平和垂直手部距离参数通常需要手动测量或专用传感系统，难以在真实环境中使用。

Method: 开发了两种多阶段视觉语言模型（VLM）管道：文本引导的仅检测管道和检测加分割管道。两种管道都使用文本引导定位任务相关感兴趣区域，从这些区域提取视觉特征，并使用基于transformer的时间回归来估计提升开始和结束时的H和V距离。

Result: 结果因管道和摄像机视角条件而异，基于分割的多视角管道表现最佳，估计H的平均绝对误差约为6-8厘米，V约为5-8厘米。分割方法使H的估计误差降低20-30%，V降低35-40%。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于静态扩散模型的自定义故事板生成方法存在三个关键问题：1）静态模型缺乏动态表现力，常采用"复制粘贴"模式；2）一次性推理无法迭代修正缺失属性或提示词遵循不佳的问题；3）多智能体框架依赖不稳健的评估器，不适合评估风格化、非写实的动画内容。

Method: 提出AnimeAgent，首个基于图像到视频的多智能体框架，灵感来源于迪士尼的"组合式直进与姿势到姿势"工作流程。利用I2V的隐式运动先验增强一致性和表现力，同时采用混合主观-客观评审机制实现可靠的迭代优化。

Result: 实验表明AnimeAgent在一致性、提示词忠实度和风格化方面达到最先进的性能水平。研究团队还收集了带有人工标注真实值的故事板生成基准数据集。

Conclusion: AnimeAgent通过创新的图像到视频多智能体框架，有效解决了现有自定义故事板生成方法在动态表现、迭代修正和评估方面的核心问题，在多个关键指标上实现了最先进的性能。
...
TL;DR: 提出一个交互式3D形状生成框架，通过迭代分割边界框实现从粗到细的创作过程，包含边界框分割生成模型和边界框到形状生成模型两个核心技术组件。
...
Method: 框架包含两个生成模型：BoxSplitGen通过迭代分割边界框生成多粒度3D部件边界框，学习聚合合并过程的逆过程；边界框到形状生成模型利用现有3D扩散模型的先验知识，并加入边界框条件。
...
<details>
  <summary>Details</summary>
Motivation: 现有文生图模型依赖于描述性语言，而专业工作流程需要精确的数值控制（如对象位置、大小、颜色）。存在参数化鸿沟：描述性语言无法提供精确的数值控制。

Method: 在统一的结构化文本框架中直接使用数值边界框和RGB三元组作为条件输入，通过训练带有参数化标注的标题来实现精确控制，无需架构修改或推理时优化。

Result: BBQ在全面评估中实现了强大的边界框对齐，并在RGB色彩保真度方面优于最先进的基线方法。支持直观的用户界面如对象拖拽和颜色选择器。

Conclusion: 研究结果支持一种新范式：将用户意图转换为中间结构化语言，由基于流的transformer作为渲染器消费，自然地容纳数值参数，取代模糊的迭代提示。
...
TL;DR: GA-Drive是一个创新的驾驶仿真框架，通过几何-外观解耦和扩散模型生成技术，能够沿用户指定的新轨迹生成高质量的可编辑相机视图。
...
Method: 采用几何-外观解耦策略：首先利用几何信息合成新的伪视图，然后通过训练的视频扩散模型将其转换为逼真的视图。这种方法支持通过先进的视频到视频编辑技术进行外观编辑，同时保持底层几何结构的一致性。

Result: 大量实验表明，GA-Drive在NTA-IoU、NTL-IoU和FID分数方面显著优于现有方法。

Conclusion: GA-Drive提供了一个有效的驾驶仿真框架，能够生成高质量、可编辑的自由视角视图，为自动驾驶系统的训练和评估提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常将空间和时间相关性分开处理，并依赖强3D几何先验，这限制了模型对不同相机设置和自运动的泛化能力。需要一种能够统一处理时空相关性的几何无关世界模型。
...
Conclusion: RAYNOVA通过几何无关的双因果自回归框架和统一的4D时空表示，实现了对真实世界演化的物理合理模拟，在多样相机设置下具有强大的泛化能力。
...
TL;DR: 提出了一种轻量级、即插即用的匹配监督模块LPP，用于端到端学习单像素宽度的清晰边缘，无需传统后处理算法。


<details>
  <summary>Details</summary>
Motivation: 现有边缘检测方法依赖非最大抑制和骨架细化等手工后处理算法来获得清晰边缘，这些算法不可微分，阻碍端到端优化，且所有现有清晰边缘检测方法仍需此类后处理才能获得满意结果。

Method: 提出LPP模块，仅需约21K额外参数，可附加到任何边缘检测模型上。在每次训练迭代中，基于空间距离和置信度对预测边缘和真实边缘进行一对一匹配，确保训练和测试协议的一致性。
...
TL;DR: 提出NGL（自然服装语言）作为中间表示，将GarmentCode参数转换为更易被语言模型理解的形式，通过NGL-Prompter训练免费流程从图像提取服装参数，实现多层服装的缝纫图案重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖合成数据微调视觉语言模型，难以泛化到真实世界图像，无法捕捉服装部件间的真实相关性，且通常仅限于单层服装。需要一种能处理真实世界图像、多层服装且无需昂贵训练的方法。

Method: 提出NGL中间语言重新构建GarmentCode参数，使其更易被语言模型理解。开发NGL-Prompter训练免费流程，查询大型视觉语言模型提取结构化服装参数，然后确定性地映射到有效的GarmentCode。

Result: 在Dress4D、CloSe和约5000张真实时尚图像数据集上评估，在标准几何指标上达到最先进性能，在人类和GPT感知评估中显著优于现有基线。能够重建多层服装，而竞争方法主要关注单层服装。

Conclusion: 通过NGL中间表示和训练免费流程，无需昂贵模型训练即可实现准确的缝纫图案重建，在真实世界图像上表现出强大的泛化能力，即使存在遮挡部分也能有效工作。
...
TL;DR: 本文提出了一种基于3D高斯泼溅的变形内窥镜组织重建方法，通过表面感知重建和物理约束实现平滑变形表面和实时渲染。


<details>
  <summary>Details</summary>
Motivation: 重建可变形内窥镜组织对机器人辅助手术至关重要。现有3D高斯泼溅方法难以实现一致的表面重建，而NeRF方法缺乏实时渲染能力，需要同时实现平滑变形表面和实时渲染。

Method: 基于3D高斯泼溅，引入表面感知重建：先用基于符号距离场的方法构建网格，再用该网格约束高斯泼溅重建过程。同时加入局部刚性和全局非刚性约束来指导高斯变形，适应软内窥镜组织的高度可变形特性。

Result: 定量和定性分析表明，该方法在纹理和几何上都实现了稳定的重建质量，提供了快速渲染过程和平滑表面外观。

Conclusion: 提出的基于3D高斯泼溅的方法成功解决了变形内窥镜组织重建中表面一致性和实时渲染的挑战，实现了高质量的实时重建。
...
TL;DR: CleanStyle是一个即插即用的扩散模型风格迁移框架，通过SVD分解过滤风格嵌入中的内容泄露噪声，并提出风格特定的无分类器引导来提升提示对齐和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 当前基于编码器的扩散模型风格迁移方法虽然高效且无需调优，但存在内容泄露问题——风格图像中的语义元素会不期望地出现在输出中，损害了提示保真度和风格一致性。

Method: 提出CleanStyleSVD（CS-SVD），通过奇异值分解（SVD）分离风格嵌入的尾部成分，并使用时间感知的指数调度动态抑制这些成分。同时提出风格特定的无分类器引导（SS-CFG），重用被抑制的尾部成分构建风格感知的无条件输入，提供针对性的负信号。

Result: 大量实验表明，CleanStyle显著减少了内容泄露，提升了风格化质量和提示对齐，适用于广泛的风格参考和提示。

Conclusion: CleanStyle是一个轻量级、可解释的框架，无需重新训练即可无缝集成到现有的基于编码器的扩散模型中，有效解决了风格迁移中的内容泄露问题。
...
TL;DR: 提出统一随机公式连接蒙特卡洛渲染与扩散生成模型，实现物理基础的扩散控制


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本/图像条件生成方面表现出色但缺乏物理基础的低级控制，而物理渲染提供精细物理控制但缺乏提示驱动的灵活性，需要结合两者优势

Method: 提出统一随机微分方程(SDE)公式，基于中心极限定理建模蒙特卡洛积分，通过基于物理的路径追踪实例化，从噪声方差角度分析路径追踪物理特性如何扩展到现有扩散模型

Result: 在多个任务上的广泛实验表明，该方法能够对扩散生成结果施加物理基础的控制，涵盖渲染和材质编辑等任务

Conclusion: 成功建立了蒙特卡洛渲染与扩散生成模型之间的理论联系，实现了物理基础控制与提示驱动灵活性的统一
...
TL;DR: COMiT是一个受人类交流启发的视觉token化框架，通过迭代观察局部图像块并递归更新离散表示来学习结构化视觉token序列，显著提升了组合泛化和关系推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有离散图像tokenizer主要针对重建和压缩优化，产生的token往往捕捉局部纹理而非对象级语义结构。受人类交流的渐进性和组合性启发，需要开发能够学习结构化离散视觉token序列的方法。

Method: COMiT在固定token预算内构建潜在消息，通过迭代观察局部图像块并递归更新离散表示。每个步骤整合新视觉信息同时细化和重组现有token序列。最终消息条件化流匹配解码器重建完整图像。编码和解码在单一transformer模型中实现，使用流匹配重建和语义表示对齐损失的组合进行端到端训练。

Result: 实验表明，语义对齐提供基础，而注意力顺序token化对于诱导可解释的、以对象为中心的token结构至关重要，相比先前方法显著提升了组合泛化和关系推理能力。

Conclusion: COMiT框架通过受人类交流启发的迭代、结构化token化方法，成功学习到具有对象级语义结构的离散视觉表示，在组合泛化和关系推理方面优于现有方法。
...
TL;DR: OrthoDiffusion是一个基于扩散模型的多任务骨骼肌肉MRI分析基础模型，通过自监督预训练学习膝关节解剖特征，支持多标签诊断和解剖分割，在数据稀缺情况下表现优异且可迁移到其他关节。


<details>
  <summary>Details</summary>
Motivation: 骨骼肌肉疾病是全球主要的致残原因，MRI诊断至关重要但解读困难。放射科医生需要在复杂的解剖结构中识别多种异常，这需要专业知识且存在主观差异。因此需要开发能够提高MRI解读效率和准确性的自动化工具。

Method: 开发了OrthoDiffusion统一扩散基础模型，使用三个方向特定的3D扩散模型（矢状面、冠状面、轴向面），在15,948个未标记的膝关节MRI扫描上进行自监督预训练，学习稳健的解剖特征。这些视图特定的表示被整合以支持解剖分割和多标签诊断等多种临床任务。

Result: 模型在11个膝关节结构分割和8种膝关节异常检测中表现出色，在不同临床中心和MRI场强下具有显著鲁棒性，始终优于传统监督模型。在标记数据稀缺情况下，仅使用10%的训练标签仍保持高诊断精度。从膝关节学到的解剖表示可高度迁移到其他关节，在踝关节和肩关节的11种疾病诊断中表现优异。
...
TL;DR: 研究提出了一种在联邦学习框架下解决跨模态医学图像分割问题的增强策略，通过全局强度非线性增强方法有效提升模型在CT和MRI之间的泛化能力，在保护数据隐私的同时达到接近集中式训练的性能。


<details>
  <summary>Details</summary>
Motivation: 医学图像分析中，由于数据分散在不同机构且受隐私保护限制，开发鲁棒且可泛化的分割模型面临挑战。联邦学习虽然能实现协作训练而不集中数据，但跨模态（如CT与MRI）的域偏移问题严重，现有方法通常需要患者的多模态配对数据或复杂架构，这在真实临床环境中不实用。

Method: 研究在联邦学习场景下，每个客户端仅持有单模态数据（CT或MRI），系统评估了多种增强策略：基于卷积的空间增强、频域操作、域特定归一化以及全局强度非线性增强。使用腹部器官分割和全心分割作为多类和二元分割基准进行评估。

Result: 全局强度非线性增强在集中式和联邦学习设置中均优于其他方法，通过模拟跨模态外观变化同时保留解剖结构。胰腺分割的Dice分数从0.073提升到0.437，提高了498%。联邦学习方法达到了集中式训练精度的93-98%，展现了强大的跨模态泛化能力。

Conclusion: 全局强度非线性增强策略在联邦学习框架下有效解决了跨模态医学图像分割的泛化问题，在不损害数据隐私的前提下实现了接近集中式训练的性能，为在不同医疗系统中部署可行的联邦AI提供了方向。
...
TL;DR: 本文提出了一种基于法向流的事件相机运动分割框架，通过将法向流作为中间表示来压缩运动信息，使用图割进行能量最小化，实现了近800倍的速度提升。
...
Result: 在多个公开数据集上验证了框架的准确性和效率，相比开源的最先进方法实现了近800倍的速度提升，显著降低了计算复杂度，确保了实时性能。

Conclusion: 提出的基于法向流的事件相机运动分割框架通过有效的中间表示和优化策略，在保持准确性的同时大幅提升了计算效率，为实时动态场景理解提供了可行解决方案。
...
TL;DR: 提出SIMSPINE数据集和生物力学感知的关键点模拟框架，为自然全身运动提供首个开放的椎骨级3D脊柱标注数据集，包含214万帧数据，并发布了预训练基线模型。
...
Method: 开发了生物力学感知的关键点模拟框架，通过肌肉骨骼建模从现有人体姿态数据集中生成解剖学一致的3D脊柱关键点，创建了SIMSPINE数据集，并提供了2D检测器、单目3D姿态提升模型和多视图重建管道的预训练基线。

Result: 创建了包含214万帧的SIMSPINE数据集，2D脊柱基线在受控环境中将AUC从0.63提升到0.80，在野外脊柱跟踪中将AP从0.91提升到0.93，建立了生物力学有效脊柱运动估计的统一基准。

Conclusion: 模拟框架和SIMSPINE数据集通过实现自然条件下可重复、解剖学基础的3D脊柱估计，推动了基于视觉的生物力学、运动分析和数字人体建模的研究。
...
TL;DR: VGGDrive通过引入跨视图3D几何赋能器，将成熟的3D基础模型的几何理解能力注入视觉语言模型，显著提升了自动驾驶任务性能


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型缺乏跨视图3D几何建模能力，导致在自动驾驶任务中表现平庸。虽然已有方法通过构建问答数据进行辅助训练，但仍无法从根本上让VLM全面处理多样化的评估协议

Method: 提出VGGDrive架构，包含可插拔的跨视图3D几何赋能器(CVGE)。CVGE将冻结的3D视觉模型的跨视图3D几何特征与VLM的2D视觉特征桥接，通过分层自适应注入机制有效赋能VLM

Result: 在五个自动驾驶基准测试中，VGGDrive显著提升了基础VLM的性能，包括跨视图风险感知、运动预测和轨迹规划等任务

Conclusion: 成熟的3D基础模型可以通过有效集成来赋能自动驾驶任务，这项工作展示了这种范式在自动驾驶领域的潜力
...
TL;DR: RU4D-SLAM：一种结合4D高斯泼溅的鲁棒SLAM框架，通过引入时间因素、不确定性感知和运动模糊渲染，在动态环境中显著提升轨迹精度和4D场景重建质量


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅与SLAM结合的方法在动态环境中表现不佳，移动物体使3D重建复杂化并影响跟踪可靠性。4D重建技术为解决这些挑战提供了有前景的方向，但其在4D感知SLAM中的应用尚未充分探索
...
Conclusion: RU4D-SLAM通过结合4D高斯泼溅、不确定性感知和动态场景处理技术，为动态环境中的鲁棒SLAM提供了有效解决方案，在轨迹精度和4D重建质量方面取得了显著改进
...
TL;DR: GatedCLIP模型通过改进CLIP架构，结合学习投影头、动态门控融合机制和对比学习目标，在仇恨表情包检测任务上显著提升性能，AUROC达到0.66，远超CLIP基线的0.49。


<details>
  <summary>Details</summary>
Motivation: 检测多模态表情包中的仇恨内容具有独特挑战，因为有害信息往往来自良性图像和文本的复杂交互。现有模型如CLIP在处理这种微妙的多模态仇恨内容时表现有限。

Method: 提出GatedCLIP模型，包含三个核心组件：1) 学习投影头将CLIP嵌入映射到任务优化的语义空间；2) 动态门控融合机制自适应加权视觉和文本特征；3) 对比学习目标保持跨模态语义对齐。

Result: 在Hateful Memes数据集上的实验表明，GatedCLIP达到AUROC 0.66，显著优于CLIP基线的0.49，同时保持计算效率，仅需35万可训练参数。

Conclusion: GatedCLIP通过专门的架构改进增强了CLIP的多模态能力，为仇恨表情包检测提供了一种有效且高效的解决方案，能够更好地捕捉图像和文本之间的复杂交互。
...
TL;DR: 提出无需训练的概念图像编辑框架，结合优化DDS与LoRA概念组合，通过预训练概念适配器实现多概念控制，在InstructPix2Pix和ComposLoRA基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于优化的零样本文本编辑方法难以保持图像身份或捕捉语言无法表达的视觉概念（如面部结构、材质纹理、物体几何），需要一种能结合文本语义指导和低层视觉概念的方法。

Method: 训练免费的概念图像编辑框架，统一优化DDS与LoRA驱动的概念组合，其中LoRA的训练数据代表概念。通过有序时间步、正则化和负提示指导改进DDS的稳定性和可控性，结合文本语义指导和预训练概念适配器的低层线索。

Result: 在InstructPix2Pix和ComposLoRA基准测试中，定量和定性结果均显示相比现有训练免费扩散编辑方法有持续改进。

Conclusion: 提出的框架成功解决了文本编辑无法表达的视觉概念问题，通过概念组合实现了更好的图像编辑效果，代码将公开提供。
...
TL;DR: 提出了一种混合图像融合框架，通过可学习的U-Net生成动态引导图来指导固定的拉普拉斯金字塔融合核，实现了高效的全分辨率训练，消除了训练-推理差距。


<details>
  <summary>Details</summary>
Motivation: 传统图像融合方法速度快但缺乏适应性和性能，而深度学习方法虽然能达到SOTA效果，但依赖缓慢、资源密集的基于补丁的训练，导致与全分辨率推理之间存在显著差距。

Method: 使用可学习的U-Net生成动态引导图，指导经典的固定拉普拉斯金字塔融合核，将策略学习与像素合成解耦，实现高效的全分辨率训练。

Result: 在RTX 4090上约1分钟或在消费级笔记本电脑GPU上约2分钟即可从零开始达到SOTA可比性能，无需任何外部模型，并在红外-可见光到医学成像等多种任务中展现出强大的零样本泛化能力。

Conclusion: 该混合框架解决了传统方法与深度学习方法之间的权衡问题，通过设计确保融合输出仅从源信息线性构建，为关键应用提供了高保真度。
...
Method: 结合零样本定位实验和人类可解释性研究，评估了七种不同的可解释AI方法。通过艺术史背景下的实验设计，分析这些方法如何解释CLIP模型的视觉推理过程。

Result: 研究结果表明，这些可解释AI方法能够捕捉人类解释的某些方面，但其有效性高度依赖于所考察类别的概念稳定性（conceptual stability）和表征可用性（representational availability）。

Conclusion: 虽然现有可解释AI方法在解释视觉语言模型的视觉推理方面取得了一定进展，但其效果受到所分析概念特性的限制。这为未来开发更有效的模型解释方法提供了重要见解，特别是在艺术史等复杂领域。
...
TL;DR: 本文提出CASG框架解决T2I扩散模型安全引导中的"有害冲突"问题，通过动态识别类别对齐的安全方向，相比现有方法将有害率降低达15.4%


<details>
  <summary>Details</summary>
Motivation: 现有基于安全引导的方法在处理多类别有害内容时存在"有害冲突"问题，即抑制一种有害类别可能无意中放大另一种有害类别，导致总体有害率增加

Method: 提出冲突感知自适应安全引导(CASG)框架，包含两个组件：冲突感知类别识别(CaCI)动态识别与生成状态最对齐的有害类别；冲突解决引导应用(CrGA)仅沿识别类别应用安全引导以避免多类别干扰

Result: 在T2I安全基准测试中，CASG实现了最先进的性能，相比现有方法将有害率降低达15.4%，可应用于潜在空间和文本空间安全防护

Conclusion: CASG通过动态识别类别对齐的安全方向有效解决了多类别有害内容生成中的冲突问题，显著提升了T2I扩散模型的安全性能
...
<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在常见视觉问答和逻辑推理方面表现出色，但在复杂现实环境中缺乏合理的决策能力，特别是空间逻辑推理能力，这需要理解复杂场景中物体的空间关系以及多步任务中步骤间的逻辑依赖关系。

Method: 提出了递归场景图辅助推理方法，利用视觉基础模型逐步将复杂场景分解为任务相关的场景图，从而增强视觉语言模型的空间逻辑推理能力。

Result: 在41个主流视觉语言模型上进行了广泛实验，结果显示即使最先进的模型在空间逻辑推理方面仍然存在困难。提出的方法优于所有先前方法。

Conclusion: SpatiaLQA基准测试揭示了当前视觉语言模型在空间逻辑推理方面的局限性，提出的递归场景图辅助推理方法能有效提升模型性能，为未来研究提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉文本渲染（VTR）存在严重问题，即使先进模型生成的文本也常出现扭曲、模糊、错位等结构异常。然而主流多模态大语言模型和专用OCR模型无法有效感知这些结构异常，这成为VTR评估和基于强化学习的优化的关键瓶颈，导致即使最先进的生成器也难以渲染结构准确的文本。

Method: 提出TextPecker方法：1）构建包含字符级结构异常标注的识别数据集；2）开发笔画编辑合成引擎以扩展结构错误覆盖范围；3）设计即插即用的结构异常感知强化学习策略，可适配任何文本到图像生成器，缓解噪声奖励信号问题。

Result: 实验表明TextPecker能持续改进多种文本到图像模型。即使在已优化的Qwen-Image模型上，也能显著提升中文文本渲染的结构保真度（平均提升4%）和语义对齐度（平均提升8.7%），在高质量VTR方面达到新的最先进水平。

Conclusion: TextPecker填补了VTR优化领域的空白，为可靠且结构准确的视觉文本生成提供了基础性步骤。该方法通过解决结构异常感知问题，显著提升了文本到图像生成中文本渲染的质量和准确性。
...
TL;DR: DropAnSH-GS：一种新颖的基于锚点的Dropout策略，通过同时移除锚点高斯及其空间邻居来打破局部冗余，并随机丢弃高阶球谐系数以缓解过拟合，提升稀疏视图下3D高斯溅射的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射Dropout方法在稀疏视图条件下通过随机置零高斯不透明度来应对过拟合，但存在邻居补偿效应（被丢弃的高斯常被邻居补偿），且忽略了高阶球谐系数对过拟合的贡献。

Method: 提出DropAnSH-GS：1）基于锚点的Dropout策略：随机选择高斯作为锚点，同时移除其空间邻居，打破局部冗余；2）扩展Dropout到颜色属性：随机丢弃高阶球谐系数，将外观信息集中在低阶系数中，便于后续模型压缩。

Result: 实验结果表明，DropAnSH-GS在计算开销可忽略的情况下显著优于现有Dropout方法，并能轻松集成到各种3DGS变体中提升性能。

Conclusion: DropAnSH-GS通过锚点邻居移除和高阶球谐系数丢弃，有效缓解了稀疏视图下的过拟合问题，提高了3D高斯溅射的鲁棒性和可压缩性。
...
<details>
  <summary>Details</summary>
Motivation: 动态驾驶场景重建对于自动驾驶仿真和闭环学习至关重要。现有前馈方法在处理长距离驾驶序列时面临序列长度的二次复杂度挑战，以及在长时间内建模动态对象的困难。

Method: 提出UFO循环范式，维护一个4D场景表示，随着新观测的到来进行迭代优化。采用基于可见性的过滤机制选择信息丰富的场景标记，实现长序列高效处理。对于动态对象，引入对象姿态引导的建模方法，支持准确的长距离运动捕捉。

Result: 在Waymo Open Dataset上的实验表明，该方法在多种序列长度上显著优于基于场景的优化方法和现有前馈方法。能够以0.5秒内重建16秒驾驶日志，同时保持优越的视觉质量和几何精度。
...
TL;DR: ArtiAgent是一个自动化系统，通过三个智能体协作生成带有丰富伪影标注的图像对，用于训练伪影检测模型，无需昂贵的人工标注。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型取得进展，AI生成图像仍常包含视觉伪影影响真实感。现有方法依赖人工标注的伪影数据集，成本高且难以扩展，需要自动化方法来可靠获取伪影标注数据。
...
Conclusion: ArtiAgent提供了一种自动化、可扩展的方法来创建伪影标注数据集，解决了人工标注的成本和可扩展性问题，为伪影缓解研究提供了有价值的资源。
...
TL;DR: TagLLM框架利用多模态大语言模型自动生成图像标签，显著降低标注成本至人工的千分之一，并通过结构化提示和交互式校准将下游任务性能提升至接近人工标注水平。


<details>
  <summary>Details</summary>
Motivation: 传统图像标注依赖人工标注数据集，成本高昂且耗时。虽然多模态大语言模型有自动化标注的潜力，但其与人工标注的差距尚未充分探索，需要研究如何缩小这一差距并实现MLLM标注替代人工标注。

Method: 提出TagLLM框架，包含两个核心组件：1）候选标签生成：使用结构化分组提示高效生成紧凑的候选标签集，尽可能覆盖真实标签同时减少后续标注工作量；2）标签消歧：通过交互式校准提示中的语义概念，有效细化候选标签。

Result: MLLM标注可将成本降至人工的千分之一（主要考虑GPU使用），标注质量达到人工的50%-80%，下游任务性能超过90%。TagLLM显著缩小了MLLM与人工标注的差距，在下游训练性能上缩小了约60%-80%的差异。
...
TL;DR: CrystaL是一个单阶段框架，通过处理完整和损坏图像的双路径设计，显式对齐注意力模式和预测分布，将潜在表示结晶为任务相关的视觉语义，提升多模态大语言模型的细粒度视觉理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有潜在思维链方法中启发式预定义的监督信号对中间潜在状态中关键视觉信息的保留指导有限，需要改进多模态大语言模型在细粒度视觉理解方面的表现。

Method: 提出CrystaL单阶段框架，采用双路径分别处理完整图像和损坏图像，通过显式对齐两个路径的注意力模式和预测分布，将潜在表示结晶为任务相关的视觉语义，无需辅助标注或外部模块。

Result: 在感知密集型基准测试中，CrystaL持续优于最先进的基线方法，在细粒度视觉理解方面取得显著提升，同时保持强大的推理能力。

Conclusion: CrystaL通过显式对齐注意力模式和预测分布，有效解决了潜在思维链方法中视觉信息保留不足的问题，为多模态大语言模型提供了更有效的细粒度视觉理解解决方案。
...
TL;DR: EW-DETR框架将DETR检测器扩展为能够处理动态环境中目标检测的三大挑战：增量学习、领域适应和未知目标检测，无需访问先前数据。


<details>
  <summary>Details</summary>
Motivation: 现实世界目标检测需要在动态环境中运行，面临新类别出现、领域漂移、未知目标识别等挑战，且无法访问先前数据，现有方法难以同时应对这些需求。

Method: 提出EW-DETR框架，包含三个协同模块：增量LoRA适配器（无示例增量学习）、查询-范数目标性适配器（从DETR解码器查询中解耦目标感知特征）、熵感知未知混合（校准的未知检测）。

Result: 在Pascal Series和Diverse Weather基准测试中，EW-DETR优于其他方法，将FOGS综合评分提高了57.24%。

Conclusion: EW-DETR框架有效解决了动态世界目标检测的三大挑战，为现实世界目标检测系统提供了强大的解决方案。
...
TL;DR: 提出一种基于上下文图像分解框架，利用大型扩散基础模型进行logo-物体分解，通过轻量级LoRA适配和循环一致性调优策略实现高质量分层分离。


<details>
  <summary>Details</summary>
Motivation: 现实世界图像中的视觉层分离（如阴影、反射、透视变形等）存在非线性全局耦合的挑战，特别是logo-物体分解需要同时保留两个层的完整性。

Method: 1) 通过轻量级LoRA适配微调预训练扩散模型；2) 引入循环一致性调优策略，联合训练分解和合成模型；3) 采用渐进式自改进过程，迭代增强训练数据。

Result: 实验表明该方法能实现准确连贯的分解，并在其他分解类型上有效泛化，显示出作为统一分层图像分解框架的潜力。

Conclusion: 提出的上下文图像分解框架通过循环一致性监督和渐进式自改进，成功解决了复杂交互下的logo-物体分解问题，为分层图像分解提供了统一解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 图像到视频生成模型具有视觉指令跟随能力，但这种能力带来了被攻击者利用的风险。攻击者可能通过图像模态注入恶意意图，而现有研究尚未充分关注这一风险。

Method: VII包含两个模块：恶意意图重编程模块从恶意文本提示中提取意图并降低其静态危害性；视觉指令接地模块将提取的意图通过视觉指令形式嵌入到安全输入图像中，保持与原始恶意文本的语义一致性。

Result: 在四个最先进的商业I2V模型上的实验表明，VII攻击成功率最高达83.5%，同时将拒绝率降至接近零，显著优于现有基线方法。

Conclusion: 该研究揭示了图像到视频生成模型中视觉指令跟随能力的安全风险，提出的VII框架能够有效绕过安全防护机制，为未来防御此类攻击提供了重要参考。
...
Method: 提出了CHAIN基准测试，这是一个交互式3D物理驱动测试平台，包含机械拼图互锁、3D堆叠和包装等任务，将评估从被动感知转向主动问题解决，并在统一交互设置下对最先进的VLM和基于扩散的模型进行全面研究。

Result: 研究显示，即使表现最佳的模型在理解物理结构和因果约束方面仍然存在困难，常常无法产生可靠的长时程计划，也不能稳健地将感知到的结构转化为有效动作。

Conclusion: CHAIN基准测试揭示了当前模型在物理结构理解和因果推理方面的局限性，为评估和改进模型在物理约束下的结构化动作规划能力提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像处理需要处理高维体数据、异构文件格式和领域特定训练流程的专用软件。现有框架要么提供需要大量集成工作的低级组件，要么采用僵化的整体流程难以修改。

Method: 基于PyTorch的模块化框架，核心是LayerT延迟配置机制，支持运行时替换卷积、归一化和激活模块而无需子类化。提供完整的数据加载、训练、推理和评估流程。

Result: 框架提供内置k折交叉验证、自动ROI检测的数据集检查、深度监督、指数移动平均、多前端实验跟踪、训练状态恢复和商回归验证分数预测等功能。

Conclusion: MIPCandy是一个开源、免费的医学图像处理框架，通过可扩展的捆绑包生态系统提供预构建模型实现，遵循一致的训练器-预测器模式，无需修改即可与核心框架集成。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（如CLIP）难以理解否定语义，常常将肯定和否定表达嵌入到相似的向量空间中（例如将"没有狗"与狗图像匹配）。现有方法通过微调CLIP的文本编码器来改进否定理解，但存在过拟合风险。

Method: 提出CLIPGlasses框架，采用双阶段设计：1) Lens模块从文本嵌入中解耦否定语义；2) Frame模块预测上下文感知的排斥强度，通过修改相似度计算来惩罚与否定语义的对齐，从而减少误匹配。

Result: 实验表明，配备CLIPGlasses的CLIP在域内性能上具有竞争力，在跨域泛化方面优于最先进方法。在低资源条件下表现尤为突出，显示出更强的跨域鲁棒性。

Conclusion: CLIPGlasses是一个有效的即插即用解决方案，能够显著提升CLIP对否定语义的理解能力，特别是在跨域泛化和低资源场景下表现出更强的鲁棒性。
...
TL;DR: OmniOCR是一个针对少数民族文字的通用OCR框架，通过动态低秩适应和稀疏正则化技术，在资源匮乏的少数民族文字识别任务上实现了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 当前OCR技术主要关注拉丁文和中文等主流文字，而少数民族文字由于书写系统复杂、标注数据稀缺、历史与现代形式多样等问题，在低资源或零样本场景下识别效果不佳，需要专门解决方案。

Method: 提出OmniOCR框架，引入动态低秩适应技术，在不同层和文字间动态分配模型容量；采用稀疏正则化修剪冗余更新，实现紧凑高效的适应，且不增加推理成本。

Result: 在藏文MNIST、水书、古彝文和东巴文四个数据集上，OmniOCR超越了零样本基础模型和标准后训练方法，达到最先进准确率，相比基线模型在这些数据集上提升了39%-66%的准确率。

Conclusion: OmniOCR为少数民族文字识别提供了有效的通用解决方案，通过动态适应机制和稀疏正则化实现了优异的参数效率和识别性能，有助于促进少数民族文字的数字保存和应用。
...
TL;DR: 本文提出了一种新颖的迭代自校正框架，通过能力反思和记忆反思两大机制，使大型视觉语言模型能够自我诊断错误、制定修正计划，避免重复尝试，从而显著提升复杂视觉理解任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型虽然通过迭代优化方法在复杂视觉理解任务上展现出潜力，但缺乏有效的自校正机制，难以独立纠正认知偏差。在多轮修正过程中，模型往往陷入重复无效的尝试，无法实现答案质量的稳定提升。

Method: 提出一种新颖的迭代自校正框架，赋予模型两大关键能力：1) 能力反思：诊断错误并生成修正计划；2) 记忆反思：回顾过往尝试以避免重复，探索新解决方案；最后通过严谨的重新推理来优化答案。
...
Conclusion: 研究表明，结构化、自我意识的反思机制能够在不增加额外训练的情况下，显著增强视觉语言模型的推理鲁棒性，为解决模型自我修正问题提供了有效方案。
...
TL;DR: VAUQ是一个视觉感知的不确定性量化框架，用于大视觉语言模型的自评估，通过图像信息评分和核心区域掩码策略来减少幻觉问题。
...
Method: 提出VAUQ框架，引入图像信息评分来量化视觉输入对预测不确定性的减少程度，采用无监督核心区域掩码策略增强显著区域的影响，结合预测熵得到无需训练的评分函数。

Result: 综合实验表明，VAUQ在多个数据集上持续优于现有的自评估方法。

Conclusion: VAUQ通过视觉感知的不确定性量化，有效提高了大视觉语言模型自评估的可靠性，减少了幻觉问题。
...
TL;DR: 提出一种混合方法，结合基础模型和优化方法的优势，从图像实现高保真3D头部几何重建，减少相机需求并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有3D头部重建方法存在根本性限制：传统摄影测量需要大量相机阵列和计算资源，而近期方法要么缺乏几何细节（单图基础模型），要么需要密集视角和昂贵计算（优化方法）。需要一种能平衡这些限制的方法。

Method: 采用混合方法：1) 提出多视角表面法线预测模型，通过跨视角注意力扩展单目基础模型，在前向传播中产生几何一致的法线；2) 将这些预测作为强几何先验，在逆渲染优化框架中恢复高频表面细节。

Result: 方法在单图和多视角重建任务中均优于现有技术，达到与密集视角摄影测量相当的高保真重建效果，同时显著减少了相机需求和计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 高速飞行的无人机在基础设施检查、地形探索和搜救等应用中具有重要价值，但高速导致图像严重运动模糊，位姿估计存在显著漂移和噪声，这使得对这类退化敏感的神经辐射场（NeRF）进行密集3D重建变得特别困难。

Method: 提出统一框架，将事件-图像融合嵌入NeRF优化过程，同时使用事件和帧模态联合优化基于事件的视觉惯性里程计先验，无需地面真值监督即可恢复清晰的辐射场和准确的相机轨迹。

Result: 在合成数据和真实世界快速飞行无人机采集的序列上验证了方法有效性。即使在高度动态的无人机飞行中，RGB帧严重退化且位姿先验不可靠的情况下，该方法仍能重建高保真辐射场并保留精细场景细节，在真实数据上相比最先进方法性能提升超过50%。

Conclusion: 该框架通过融合事件流和运动模糊帧，成功解决了高速无人机飞行中NeRF重建的挑战，实现了无需地面真值监督的高质量辐射场重建和相机轨迹恢复，为快速飞行的机器人应用提供了有效的3D重建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 从非结构化数据恢复B-rep表示是计算机视觉和图形学中的重要挑战任务。现有深度学习方法依赖密集干净的点云，难以泛化到新形状，需要从2D图像直接学习3D参数化表示

Method: 提出B-rep高斯泼溅框架，使用可学习特征的高斯泼溅渲染器，配合特定拟合策略。采用两阶段学习：第一阶段捕获几何和边缘，第二阶段细化面片特征，实现几何重建和特征学习的解耦

Result: 大量实验证明该方法在性能上优于现有最先进方法，能够实现干净的几何结构和一致的实例表示

Conclusion: BrepGaussian是从2D图像学习3D参数化表示的有效框架，通过两阶段学习策略成功解决了几何重建和特征学习的耦合问题，在B-rep恢复任务上表现出色
...
TL;DR: SynthRender是一个开源合成图像生成框架，结合领域随机化和Reality-to-Simulation技术，为缺乏3D文件的工业零件提供低成本、可迁移的合成数据，在多个基准测试中取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 工业物体感知需要大量标注数据，但获取和标注专有零件数据的成本很高，成为广泛部署的主要障碍。需要开发低成本、高效的合成数据生成方法来解决这一问题。

Method: 1. 开发SynthRender开源框架，具有引导领域随机化功能；2. 基准测试从2D图像创建3D资产的Reality-to-Simulation技术；3. 结合领域随机化生成合成资产；4. 引入IRIS工业真实-模拟图像集，包含32个类别和约20,000个标签。

Result: 在多个基准测试中表现优异：在公共机器人数据集上达到99.1% mAP@50，在汽车基准测试上达到98.3% mAP@50，在IRIS数据集上达到95.3% mAP@50。消融实验为高效数据生成提供了指导。

Conclusion: SynthRender框架结合领域随机化和Reality-to-Simulation技术，能够为缺乏3D文件的工业零件生成低成本、可迁移的合成数据，有效解决了工业物体感知中的数据获取难题。
...
Result: 在诊断VQA任务上显著优于基线模型，更重要的是展现出有前景的预后能力，证明指令调优VLM在纵向放射影像解读中的价值。

Conclusion: 精心设计的指令调优视觉语言模型能够实现更准确、更具临床意义的纵向放射影像解读，为临床决策支持提供新工具。
...
TL;DR: 本文提出了一种质量可控的检索新范式，通过生成语言模型将简短查询扩展为包含细粒度视觉属性和质量控制的描述性查询，从而改善文本到图像检索效果。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的文本到图像检索经常面临简短、不明确的用户查询挑战，这些查询通常只有一两个词，导致语义模糊、视觉解释冲突，且缺乏对检索图像质量的明确控制。

Method: 提出质量可控检索框架，利用生成语言模型作为查询补全函数，将不明确的查询扩展为包含姿态、场景、美学等细粒度视觉属性的描述性形式，并将查询补全条件化于从相关性和美学评分模型导出的离散化质量级别。

Result: 大量实验表明，该方法显著改善了检索结果，提供了有效的质量控制，弥合了现代视觉语言模型的表达能力与简短用户查询不明确性之间的差距。

Conclusion: 提出的质量可控检索新范式具有灵活性（兼容任何预训练VLM）、透明性（扩展查询可解释）和可控性（可引导检索结果到用户偏好的质量级别）三大优势，有效解决了简短查询的检索挑战。
...
TL;DR: Mask-HybridGNet：无需手动标注地标，直接从像素级分割掩码训练图结构分割模型，实现解剖结构边界表示和隐式对应关系学习


<details>
  <summary>Details</summary>
Motivation: 图结构医学图像分割方法需要跨患者点对点对应的手动标注地标数据集，这在实践中很少存在，限制了临床采用。需要一种能从标准像素级分割掩码直接训练的方法。

Method: 结合Chamfer距离监督和基于边的正则化，将可变长度的真实边界与固定长度的地标预测对齐，通过可微分光栅化进行细化。使用固定图邻接矩阵确保解剖合理性。

Result: 在胸部X光、心脏超声、心脏MRI和胎儿成像实验中，模型达到与最先进像素级方法竞争的结果，同时确保解剖合理性。预测地标位置在不同患者间一致对应特定解剖位置。

Conclusion: Mask-HybridGNet利用大量标准分割掩码构建结构化模型，保持拓扑完整性并提供隐式对应关系，支持时间跟踪、跨切片重建和形态学群体分析。
...
TL;DR: Spa3R是一个自监督框架，通过预测性空间场建模从无姿态多视角图像学习统一、视角不变的空间表示，无需显式3D数据或空间指令调优，显著提升VLM的3D视觉问答能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在3D空间理解能力有限，现有方法依赖显式3D模态或部分几何先验，限制了可扩展性，且让语言模型承担从稀疏线索隐式重建整体3D几何的不适定任务。作者认为空间智能可以从2D视觉中自然涌现，无需显式空间指令调优。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像到视频合成方法在人体视频生成方面仍面临挑战，特别是从单图像推断视角一致、运动相关的服装褶皱等问题

Method: 采用三个关键设计：1) 关节姿态调制，通过双维骨骼图捕获3D关节解剖关系并引入3D信息解决跨视角自遮挡；2) 视角和时间对齐，确保多视角一致性和参考图像与姿态序列的对齐；3) 渐进时空采样与时间对齐，保持长多视角动画的平滑过渡
...
<details>
  <summary>Details</summary>
Motivation: 传统方法通常通过完整结构分割后再进行事后分析来识别感兴趣区域，这种方法对于连续且几何复杂的结构（如膜）尤其受限，因为它们被分割为单一实体。需要一种能够直接进行ROI分割和形态分析的方法。

Method: 开发了TomoROIS-SurfORA两步框架：1) TomoROIS使用深度学习进行ROI分割，可以用小标注数据集从头训练；2) SurfORA将分割结构处理为点云和表面网格，提取定量形态特征（膜间距离、曲率、表面粗糙度等），支持封闭和开放表面。

Result: 在体外重构的膜系统（包含具有复杂几何形状的可变形囊泡）上展示了该工具，实现了膜接触位点和重塑事件（如内陷）的自动定量分析。方法适用于更广泛的科学成像场景。

Conclusion: TomoROIS-SurfORA为冷冻电子断层扫描中的膜结构分析提供了直接的ROI分割和表面形态分析框架，克服了传统间接方法的局限性，具有广泛的科学成像应用潜力。
